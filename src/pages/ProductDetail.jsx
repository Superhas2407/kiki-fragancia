...
export default function ProductDetail() {
  ...
  function handleShare() {
    const shareData = {
      title: `${product.house} ${product.name}${product.ml ? ` ${product.ml}ml` : ''}`,
      text: `${product.house} ${product.name}${product.ml ? ` ${product.ml}ml` : ''}${product.precioUSD > 0 ? ` — REF $${product.precioUSD}` : ''} · KiKi Fragancia`,
      url: canonicalUrl,
    }
    if (navigator.share) {
                ...
                <div className="pd-img-wrap" style={{
                  ...
                  {theme === 'halloween' && (
                    <SlimeTopBorder
                      ...
                      style={{ position: 'absolute', top: 0, left: 0, right: 0, zIndex: 3, pointerEvents: 'none' }}
                      aria-hidden="true"
                    />
                  )}
                  {resolvedImg ? (
