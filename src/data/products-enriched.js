export const products = [
  {
    "id": 1,
    "house": "AFNAN",
    "name": "Afnan 9 AM",
    "image": "afnan-9am-100ml-f.webp",
    "familia": "Floral Frutal",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Una fragancia elegante y luminosa para la mujer, con un perfil floral-frutal fresco y sofisticado que resulta ideal para el día a día. Inspirada en el estilo de perfumes femeninos modernos de diseñador, combina un carácter vibrante con un acabado limpio y refinado.",
    "description": "Una fragancia elegante y luminosa para la mujer, con un perfil floral-frutal fresco y sofisticado que resulta ideal para el día a día. Inspirada en el estilo de perfumes femeninos modernos de diseñador, combina un carácter vibrante con un acabado limpio y refinado.",
    "notasSalida": "Limón, mandarina, cardamomo, pimienta rosa",
    "notasCorazon": "lavanda, manzana verde, flor de azahar del naranjo, rosa",
    "notasFondo": "almizcle, musgo, cedro, pachulí",
    "notes": [
      "Limón",
      "mandarina",
      "cardamomo",
      "pimienta rosa",
      "lavanda",
      "manzana verde",
      "flor de azahar del naranjo",
      "rosa",
      "almizcle",
      "musgo",
      "cedro",
      "pachulí"
    ],
    "ml": 100,
    "precioUSD": 40,
    "categoria": "arabes"
  },
  {
    "id": 2,
    "house": "AFNAN",
    "name": "Afnan 9 AM Dive",
    "image": "afnan-9am-dive-100ml-u.webp",
    "familia": "Floral Frutal",
    "tipo": "Eau de Parfum",
    "genero": "Unisex",
    "descripcion": "Afnan 9 AM Dive es una fragancia unisex de estilo aromático acuático, fresca y vibrante, ideal para quienes buscan una presencia limpia, moderna y versátil. Su perfil recuerda a un perfume de diseñador en la línea de los cítricos marinos contemporáneos, con un toque especiado y elegante que aporta personalidad durante todo el día.",
    "description": "Afnan 9 AM Dive es una fragancia unisex de estilo aromático acuático, fresca y vibrante, ideal para quienes buscan una presencia limpia, moderna y versátil. Su perfil recuerda a un perfume de diseñador en la línea de los cítricos marinos contemporáneos, con un toque especiado y elegante que aporta personalidad durante todo el día.",
    "notasSalida": "Limón, Menta, Grosella negra, Pimienta rosa",
    "notasCorazon": "Manzana, Incienso, Cedro, Jengibre",
    "notasFondo": "Sándalo, Pachulí, Jazmín",
    "notes": [
      "Limón",
      "Menta",
      "Grosella negra",
      "Pimienta rosa",
      "Manzana",
      "Incienso",
      "Cedro",
      "Jengibre",
      "Sándalo",
      "Pachulí",
      "Jazmín"
    ],
    "ml": 100,
    "precioUSD": 40,
    "categoria": "arabes"
  },
  {
    "id": 3,
    "house": "AFNAN",
    "name": "Afnan 9 PM",
    "image": "afnan-9pm-100ml-m.webp",
    "familia": "Floral Frutal",
    "tipo": "Eau de Parfum",
    "genero": "Masculino",
    "descripcion": "Un perfume para el hombre de perfil oriental vainilla, moderno y muy seductor, con un equilibrio entre frescura frutal y una estela cálida y dulce. Afnan 9 PM se inspira claramente en el estilo de Ultra Male de Jean Paul Gaultier, ofreciendo una propuesta juvenil, intensa y muy agradable para la noche.",
    "description": "Un perfume para el hombre de perfil oriental vainilla, moderno y muy seductor, con un equilibrio entre frescura frutal y una estela cálida y dulce. Afnan 9 PM se inspira claramente en el estilo de Ultra Male de Jean Paul Gaultier, ofreciendo una propuesta juvenil, intensa y muy agradable para la noche.",
    "notasSalida": "Bergamota, Manzana, Canela, Lavandín",
    "notasCorazon": "Lirio de los valles, Flor de azahar del naranjo, Vainilla",
    "notasFondo": "Haba tonka, Ámbar, Pachulí",
    "notes": [
      "Bergamota",
      "Manzana",
      "Canela",
      "Lavandín",
      "Lirio de los valles",
      "Flor de azahar del naranjo",
      "Vainilla",
      "Haba tonka",
      "Ámbar",
      "Pachulí"
    ],
    "ml": 100,
    "precioUSD": 40,
    "categoria": "arabes"
  },
  {
    "id": 4,
    "house": "AFNAN",
    "name": "Supremacy Collector's Edition Pour Homme",
    "image": "afnan-supremacy-collectors-edition-pour-homme-100ml-m.webp",
    "familia": "Cítrico",
    "tipo": "Eau de Parfum",
    "genero": "Masculino",
    "descripcion": "Fragancia para el hombre de carácter chipre frutal, moderna y sofisticada, con una salida fresca y vibrante que aporta presencia inmediata. Su estilo se inspira claramente en el perfil elegante y magnético de perfumes de diseñador como *Aventus*, ofreciendo una estela refinada y muy versátil para uso diario o nocturno.",
    "description": "Fragancia para el hombre de carácter chipre frutal, moderna y sofisticada, con una salida fresca y vibrante que aporta presencia inmediata. Su estilo se inspira claramente en el perfil elegante y magnético de perfumes de diseñador como *Aventus*, ofreciendo una estela refinada y muy versátil para uso diario o nocturno.",
    "notasSalida": "Piña, Bergamota, Manzana, Flores blancas",
    "notasCorazon": "Abedul, Flor de azahar del naranjo, Ámbar",
    "notasFondo": "Ámbar gris, Musgo de roble, Almizcle",
    "notes": [
      "Piña",
      "Bergamota",
      "Manzana",
      "Flores blancas",
      "Abedul",
      "Flor de azahar del naranjo",
      "Ámbar",
      "Ámbar gris",
      "Musgo de roble",
      "Almizcle"
    ],
    "ml": 100,
    "precioUSD": 60,
    "categoria": "arabes",
    "agotado": true
  },
  {
    "id": 5,
    "house": "AFNAN",
    "name": "Afnan Turathi Blue",
    "image": "afnan-turathi-blue-90ml-m.webp",
    "familia": "Cítrico",
    "tipo": "Eau de Parfum",
    "genero": "Masculino",
    "descripcion": "Una fragancia para el hombre de perfil fresco, elegante y moderno, con un carácter aromático acuático que proyecta limpieza y sofisticación. Turathi Blue de Afnan destaca por su aire cítrico y amaderado, y suele compararse con el estilo de fragancias masculinas de diseñador de corte deportivo y refinado.",
    "description": "Una fragancia para el hombre de perfil fresco, elegante y moderno, con un carácter aromático acuático que proyecta limpieza y sofisticación. Turathi Blue de Afnan destaca por su aire cítrico y amaderado, y suele compararse con el estilo de fragancias masculinas de diseñador de corte deportivo y refinado.",
    "notasSalida": "Bergamota, Mandarina, Ámbar",
    "notasCorazon": "Notas amaderadas, Almizcle",
    "notasFondo": "Pachulí, Especias frescas",
    "notes": [
      "Bergamota",
      "Mandarina",
      "Ámbar",
      "Notas amaderadas",
      "Almizcle",
      "Pachulí",
      "Especias frescas"
    ],
    "ml": 90,
    "precioUSD": 45,
    "categoria": "arabes"
  },
  {
    "id": 6,
    "house": "AL HARAMAIN",
    "name": "Al Haramain Amber Oud Carbon Edition",
    "image": "al-haramain-amber-oud-carbon-edition-60ml-u.webp",
    "familia": "Amaderado",
    "tipo": "Eau de Parfum",
    "genero": "Unisex",
    "descripcion": "Una fragancia unisex de estilo amaderado aromático, fresca, limpia y elegante, ideal para quienes buscan un perfil sofisticado con carácter moderno. Se inspira claramente en Terre d’Hermès, ofreciendo un acorde mineral y seco con una salida aromática muy refinada.",
    "description": "Una fragancia unisex de estilo amaderado aromático, fresca, limpia y elegante, ideal para quienes buscan un perfil sofisticado con carácter moderno. Se inspira claramente en Terre d’Hermès, ofreciendo un acorde mineral y seco con una salida aromática muy refinada.",
    "notasSalida": "Lavanda, Bergamota, Romero, Notas marinas",
    "notasCorazon": "Geranio, Salvia, Ámbar",
    "notasFondo": "Vetiver, Musgo, Cedro",
    "notes": [
      "Lavanda",
      "Bergamota",
      "Romero",
      "Notas marinas",
      "Geranio",
      "Salvia",
      "Ámbar",
      "Vetiver",
      "Musgo",
      "Cedro"
    ],
    "ml": 60,
    "precioUSD": 60,
    "categoria": "arabes",
    "variantIds": [
      445
    ],
    "agotado": true
  },
  {
    "id": 7,
    "house": "AL HARAMAIN",
    "name": "Al Haramain Amber Oud Dubai Night",
    "image": "al-haramain-amber-oud-dubai-night-75ml-u.webp",
    "familia": "Floral Frutal",
    "tipo": "Eau de Parfum",
    "genero": "Unisex",
    "descripcion": "Unisex y sofisticado, Amber Oud Dubai Night de Al Haramain despliega un carácter oriental amaderado con una apertura especiada y cítrica, seguida de un corazón floral-oud de gran presencia. Su perfil opulento y envolvente lo convierte en una propuesta de lujo ideal para noches especiales, con un aire de inspiración nicho que recuerda el estilo de fragancias ámbar-oud de alta gama.",
    "description": "Unisex y sofisticado, Amber Oud Dubai Night de Al Haramain despliega un carácter oriental amaderado con una apertura especiada y cítrica, seguida de un corazón floral-oud de gran presencia. Su perfil opulento y envolvente lo convierte en una propuesta de lujo ideal para noches especiales, con un aire de inspiración nicho que recuerda el estilo de fragancias ámbar-oud de alta gama.",
    "notasSalida": "Azafrán, Bergamota, Resina de elemí, Rosa búlgara",
    "notasCorazon": "Oud, Lirio de los valles, Haba tonka",
    "notasFondo": "Ámbar, Almizcle blanco, Musgo de roble",
    "notes": [
      "Azafrán",
      "Bergamota",
      "Resina de elemí",
      "Rosa búlgara",
      "Oud",
      "Lirio de los valles",
      "Haba tonka",
      "Ámbar",
      "Almizcle blanco",
      "Musgo de roble"
    ],
    "ml": 75,
    "precioUSD": 65,
    "categoria": "arabes",
    "variantIds": [
      446
    ]
  },
  {
    "id": 8,
    "house": "AL HARAMAIN",
    "name": "Al Haramain Amber Oud Gold Edition",
    "image": "al-haramain-amber-oud-gold-edition-120ml-u.webp",
    "familia": "Cítrico",
    "tipo": "Eau de Parfum",
    "genero": "Unisex",
    "descripcion": "Unisex, sofisticado y envolvente, Amber Oud Gold Edition de Al Haramain destaca por su faceta dulce-ámbar con un acabado afrutado y cremoso, ideal para quienes buscan una firma elegante y moderna. Se inspira en el estilo de fragancias de lujo tipo niche con aire adictivo, y en muchas descripciones se asocia a un perfil similar al de composiciones frutales-ambaradas de diseñador.",
    "description": "Unisex, sofisticado y envolvente, Amber Oud Gold Edition de Al Haramain destaca por su faceta dulce-ámbar con un acabado afrutado y cremoso, ideal para quienes buscan una firma elegante y moderna. Se inspira en el estilo de fragancias de lujo tipo niche con aire adictivo, y en muchas descripciones se asocia a un perfil similar al de composiciones frutales-ambaradas de diseñador.",
    "notasSalida": "Bergamota, Notas verdes, Melón",
    "notasCorazon": "Piña, Notas dulces, Ámbar",
    "notasFondo": "Almizcle, Vainilla, Notas amaderadas",
    "notes": [
      "Bergamota",
      "Notas verdes",
      "Melón",
      "Piña",
      "Notas dulces",
      "Ámbar",
      "Almizcle",
      "Vainilla",
      "Notas amaderadas"
    ],
    "ml": 120,
    "precioUSD": 80,
    "categoria": "arabes"
  },
  {
    "id": 9,
    "house": "AL HARAMAIN",
    "name": "Al Haramain Amber Oud Gold Edition",
    "image": "al-haramain-amber-oud-gold-edition-200ml-u.webp",
    "familia": "Cítrico",
    "tipo": "Eau de Parfum",
    "genero": "Unisex",
    "descripcion": "Una fragancia unisex luminosa, dulce y sofisticada, con un perfil ámbar-vainilla de gran presencia y un toque afrutado que la hace muy versátil. Al Haramain Amber Oud Gold Edition se inspira en el estilo de los perfumes de diseñador de aire opulento y moderno, con una estela cremosa y elegante ideal para quienes buscan lujo accesible.",
    "description": "Una fragancia unisex luminosa, dulce y sofisticada, con un perfil ámbar-vainilla de gran presencia y un toque afrutado que la hace muy versátil. Al Haramain Amber Oud Gold Edition se inspira en el estilo de los perfumes de diseñador de aire opulento y moderno, con una estela cremosa y elegante ideal para quienes buscan lujo accesible.",
    "notasSalida": "Bergamota, Notas verdes, Melón",
    "notasCorazon": "Piña, Notas dulces, Ámbar",
    "notasFondo": "Almizcle, Vainilla, Notas amaderadas",
    "notes": [
      "Bergamota",
      "Notas verdes",
      "Melón",
      "Piña",
      "Notas dulces",
      "Ámbar",
      "Almizcle",
      "Vainilla",
      "Notas amaderadas"
    ],
    "ml": 200,
    "precioUSD": 95,
    "categoria": "arabes"
  },
  {
    "id": 10,
    "house": "AL HARAMAIN",
    "name": "Al Haramain Amber Oud Gold Edition",
    "image": "al-haramain-amber-oud-gold-edition-60ml-u.webp",
    "familia": "Cítrico",
    "tipo": "Eau de Parfum",
    "genero": "Unisex",
    "descripcion": "Una fragancia unisex opulenta y luminosa, con un perfil oriental-vainilla de aire goloso y elegante que envuelve con sensación de lujo. Amber Oud Gold Edition de Al Haramain se inspira en el estilo de fragancias nicho modernas de gran difusión, con un carácter que puede recordar a creaciones dulces y afrutadas de diseñador.",
    "description": "Una fragancia unisex opulenta y luminosa, con un perfil oriental-vainilla de aire goloso y elegante que envuelve con sensación de lujo. Amber Oud Gold Edition de Al Haramain se inspira en el estilo de fragancias nicho modernas de gran difusión, con un carácter que puede recordar a creaciones dulces y afrutadas de diseñador.",
    "notasSalida": "Bergamota, Notas verdes, Melón",
    "notasCorazon": "Piña, Notas dulces, Ámbar",
    "notasFondo": "Almizcle, Vainilla, Notas amaderadas",
    "notes": [
      "Bergamota",
      "Notas verdes",
      "Melón",
      "Piña",
      "Notas dulces",
      "Ámbar",
      "Almizcle",
      "Vainilla",
      "Notas amaderadas"
    ],
    "ml": 60,
    "precioUSD": 55,
    "categoria": "arabes",
    "variantIds": [
      8,
      9
    ]
  },
  {
    "id": 11,
    "house": "AL HARAMAIN",
    "name": "Al Haramain Junoon Rose",
    "image": "al-haramain-junoon-rose-75ml-f.webp",
    "familia": "Floral Amaderado",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Una fragancia para la mujer de estilo oriental floral, elegante y envolvente, donde la rosa se combina con matices empolvados y un fondo cálido y aterciopelado. Junoon Rose de Al Haramain transmite una feminidad sofisticada y romántica, con un aire refinado que puede recordar al universo de perfumería de diseñador en clave árabe y lujosa.",
    "description": "Una fragancia para la mujer de estilo oriental floral, elegante y envolvente, donde la rosa se combina con matices empolvados y un fondo cálido y aterciopelado. Junoon Rose de Al Haramain transmite una feminidad sofisticada y romántica, con un aire refinado que puede recordar al universo de perfumería de diseñador en clave árabe y lujosa.",
    "notasSalida": "Bergamota, Flor de cerezo, Amaretto, Rosa",
    "notasCorazon": "Heliotropo, Gamuza, Haba tonka, Ámbar",
    "notasFondo": "Almizcle, Sándalo, Cedro",
    "notes": [
      "Bergamota",
      "Flor de cerezo",
      "Amaretto",
      "Rosa",
      "Heliotropo",
      "Gamuza",
      "Haba tonka",
      "Ámbar",
      "Almizcle",
      "Sándalo",
      "Cedro"
    ],
    "ml": 75,
    "precioUSD": 65,
    "categoria": "arabes"
  },
  {
    "id": 12,
    "house": "ANTONIO BANDERAS",
    "name": "Antonio Banderas Blue Seduction Woman",
    "image": "antonio-banderas-blue-seduction-200ml-f.webp",
    "familia": "Cítrico",
    "tipo": "Eau de Toilette",
    "genero": "Femenino",
    "descripcion": "Una fragancia fresca y seductora para el hombre, con un carácter acuático-frutal que aporta sensación de limpieza, energía y magnetismo. Blue Seduction de Antonio Banderas se inspira en un estilo moderno, casual y atractivo, muy en la línea de un perfume de diseñador para el día a día.",
    "description": "Una fragancia fresca y seductora para el hombre, con un carácter acuático-frutal que aporta sensación de limpieza, energía y magnetismo. Blue Seduction de Antonio Banderas se inspira en un estilo moderno, casual y atractivo, muy en la línea de un perfume de diseñador para el día a día.",
    "notasSalida": "Melón, Bergamota, Menta, Grosella negra",
    "notasCorazon": "Agua de mar, Manzana verde, Capuchino, Cardamomo",
    "notasFondo": "Nuez moscada, Ámbar, Notas amaderadas",
    "notes": [
      "Melón",
      "Bergamota",
      "Menta",
      "Grosella negra",
      "Agua de mar",
      "Manzana verde",
      "Capuchino",
      "Cardamomo",
      "Nuez moscada",
      "Ámbar",
      "Notas amaderadas"
    ],
    "ml": 200,
    "precioUSD": 35,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 13,
    "house": "ANTONIO BANDERAS",
    "name": "Antonio Banderas Her Golden Secret",
    "image": "antonio-banderas-her-golden-secret-80ml-f.webp",
    "familia": "Floral Frutal",
    "tipo": "Eau de Toilette",
    "genero": "Femenino",
    "descripcion": "Una fragancia para la mujer sensual y envolvente, de estilo oriental floral con un toque frutal elegante y muy seductor. *Her Golden Secret* de Antonio Banderas destaca por su carácter misterioso y femenino, con una firma olfativa que recuerda a un perfume de diseñador por su mezcla de gardenia, vainilla y maderas cálidas.",
    "description": "Una fragancia para la mujer sensual y envolvente, de estilo oriental floral con un toque frutal elegante y muy seductor. *Her Golden Secret* de Antonio Banderas destaca por su carácter misterioso y femenino, con una firma olfativa que recuerda a un perfume de diseñador por su mezcla de gardenia, vainilla y maderas cálidas.",
    "notasSalida": "Bergamota, Mandarina, Manzana, Melocotón",
    "notasCorazon": "Grosella negra, Gardenia, Flor de azahar del naranjo, Pachulí",
    "notasFondo": "Cedro, Vainilla, Haba tonka, Almizcle",
    "notes": [
      "Bergamota",
      "Mandarina",
      "Manzana",
      "Melocotón",
      "Grosella negra",
      "Gardenia",
      "Flor de azahar del naranjo",
      "Pachulí",
      "Cedro",
      "Vainilla",
      "Haba tonka",
      "Almizcle"
    ],
    "ml": 80,
    "precioUSD": 25,
    "categoria": "disenador"
  },
  {
    "id": 14,
    "house": "ANTONIO BANDERAS",
    "name": "Antonio Banderas Her Secret Absolu",
    "image": "antonio-banderas-her-secret-absolu-80ml-f.webp",
    "familia": "Floral Frutal",
    "tipo": "Eau de Toilette",
    "genero": "Femenino",
    "descripcion": "Una fragancia sofisticada y sensual para la mujer, con un carácter envolvente y femenino que combina dulzura, elegancia y un halo nocturno. Her Secret Absolu de Antonio Banderas se presenta como un perfume de estilo oriental-gourmand, pensado para quienes buscan una estela moderna y seductora, con inspiración en el universo de perfumes comerciales femeninos de diseñador.",
    "description": "Una fragancia sofisticada y sensual para la mujer, con un carácter envolvente y femenino que combina dulzura, elegancia y un halo nocturno. Her Secret Absolu de Antonio Banderas se presenta como un perfume de estilo oriental-gourmand, pensado para quienes buscan una estela moderna y seductora, con inspiración en el universo de perfumes comerciales femeninos de diseñador.",
    "notasSalida": "Ciruela, Naranja amarga, Jengibre",
    "notasCorazon": "Rosa, Jazmín, Tuberosa",
    "notasFondo": "Vainilla, Almizcle, Pachulí",
    "notes": [
      "Ciruela",
      "Naranja amarga",
      "Jengibre",
      "Rosa",
      "Jazmín",
      "Tuberosa",
      "Vainilla",
      "Almizcle",
      "Pachulí"
    ],
    "ml": 80,
    "precioUSD": 25,
    "categoria": "disenador"
  },
  {
    "id": 15,
    "house": "ANTONIO BANDERAS",
    "name": "Antonio Banderas Her Secret Desire",
    "image": "antonio-banderas-her-secret-desire-80ml-f.webp",
    "familia": "Floral Frutal",
    "tipo": "Eau de Toilette",
    "genero": "Femenino",
    "descripcion": "Un eau de toilette para la mujer de carácter floral frutal con un toque oriental, pensado para una feminidad seductora, dulce y moderna. Su estilo evocador recuerda a una fragancia de diseñador de inspiración gourmand-floral, con un aura íntima y elegante para el día a día o para la noche.",
    "description": "Un eau de toilette para la mujer de carácter floral frutal con un toque oriental, pensado para una feminidad seductora, dulce y moderna. Su estilo evocador recuerda a una fragancia de diseñador de inspiración gourmand-floral, con un aura íntima y elegante para el día a día o para la noche.",
    "notasSalida": "Frambuesa, Bergamota, Violeta",
    "notasCorazon": "Raíz de lirio, Jazmín de la India, Néroli",
    "notasFondo": "Vainilla, Sándalo, Abeto balsámico",
    "notes": [
      "Frambuesa",
      "Bergamota",
      "Violeta",
      "Raíz de lirio",
      "Jazmín de la India",
      "Néroli",
      "Vainilla",
      "Sándalo",
      "Abeto balsámico"
    ],
    "ml": 80,
    "precioUSD": 25,
    "categoria": "disenador"
  },
  {
    "id": 16,
    "house": "ANTONIO BANDERAS",
    "name": "Antonio Banderas Her Secret Temptation",
    "image": "antonio-banderas-her-secret-temptation-80ml-f.webp",
    "familia": "Floral",
    "tipo": "Eau de Toilette",
    "genero": "Femenino",
    "descripcion": "Una fragancia para la mujer que envuelve con una sensualidad elegante y un carácter seductor, pensada para dejar una estela dulce, floral y sofisticada. Se inspira en la faceta más femenina y atractiva de la línea Secret de Antonio Banderas, con un aire de tentación moderna y nocturna.",
    "description": "Una fragancia para la mujer que envuelve con una sensualidad elegante y un carácter seductor, pensada para dejar una estela dulce, floral y sofisticada. Se inspira en la faceta más femenina y atractiva de la línea Secret de Antonio Banderas, con un aire de tentación moderna y nocturna.",
    "notasSalida": "Pimienta rosa, neroli, rosa",
    "notasCorazon": "iris, jazmín, ámbar",
    "notasFondo": "pachulí, azúcar, almizcle",
    "notes": [
      "Pimienta rosa",
      "neroli",
      "rosa",
      "iris",
      "jazmín",
      "ámbar",
      "pachulí",
      "azúcar",
      "almizcle"
    ],
    "ml": 80,
    "precioUSD": 25,
    "categoria": "disenador"
  },
  {
    "id": 17,
    "house": "ARMAF",
    "name": "Armaf Aura",
    "image": "armaf-aura-100ml-m.webp",
    "familia": "Floral Frutal",
    "tipo": "Eau de Parfum",
    "genero": "Masculino",
    "descripcion": "Una fragancia para el hombre fresca, especiada y elegante, pensada para destacar con un carácter moderno y versátil. Su estilo olfativo combina cítricos vibrantes, acordes aromáticos y una base amaderada cálida; a menudo se compara con perfumes masculinos de diseñador de perfil fresco y refinado.",
    "description": "Una fragancia para el hombre fresca, especiada y elegante, pensada para destacar con un carácter moderno y versátil. Su estilo olfativo combina cítricos vibrantes, acordes aromáticos y una base amaderada cálida; a menudo se compara con perfumes masculinos de diseñador de perfil fresco y refinado.",
    "notasSalida": "Bergamota, Cítricos, Cardamomo, Manzana, Lirio",
    "notasCorazon": "Violeta, Pimienta, Lavanda, Geranio",
    "notasFondo": "Vetiver, Sándalo, Almizcle, Ámbar",
    "notes": [
      "Bergamota",
      "Cítricos",
      "Cardamomo",
      "Manzana",
      "Lirio",
      "Violeta",
      "Pimienta",
      "Lavanda",
      "Geranio",
      "Vetiver",
      "Sándalo",
      "Almizcle",
      "Ámbar"
    ],
    "ml": 100,
    "precioUSD": 40,
    "categoria": "arabes"
  },
  {
    "id": 18,
    "house": "ARMAF",
    "name": "Armaf Checkmate Queen",
    "image": "armaf-checkmate-queen-100ml-f.webp",
    "familia": "Floral Amaderado",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Armaf Checkmate Queen es una fragancia oriental floral para la mujer, con un carácter elegante, moderno y sensual. Se inspira en un estilo sofisticado tipo diseñador, con un aire refinado y envolvente pensado para destacar con presencia y confianza.",
    "description": "Armaf Checkmate Queen es una fragancia oriental floral para la mujer, con un carácter elegante, moderno y sensual. Se inspira en un estilo sofisticado tipo diseñador, con un aire refinado y envolvente pensado para destacar con presencia y confianza.",
    "notasSalida": "Nardo, Notas verdes",
    "notasCorazon": "Higo, Especias",
    "notasFondo": "Sándalo, Notas empolvadas",
    "notes": [
      "Nardo",
      "Notas verdes",
      "Higo",
      "Especias",
      "Sándalo",
      "Notas empolvadas"
    ],
    "ml": 100,
    "precioUSD": 55,
    "categoria": "arabes",
    "agotado": true
  },
  {
    "id": 19,
    "house": "ARMAF",
    "name": "Armaf Club de Nuit Blue Iconic",
    "image": "armaf-club-de-nuit-blue-iconic-105ml-m.webp",
    "familia": "Floral Amaderado",
    "tipo": "Eau de Parfum",
    "genero": "Masculino",
    "descripcion": "Armaf Club de Nuit Blue Iconic es una fragancia masculina audaz y sofisticada, inspirada en el icónico Bleu de Chanel. Un aroma fresco y amaderado que irradia confianza y deja una estela inolvidable.",
    "description": "Armaf Club de Nuit Blue Iconic es una fragancia masculina audaz y sofisticada, inspirada en el icónico Bleu de Chanel. Un aroma fresco y amaderado que irradia confianza y deja una estela inolvidable.",
    "notasSalida": "Pomelo, Limón, Menta, Pimienta Rosa, Jengibre",
    "notasCorazon": "Nuez Moscada, Jazmín, Iso E Super, Incienso, Vetiver",
    "notasFondo": "Cedro, Sándalo, Pachulí, Ámbar",
    "notes": [
      "Pomelo",
      "Limón",
      "Menta",
      "Pimienta Rosa",
      "Jengibre",
      "Nuez Moscada",
      "Jazmín",
      "Iso E Super",
      "Incienso",
      "Vetiver",
      "Cedro",
      "Sándalo",
      "Pachulí",
      "Ámbar"
    ],
    "ml": 105,
    "precioUSD": 55,
    "categoria": "arabes",
    "agotado": true
  },
  {
    "id": 20,
    "house": "ARMAF",
    "name": "Armaf Club de Nuit Intense",
    "image": "armaf-club-de-nuit-intense-105ml-m.webp",
    "familia": "Floral Frutal",
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": "Club de Nuit Intense es un perfume seductor diseñado para el hombre moderno que busca destacar con un aroma intenso y sofisticado. Inspirado en el legendario Aventus de Creed, este fragancia combina notas audaces y elegantes que transmiten poder y confianza masculina. Un perfume perfecto para ocasiones formales y eventos nocturnos.",
    "description": "Club de Nuit Intense es un perfume seductor diseñado para el hombre moderno que busca destacar con un aroma intenso y sofisticado. Inspirado en el legendario Aventus de Creed, este fragancia combina notas audaces y elegantes que transmiten poder y confianza masculina. Un perfume perfecto para ocasiones formales y eventos nocturnos.",
    "notasSalida": "Limón, Piña, Jazmín",
    "notasCorazon": "Birch, Pachulí, Almizcle",
    "notasFondo": "Amaderada, Vainilla, Musgo de roble",
    "notes": [
      "Limón",
      "Piña",
      "Jazmín",
      "Birch",
      "Pachulí",
      "Almizcle",
      "Amaderada",
      "Vainilla",
      "Musgo de roble"
    ],
    "ml": 105,
    "precioUSD": 45,
    "categoria": "arabes"
  },
  {
    "id": 21,
    "house": "ARMAF",
    "name": "Armaf Club de Nuit Maleka",
    "image": "armaf-club-de-nuit-maleka-105ml-f.webp",
    "familia": "Floral Frutal",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Una fragancia para la mujer de estilo floral afrutado, luminosa y sofisticada, con un aire moderno y seductor. Se inspira en el universo de los perfumes de diseñador de perfil elegante y contemporáneo, aportando una presencia refinada con un toque dulce, misterioso y envolvente.",
    "description": "Una fragancia para la mujer de estilo floral afrutado, luminosa y sofisticada, con un aire moderno y seductor. Se inspira en el universo de los perfumes de diseñador de perfil elegante y contemporáneo, aportando una presencia refinada con un toque dulce, misterioso y envolvente.",
    "notasSalida": "Bergamota, Lichi, Pimienta rosa",
    "notasCorazon": "Raíz de iris, Ambroxan",
    "notasFondo": "Sándalo, Praliné",
    "notes": [
      "Bergamota",
      "Lichi",
      "Pimienta rosa",
      "Raíz de iris",
      "Ambroxan",
      "Sándalo",
      "Praliné"
    ],
    "ml": 105,
    "precioUSD": 50,
    "categoria": "arabes",
    "agotado": true
  },
  {
    "id": 22,
    "house": "ARMAF",
    "name": "Armaf Club de Nuit Man",
    "image": "armaf-club-de-nuit-man-105ml-m.webp",
    "familia": "Floral Frutal",
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": "Club de Nuit Man es una fragancia masculina audaz y seductora que irradia confianza y sofisticación. Inspirada en el legendario Creed Aventus, esta creación ofrece una experiencia olfativa cítrica y amaderada inolvidable, perfecta para el hombre moderno y ambicioso.",
    "description": "Club de Nuit Man es una fragancia masculina audaz y seductora que irradia confianza y sofisticación. Inspirada en el legendario Creed Aventus, esta creación ofrece una experiencia olfativa cítrica y amaderada inolvidable, perfecta para el hombre moderno y ambicioso.",
    "notasSalida": "Limón, Grosellas negras, Manzana, Bergamota",
    "notasCorazon": "Piña, Jazmín, Abedul, Rosa",
    "notasFondo": "Pachulí, Ámbar gris, Vainilla, Almizcle",
    "notes": [
      "Limón",
      "Grosellas negras",
      "Manzana",
      "Bergamota",
      "Piña",
      "Jazmín",
      "Abedul",
      "Rosa",
      "Pachulí",
      "Ámbar gris",
      "Vainilla",
      "Almizcle"
    ],
    "ml": 105,
    "precioUSD": 40,
    "categoria": "arabes"
  },
  {
    "id": 23,
    "house": "ARMAF",
    "name": "Armaf Club de Nuit Milestone",
    "image": "armaf-club-de-nuit-milestone-105ml-u.webp",
    "familia": "Amaderado",
    "tipo": "Eau de Parfum",
    "genero": "Unisex",
    "descripcion": "Una fragancia unisex de aura elegante y contemporánea, con un estilo fresco, almizclado y amaderado que transmite sofisticación discreta. Está inspirada en el perfil de lujo acuático y limpio de Creed Millésime Imperial, ideal para quienes buscan una estela refinada y versátil.",
    "description": "Una fragancia unisex de aura elegante y contemporánea, con un estilo fresco, almizclado y amaderado que transmite sofisticación discreta. Está inspirada en el perfil de lujo acuático y limpio de Creed Millésime Imperial, ideal para quienes buscan una estela refinada y versátil.",
    "notasSalida": "Notas marinas, Frutas rojas, Bergamota",
    "notasCorazon": "Violeta, Maderas blancas, Sándalo",
    "notasFondo": "Almizcle, Ambroxan, Vetiver",
    "notes": [
      "Notas marinas",
      "Frutas rojas",
      "Bergamota",
      "Violeta",
      "Maderas blancas",
      "Sándalo",
      "Almizcle",
      "Ambroxan",
      "Vetiver"
    ],
    "ml": 105,
    "precioUSD": 50,
    "categoria": "arabes"
  },
  {
    "id": 24,
    "house": "ARMAF",
    "name": "Armaf Club de Nuit Précieux I",
    "image": "armaf-club-de-nuit-precieux-i-55ml-u.webp",
    "familia": "Floral Frutal",
    "tipo": "Eau de Parfum",
    "genero": "Unisex",
    "descripcion": "Unisex y sofisticado, este extrait de parfum envuelve con un estilo oriental amaderado de gran presencia, pensado para quienes buscan una estela elegante y moderna. Club de Nuit Precieux I de Armaf se inspira en el universo de lujo contemporáneo y recuerda el carácter vibrante y adictivo de fragancias tipo Aventus, con un giro más cálido y cremoso.",
    "description": "Unisex y sofisticado, este extrait de parfum envuelve con un estilo oriental amaderado de gran presencia, pensado para quienes buscan una estela elegante y moderna. Club de Nuit Precieux I de Armaf se inspira en el universo de lujo contemporáneo y recuerda el carácter vibrante y adictivo de fragancias tipo Aventus, con un giro más cálido y cremoso.",
    "notasSalida": "Bergamota, Caramelo, Limón, Pera, Pimienta negra, Pimienta rosa, Piña",
    "notasCorazon": "Musgo de roble, Madera blanca, Anís, Jazmín, Lirio de los valles, Ambroxan",
    "notasFondo": "Vainilla, Ámbar, Almizcle blanco, Cuero, Pachulí, Cedro",
    "notes": [
      "Bergamota",
      "Caramelo",
      "Limón",
      "Pera",
      "Pimienta negra",
      "Pimienta rosa",
      "Piña",
      "Musgo de roble",
      "Madera blanca",
      "Anís",
      "Jazmín",
      "Lirio de los valles",
      "Ambroxan",
      "Vainilla",
      "Ámbar",
      "Almizcle blanco",
      "Cuero",
      "Pachulí",
      "Cedro"
    ],
    "ml": 55,
    "precioUSD": 75,
    "categoria": "arabes"
  },
  {
    "id": 25,
    "house": "ARMAF",
    "name": "Armaf Club de Nuit Sillage",
    "image": "armaf-club-de-nuit-sillage-105ml-u.webp",
    "familia": "Floral Amaderado",
    "tipo": "Eau de Parfum",
    "genero": "Unisex",
    "descripcion": null,
    "description": null,
    "notasSalida": "Bergamota, Grosella negra, Limón, Lima, Jengibre",
    "notasCorazon": "Hojas de violeta, Rosa, Iris, Jazmín",
    "notasFondo": "Ambroxan, Almizcle, Sándalo, Cedro",
    "notes": [
      "Bergamota",
      "Grosella negra",
      "Limón",
      "Lima",
      "Jengibre",
      "Hojas de violeta",
      "Rosa",
      "Iris",
      "Jazmín",
      "Ambroxan",
      "Almizcle",
      "Sándalo",
      "Cedro"
    ],
    "ml": 105,
    "precioUSD": 50,
    "categoria": "arabes",
    "agotado": true
  },
  {
    "id": 26,
    "house": "ARMAF",
    "name": "Armaf Club de Nuit Urban Elixir",
    "image": "armaf-club-de-nuit-urban-elixir-105ml-m.webp",
    "familia": "Floral Amaderado",
    "tipo": "Elixir",
    "genero": "Masculino",
    "descripcion": "Un perfume para el hombre intenso y moderno, con un perfil ámbar especiado-aromático de gran presencia y elegancia urbana. Se inspira claramente en el estilo de Yves Saint Laurent Y EDP, ofreciendo una estela magnética, fresca al inicio y profunda en el secado.",
    "description": "Un perfume para el hombre intenso y moderno, con un perfil ámbar especiado-aromático de gran presencia y elegancia urbana. Se inspira claramente en el estilo de Yves Saint Laurent Y EDP, ofreciendo una estela magnética, fresca al inicio y profunda en el secado.",
    "notasSalida": "Bergamota, Pimienta rosa, Jazmín, Flor de azahar, Lavanda",
    "notasCorazon": "Geranio, Elemí, Vetiver, Azafrán, Ambroxan",
    "notasFondo": "Ámbar, Pachulí, Cedro, Ládano",
    "notes": [
      "Bergamota",
      "Pimienta rosa",
      "Jazmín",
      "Flor de azahar",
      "Lavanda",
      "Geranio",
      "Elemí",
      "Vetiver",
      "Azafrán",
      "Ambroxan",
      "Ámbar",
      "Pachulí",
      "Cedro",
      "Ládano"
    ],
    "ml": 105,
    "precioUSD": 50,
    "categoria": "arabes"
  },
  {
    "id": 27,
    "house": "ARMAF",
    "name": "Armaf Connoisseur Man",
    "image": "armaf-connoisseur-man-100ml-m.webp",
    "familia": "Oriental",
    "tipo": "Eau de Parfum",
    "genero": "Masculino",
    "descripcion": "Un perfume para el hombre con un carácter elegante, fresco y sofisticado, ideal para quienes buscan presencia con un toque moderno. Su perfil oriental floral combina una salida vibrante y aromática con un fondo amaderado e incienso, evocando el estilo de un diseñador contemporáneo con guiños a fragancias masculinas refinadas y versátiles.",
    "description": "Un perfume para el hombre con un carácter elegante, fresco y sofisticado, ideal para quienes buscan presencia con un toque moderno. Su perfil oriental floral combina una salida vibrante y aromática con un fondo amaderado e incienso, evocando el estilo de un diseñador contemporáneo con guiños a fragancias masculinas refinadas y versátiles.",
    "notasSalida": "Aldehídos, Manzana verde, Pomelo, Jengibre",
    "notasCorazon": "Geranio, Lavanda, Salvia, Incienso de olíbano",
    "notasFondo": "Haba tonka, Pachulí, Cedro",
    "notes": [
      "Aldehídos",
      "Manzana verde",
      "Pomelo",
      "Jengibre",
      "Geranio",
      "Lavanda",
      "Salvia",
      "Incienso de olíbano",
      "Haba tonka",
      "Pachulí",
      "Cedro"
    ],
    "ml": 100,
    "precioUSD": 50,
    "categoria": "arabes"
  },
  {
    "id": 28,
    "house": "ARMAF",
    "name": "Armaf Connoisseur Women",
    "image": "armaf-connoisseur-women-100ml-f.webp",
    "familia": "Floral Frutal",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Un perfume para la mujer que combina frescura cítrica, florales elegantes y un fondo cálido y envolvente. Connoisseur Women de Armaf se inspira en el estilo de Prada Paradoxe, ofreciendo una estela sofisticada, moderna y muy femenina.",
    "description": "Un perfume para la mujer que combina frescura cítrica, florales elegantes y un fondo cálido y envolvente. Connoisseur Women de Armaf se inspira en el estilo de Prada Paradoxe, ofreciendo una estela sofisticada, moderna y muy femenina.",
    "notasSalida": "Mandarina, bergamota, pera, flor de azahar del naranjo",
    "notasCorazon": "jazmín sambac, neroli, almizcle blanco, vainilla",
    "notasFondo": "benjuí, ámbar, haba tonka",
    "notes": [
      "Mandarina",
      "bergamota",
      "pera",
      "flor de azahar del naranjo",
      "jazmín sambac",
      "neroli",
      "almizcle blanco",
      "vainilla",
      "benjuí",
      "ámbar",
      "haba tonka"
    ],
    "ml": 100,
    "precioUSD": 45,
    "categoria": "arabes"
  },
  {
    "id": 29,
    "house": "ARMAF",
    "name": "Armaf El Cielo Pour Homme",
    "image": "armaf-el-cielo-pour-homme-100ml-m.webp",
    "familia": "Frutal",
    "tipo": "Eau de Parfum",
    "genero": "Masculino",
    "descripcion": "Fragancia para el hombre de estilo cítrico aromático, fresca y elegante, pensada para un perfil moderno y versátil. Su carácter recuerda al ADN de Invictus por su combinación de frescura frutal, matices acuáticos y un fondo cálido y amaderado.",
    "description": "Fragancia para el hombre de estilo cítrico aromático, fresca y elegante, pensada para un perfil moderno y versátil. Su carácter recuerda al ADN de Invictus por su combinación de frescura frutal, matices acuáticos y un fondo cálido y amaderado.",
    "notasSalida": "Melón, Manzana, Bergamota, Limón",
    "notasCorazon": "Lavanda, Violeta, Ciprés, Ámbar",
    "notasFondo": "Almizcle, Vainilla, Cedro",
    "notes": [
      "Melón",
      "Manzana",
      "Bergamota",
      "Limón",
      "Lavanda",
      "Violeta",
      "Ciprés",
      "Ámbar",
      "Almizcle",
      "Vainilla",
      "Cedro"
    ],
    "ml": 100,
    "precioUSD": 35,
    "categoria": "arabes"
  },
  {
    "id": 30,
    "house": "ARMAF",
    "name": "Armaf Hunter",
    "image": "armaf-hunter-100ml-m.webp",
    "familia": "Oriental",
    "tipo": "Eau de Parfum",
    "genero": "Masculino",
    "descripcion": "Fragancia para el hombre de carácter fresco, aromático y amaderado, con un perfil elegante que combina energía cítrica con un fondo cálido y refinado. Armaf Hunter Men se inspira en el estilo de los grandes perfumes masculinos de diseñador, con un aire sofisticado y versátil para el día a día.",
    "description": "Fragancia para el hombre de carácter fresco, aromático y amaderado, con un perfil elegante que combina energía cítrica con un fondo cálido y refinado. Armaf Hunter Men se inspira en el estilo de los grandes perfumes masculinos de diseñador, con un aire sofisticado y versátil para el día a día.",
    "notasSalida": "Pomelo, Agujas de cedro, Romero, Cardamomo",
    "notasCorazon": "Ylang-ylang, Nardos, Incienso",
    "notasFondo": "Cedro, Sándalo, Vetiver",
    "notes": [
      "Pomelo",
      "Agujas de cedro",
      "Romero",
      "Cardamomo",
      "Ylang-ylang",
      "Nardos",
      "Incienso",
      "Cedro",
      "Sándalo",
      "Vetiver"
    ],
    "ml": 100,
    "precioUSD": 35,
    "categoria": "arabes"
  },
  {
    "id": 31,
    "house": "ARMAF",
    "name": "Armaf Hunter Intense",
    "image": "armaf-hunter-intense-100ml-m.webp",
    "familia": "Cítrico",
    "tipo": "Eau de Parfum",
    "genero": "Masculino",
    "descripcion": "Una fragancia para el hombre de carácter fresco, aromático y amaderado, creada para transmitir energía, presencia y elegancia moderna. Armaf Hunter Intense se inspira en el estilo vibrante de perfumes deportivos de diseñador, con un perfil que recuerda a combinaciones como Invictus Aqua y Dior Sauvage por su equilibrio entre frescura cítrica y fondo especiado.",
    "description": "Una fragancia para el hombre de carácter fresco, aromático y amaderado, creada para transmitir energía, presencia y elegancia moderna. Armaf Hunter Intense se inspira en el estilo vibrante de perfumes deportivos de diseñador, con un perfil que recuerda a combinaciones como Invictus Aqua y Dior Sauvage por su equilibrio entre frescura cítrica y fondo especiado.",
    "notasSalida": "Bergamota, Limón, Mandarina, Pomelo",
    "notasCorazon": "Lavanda, Ylang-ylang, Nardos",
    "notasFondo": "Geranio, Pimienta negra, Cardamomo",
    "notes": [
      "Bergamota",
      "Limón",
      "Mandarina",
      "Pomelo",
      "Lavanda",
      "Ylang-ylang",
      "Nardos",
      "Geranio",
      "Pimienta negra",
      "Cardamomo"
    ],
    "ml": 100,
    "precioUSD": 35,
    "categoria": "arabes"
  },
  {
    "id": 32,
    "house": "ARMAF",
    "name": "Armaf Island Bliss",
    "image": "armaf-island-bliss-100ml-f.webp",
    "familia": "Floral Frutal",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Una fragancia para la mujer de estilo floral-frutal gourmand, luminosa y tropical, con un aire dulce y cremoso que transmite frescura y sofisticación. Se inspira en el universo de perfumes femeninos veraniegos y evoca una sensación similar a propuestas de diseñador con coco, flores blancas y vainilla.",
    "description": "Una fragancia para la mujer de estilo floral-frutal gourmand, luminosa y tropical, con un aire dulce y cremoso que transmite frescura y sofisticación. Se inspira en el universo de perfumes femeninos veraniegos y evoca una sensación similar a propuestas de diseñador con coco, flores blancas y vainilla.",
    "notasSalida": "Bayas silvestres, Notas verdes, Bergamota, Naranja, Cereza",
    "notasCorazon": "Flor de azahar, Lirio de agua, Coco, Flores blancas",
    "notasFondo": "Haba tonka, Vainilla, Ámbar, Almizcle",
    "notes": [
      "Bayas silvestres",
      "Notas verdes",
      "Bergamota",
      "Naranja",
      "Cereza",
      "Flor de azahar",
      "Lirio de agua",
      "Coco",
      "Flores blancas",
      "Haba tonka",
      "Vainilla",
      "Ámbar",
      "Almizcle"
    ],
    "ml": 100,
    "precioUSD": 60,
    "categoria": "arabes"
  },
  {
    "id": 33,
    "house": "ARMAF",
    "name": "Armaf La Rosa",
    "image": "armaf-la-rosa-100ml-f.webp",
    "familia": "Floral Frutal",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Un perfume para la mujer elegante y romántica, de perfil floral-frutal con un fondo dulce y cremoso que aporta sofisticación y calidez. Se inspira en el estilo de los grandes florales modernos de diseñador, con un aura femenina, luminosa y envolvente.",
    "description": "Un perfume para la mujer elegante y romántica, de perfil floral-frutal con un fondo dulce y cremoso que aporta sofisticación y calidez. Se inspira en el estilo de los grandes florales modernos de diseñador, con un aura femenina, luminosa y envolvente.",
    "notasSalida": "Pera, Grosellas negras, Pimienta rosa, Bergamota, Flor de azahar del naranjo",
    "notasCorazon": "Nardos, Jazmín, Mimosa, Ylang-ylang, Avellana",
    "notasFondo": "Praliné, Pachulí, Haba tonka, Vetiver",
    "notes": [
      "Pera",
      "Grosellas negras",
      "Pimienta rosa",
      "Bergamota",
      "Flor de azahar del naranjo",
      "Nardos",
      "Jazmín",
      "Mimosa",
      "Ylang-ylang",
      "Avellana",
      "Praliné",
      "Pachulí",
      "Haba tonka",
      "Vetiver"
    ],
    "ml": 100,
    "precioUSD": 35,
    "categoria": "arabes"
  },
  {
    "id": 34,
    "house": "ARMAF",
    "name": "Armaf Odyssey Aoud",
    "image": "armaf-odyssey-aoud-100ml-m.webp",
    "familia": "Aromático",
    "tipo": "Eau de Parfum",
    "genero": "Masculino",
    "descripcion": "Una fragancia intensa y elegante para el hombre, de carácter amaderado especiado con un acorde protagonista de oud oscuro y sofisticado. Su estilo evoca la riqueza de ciertos perfumes de diseñador de oud moderno, con un acabado cálido, sensual y muy masculino.",
    "description": "Una fragancia intensa y elegante para el hombre, de carácter amaderado especiado con un acorde protagonista de oud oscuro y sofisticado. Su estilo evoca la riqueza de ciertos perfumes de diseñador de oud moderno, con un acabado cálido, sensual y muy masculino.",
    "notasSalida": "Lavanda, Nuez moscada, Azafrán",
    "notasCorazon": "Oud, Notas amaderadas, Ambroxan",
    "notasFondo": "Almizcle, Pachulí",
    "notes": [
      "Lavanda",
      "Nuez moscada",
      "Azafrán",
      "Oud",
      "Notas amaderadas",
      "Ambroxan",
      "Almizcle",
      "Pachulí"
    ],
    "ml": 100,
    "precioUSD": 35,
    "categoria": "arabes"
  },
  {
    "id": 35,
    "house": "ARMAF",
    "name": "Armaf Odyssey Artisto",
    "image": "armaf-odyssey-artisto-100ml-u.webp",
    "familia": "Floral Amaderado",
    "tipo": "Eau de Parfum",
    "genero": "Unisex",
    "descripcion": "Odyssey Artisto es un perfume de carácter versátil y sofisticado, diseñado para aquellos que buscan una fragancia moderna y provocativa. Su aroma audaz transita entre lo clásico y lo contemporáneo, recordando sutilmente las elegantes creaciones de fragancias de nicho internacional. Un perfume que desafía los límites de género y se adapta a personalidades únicas.",
    "description": "Odyssey Artisto es un perfume de carácter versátil y sofisticado, diseñado para aquellos que buscan una fragancia moderna y provocativa. Su aroma audaz transita entre lo clásico y lo contemporáneo, recordando sutilmente las elegantes creaciones de fragancias de nicho internacional. Un perfume que desafía los límites de género y se adapta a personalidades únicas.",
    "notasSalida": "Bergamota, Pimienta rosa, Jengibre",
    "notasCorazon": "Madera de cedro, Vetiver",
    "notasFondo": "Ámbar, Almizcle",
    "notes": [
      "Bergamota",
      "Pimienta rosa",
      "Jengibre",
      "Madera de cedro",
      "Vetiver",
      "Ámbar",
      "Almizcle"
    ],
    "ml": 100,
    "precioUSD": 50,
    "categoria": "arabes"
  },
  {
    "id": 36,
    "house": "ARMAF",
    "name": "Armaf Odyssey Bahamas",
    "image": "armaf-odyssey-ba-ha-mas-100ml-u.webp",
    "familia": "Floral Amaderado",
    "tipo": "Eau de Parfum",
    "genero": "Unisex",
    "descripcion": "Odyssey BA HA MAS es un perfume contemporáneo y versátil que desafía los límites de género tradicionales. Su composición audaz y sofisticada captura la esencia de un viaje sensorial que se adapta a personalidades modernas y cosmopolitas, inspirándose en la elegancia de fragancias de alta gama como Acqua di Gio de Giorgio Armani.",
    "description": "Odyssey BA HA MAS es un perfume contemporáneo y versátil que desafía los límites de género tradicionales. Su composición audaz y sofisticada captura la esencia de un viaje sensorial que se adapta a personalidades modernas y cosmopolitas, inspirándose en la elegancia de fragancias de alta gama como Acqua di Gio de Giorgio Armani.",
    "notasSalida": "Bergamota, Lavanda, Madera de sándalo",
    "notasCorazon": "Vetiver, Almizcle",
    "notasFondo": "Pimienta rosa, Cedro",
    "notes": [
      "Bergamota",
      "Lavanda",
      "Madera de sándalo",
      "Vetiver",
      "Almizcle",
      "Pimienta rosa",
      "Cedro"
    ],
    "ml": 100,
    "precioUSD": 60,
    "categoria": "arabes"
  },
  {
    "id": 37,
    "house": "ARMAF",
    "name": "Armaf Odyssey Black Forest",
    "image": "armaf-odyssey-black-forest-100ml-u.webp",
    "familia": "Amaderado",
    "tipo": "Eau de Parfum",
    "genero": "Unisex",
    "descripcion": "Fragancia unisex de estilo oriental vainilla, cálida y golosa, con una faceta oscura y elegante que la hace perfecta para quienes buscan un perfume envolvente y sofisticado. Se inspira en el espíritu de la Selva Negra y evoca el perfil de un postre gourmand de cereza, caramelo y vainilla, con un aire adictivo y moderno.",
    "description": "Fragancia unisex de estilo oriental vainilla, cálida y golosa, con una faceta oscura y elegante que la hace perfecta para quienes buscan un perfume envolvente y sofisticado. Se inspira en el espíritu de la Selva Negra y evoca el perfil de un postre gourmand de cereza, caramelo y vainilla, con un aire adictivo y moderno.",
    "notasSalida": "Caramelo, Cereza, Canela, Lactonas, Flores blancas",
    "notasCorazon": "Dátiles, Miel, Nuez moscada, Haba tonka",
    "notasFondo": "Vainilla, Praliné, Almizcle, Notas amaderadas",
    "notes": [
      "Caramelo",
      "Cereza",
      "Canela",
      "Lactonas",
      "Flores blancas",
      "Dátiles",
      "Miel",
      "Nuez moscada",
      "Haba tonka",
      "Vainilla",
      "Praliné",
      "Almizcle",
      "Notas amaderadas"
    ],
    "ml": 100,
    "precioUSD": 60,
    "categoria": "arabes"
  },
  {
    "id": 38,
    "house": "ARMAF",
    "name": "Armaf Odyssey Candee",
    "image": "armaf-odyssey-candee-100ml-f.webp",
    "familia": "Floral Frutal",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Una fragancia para la mujer de estilo dulce, afrutado y gourmand, con un carácter moderno, alegre y muy envolvente. Odyssey Candee de Armaf destaca por su faceta jugosa y adictiva, con un perfil que recuerda a los perfumes femeninos de diseñador de tendencia frutal-gourmand.",
    "description": "Una fragancia para la mujer de estilo dulce, afrutado y gourmand, con un carácter moderno, alegre y muy envolvente. Odyssey Candee de Armaf destaca por su faceta jugosa y adictiva, con un perfil que recuerda a los perfumes femeninos de diseñador de tendencia frutal-gourmand.",
    "notasSalida": "Fresa, Frambuesa, Melocotón, Bergamota",
    "notasCorazon": "Geranio, Jazmín, Caramelo, Maracuyá",
    "notasFondo": "Almizcle, Pachulí, Ámbar",
    "notes": [
      "Fresa",
      "Frambuesa",
      "Melocotón",
      "Bergamota",
      "Geranio",
      "Jazmín",
      "Caramelo",
      "Maracuyá",
      "Almizcle",
      "Pachulí",
      "Ámbar"
    ],
    "ml": 100,
    "precioUSD": 35,
    "categoria": "arabes"
  },
  {
    "id": 39,
    "house": "ARMAF",
    "name": "Armaf Odyssey Dubai Chocolat",
    "image": "armaf-odyssey-dubai-chocolat-unisex-100ml-u.webp",
    "familia": "Gourmand",
    "tipo": "Eau de Parfum",
    "genero": "Unisex",
    "descripcion": "Odyssey Dubai Chocolat es un perfume unisex que seduce con su aroma decadente y sofisticado, evocando la riqueza de los chocolates árabes de lujo. Su composición audaz y seductora es perfecta para amantes de fragancias gourmands que buscan una experiencia olfativa verdaderamente única y memorable.",
    "description": "Odyssey Dubai Chocolat es un perfume unisex que seduce con su aroma decadente y sofisticado, evocando la riqueza de los chocolates árabes de lujo. Su composición audaz y seductora es perfecta para amantes de fragancias gourmands que buscan una experiencia olfativa verdaderamente única y memorable.",
    "notasSalida": "Chocolate, Vainilla",
    "notasCorazon": "Almendra, Cacao",
    "notasFondo": "Caramelo",
    "notes": [
      "Chocolate",
      "Vainilla",
      "Almendra",
      "Cacao",
      "Caramelo"
    ],
    "ml": 100,
    "precioUSD": 35,
    "categoria": "arabes"
  },
  {
    "id": 40,
    "house": "ARMAF",
    "name": "Armaf Odyssey Eau de Montagne",
    "image": "armaf-odyssey-eau-de-montagne-100ml-u.webp",
    "familia": "Gourmand",
    "tipo": "Eau de Parfum",
    "genero": "Unisex",
    "descripcion": "Odyssey Eau de Montagne de Armaf es una fragancia unisex de estilo oriental vainilla, con un perfil elegante, cremoso y envolvente. Su carácter refinado combina calidez especiada y una base amaderada-almizclada, con una inspiración claramente cercana a creaciones gourmand de diseñador.",
    "description": "Odyssey Eau de Montagne de Armaf es una fragancia unisex de estilo oriental vainilla, con un perfil elegante, cremoso y envolvente. Su carácter refinado combina calidez especiada y una base amaderada-almizclada, con una inspiración claramente cercana a creaciones gourmand de diseñador.",
    "notasSalida": "Almendra, especias, lactonas",
    "notasCorazon": "flores blancas, vainilla, cacao",
    "notasFondo": "sándalo, almizcle",
    "notes": [
      "Almendra",
      "especias",
      "lactonas",
      "flores blancas",
      "vainilla",
      "cacao",
      "sándalo",
      "almizcle"
    ],
    "ml": 100,
    "precioUSD": 60,
    "categoria": "arabes"
  },
  {
    "id": 41,
    "house": "ARMAF",
    "name": "Armaf Odyssey Femme",
    "image": "armaf-odyssey-femme-100ml-f.webp",
    "familia": "Gourmand",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Una fragancia oriental floral para la mujer, elegante y seductora, con una estela cálida y sofisticada ideal para ocasiones especiales. Odyssey Femme de Armaf evoca un estilo femenino de inspiración nicho, con un carácter que puede recordar a perfumes florales gourmand de diseñador por su mezcla de flores blancas, miel y cacao.",
    "description": "Una fragancia oriental floral para la mujer, elegante y seductora, con una estela cálida y sofisticada ideal para ocasiones especiales. Odyssey Femme de Armaf evoca un estilo femenino de inspiración nicho, con un carácter que puede recordar a perfumes florales gourmand de diseñador por su mezcla de flores blancas, miel y cacao.",
    "notasSalida": "Ciruela, miel, bergamota, gardenia",
    "notasCorazon": "nardos, lirio de los valles, jazmín, rosa",
    "notasFondo": "azafrán, cacao, haba tonka, almizcle",
    "notes": [
      "Ciruela",
      "miel",
      "bergamota",
      "gardenia",
      "nardos",
      "lirio de los valles",
      "jazmín",
      "rosa",
      "azafrán",
      "cacao",
      "haba tonka",
      "almizcle"
    ],
    "ml": 100,
    "precioUSD": 35,
    "categoria": "arabes"
  },
  {
    "id": 42,
    "house": "ARMAF",
    "name": "Armaf Odyssey Go Mango",
    "image": "armaf-odyssey-go-mango-100ml-u.webp",
    "familia": "Floral Frutal",
    "tipo": "Eau de Parfum",
    "genero": "Unisex",
    "descripcion": "Odyssey Go Mango de Armaf es una fragancia unisex de estilo afrutado, tropical y gourmand, pensada para quienes buscan un aroma luminoso, moderno y adictivo. Su perfil jugoso y exótico evoca el espíritu de los perfumes de nicho tropicales, con una vibra contemporánea que puede atraer a amantes de fragancias inspiradas en el mango y los dulces cremosos.",
    "description": "Odyssey Go Mango de Armaf es una fragancia unisex de estilo afrutado, tropical y gourmand, pensada para quienes buscan un aroma luminoso, moderno y adictivo. Su perfil jugoso y exótico evoca el espíritu de los perfumes de nicho tropicales, con una vibra contemporánea que puede atraer a amantes de fragancias inspiradas en el mango y los dulces cremosos.",
    "notasSalida": "Pimienta rosa, Jengibre, Limón, Flores blancas",
    "notasCorazon": "Mango, Acorde dulce gourmand, Flor tropical, Almizcle",
    "notasFondo": "Madera, Crema, Vainilla",
    "notes": [
      "Pimienta rosa",
      "Jengibre",
      "Limón",
      "Flores blancas",
      "Mango",
      "Acorde dulce gourmand",
      "Flor tropical",
      "Almizcle",
      "Madera",
      "Crema",
      "Vainilla"
    ],
    "ml": 100,
    "precioUSD": 45,
    "categoria": "arabes"
  },
  {
    "id": 43,
    "house": "ARMAF",
    "name": "Armaf Odyssey Homme",
    "image": "armaf-odyssey-homme-100ml-m.webp",
    "familia": "Floral Amaderado",
    "tipo": "Eau de Parfum",
    "genero": "Masculino",
    "descripcion": "Armaf Odyssey Homme es una fragancia para el hombre de carácter cálido, sofisticado y seductor, con un perfil ambarado, especiado y amaderado. Su estilo envolvente recuerda a una inspiración moderna de aromas de diseñador con un toque elegante y nocturno, ideal para quien busca presencia y distinción.",
    "description": "Armaf Odyssey Homme es una fragancia para el hombre de carácter cálido, sofisticado y seductor, con un perfil ambarado, especiado y amaderado. Su estilo envolvente recuerda a una inspiración moderna de aromas de diseñador con un toque elegante y nocturno, ideal para quien busca presencia y distinción.",
    "notasSalida": "Vainilla, Ámbar, Especias, Iris",
    "notasCorazon": "Cuero, Jazmín, Sándalo, Bergamota",
    "notasFondo": "Lavanda, Vetiver, Pachulí, Cedro",
    "notes": [
      "Vainilla",
      "Ámbar",
      "Especias",
      "Iris",
      "Cuero",
      "Jazmín",
      "Sándalo",
      "Bergamota",
      "Lavanda",
      "Vetiver",
      "Pachulí",
      "Cedro"
    ],
    "ml": 100,
    "precioUSD": 40,
    "categoria": "arabes"
  },
  {
    "id": 44,
    "house": "ARMAF",
    "name": "Armaf Odyssey Homme White Edition",
    "image": "armaf-odyssey-homme-white-edition-100ml-m.webp",
    "familia": "Floral Amaderado",
    "tipo": "Eau de Parfum",
    "genero": "Masculino",
    "descripcion": "Odyssey Homme White Edition es una fragancia para el hombre moderno que busca un aroma fresco, elegante y con presencia. Su perfil ámbar fougère combina un inicio cítrico y especiado con un fondo cálido y amaderado, evocando un estilo sofisticado inspirado en perfumes de diseñador de corte masculino contemporáneo.",
    "description": "Odyssey Homme White Edition es una fragancia para el hombre moderno que busca un aroma fresco, elegante y con presencia. Su perfil ámbar fougère combina un inicio cítrico y especiado con un fondo cálido y amaderado, evocando un estilo sofisticado inspirado en perfumes de diseñador de corte masculino contemporáneo.",
    "notasSalida": "Pimienta rosa, Pomelo, Yuzu",
    "notasCorazon": "Hojas de violeta, Notas marinas, Ámbar",
    "notasFondo": "Amberwood, Madera de gaiac",
    "notes": [
      "Pimienta rosa",
      "Pomelo",
      "Yuzu",
      "Hojas de violeta",
      "Notas marinas",
      "Ámbar",
      "Amberwood",
      "Madera de gaiac"
    ],
    "ml": 100,
    "precioUSD": 40,
    "categoria": "arabes"
  },
  {
    "id": 45,
    "house": "ARMAF",
    "name": "Armaf Odyssey Limoni Fresh",
    "image": "armaf-odyssey-limoni-fresh-unisex-100ml-u.webp",
    "familia": "Floral Amaderado",
    "tipo": "Eau de Parfum",
    "genero": "Unisex",
    "descripcion": "Odyssey Limoni Fresh es una fragancia refrescante y versátil que invita a una aventura sensorial para hombres y mujeres. Con un perfil fresco y luminoso, este aroma despierta la energía y la vitalidad, inspirándose en los códigos de frescura de fragancias mediterráneas de alta gama.",
    "description": "Odyssey Limoni Fresh es una fragancia refrescante y versátil que invita a una aventura sensorial para hombres y mujeres. Con un perfil fresco y luminoso, este aroma despierta la energía y la vitalidad, inspirándose en los códigos de frescura de fragancias mediterráneas de alta gama.",
    "notasSalida": "Limón, Bergamota, Mandarina",
    "notasCorazon": "Jengibre, Pimienta rosa",
    "notasFondo": "Madera blanca, Almizcle",
    "notes": [
      "Limón",
      "Bergamota",
      "Mandarina",
      "Jengibre",
      "Pimienta rosa",
      "Madera blanca",
      "Almizcle"
    ],
    "ml": 100,
    "precioUSD": 35,
    "categoria": "arabes"
  },
  {
    "id": 46,
    "house": "ARMAF",
    "name": "Armaf Odyssey Mandarin Sky",
    "image": "armaf-odyssey-mandarin-sky-100ml-m.webp",
    "familia": "Amaderado",
    "tipo": "Eau de Parfum",
    "genero": "Masculino",
    "descripcion": "Una fragancia para el hombre de estilo cítrico-gourmand, vibrante y cálida, ideal para quien busca un aroma moderno con presencia y facilidad de uso. Odyssey Mandarin Sky de Armaf se inspira en el perfil dulce y luminoso de los diseños masculinos contemporáneos, con un aire elegante y versátil para el día a día.",
    "description": "Una fragancia para el hombre de estilo cítrico-gourmand, vibrante y cálida, ideal para quien busca un aroma moderno con presencia y facilidad de uso. Odyssey Mandarin Sky de Armaf se inspira en el perfil dulce y luminoso de los diseños masculinos contemporáneos, con un aire elegante y versátil para el día a día.",
    "notasSalida": "Mandarina, Naranja, Azafrán, Salvia",
    "notasCorazon": "Caramelo, Haba tonka, Cempasúchil",
    "notasFondo": "Ambroxan, Vetiver, Cedro",
    "notes": [
      "Mandarina",
      "Naranja",
      "Azafrán",
      "Salvia",
      "Caramelo",
      "Haba tonka",
      "Cempasúchil",
      "Ambroxan",
      "Vetiver",
      "Cedro"
    ],
    "ml": 100,
    "precioUSD": 45,
    "categoria": "arabes"
  },
  {
    "id": 47,
    "house": "ARMAF",
    "name": "Armaf Odyssey Mandarin Sky Elixir",
    "image": "armaf-odyssey-mandarin-sky-elixir-100ml-u.webp",
    "familia": "Oriental",
    "tipo": "Elixir",
    "genero": "Unisex",
    "descripcion": "Una fragancia unisex vibrante y sofisticada, de perfil oriental amaderado con un giro dulce y especiado que resulta moderno y adictivo. Su apertura cítrica y aromática recuerda a una interpretación luminosa y gourmand, con un aire que puede gustar a quienes disfrutan de perfumes de diseñador de estilo contemporáneo.",
    "description": "Una fragancia unisex vibrante y sofisticada, de perfil oriental amaderado con un giro dulce y especiado que resulta moderno y adictivo. Su apertura cítrica y aromática recuerda a una interpretación luminosa y gourmand, con un aire que puede gustar a quienes disfrutan de perfumes de diseñador de estilo contemporáneo.",
    "notasSalida": "Mandarina, Naranja, Bergamota, Lavanda",
    "notasCorazon": "Cardamomo, Pimienta negra, Caramelo, Haba tonka",
    "notasFondo": "Pachulí, Incienso, Vainilla, Vetiver",
    "notes": [
      "Mandarina",
      "Naranja",
      "Bergamota",
      "Lavanda",
      "Cardamomo",
      "Pimienta negra",
      "Caramelo",
      "Haba tonka",
      "Pachulí",
      "Incienso",
      "Vainilla",
      "Vetiver"
    ],
    "ml": 100,
    "precioUSD": 55,
    "categoria": "arabes"
  },
  {
    "id": 47,
    "house": "ARMAF",
    "name": "Armaf Odyssey Mandarin Sky Elixir",
    "image": "armaf-odyssey-mandarin-sky-elixir-100ml-u.webp",
    "familia": "Oriental",
    "tipo": "Elixir",
    "genero": "Unisex",
    "descripcion": "Una fragancia unisex vibrante y sofisticada, de perfil oriental amaderado con un giro dulce y especiado que resulta moderno y adictivo. Su apertura cítrica y aromática recuerda a una interpretación luminosa y gourmand, con un aire que puede gustar a quienes disfrutan de perfumes de diseñador de estilo contemporáneo.",
    "description": "Una fragancia unisex vibrante y sofisticada, de perfil oriental amaderado con un giro dulce y especiado que resulta moderno y adictivo. Su apertura cítrica y aromática recuerda a una interpretación luminosa y gourmand, con un aire que puede gustar a quienes disfrutan de perfumes de diseñador de estilo contemporáneo.",
    "notasSalida": "Mandarina, Naranja, Bergamota, Lavanda",
    "notasCorazon": "Cardamomo, Pimienta negra, Caramelo, Haba tonka",
    "notasFondo": "Pachulí, Incienso, Vainilla, Vetiver",
    "notes": [
      "Mandarina",
      "Naranja",
      "Bergamota",
      "Lavanda",
      "Cardamomo",
      "Pimienta negra",
      "Caramelo",
      "Haba tonka",
      "Pachulí",
      "Incienso",
      "Vainilla",
      "Vetiver"
    ],
    "ml": 100,
    "precioUSD": 55,
    "categoria": "arabes",
    "agotado": true
  },
  {
    "id": 48,
    "house": "ARMAF",
    "name": "Armaf Odyssey Mega Man",
    "image": "armaf-odyssey-mega-man-100ml-m.webp",
    "familia": "Frutal",
    "tipo": "Eau de Parfum",
    "genero": "Masculino",
    "descripcion": "Fragancia amaderada aromática para el hombre, moderna y enérgica, pensada para proyectar frescura, carácter y sofisticación en el día a día. Su perfil recuerda el estilo de un diseñador deportivo y elegante, con un aire vibrante que puede evocar propuestas tipo Bleu de Chanel por su frescura limpia y masculina.",
    "description": "Fragancia amaderada aromática para el hombre, moderna y enérgica, pensada para proyectar frescura, carácter y sofisticación en el día a día. Su perfil recuerda el estilo de un diseñador deportivo y elegante, con un aire vibrante que puede evocar propuestas tipo Bleu de Chanel por su frescura limpia y masculina.",
    "notasSalida": "Naranja, Jengibre, Bergamota, Limón, Menta",
    "notasCorazon": "Piña, Enebro de Virginia, Geranio, Salvia",
    "notasFondo": "Haba tonka, Almizcle, Cedro, Vetiver",
    "notes": [
      "Naranja",
      "Jengibre",
      "Bergamota",
      "Limón",
      "Menta",
      "Piña",
      "Enebro de Virginia",
      "Geranio",
      "Salvia",
      "Haba tonka",
      "Almizcle",
      "Cedro",
      "Vetiver"
    ],
    "ml": 100,
    "precioUSD": 35,
    "categoria": "arabes"
  },
  {
    "id": 49,
    "house": "ARMAF",
    "name": "Armaf Odyssey Revolution",
    "image": "armaf-odyssey-revolution-100ml-m.webp",
    "familia": "Gourmand",
    "tipo": "Eau de Parfum",
    "genero": "Masculino",
    "descripcion": "Fragancia para el hombre de estilo moderno, dulce y sofisticado, con una presencia intensa pensada para destacar en salidas y ocasiones especiales. Su perfil recuerda a un perfume de diseñador contemporáneo por su mezcla frutal-gourmand con fondo amaderado y especiado.",
    "description": "Fragancia para el hombre de estilo moderno, dulce y sofisticado, con una presencia intensa pensada para destacar en salidas y ocasiones especiales. Su perfil recuerda a un perfume de diseñador contemporáneo por su mezcla frutal-gourmand con fondo amaderado y especiado.",
    "notasSalida": "Piña, naranja, ciruela",
    "notasCorazon": "chocolate, vainilla, cardamomo",
    "notasFondo": "haba tonka, pachulí",
    "notes": [
      "Piña",
      "naranja",
      "ciruela",
      "chocolate",
      "vainilla",
      "cardamomo",
      "haba tonka",
      "pachulí"
    ],
    "ml": 100,
    "precioUSD": 45,
    "categoria": "arabes"
  },
  {
    "id": 50,
    "house": "ARMAF",
    "name": "Armaf Odyssey Spectra",
    "image": "armaf-odyssey-spectra-100ml-u.webp",
    "familia": "Floral Frutal",
    "tipo": "Eau de Parfum",
    "genero": "Unisex",
    "descripcion": "Armaf Odyssey Spectra es una fragancia unisex de estilo oriental especiado, con un carácter moderno, cálido y envolvente. Su perfil combina frescura frutal y especias con un fondo dulce y ambarado, ideal para quien busca una estela elegante y seductora; se inspira en el universo de los perfumes intensos y nocturnos de diseñador.",
    "description": "Armaf Odyssey Spectra es una fragancia unisex de estilo oriental especiado, con un carácter moderno, cálido y envolvente. Su perfil combina frescura frutal y especias con un fondo dulce y ambarado, ideal para quien busca una estela elegante y seductora; se inspira en el universo de los perfumes intensos y nocturnos de diseñador.",
    "notasSalida": "Bergamota, Canela, Manzana, Flor de azahar",
    "notasCorazon": "Lavanda, Lirio del valle, Haba tonka, Pachulí",
    "notasFondo": "Tabaco, Vainilla, Ámbar",
    "notes": [
      "Bergamota",
      "Canela",
      "Manzana",
      "Flor de azahar",
      "Lavanda",
      "Lirio del valle",
      "Haba tonka",
      "Pachulí",
      "Tabaco",
      "Vainilla",
      "Ámbar"
    ],
    "ml": 100,
    "precioUSD": 35,
    "categoria": "arabes",
    "agotado": true
  },
  {
    "id": 51,
    "house": "ARMAF",
    "name": "Armaf Odyssey Tyrant Special Edition",
    "image": "armaf-odyssey-tyrant-special-edition-100ml-m.webp",
    "familia": "Amaderado",
    "tipo": "Eau de Parfum",
    "genero": "Masculino",
    "descripcion": "Odyssey Tyrant es un perfume audaz y sofisticado diseñado para el hombre contemporáneo que busca destacar con un aroma poderoso y seductor. Con un carácter intenso y elegante, este perfume evoca la energía y la determinación de un conquistador moderno, inspirándose en fragancias de alta gama como las de diseñadores de lujo europeos.",
    "description": "Odyssey Tyrant es un perfume audaz y sofisticado diseñado para el hombre contemporáneo que busca destacar con un aroma poderoso y seductor. Con un carácter intenso y elegante, este perfume evoca la energía y la determinación de un conquistador moderno, inspirándose en fragancias de alta gama como las de diseñadores de lujo europeos.",
    "notasSalida": "Bergamota, Pimienta negra, Lavanda",
    "notasCorazon": "Cedro, Sándalo",
    "notasFondo": "Ámbar, Almizcle",
    "notes": [
      "Bergamota",
      "Pimienta negra",
      "Lavanda",
      "Cedro",
      "Sándalo",
      "Ámbar",
      "Almizcle"
    ],
    "ml": 100,
    "precioUSD": 35,
    "categoria": "arabes"
  },
  {
    "id": 52,
    "house": "ARMAF",
    "name": "Armaf Tag Her",
    "image": "armaf-tag-her-100ml-f.webp",
    "familia": "Floral Frutal",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Un perfume para la mujer de carácter floral ámbar, luminoso y envolvente, ideal para quienes buscan una elegancia moderna con un toque adictivo. Se inspira en el estilo de fragancias femeninas tipo diseñador, con un perfil cítrico-floral cremoso que recuerda a composiciones sofisticadas y muy llevables a diario.",
    "description": "Un perfume para la mujer de carácter floral ámbar, luminoso y envolvente, ideal para quienes buscan una elegancia moderna con un toque adictivo. Se inspira en el estilo de fragancias femeninas tipo diseñador, con un perfil cítrico-floral cremoso que recuerda a composiciones sofisticadas y muy llevables a diario.",
    "notasSalida": "Bergamota, Neroli, Pimienta rosa, Flor de azahar del naranjo",
    "notasCorazon": "Jazmín, Madreselva, Rosa, Iris",
    "notasFondo": "Vainilla, Almizcle, Caramelo, Ládano",
    "notes": [
      "Bergamota",
      "Neroli",
      "Pimienta rosa",
      "Flor de azahar del naranjo",
      "Jazmín",
      "Madreselva",
      "Rosa",
      "Iris",
      "Vainilla",
      "Almizcle",
      "Caramelo",
      "Ládano"
    ],
    "ml": 100,
    "precioUSD": 35,
    "categoria": "arabes"
  },
  {
    "id": 53,
    "house": "ARMAF",
    "name": "Armaf Tag Him",
    "image": "armaf-tag-him-100ml-m.webp",
    "familia": "Floral Amaderado",
    "tipo": "Eau de Parfum",
    "genero": "Masculino",
    "descripcion": "Una fragancia para el hombre moderna y sofisticada, de carácter cítrico especiado con un fondo amaderado elegante y seguro. Armaf Tag-Him se inspira claramente en el estilo de Bleu de Chanel, ofreciendo una alternativa fresca, masculina y versátil para el día a día o la noche.",
    "description": "Una fragancia para el hombre moderna y sofisticada, de carácter cítrico especiado con un fondo amaderado elegante y seguro. Armaf Tag-Him se inspira claramente en el estilo de Bleu de Chanel, ofreciendo una alternativa fresca, masculina y versátil para el día a día o la noche.",
    "notasSalida": "Toronja, Limón, Pimienta rosa, Bergamota",
    "notasCorazon": "Jengibre, Menta, Lavanda, Nuez moscada",
    "notasFondo": "Vetiver, Sándalo, Pachulí, Cedro",
    "notes": [
      "Toronja",
      "Limón",
      "Pimienta rosa",
      "Bergamota",
      "Jengibre",
      "Menta",
      "Lavanda",
      "Nuez moscada",
      "Vetiver",
      "Sándalo",
      "Pachulí",
      "Cedro"
    ],
    "ml": 100,
    "precioUSD": 45,
    "categoria": "arabes",
    "agotado": true
  },
  {
    "id": 54,
    "house": "ARMAF",
    "name": "Armaf The Pride of Armaf for Women",
    "image": "armaf-the-pride-of-armaf-for-women-100ml-f.webp",
    "familia": "Gourmand",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Una fragancia para la mujer elegante y seductora, de estilo floral oriental con un toque gourmand que aporta calidez y personalidad. Se inspira en el ADN olfativo de Carolina Herrera Good Girl, combinando brillo floral con un fondo cremoso, oscuro y adictivo.",
    "description": "Una fragancia para la mujer elegante y seductora, de estilo floral oriental con un toque gourmand que aporta calidez y personalidad. Se inspira en el ADN olfativo de Carolina Herrera Good Girl, combinando brillo floral con un fondo cremoso, oscuro y adictivo.",
    "notasSalida": "Flor de azahar del naranjo, Mandarina, Jazmín",
    "notasCorazon": "Café, Almendra, Haba tonka",
    "notasFondo": "Cacao, Pachulí",
    "notes": [
      "Flor de azahar del naranjo",
      "Mandarina",
      "Jazmín",
      "Café",
      "Almendra",
      "Haba tonka",
      "Cacao",
      "Pachulí"
    ],
    "ml": 100,
    "precioUSD": 45,
    "categoria": "arabes"
  },
  {
    "id": 55,
    "house": "ARMAF",
    "name": "Armaf Tres Jour Pour Femme",
    "image": "armaf-tres-jour-pour-femme-100ml-f.webp",
    "familia": "Floral Amaderado",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Una fragancia floral amaderada para la mujer, fresca y elegante, con un carácter limpio y sofisticado ideal para el día a día. Tres Jour Armaf pour femme se inspira en el estilo de los perfumes femeninos clásicos de aire refinado y luminoso, evocando una sensación delicada y moderna.",
    "description": "Una fragancia floral amaderada para la mujer, fresca y elegante, con un carácter limpio y sofisticado ideal para el día a día. Tres Jour Armaf pour femme se inspira en el estilo de los perfumes femeninos clásicos de aire refinado y luminoso, evocando una sensación delicada y moderna.",
    "notasSalida": "Té verde, Limón, Neroli, Flores blancas",
    "notasCorazon": "Jazmín, Nardo, Almizcle",
    "notasFondo": "Cedro, Pachulí, Vainilla",
    "notes": [
      "Té verde",
      "Limón",
      "Neroli",
      "Flores blancas",
      "Jazmín",
      "Nardo",
      "Almizcle",
      "Cedro",
      "Pachulí",
      "Vainilla"
    ],
    "ml": 100,
    "precioUSD": 40,
    "categoria": "arabes"
  },
  {
    "id": 56,
    "house": "ARMAF",
    "name": "Armaf Tres Nuit",
    "image": "armaf-tres-nuit-100ml-m.webp",
    "familia": "Amaderado",
    "tipo": "Eau de Parfum",
    "genero": "Masculino",
    "descripcion": "Una fragancia para el hombre de carácter fresco, especiado y elegante, ideal para quienes buscan un aroma limpio con presencia. Armaf Tres Nuit se inspira en el estilo de Green Irish Tweed de Creed, aportando un perfil aromático-verde, refinado y versátil para el día a día o ocasiones especiales.",
    "description": "Una fragancia para el hombre de carácter fresco, especiado y elegante, ideal para quienes buscan un aroma limpio con presencia. Armaf Tres Nuit se inspira en el estilo de Green Irish Tweed de Creed, aportando un perfil aromático-verde, refinado y versátil para el día a día o ocasiones especiales.",
    "notasSalida": "Limón, Verbena, Iris",
    "notasCorazon": "Lavanda, Violeta",
    "notasFondo": "Ámbar gris, Sándalo",
    "notes": [
      "Limón",
      "Verbena",
      "Iris",
      "Lavanda",
      "Violeta",
      "Ámbar gris",
      "Sándalo"
    ],
    "ml": 100,
    "precioUSD": 45,
    "categoria": "arabes",
    "agotado": true
  },
  {
    "id": 57,
    "house": "ARMAF",
    "name": "Armaf Ventana Pour Homme",
    "image": "armaf-ventana-pour-homme-100ml-m.webp",
    "familia": "Amaderado",
    "tipo": "Eau de Parfum",
    "genero": "Masculino",
    "descripcion": "Un Eau de Parfum para el hombre contemporáneo, con un perfil aromático-amaderado fresco, elegante y versátil. Armaf Ventana Pour Homme se inspira en el estilo vibrante de los grandes cítricos masculinos de diseñador, con una estela limpia y moderna ideal para el día a día.",
    "description": "Un Eau de Parfum para el hombre contemporáneo, con un perfil aromático-amaderado fresco, elegante y versátil. Armaf Ventana Pour Homme se inspira en el estilo vibrante de los grandes cítricos masculinos de diseñador, con una estela limpia y moderna ideal para el día a día.",
    "notasSalida": "Toronja, Limón, Mandarina, Lavanda",
    "notasCorazon": "Geranio, Pachulí, Cedro",
    "notasFondo": "Vetiver, Ámbar, Almizcle",
    "notes": [
      "Toronja",
      "Limón",
      "Mandarina",
      "Lavanda",
      "Geranio",
      "Pachulí",
      "Cedro",
      "Vetiver",
      "Ámbar",
      "Almizcle"
    ],
    "ml": 100,
    "precioUSD": 35,
    "categoria": "arabes"
  },
  {
    "id": 58,
    "house": "BENETTON",
    "name": "Benetton Hot Gold",
    "image": "benetton-hot-gold-100ml-f.webp",
    "familia": "Floral Frutal",
    "tipo": "Eau de Toilette",
    "genero": "Femenino",
    "descripcion": "Hot Gold Benetton es una fragancia para la mujer de estilo floral frutal con un brillo cálido y elegante, pensada para quienes buscan un perfume luminoso y refinado. Su carácter recuerda a un femenino dorado de aire moderno, con una sensualidad suave que evoca el estilo de algunos florales frutales de diseñador.",
    "description": "Hot Gold Benetton es una fragancia para la mujer de estilo floral frutal con un brillo cálido y elegante, pensada para quienes buscan un perfume luminoso y refinado. Su carácter recuerda a un femenino dorado de aire moderno, con una sensualidad suave que evoca el estilo de algunos florales frutales de diseñador.",
    "notasSalida": "Durazno, Bergamota, Manzana roja",
    "notasCorazon": "Rosa, Carambola, Mimosa",
    "notasFondo": "Sándalo, Almizcle, Ámbar",
    "notes": [
      "Durazno",
      "Bergamota",
      "Manzana roja",
      "Rosa",
      "Carambola",
      "Mimosa",
      "Sándalo",
      "Almizcle",
      "Ámbar"
    ],
    "ml": 100,
    "precioUSD": 20,
    "categoria": "disenador"
  },
  {
    "id": 59,
    "house": "BENETTON",
    "name": "Benetton Sisterland Green Jasmine",
    "image": "benetton-sisterland-green-jasmine-80ml-f.webp",
    "familia": "Floral Frutal",
    "tipo": "Eau de Toilette",
    "genero": "Femenino",
    "descripcion": "Sisterland Green Jasmine de Benetton es una fragancia floral frutal para la mujer, fresca, luminosa y fácil de llevar. Su aroma jugoso de mandarina y pera se funde con un corazón de jazmín y azahar, con un fondo suave de vainilla y almizcle que le da un acabado delicado y elegante.",
    "description": "Sisterland Green Jasmine de Benetton es una fragancia floral frutal para la mujer, fresca, luminosa y fácil de llevar. Su aroma jugoso de mandarina y pera se funde con un corazón de jazmín y azahar, con un fondo suave de vainilla y almizcle que le da un acabado delicado y elegante.",
    "notasSalida": "Mandarina, Pera",
    "notasCorazon": "Jazmín, Azahar",
    "notasFondo": "Vainilla, Almizcle",
    "notes": [
      "Mandarina",
      "Pera",
      "Jazmín",
      "Azahar",
      "Vainilla",
      "Almizcle"
    ],
    "ml": 80,
    "precioUSD": 25,
    "categoria": "disenador"
  },
  {
    "id": 60,
    "house": "BHARARA",
    "name": "Bharara Champagne Pink",
    "image": "bharara-champagne-pink-100ml-f.webp",
    "familia": "Floral Amaderado",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Champagne Pink de Bharara es un perfume para la mujer de estilo oriental vainilla, dulce, cálido y con un brillo elegante que lo hace ideal para ocasiones especiales. Su carácter floral y gourmand evoca una sofisticación moderna, con una vibra refinada que recuerda a las fragancias femeninas más seductoras y envolventes del mercado.",
    "description": "Champagne Pink de Bharara es un perfume para la mujer de estilo oriental vainilla, dulce, cálido y con un brillo elegante que lo hace ideal para ocasiones especiales. Su carácter floral y gourmand evoca una sofisticación moderna, con una vibra refinada que recuerda a las fragancias femeninas más seductoras y envolventes del mercado.",
    "notasSalida": "Jazmín, Rosa, Caramelo, Nuez moscada, Nardos",
    "notasCorazon": "Lirio de los valles, Acordes marinos, Ámbar, Haba tonka",
    "notasFondo": "Vainilla, Sándalo, Almizcle, Vetiver",
    "notes": [
      "Jazmín",
      "Rosa",
      "Caramelo",
      "Nuez moscada",
      "Nardos",
      "Lirio de los valles",
      "Acordes marinos",
      "Ámbar",
      "Haba tonka",
      "Vainilla",
      "Sándalo",
      "Almizcle",
      "Vetiver"
    ],
    "ml": 100,
    "precioUSD": 45,
    "categoria": "arabes"
  },
  {
    "id": 61,
    "house": "BHARARA",
    "name": "Bharara King Eau de Parfum",
    "image": "bharara-king-eau-de-parfum-100ml-m.webp",
    "familia": "Cítrico",
    "tipo": "Eau de Parfum",
    "genero": "Masculino",
    "descripcion": "Bharara King Eau de Parfum es una fragancia para el hombre de carácter cítrico-frutal, fresca y luminosa al inicio, con un fondo dulce y envolvente que aporta presencia y sofisticación. Su perfil olfativo recuerda a un estilo moderno y magnético, ideal para quien busca una firma masculina llamativa con aire de lujo contemporáneo.",
    "description": "Bharara King Eau de Parfum es una fragancia para el hombre de carácter cítrico-frutal, fresca y luminosa al inicio, con un fondo dulce y envolvente que aporta presencia y sofisticación. Su perfil olfativo recuerda a un estilo moderno y magnético, ideal para quien busca una firma masculina llamativa con aire de lujo contemporáneo.",
    "notasSalida": "Naranja, Bergamota, Limón",
    "notasCorazon": "Notas afrutadas, Vainilla",
    "notasFondo": "Almizcle blanco, Ámbar",
    "notes": [
      "Naranja",
      "Bergamota",
      "Limón",
      "Notas afrutadas",
      "Vainilla",
      "Almizcle blanco",
      "Ámbar"
    ],
    "ml": 100,
    "precioUSD": 80,
    "categoria": "arabes",
    "agotado": true
  },
  {
    "id": 62,
    "house": "BHARARA",
    "name": "Bharara King Parfum",
    "image": "bharara-king-parfum-100ml-m.webp",
    "familia": "Cítrico",
    "tipo": "Parfum",
    "genero": "Masculino",
    "descripcion": "Una fragancia para el hombre de estilo aromático, cítrico y afrutado, con un acabado dulce y amaderado que transmite presencia y elegancia. Bharara King Parfum Men se inspira en un perfil masculino moderno, versátil y llamativo, ideal para quien busca un aroma sofisticado con gran estela y carácter.",
    "description": "Una fragancia para el hombre de estilo aromático, cítrico y afrutado, con un acabado dulce y amaderado que transmite presencia y elegancia. Bharara King Parfum Men se inspira en un perfil masculino moderno, versátil y llamativo, ideal para quien busca un aroma sofisticado con gran estela y carácter.",
    "notasSalida": "Naranja, Bergamota, Limón",
    "notasCorazon": "Notas afrutadas, Vainilla",
    "notasFondo": "Almizcle blanco, Ámbar",
    "notes": [
      "Naranja",
      "Bergamota",
      "Limón",
      "Notas afrutadas",
      "Vainilla",
      "Almizcle blanco",
      "Ámbar"
    ],
    "ml": 100,
    "precioUSD": 100,
    "categoria": "arabes"
  },
  {
    "id": 63,
    "house": "BHARARA",
    "name": "Bharara Onyx",
    "image": "bharara-onyx-100ml-u.webp",
    "familia": "Floral Frutal",
    "tipo": "Eau de Parfum",
    "genero": "Unisex",
    "descripcion": "Bharara Onyx es una fragancia unisex de carácter elegante y contemporáneo, con una salida cítrica luminosa y un corazón aromático, floral y especiado. Se inspira en Lafayette Street de Bond No. 9, ofreciendo un perfil sofisticado, versátil y con un fondo cálido de ámbar, vainilla y sándalo.",
    "description": "Bharara Onyx es una fragancia unisex de carácter elegante y contemporáneo, con una salida cítrica luminosa y un corazón aromático, floral y especiado. Se inspira en Lafayette Street de Bond No. 9, ofreciendo un perfil sofisticado, versátil y con un fondo cálido de ámbar, vainilla y sándalo.",
    "notasSalida": "Bergamota, Limón, Manzana, Cardamomo, Davana",
    "notasCorazon": "Cilantro, Lavanda, Lavandín, Flor de azahar, Jazmín",
    "notasFondo": "Rosa, Ámbar, Haba tonka, Sándalo, Vainilla",
    "notes": [
      "Bergamota",
      "Limón",
      "Manzana",
      "Cardamomo",
      "Davana",
      "Cilantro",
      "Lavanda",
      "Lavandín",
      "Flor de azahar",
      "Jazmín",
      "Rosa",
      "Ámbar",
      "Haba tonka",
      "Sándalo",
      "Vainilla"
    ],
    "ml": 100,
    "precioUSD": 90,
    "categoria": "arabes",
    "agotado": true
  },
  {
    "id": 64,
    "house": "BHARARA",
    "name": "Bharara Rome Pour Femme",
    "image": "bharara-rome-pour-femme-100ml-f.webp",
    "familia": "Floral",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Una fragancia para la mujer de estilo oriental floral, dulce, elegante y moderna, con una presencia sofisticada que combina sensualidad y carácter. Se inspira en el universo de los grandes florales ambarados de diseñador, con un aura envolvente y femenina ideal para destacar con lujo discreto.",
    "description": "Una fragancia para la mujer de estilo oriental floral, dulce, elegante y moderna, con una presencia sofisticada que combina sensualidad y carácter. Se inspira en el universo de los grandes florales ambarados de diseñador, con un aura envolvente y femenina ideal para destacar con lujo discreto.",
    "notasSalida": "Grosella negra, jazmín, vainilla Bourbon",
    "notasCorazon": "frutos rojos, cítricos suaves, flores blancas",
    "notasFondo": "rosa, almizcle, vainilla",
    "notes": [
      "Grosella negra",
      "jazmín",
      "vainilla Bourbon",
      "frutos rojos",
      "cítricos suaves",
      "flores blancas",
      "rosa",
      "almizcle",
      "vainilla"
    ],
    "ml": 100,
    "precioUSD": 45,
    "categoria": "arabes",
    "agotado": true
  },
  {
    "id": 65,
    "house": "BHARARA",
    "name": "Bharara Rome Pour Homme",
    "image": "bharara-rome-pour-homme-100ml-m.webp",
    "familia": "Floral Amaderado",
    "tipo": "Eau de Parfum",
    "genero": "Masculino",
    "descripcion": "Fragancia para el hombre de carácter fresco, especiado y amaderado, ideal para una presencia elegante y segura. Se inspira en el estilo moderno y sofisticado de Valentino Uomo Born in Roma, con un perfil aromático refinado y versátil para uso diario o noche.",
    "description": "Fragancia para el hombre de carácter fresco, especiado y amaderado, ideal para una presencia elegante y segura. Se inspira en el estilo moderno y sofisticado de Valentino Uomo Born in Roma, con un perfil aromático refinado y versátil para uso diario o noche.",
    "notasSalida": "Geranio, Rosa",
    "notasCorazon": "Salvia, Jengibre",
    "notasFondo": "Cedro, Vetiver",
    "notes": [
      "Geranio",
      "Rosa",
      "Salvia",
      "Jengibre",
      "Cedro",
      "Vetiver"
    ],
    "ml": 100,
    "precioUSD": 40,
    "categoria": "arabes"
  },
  {
    "id": 66,
    "house": "BRITNEY SPEARS",
    "name": "Britney Spears Circus Fantasy",
    "image": "britney-spears-circus-fantasy-100ml-f.webp",
    "familia": "Floral Frutal",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Una fragancia para la mujer de estilo floral frutal dulce, con un aire juvenil, divertido y encantador. Circus Fantasy de Britney Spears destaca por su faceta golosa y vibrante, ideal para quien busca un perfume femenino con personalidad y un toque de fantasía; no está inspirada en un diseñador concreto, sino en el universo propio de la línea Fantasy.",
    "description": "Una fragancia para la mujer de estilo floral frutal dulce, con un aire juvenil, divertido y encantador. Circus Fantasy de Britney Spears destaca por su faceta golosa y vibrante, ideal para quien busca un perfume femenino con personalidad y un toque de fantasía; no está inspirada en un diseñador concreto, sino en el universo propio de la línea Fantasy.",
    "notasSalida": "Frambuesa, Azúcar, Flor de albaricoquero, Orquídea",
    "notasCorazon": "Peonía, Nenúfar, Vainilla",
    "notasFondo": "Almizcle, Violeta, Notas amaderadas",
    "notes": [
      "Frambuesa",
      "Azúcar",
      "Flor de albaricoquero",
      "Orquídea",
      "Peonía",
      "Nenúfar",
      "Vainilla",
      "Almizcle",
      "Violeta",
      "Notas amaderadas"
    ],
    "ml": 100,
    "precioUSD": 30,
    "categoria": "disenador"
  },
  {
    "id": 67,
    "house": "BRITNEY SPEARS",
    "name": "Britney Spears Curious",
    "image": "britney-spears-curious-100ml-f.webp",
    "familia": "Floral Frutal",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Un floral luminoso y femenino para la mujer, fresco y elegante, con un aura limpia y sensual que combina delicadeza y carácter. Curious de Britney Spears se inspira en el estilo de un floral blanco de diseñador, con un toque moderno y juvenil que recuerda a composiciones clásicas de lujo.",
    "description": "Un floral luminoso y femenino para la mujer, fresco y elegante, con un aura limpia y sensual que combina delicadeza y carácter. Curious de Britney Spears se inspira en el estilo de un floral blanco de diseñador, con un toque moderno y juvenil que recuerda a composiciones clásicas de lujo.",
    "notasSalida": "Magnolia, Pera, Flor de loto, Jazmín",
    "notasCorazon": "Nardos, Ciclamen, Almizcle",
    "notasFondo": "Vainilla, Maderas blancas, Sándalo",
    "notes": [
      "Magnolia",
      "Pera",
      "Flor de loto",
      "Jazmín",
      "Nardos",
      "Ciclamen",
      "Almizcle",
      "Vainilla",
      "Maderas blancas",
      "Sándalo"
    ],
    "ml": 100,
    "precioUSD": 30,
    "categoria": "disenador"
  },
  {
    "id": 68,
    "house": "BRITNEY SPEARS",
    "name": "Britney Spears Fantasy",
    "image": "britney-spears-fantasy-100ml-f.webp",
    "familia": "Gourmand",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Un perfume para la mujer de estilo floral frutal gourmand, alegre y seductor, ideal para quienes buscan una firma dulce con carácter glamuroso. Britney Spears Fantasy es una creación icónica que se inspira en el universo de los perfumes de diseñador, con un toque juguetón y femenino que lo ha convertido en un clásico moderno.",
    "description": "Un perfume para la mujer de estilo floral frutal gourmand, alegre y seductor, ideal para quienes buscan una firma dulce con carácter glamuroso. Britney Spears Fantasy es una creación icónica que se inspira en el universo de los perfumes de diseñador, con un toque juguetón y femenino que lo ha convertido en un clásico moderno.",
    "notasSalida": "Kiwi, Lichi rojo, Membrillo, Chocolate blanco",
    "notasCorazon": "Orquídea, Jazmín, Quequito",
    "notasFondo": "Almizcle, Raíz de lirio, Notas amaderadas",
    "notes": [
      "Kiwi",
      "Lichi rojo",
      "Membrillo",
      "Chocolate blanco",
      "Orquídea",
      "Jazmín",
      "Quequito",
      "Almizcle",
      "Raíz de lirio",
      "Notas amaderadas"
    ],
    "ml": 100,
    "precioUSD": 30,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 69,
    "house": "BRITNEY SPEARS",
    "name": "Britney Spears Hidden Fantasy",
    "image": "britney-spears-hidden-fantasy-100ml-f.webp",
    "familia": "Floral Amaderado",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Una fragancia para la mujer de carácter floral frutal y goloso, luminosa y con un toque especiado muy seductor. Hidden Fantasy de Britney Spears evoca un estilo divertido y femenino, con un perfil dulce que la hace perfecta para quienes buscan un aroma encantador y envolvente.",
    "description": "Una fragancia para la mujer de carácter floral frutal y goloso, luminosa y con un toque especiado muy seductor. Hidden Fantasy de Britney Spears evoca un estilo divertido y femenino, con un perfil dulce que la hace perfecta para quienes buscan un aroma encantador y envolvente.",
    "notasSalida": "Naranja, Mandarina, Verbena limonera, Clavo de olor",
    "notasCorazon": "Gardenia, Lirio Stargazer, Jazmín",
    "notasFondo": "Sándalo, Ámbar, Vainilla",
    "notes": [
      "Naranja",
      "Mandarina",
      "Verbena limonera",
      "Clavo de olor",
      "Gardenia",
      "Lirio Stargazer",
      "Jazmín",
      "Sándalo",
      "Ámbar",
      "Vainilla"
    ],
    "ml": 100,
    "precioUSD": 30,
    "categoria": "disenador"
  },
  {
    "id": 70,
    "house": "BRITNEY SPEARS",
    "name": "Britney Spears Midnight Fantasy",
    "image": "britney-spears-midnight-fantasy-100ml-f.webp",
    "familia": "Floral Frutal",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Una fragancia floral frutal para la mujer, intensa y envolvente, con un aire nocturno y seductor que combina dulzura jugosa con un fondo cremoso y cálido. Midnight Fantasy de Britney Spears ofrece una vibra glamourosa y fantasiosa, ideal para quienes buscan un perfume femenino con presencia y un toque juvenil de lujo accesible.",
    "description": "Una fragancia floral frutal para la mujer, intensa y envolvente, con un aire nocturno y seductor que combina dulzura jugosa con un fondo cremoso y cálido. Midnight Fantasy de Britney Spears ofrece una vibra glamourosa y fantasiosa, ideal para quienes buscan un perfume femenino con presencia y un toque juvenil de lujo accesible.",
    "notasSalida": "Ciruela, Cereza ácida, Frambuesa",
    "notasCorazon": "Orquídea, Iris, Fresia",
    "notasFondo": "Vainilla, Ámbar, Almizcle",
    "notes": [
      "Ciruela",
      "Cereza ácida",
      "Frambuesa",
      "Orquídea",
      "Iris",
      "Fresia",
      "Vainilla",
      "Ámbar",
      "Almizcle"
    ],
    "ml": 100,
    "precioUSD": 30,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 71,
    "house": "BVLGARI",
    "name": "Bvlgari Aqva Pour Homme Marine",
    "image": "bvlgari-aqva-pour-homme-marine-100ml-m.webp",
    "familia": "Amaderado",
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": "Una fragancia aromática acuática para el hombre que transmite frescura marina, limpieza elegante y un carácter sofisticado ideal para climas cálidos. Aqva Pour Homme Marine de Bvlgari se inspira en una estética masculina moderna y refrescante, con un perfil que evoca el mar y recuerda al estilo de un diseñador contemporáneo de lujo.",
    "description": "Una fragancia aromática acuática para el hombre que transmite frescura marina, limpieza elegante y un carácter sofisticado ideal para climas cálidos. Aqva Pour Homme Marine de Bvlgari se inspira en una estética masculina moderna y refrescante, con un perfil que evoca el mar y recuerda al estilo de un diseñador contemporáneo de lujo.",
    "notasSalida": "Toronja (pomelo), Neroli, Mandarina",
    "notasCorazon": "Petit grain, Notas acuáticas, Algas marinas",
    "notasFondo": "Romero, Cedro de Virginia, Ámbar",
    "notes": [
      "Toronja (pomelo)",
      "Neroli",
      "Mandarina",
      "Petit grain",
      "Notas acuáticas",
      "Algas marinas",
      "Romero",
      "Cedro de Virginia",
      "Ámbar"
    ],
    "ml": 100,
    "precioUSD": 155,
    "categoria": "disenador"
  },
  {
    "id": 72,
    "house": "BVLGARI",
    "name": "Bvlgari Man in Black",
    "image": "bvlgari-man-in-black-100ml-m.webp",
    "familia": "Floral Amaderado",
    "tipo": "Eau de Parfum",
    "genero": "Masculino",
    "descripcion": "Para el hombre, Bvlgari Man In Black es una fragancia intensa, oscura y seductora, de estilo ambarado especiado con un marcado acorde de cuero. Se inspira en el universo de un perfume de diseñador moderno y sofisticado, con un carácter cálido y magnético ideal para la noche.",
    "description": "Para el hombre, Bvlgari Man In Black es una fragancia intensa, oscura y seductora, de estilo ambarado especiado con un marcado acorde de cuero. Se inspira en el universo de un perfume de diseñador moderno y sofisticado, con un carácter cálido y magnético ideal para la noche.",
    "notasSalida": "Especias, Ron, Tabaco",
    "notasCorazon": "Cuero, Iris, Nardo",
    "notasFondo": "Haba tonka, Madera de gaiac, Benjuí",
    "notes": [
      "Especias",
      "Ron",
      "Tabaco",
      "Cuero",
      "Iris",
      "Nardo",
      "Haba tonka",
      "Madera de gaiac",
      "Benjuí"
    ],
    "ml": 100,
    "precioUSD": 150,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 73,
    "house": "CACHAREL",
    "name": "Anaïs Anaïs L'Original",
    "image": "cacharel-anais-anais-loriginal-eau-de-toilette-50ml-f.webp",
    "familia": "Floral Amaderado",
    "tipo": "Eau de Toilette",
    "genero": "Femenino",
    "descripcion": "Una fragancia floral y delicada para la mujer, con una estela clásica, romántica y elegante que envuelve con suavidad. Anais Anais L’Original de Cacharel es un perfume atemporal de inspiración femenina, ideal para quienes buscan un estilo refinado y luminoso.",
    "description": "Una fragancia floral y delicada para la mujer, con una estela clásica, romántica y elegante que envuelve con suavidad. Anais Anais L’Original de Cacharel es un perfume atemporal de inspiración femenina, ideal para quienes buscan un estilo refinado y luminoso.",
    "notasSalida": "Jacinto, Madreselva, Azucena, Lirio del valle",
    "notasCorazon": "Jazmín, Ylang-ylang, Rosa, Musgo",
    "notasFondo": "Almizcle, Vetiver, Sándalo, Grosella negra",
    "notes": [
      "Jacinto",
      "Madreselva",
      "Azucena",
      "Lirio del valle",
      "Jazmín",
      "Ylang-ylang",
      "Rosa",
      "Musgo",
      "Almizcle",
      "Vetiver",
      "Sándalo",
      "Grosella negra"
    ],
    "ml": 100,
    "precioUSD": 45,
    "categoria": "disenador"
  },
  {
    "id": 74,
    "house": "CALVIN KLEIN",
    "name": "Calvin Klein CK IN2U for Her",
    "image": "calvin-klein-ck-in2u-for-her-100ml-f.webp",
    "familia": "Floral Amaderado",
    "tipo": "Eau de Toilette",
    "genero": "Femenino",
    "descripcion": "Una fragancia para la mujer de estilo oriental floral, fresca y juvenil, con un carácter moderno y desenfadado. CK IN2U for Her de Calvin Klein se inspira en la energía urbana contemporánea y ofrece un aire similar al de un perfume de diseñador casual, luminoso y adictivo.",
    "description": "Una fragancia para la mujer de estilo oriental floral, fresca y juvenil, con un carácter moderno y desenfadado. CK IN2U for Her de Calvin Klein se inspira en la energía urbana contemporánea y ofrece un aire similar al de un perfume de diseñador casual, luminoso y adictivo.",
    "notasSalida": "Toronja rosa, Bergamota, Grosella negra",
    "notasCorazon": "Orquídea, Cactus, Vainilla",
    "notasFondo": "Cedro, Ámbar",
    "notes": [
      "Toronja rosa",
      "Bergamota",
      "Grosella negra",
      "Orquídea",
      "Cactus",
      "Vainilla",
      "Cedro",
      "Ámbar"
    ],
    "ml": 100,
    "precioUSD": 40,
    "categoria": "disenador"
  },
  {
    "id": 75,
    "house": "CALVIN KLEIN",
    "name": "Calvin Klein CK One Shock for Her",
    "image": "calvin-klein-ck-one-shock-for-her-100ml-f.webp",
    "familia": "Gourmand",
    "tipo": "Eau de Toilette",
    "genero": "Femenino",
    "descripcion": "CK One Shock For Her de Calvin Klein es una fragancia para la mujer de estilo oriental especiado, intensa y adictiva, que combina dulzura, misterio y un toque provocador. Inspirada en el ADN juvenil y urbano de Calvin Klein, ofrece un carácter moderno con un fondo cremoso y sensual ideal para destacar con personalidad.",
    "description": "CK One Shock For Her de Calvin Klein es una fragancia para la mujer de estilo oriental especiado, intensa y adictiva, que combina dulzura, misterio y un toque provocador. Inspirada en el ADN juvenil y urbano de Calvin Klein, ofrece un carácter moderno con un fondo cremoso y sensual ideal para destacar con personalidad.",
    "notasSalida": "Flor de la pasión, Amapola roja, Peonía, Chocolate mexicano",
    "notasCorazon": "Zarzamora, Narciso, Jazmín, Vainilla",
    "notasFondo": "Almizcle, Ámbar, Pachulí",
    "notes": [
      "Flor de la pasión",
      "Amapola roja",
      "Peonía",
      "Chocolate mexicano",
      "Zarzamora",
      "Narciso",
      "Jazmín",
      "Vainilla",
      "Almizcle",
      "Ámbar",
      "Pachulí"
    ],
    "ml": 100,
    "precioUSD": 40,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 76,
    "house": "CALVIN KLEIN",
    "name": "Calvin Klein Eternity for Women",
    "image": "calvin-klein-eternity-for-women-100ml-f.webp",
    "familia": "Floral Amaderado",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Un clásico floral para la mujer, elegante y atemporal, que transmite pureza, romanticismo y sofisticación. Inspirado en el estilo de un gran floral de diseñador, se siente luminoso, limpio y muy refinado, ideal para un uso diario distinguido.",
    "description": "Un clásico floral para la mujer, elegante y atemporal, que transmite pureza, romanticismo y sofisticación. Inspirado en el estilo de un gran floral de diseñador, se siente luminoso, limpio y muy refinado, ideal para un uso diario distinguido.",
    "notasSalida": "Mandarina, fresia, salvia, lirio de los valles",
    "notasCorazon": "rosa, jazmín, narciso",
    "notasFondo": "sándalo, ámbar, almizcle",
    "notes": [
      "Mandarina",
      "fresia",
      "salvia",
      "lirio de los valles",
      "rosa",
      "jazmín",
      "narciso",
      "sándalo",
      "ámbar",
      "almizcle"
    ],
    "ml": 100,
    "precioUSD": 45,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 77,
    "house": "CALVIN KLEIN",
    "name": "Calvin Klein Euphoria",
    "image": "calvin-klein-euphoria-100ml-f.webp",
    "familia": "Amaderado",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Una fragancia para la mujer misteriosa, elegante y seductora, de estilo oriental floral con un toque frutal jugoso. Calvin Klein Euphoria se inspira en una perfumería de diseño sofisticada y adictiva, ideal para quien busca una estela envolvente y femenina.",
    "description": "Una fragancia para la mujer misteriosa, elegante y seductora, de estilo oriental floral con un toque frutal jugoso. Calvin Klein Euphoria se inspira en una perfumería de diseño sofisticada y adictiva, ideal para quien busca una estela envolvente y femenina.",
    "notasSalida": "Granada, Caqui, Acordes verdes, Orquídea negra",
    "notasCorazon": "Flor de loto, Champaca, Caoba, Ámbar",
    "notasFondo": "Violeta, Almizcle, Pachulí, Vainilla",
    "notes": [
      "Granada",
      "Caqui",
      "Acordes verdes",
      "Orquídea negra",
      "Flor de loto",
      "Champaca",
      "Caoba",
      "Ámbar",
      "Violeta",
      "Almizcle",
      "Pachulí",
      "Vainilla"
    ],
    "ml": 100,
    "precioUSD": 65,
    "categoria": "disenador"
  },
  {
    "id": 78,
    "house": "CALVIN KLEIN",
    "name": "Calvin Klein Obsession",
    "image": "calvin-klein-obsession-100ml-f.webp",
    "familia": "Cítrico",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Una fragancia opulenta para la mujer, de estilo ámbar especiado con un aura cálida, sensual y envolvente. Obsession de Calvin Klein es un clásico de diseñador que se reconoce por su intensidad y su carácter adictivo, ideal para quien busca un perfume sofisticado con presencia.",
    "description": "Una fragancia opulenta para la mujer, de estilo ámbar especiado con un aura cálida, sensual y envolvente. Obsession de Calvin Klein es un clásico de diseñador que se reconoce por su intensidad y su carácter adictivo, ideal para quien busca un perfume sofisticado con presencia.",
    "notasSalida": "Bergamota, Mandarina, Notas verdes, Albahaca",
    "notasCorazon": "Vainilla, Ámbar, Especias",
    "notasFondo": "Maderas, Almizcle, Algalia",
    "notes": [
      "Bergamota",
      "Mandarina",
      "Notas verdes",
      "Albahaca",
      "Vainilla",
      "Ámbar",
      "Especias",
      "Maderas",
      "Almizcle",
      "Algalia"
    ],
    "ml": 100,
    "precioUSD": 30,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 79,
    "house": "CALVIN KLEIN",
    "name": "Calvin Klein Truth",
    "image": "calvin-klein-truth-100ml-f.webp",
    "familia": "Floral Amaderado",
    "tipo": "Eau de Toilette",
    "genero": "Femenino",
    "descripcion": "Una fragancia para la mujer de carácter oriental amaderado, elegante y natural, que envuelve la piel con una frescura serena y un fondo cálido y sensual. Truth de Calvin Klein se inspira en el estilo limpio y sofisticado de la perfumería de diseñador, con un perfil moderno y atemporal.",
    "description": "Una fragancia para la mujer de carácter oriental amaderado, elegante y natural, que envuelve la piel con una frescura serena y un fondo cálido y sensual. Truth de Calvin Klein se inspira en el estilo limpio y sofisticado de la perfumería de diseñador, con un perfil moderno y atemporal.",
    "notasSalida": "Vetiver, Pachulí, Bambú, Bergamota, Limón",
    "notasCorazon": "Trébol blanco, Sándalo, Peonía, Azucena, Mimosa",
    "notasFondo": "Zarzamora, Flor del árbol de la seda, Ámbar, Almizcle, Vainilla",
    "notes": [
      "Vetiver",
      "Pachulí",
      "Bambú",
      "Bergamota",
      "Limón",
      "Trébol blanco",
      "Sándalo",
      "Peonía",
      "Azucena",
      "Mimosa",
      "Zarzamora",
      "Flor del árbol de la seda",
      "Ámbar",
      "Almizcle",
      "Vainilla"
    ],
    "ml": 100,
    "precioUSD": 45,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 80,
    "house": "CAROLINA HERRERA",
    "name": "Carolina Herrera Eau de Parfum",
    "image": "carolina-herrera-carolina-herrera-eau-de-parfum-100ml-f.webp",
    "familia": "Floral Amaderado",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Una fragancia floral sofisticada para la mujer, elegante y con un aire clásico de lujo atemporal. Su carácter luminosa y refinado evoca el estilo de los grandes perfumes florales de diseñador, con una estela sensual y femenina.",
    "description": "Una fragancia floral sofisticada para la mujer, elegante y con un aire clásico de lujo atemporal. Su carácter luminosa y refinado evoca el estilo de los grandes perfumes florales de diseñador, con una estela sensual y femenina.",
    "notasSalida": "Bergamota, Flor de azahar, Rosa, Notas verdes, Chabacano, Jazmín",
    "notasCorazon": "Nardo, Madreselva, Lirio, Ylang ylang, Cedro, Almizcle",
    "notasFondo": "Ámbar, Vetiver, Sándalo, Musgo de roble, Algalia",
    "notes": [
      "Bergamota",
      "Flor de azahar",
      "Rosa",
      "Notas verdes",
      "Chabacano",
      "Jazmín",
      "Nardo",
      "Madreselva",
      "Lirio",
      "Ylang ylang",
      "Cedro",
      "Almizcle",
      "Ámbar",
      "Vetiver",
      "Sándalo",
      "Musgo de roble",
      "Algalia"
    ],
    "ml": 100,
    "precioUSD": 80,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 81,
    "house": "CLINIQUE",
    "name": "Clinique Happy Heart",
    "image": "clinique-happy-heart-2012-50ml-f.webp",
    "familia": "Amaderado",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Clinique Happy Heart 2012 es una fragancia luminosa y elegante para la mujer, de perfil floral frutal con un aire fresco, limpio y delicadamente acuático. Su estilo femenino y refinado evoca la frescura de la mandarina y las notas verdes, con un carácter que se inspira en la estética fresca y optimista de Clinique Happy.",
    "description": "Clinique Happy Heart 2012 es una fragancia luminosa y elegante para la mujer, de perfil floral frutal con un aire fresco, limpio y delicadamente acuático. Su estilo femenino y refinado evoca la frescura de la mandarina y las notas verdes, con un carácter que se inspira en la estética fresca y optimista de Clinique Happy.",
    "notasSalida": "Mandarina",
    "notasCorazon": "Jacinto de agua",
    "notasFondo": "Madera blanca",
    "notes": [
      "Mandarina",
      "Jacinto de agua",
      "Madera blanca"
    ],
    "ml": 50,
    "precioUSD": 50,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 82,
    "house": "DAVIDOFF",
    "name": "Davidoff Cool Water for Women",
    "image": "davidoff-cool-water-eau-de-toilette-for-women-100ml-f.webp",
    "familia": "Floral Frutal",
    "tipo": "Eau de Toilette",
    "genero": "Femenino",
    "descripcion": "Una fragancia floral acuática para la mujer, fresca, luminosa y atemporal, ideal para quienes buscan una estela limpia con un toque elegante. Cool Water Woman de Davidoff se inspira en el estilo icónico de los perfumes acuáticos de diseñador, con una sensación revitalizante y femenina.",
    "description": "Una fragancia floral acuática para la mujer, fresca, luminosa y atemporal, ideal para quienes buscan una estela limpia con un toque elegante. Cool Water Woman de Davidoff se inspira en el estilo icónico de los perfumes acuáticos de diseñador, con una sensación revitalizante y femenina.",
    "notasSalida": "Piña, Lirio, Loto, Melón, Membrillo, Grosella negra, Frutas cítricas",
    "notasCorazon": "Miel, Jazmín, Rosa, Nenúfar, Espino blanco, Muguete, Mora",
    "notasFondo": "Frambuesa, Almizcle, Melocotón, Sándalo, Vainilla, Violeta, Vetiver",
    "notes": [
      "Piña",
      "Lirio",
      "Loto",
      "Melón",
      "Membrillo",
      "Grosella negra",
      "Frutas cítricas",
      "Miel",
      "Jazmín",
      "Rosa",
      "Nenúfar",
      "Espino blanco",
      "Muguete",
      "Mora",
      "Frambuesa",
      "Almizcle",
      "Melocotón",
      "Sándalo",
      "Vainilla",
      "Violeta",
      "Vetiver"
    ],
    "ml": 100,
    "precioUSD": 40,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 83,
    "house": "DIESEL",
    "name": "Diesel Plus Plus Feminine",
    "image": "diesel-plus-plus-feminine-75ml-f.webp",
    "familia": "Floral Frutal",
    "tipo": "Eau de Toilette",
    "genero": "Femenino",
    "descripcion": "Un perfume para la mujer de estilo oriental floral, dulce y lactónico, con un carácter cremoso y juvenil que combina frutas jugosas con un fondo cálido y envolvente. Diesel Plus Plus Feminine destaca por su faceta gourmand de coco y vainilla, y se inspira en un estilo de perfumería femenina noventera, alegre y original.",
    "description": "Un perfume para la mujer de estilo oriental floral, dulce y lactónico, con un carácter cremoso y juvenil que combina frutas jugosas con un fondo cálido y envolvente. Diesel Plus Plus Feminine destaca por su faceta gourmand de coco y vainilla, y se inspira en un estilo de perfumería femenina noventera, alegre y original.",
    "notasSalida": "Piña, Naranja amarga, Casis, Notas afrutadas, Lirio de los valles, Leche, Coco",
    "notasCorazon": "Azucena, Cereza, Fresa, Manzana roja, Orquídea, Jazmín, Magnolia",
    "notasFondo": "Vainilla, Almizcle, Nuez moscada, Sándalo, Raíz de lirio, Ámbar, Cedro de Virginia",
    "notes": [
      "Piña",
      "Naranja amarga",
      "Casis",
      "Notas afrutadas",
      "Lirio de los valles",
      "Leche",
      "Coco",
      "Azucena",
      "Cereza",
      "Fresa",
      "Manzana roja",
      "Orquídea",
      "Jazmín",
      "Magnolia",
      "Vainilla",
      "Almizcle",
      "Nuez moscada",
      "Sándalo",
      "Raíz de lirio",
      "Ámbar",
      "Cedro de Virginia"
    ],
    "ml": 75,
    "precioUSD": 20,
    "categoria": "disenador"
  },
  {
    "id": 84,
    "house": "DIOR",
    "name": "Dior Dolce Vita",
    "image": "dior-dolce-vita-100ml-f.webp",
    "familia": "Floral Frutal",
    "tipo": "Eau de Toilette",
    "genero": "Femenino",
    "descripcion": "Una fragancia para la mujer con un carácter frutal-floral y un fondo cálido, elegante y ligeramente especiado. Dolce Vita de Dior evoca una feminidad luminosa y optimista, con un aire clásico de lujo inspirado en el universo olfativo de Dior.",
    "description": "Una fragancia para la mujer con un carácter frutal-floral y un fondo cálido, elegante y ligeramente especiado. Dolce Vita de Dior evoca una feminidad luminosa y optimista, con un aire clásico de lujo inspirado en el universo olfativo de Dior.",
    "notasSalida": "Flor de naranjo, Melocotón, Bergamota, Rosa, Cardamomo",
    "notasCorazon": "Pomelo, Albaricoque, Magnolia, Azucena, Canela",
    "notasFondo": "Heliotropo, Sándalo, Coco, Vainilla, Cedro",
    "notes": [
      "Flor de naranjo",
      "Melocotón",
      "Bergamota",
      "Rosa",
      "Cardamomo",
      "Pomelo",
      "Albaricoque",
      "Magnolia",
      "Azucena",
      "Canela",
      "Heliotropo",
      "Sándalo",
      "Coco",
      "Vainilla",
      "Cedro"
    ],
    "ml": 100,
    "precioUSD": 220,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 86,
    "house": "DIOR",
    "name": "Dior Homme Intense",
    "image": "dior-homme-intense-2025-100ml-m.webp",
    "familia": "Frutal",
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": "Dior Homme Intense 2025 es una fragancia sofisticada y envolvente para el hombre, con un iris aterciopelado y elegante que aporta una firma refinada y muy moderna. Su estilo olfativo es amaderado floral almizclado, con un aire inspirado en la elegancia característica de Dior Homme Intense, ideal para quien busca presencia, distinción y un toque seductor.",
    "description": "Dior Homme Intense 2025 es una fragancia sofisticada y envolvente para el hombre, con un iris aterciopelado y elegante que aporta una firma refinada y muy moderna. Su estilo olfativo es amaderado floral almizclado, con un aire inspirado en la elegancia característica de Dior Homme Intense, ideal para quien busca presencia, distinción y un toque seductor.",
    "notasSalida": "Lavanda, Iris",
    "notasCorazon": "Almizcle ambreta, Pera",
    "notasFondo": "Vetiver, Cedro de Virginia",
    "notes": [
      "Lavanda",
      "Iris",
      "Almizcle ambreta",
      "Pera",
      "Vetiver",
      "Cedro de Virginia"
    ],
    "ml": 100,
    "precioUSD": 220,
    "categoria": "disenador"
  },
  {
    "id": 87,
    "house": "DIOR",
    "name": "Dior Poison Girl Eau de Toilette",
    "image": "dior-poison-girl-eau-de-toilette-100ml-f.webp",
    "familia": "Floral Amaderado",
    "tipo": "Eau de Toilette",
    "genero": "Femenino",
    "descripcion": "Poison Girl Eau de Toilette de Dior es una fragancia para la mujer joven, sensual y con carácter, de estilo oriental floral gourmand. Se inspira en el universo seductor y rebelde de la línea Poison Girl de Dior, con un aire moderno y adictivo ideal para destacar día y noche.",
    "description": "Poison Girl Eau de Toilette de Dior es una fragancia para la mujer joven, sensual y con carácter, de estilo oriental floral gourmand. Se inspira en el universo seductor y rebelde de la línea Poison Girl de Dior, con un aire moderno y adictivo ideal para destacar día y noche.",
    "notasSalida": "Naranja amarga, Limón, Rosa de Grasse, Rosa de Damasco",
    "notasCorazon": "Flor de azahar del naranjo, Haba tonka, Sándalo, Almendra",
    "notasFondo": "Heliotropo, Bálsamo de Tolú, Vainilla",
    "notes": [
      "Naranja amarga",
      "Limón",
      "Rosa de Grasse",
      "Rosa de Damasco",
      "Flor de azahar del naranjo",
      "Haba tonka",
      "Sándalo",
      "Almendra",
      "Heliotropo",
      "Bálsamo de Tolú",
      "Vainilla"
    ],
    "ml": 100,
    "precioUSD": 150,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 88,
    "house": "DIOR",
    "name": "Dior Sauvage",
    "image": "dior-sauvage-100ml-m.webp",
    "familia": "Oriental",
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": "Una fragancia para el hombre de carácter aromático y amaderado, creada para proyectar frescura, fuerza y elegancia contemporánea. Inspirada en el icónico Sauvage de Dior, combina un inicio vibrante con un fondo sensual y magnético, ideal para un estilo sofisticado y versátil.",
    "description": "Una fragancia para el hombre de carácter aromático y amaderado, creada para proyectar frescura, fuerza y elegancia contemporánea. Inspirada en el icónico Sauvage de Dior, combina un inicio vibrante con un fondo sensual y magnético, ideal para un estilo sofisticado y versátil.",
    "notasSalida": "Bergamota de Calabria, Pimienta, Lavanda",
    "notasCorazon": "Pimienta de Sichuan, Anís estrellado, Nuez moscada",
    "notasFondo": "Ambroxan, Cedro, Ládano",
    "notes": [
      "Bergamota de Calabria",
      "Pimienta",
      "Lavanda",
      "Pimienta de Sichuan",
      "Anís estrellado",
      "Nuez moscada",
      "Ambroxan",
      "Cedro",
      "Ládano"
    ],
    "ml": 100,
    "precioUSD": 250,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 89,
    "house": "DIOR",
    "name": "Dior Sauvage Elixir",
    "image": "dior-sauvage-elixir-60ml-m.webp",
    "familia": "Amaderado",
    "tipo": "Elixir",
    "genero": "Masculino",
    "descripcion": "Sauvage Elixir de Dior es una fragancia para el hombre intensa, sofisticada y de carácter magnético, con un perfil aromático-especiado y amaderado de gran presencia. Se inspira en la estela inconfundible de *Sauvage* de Dior, llevándola a una versión más oscura, densa y lujosa, pensada para destacar con elegancia.",
    "description": "Sauvage Elixir de Dior es una fragancia para el hombre intensa, sofisticada y de carácter magnético, con un perfil aromático-especiado y amaderado de gran presencia. Se inspira en la estela inconfundible de *Sauvage* de Dior, llevándola a una versión más oscura, densa y lujosa, pensada para destacar con elegancia.",
    "notasSalida": "Nuez moscada, Canela, Cardamomo, Toronja",
    "notasCorazon": "Lavanda, Regaliz, Sándalo",
    "notasFondo": "Ámbar, Pachulí, Vetiver de Haití",
    "notes": [
      "Nuez moscada",
      "Canela",
      "Cardamomo",
      "Toronja",
      "Lavanda",
      "Regaliz",
      "Sándalo",
      "Ámbar",
      "Pachulí",
      "Vetiver de Haití"
    ],
    "ml": 60,
    "precioUSD": 250,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 90,
    "house": "DOLCE & GABBANA",
    "name": "Dolce & Gabbana Devotion Pour Homme",
    "image": "dolcegabbana-devotion-pour-homme-100ml-m.webp",
    "familia": "Cítrico",
    "tipo": "Eau de Parfum",
    "genero": "Masculino",
    "descripcion": "Perfume de lujo para el hombre, con un perfil oriental especiado y fougère que combina elegancia moderna con una presencia cálida y sofisticada. Devotion Pour Homme de Dolce&Gabbana está pensado para quien busca un aroma masculino, envolvente y refinado, con un carácter contemporáneo inspirado en la alta perfumería italiana.",
    "description": "Perfume de lujo para el hombre, con un perfil oriental especiado y fougère que combina elegancia moderna con una presencia cálida y sofisticada. Devotion Pour Homme de Dolce&Gabbana está pensado para quien busca un aroma masculino, envolvente y refinado, con un carácter contemporáneo inspirado en la alta perfumería italiana.",
    "notasSalida": "Limón, café",
    "notasCorazon": "pachulí, lavanda",
    "notasFondo": "especias, haba tonka",
    "notes": [
      "Limón",
      "café",
      "pachulí",
      "lavanda",
      "especias",
      "haba tonka"
    ],
    "ml": 100,
    "precioUSD": 130,
    "categoria": "disenador"
  },
  {
    "id": 91,
    "house": "DOLCE & GABBANA",
    "name": "Dolce & Gabbana Dolce Peony",
    "image": "dolcegabbana-dolce-peony-75ml-f.webp",
    "familia": "Floral Frutal",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Una fragancia floral frutal gourmand para la mujer, luminosa y femenina, que combina frescura chispeante con un fondo dulce y seductor. Dolce Peony de Dolce&Gabbana se inspira en el universo elegante y delicado de la línea Dolce, con un carácter sofisticado y moderno.",
    "description": "Una fragancia floral frutal gourmand para la mujer, luminosa y femenina, que combina frescura chispeante con un fondo dulce y seductor. Dolce Peony de Dolce&Gabbana se inspira en el universo elegante y delicado de la línea Dolce, con un carácter sofisticado y moderno.",
    "notasSalida": "Bergamota, Pera, Pimienta rosa, Violeta",
    "notasCorazon": "Peonía, Rosa búlgara, Fresia",
    "notasFondo": "Miel, Ambroxán, Pachulí",
    "notes": [
      "Bergamota",
      "Pera",
      "Pimienta rosa",
      "Violeta",
      "Peonía",
      "Rosa búlgara",
      "Fresia",
      "Miel",
      "Ambroxán",
      "Pachulí"
    ],
    "ml": 75,
    "precioUSD": 80,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 92,
    "house": "DONNA KARAN",
    "name": "DKNY Be Delicious",
    "image": "donna-karan-dkny-be-delicious-100ml-f.webp",
    "familia": "Floral Frutal",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Un perfume para la mujer de estilo floral frutal, fresco y vibrante, con una salida jugosa y luminosa que aporta una sensación limpia y moderna. DKNY Be Delicious de Donna Karan se inspira en el espíritu urbano y desenfadado de un clásico de diseñador, con un aire icónico y muy reconocible.",
    "description": "Un perfume para la mujer de estilo floral frutal, fresco y vibrante, con una salida jugosa y luminosa que aporta una sensación limpia y moderna. DKNY Be Delicious de Donna Karan se inspira en el espíritu urbano y desenfadado de un clásico de diseñador, con un aire icónico y muy reconocible.",
    "notasSalida": "Pepino, Pomelo, Magnolia, Manzana verde",
    "notasCorazon": "Lirio de los valles, Nardos, Violeta, Rosa",
    "notasFondo": "Sándalo, Ámbar, Notas amaderadas",
    "notes": [
      "Pepino",
      "Pomelo",
      "Magnolia",
      "Manzana verde",
      "Lirio de los valles",
      "Nardos",
      "Violeta",
      "Rosa",
      "Sándalo",
      "Ámbar",
      "Notas amaderadas"
    ],
    "ml": 100,
    "precioUSD": 90,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 93,
    "house": "DUMONT",
    "name": "Dumont Nitro Black",
    "image": "dumont-nitro-black-100ml-m.webp",
    "familia": "Floral Frutal",
    "tipo": "Eau de Parfum",
    "genero": "Masculino",
    "descripcion": "Nitro Black es una fragancia para el hombre, de perfil aromático fresco, frutal y especiado, con un carácter elegante y moderno. Está inspirada en el estilo de Creed Aventus, aportando una estela sofisticada, versátil y masculina ideal para uso diario o ocasiones especiales.",
    "description": "Nitro Black es una fragancia para el hombre, de perfil aromático fresco, frutal y especiado, con un carácter elegante y moderno. Está inspirada en el estilo de Creed Aventus, aportando una estela sofisticada, versátil y masculina ideal para uso diario o ocasiones especiales.",
    "notasSalida": "Bergamota, Piña, Grosellas negras, Manzana, Abedul",
    "notasCorazon": "Pachulí, Pimienta, Lavanda, Jazmín",
    "notasFondo": "Rosa, Almizcle, Ámbar, Vainilla",
    "notes": [
      "Bergamota",
      "Piña",
      "Grosellas negras",
      "Manzana",
      "Abedul",
      "Pachulí",
      "Pimienta",
      "Lavanda",
      "Jazmín",
      "Rosa",
      "Almizcle",
      "Ámbar",
      "Vainilla"
    ],
    "ml": 100,
    "precioUSD": 45,
    "categoria": "arabes",
    "agotado": true
  },
  {
    "id": 94,
    "house": "DUMONT",
    "name": "Dumont Nitro Blue",
    "image": "dumont-nitro-blue-100ml-m.webp",
    "familia": "Gourmand",
    "tipo": "Eau de Parfum",
    "genero": "Masculino",
    "descripcion": "Nitro Blue de Dumont es una fragancia oriental amaderada para el hombre, intensa y sofisticada, con un perfil dulce-especiado que deja una estela cálida y envolvente. Su carácter recuerda a un gourmand moderno con aire de lujo, ideal para quienes buscan un aroma llamativo, elegante y duradero.",
    "description": "Nitro Blue de Dumont es una fragancia oriental amaderada para el hombre, intensa y sofisticada, con un perfil dulce-especiado que deja una estela cálida y envolvente. Su carácter recuerda a un gourmand moderno con aire de lujo, ideal para quienes buscan un aroma llamativo, elegante y duradero.",
    "notasSalida": "Canela, salvia, limón, flor de azahar del naranjo",
    "notasCorazon": "albahaca, cardamomo, chocolate, praliné",
    "notasFondo": "notas amaderadas, pachulí, ámbar negro",
    "notes": [
      "Canela",
      "salvia",
      "limón",
      "flor de azahar del naranjo",
      "albahaca",
      "cardamomo",
      "chocolate",
      "praliné",
      "notas amaderadas",
      "pachulí",
      "ámbar negro"
    ],
    "ml": 100,
    "precioUSD": 45,
    "categoria": "arabes",
    "agotado": true
  },
  {
    "id": 95,
    "house": "DUMONT",
    "name": "Dumont Nitro Platinum",
    "image": "dumont-nitro-platinum-100ml-m.webp",
    "familia": "Floral Frutal",
    "tipo": "Eau de Parfum",
    "genero": "Masculino",
    "descripcion": "Un Eau de Parfum sofisticado para el hombre, con un perfil fresco, amaderado y elegante que proyecta presencia moderna. Su carácter versátil y envolvente lo convierte en una opción ideal para quienes buscan una fragancia masculina con aire refinado y un toque de inspiración en perfumes de diseñador contemporáneos.",
    "description": "Un Eau de Parfum sofisticado para el hombre, con un perfil fresco, amaderado y elegante que proyecta presencia moderna. Su carácter versátil y envolvente lo convierte en una opción ideal para quienes buscan una fragancia masculina con aire refinado y un toque de inspiración en perfumes de diseñador contemporáneos.",
    "notasSalida": "Bergamota, Manzana, Grosella negra, Naranjo, Pimienta rosa",
    "notasCorazon": "Ámbar, Caramelo, Lirio del valle, Azafrán, Madera seca",
    "notasFondo": "Vainilla, Maderas preciosas, Musgo, Almizcle, Madera de guayaco",
    "notes": [
      "Bergamota",
      "Manzana",
      "Grosella negra",
      "Naranjo",
      "Pimienta rosa",
      "Ámbar",
      "Caramelo",
      "Lirio del valle",
      "Azafrán",
      "Madera seca",
      "Vainilla",
      "Maderas preciosas",
      "Musgo",
      "Almizcle",
      "Madera de guayaco"
    ],
    "ml": 100,
    "precioUSD": 45,
    "categoria": "arabes"
  },
  {
    "id": 96,
    "house": "DUMONT",
    "name": "Dumont Nitro Red",
    "image": "dumont-nitro-red-100ml-m.webp",
    "familia": "Frutal",
    "tipo": "Eau de Parfum",
    "genero": "Masculino",
    "descripcion": "Fragancia para el hombre de estilo aromático frutal, fresca y moderna, con un perfil limpio y atractivo que combina energía y carácter. Se inspira en el estilo de los perfumes masculinos actuales de diseñador, con una vibra luminosa y versátil ideal para uso diario.",
    "description": "Fragancia para el hombre de estilo aromático frutal, fresca y moderna, con un perfil limpio y atractivo que combina energía y carácter. Se inspira en el estilo de los perfumes masculinos actuales de diseñador, con una vibra luminosa y versátil ideal para uso diario.",
    "notasSalida": "Manzana, Lavanda, Bergamota",
    "notasCorazon": "Sandía, Cedro, Cálamo aromático",
    "notasFondo": "Ámbar, Sándalo, Pachulí",
    "notes": [
      "Manzana",
      "Lavanda",
      "Bergamota",
      "Sandía",
      "Cedro",
      "Cálamo aromático",
      "Ámbar",
      "Sándalo",
      "Pachulí"
    ],
    "ml": 100,
    "precioUSD": 45,
    "categoria": "arabes",
    "agotado": true
  },
  {
    "id": 97,
    "house": "DUMONT",
    "name": "Dumont Nitro Red Intensely",
    "image": "dumont-nitro-red-intensely-100ml-m.webp",
    "familia": "Frutal",
    "tipo": "Eau de Parfum",
    "genero": "Masculino",
    "descripcion": "Una fragancia para el hombre moderna y seductora, de perfil afrutado, acuático y amaderado, con una salida vibrante y un fondo cálido de gran presencia. Se inspira en el estilo de los perfumes masculinos frescos y adictivos de diseñador, con una vibra que recuerda a composiciones juveniles y magnéticas de gran proyección.",
    "description": "Una fragancia para el hombre moderna y seductora, de perfil afrutado, acuático y amaderado, con una salida vibrante y un fondo cálido de gran presencia. Se inspira en el estilo de los perfumes masculinos frescos y adictivos de diseñador, con una vibra que recuerda a composiciones juveniles y magnéticas de gran proyección.",
    "notasSalida": "Bergamota, manzana roja, melocotón, sandía",
    "notasCorazon": "lavanda, cedro, acorde marino, sándalo",
    "notasFondo": "pachulí, ámbar, musgo",
    "notes": [
      "Bergamota",
      "manzana roja",
      "melocotón",
      "sandía",
      "lavanda",
      "cedro",
      "acorde marino",
      "sándalo",
      "pachulí",
      "ámbar",
      "musgo"
    ],
    "ml": 100,
    "precioUSD": 60,
    "categoria": "arabes",
    "agotado": true
  },
  {
    "id": 98,
    "house": "DUMONT",
    "name": "Dumont Nitro White",
    "image": "dumont-nitro-white-100ml-m.webp",
    "familia": "Oriental",
    "tipo": "Eau de Parfum",
    "genero": "Masculino",
    "descripcion": "Fragancia de cuero intensa y sofisticada, pensada para el hombre que busca una estela elegante, moderna y con carácter. Nitro White de Dumont se mueve entre un perfil cálido de miel y ámbar con un fondo cremoso de vainilla, cuero y almizcle, evocando un estilo masculino refinado con aire inspirado en la perfumería de diseñador contemporánea.",
    "description": "Fragancia de cuero intensa y sofisticada, pensada para el hombre que busca una estela elegante, moderna y con carácter. Nitro White de Dumont se mueve entre un perfil cálido de miel y ámbar con un fondo cremoso de vainilla, cuero y almizcle, evocando un estilo masculino refinado con aire inspirado en la perfumería de diseñador contemporánea.",
    "notasSalida": "Bayas de enebro, iris, ciprés, mirra",
    "notasCorazon": "pachulí, miel, vainilla",
    "notasFondo": "ámbar, almizcle, cuero",
    "notes": [
      "Bayas de enebro",
      "iris",
      "ciprés",
      "mirra",
      "pachulí",
      "miel",
      "vainilla",
      "ámbar",
      "almizcle",
      "cuero"
    ],
    "ml": 100,
    "precioUSD": 45,
    "categoria": "arabes",
    "agotado": true
  },
  {
    "id": 99,
    "house": "ELIZABETH ARDEN",
    "name": "Elizabeth Arden Green Tea",
    "image": "elizabeth-arden-green-tea-elizabeth-arden-100ml-f.webp",
    "familia": "Floral Frutal",
    "tipo": "Eau de Toilette",
    "genero": "Femenino",
    "descripcion": "Una fragancia cítrica aromática para la mujer, fresca, luminosa y fácil de llevar en el día a día. Su perfil limpio y revitalizante, dominado por el té verde y los cítricos, le da un aire clásico y natural; no se presenta como inspiración directa de un perfume de diseñador, sino como una creación propia de Elizabeth Arden.",
    "description": "Una fragancia cítrica aromática para la mujer, fresca, luminosa y fácil de llevar en el día a día. Su perfil limpio y revitalizante, dominado por el té verde y los cítricos, le da un aire clásico y natural; no se presenta como inspiración directa de un perfume de diseñador, sino como una creación propia de Elizabeth Arden.",
    "notasSalida": "Limón, Bergamota, Menta, Ruibarbo, Cáscara de naranja",
    "notasCorazon": "Jazmín, Hinojo, Almizcle, Ámbar, Musgo de roble",
    "notasFondo": "Té verde, Semillas de apio, Alcaravea, Clavo",
    "notes": [
      "Limón",
      "Bergamota",
      "Menta",
      "Ruibarbo",
      "Cáscara de naranja",
      "Jazmín",
      "Hinojo",
      "Almizcle",
      "Ámbar",
      "Musgo de roble",
      "Té verde",
      "Semillas de apio",
      "Alcaravea",
      "Clavo"
    ],
    "ml": 100,
    "precioUSD": 20,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 100,
    "house": "ELIZABETH ARDEN",
    "name": "Elizabeth Arden Sunflowers",
    "image": "elizabeth-arden-sunflowers-100ml-f.webp",
    "familia": "Floral Frutal",
    "tipo": "Eau de Toilette",
    "genero": "Femenino",
    "descripcion": "Una fragancia luminosa y alegre para la mujer, con un estilo floral-frutal fresco, jugoso y delicadamente cálido. Sunflowers de Elizabeth Arden evoca la esencia de un clásico femenino atemporal, inspirado en la vitalidad del verano y el encanto de las fragancias florales de diseñador.",
    "description": "Una fragancia luminosa y alegre para la mujer, con un estilo floral-frutal fresco, jugoso y delicadamente cálido. Sunflowers de Elizabeth Arden evoca la esencia de un clásico femenino atemporal, inspirado en la vitalidad del verano y el encanto de las fragancias florales de diseñador.",
    "notasSalida": "Flor de azahar del naranjo, Melón, Mandarina, Melocotón, Bergamota, Limón",
    "notasCorazon": "Palo de rosa de Brasil, Ciclamen, Osmanto, Raíz de lirio, Jazmín, Rosa",
    "notasFondo": "Sándalo, Ámbar, Almizcle, Musgo de roble, Cedro",
    "notes": [
      "Flor de azahar del naranjo",
      "Melón",
      "Mandarina",
      "Melocotón",
      "Bergamota",
      "Limón",
      "Palo de rosa de Brasil",
      "Ciclamen",
      "Osmanto",
      "Raíz de lirio",
      "Jazmín",
      "Rosa",
      "Sándalo",
      "Ámbar",
      "Almizcle",
      "Musgo de roble",
      "Cedro"
    ],
    "ml": 100,
    "precioUSD": 25,
    "categoria": "disenador"
  },
  {
    "id": 101,
    "house": "EMPER",
    "name": "Emper Ilang 62",
    "image": "emper-ilang-62-100ml-u.webp",
    "familia": "Floral Amaderado",
    "tipo": "Eau de Parfum",
    "genero": "Unisex",
    "descripcion": "Emper Ilang 62 es una fragancia unisex con un carácter floral amaderado sofisticado y elegante, ideal para quienes buscan un aroma distintivo y moderno. Inspirada en Ylang 49 de Le Labo, ofrece una estela refinada con un aire de lujo contemporáneo.",
    "description": "Emper Ilang 62 es una fragancia unisex con un carácter floral amaderado sofisticado y elegante, ideal para quienes buscan un aroma distintivo y moderno. Inspirada en Ylang 49 de Le Labo, ofrece una estela refinada con un aire de lujo contemporáneo.",
    "notasSalida": "Pachulí, Ylang-ylang, Musgo de roble",
    "notasCorazon": "Gardenia de Tahití, Jazmín, Vetiver",
    "notasFondo": "Benjuí, Sándalo",
    "notes": [
      "Pachulí",
      "Ylang-ylang",
      "Musgo de roble",
      "Gardenia de Tahití",
      "Jazmín",
      "Vetiver",
      "Benjuí",
      "Sándalo"
    ],
    "ml": 100,
    "precioUSD": 50,
    "categoria": "arabes"
  },
  {
    "id": 102,
    "house": "EMPER",
    "name": "Emper Stallion 53",
    "image": "emper-stallion-53-100ml-u.webp",
    "familia": "Amaderado",
    "tipo": "Eau de Parfum",
    "genero": "Unisex",
    "descripcion": "Un perfume unisex de carácter sofisticado, con un perfil amaderado, especiado y floral que proyecta elegancia y presencia. Se inspira en el estilo de Santal 33 de Le Labo, reinterpretado con un aire más cálido y envolvente para uso diario o de noche.",
    "description": "Un perfume unisex de carácter sofisticado, con un perfil amaderado, especiado y floral que proyecta elegancia y presencia. Se inspira en el estilo de Santal 33 de Le Labo, reinterpretado con un aire más cálido y envolvente para uso diario o de noche.",
    "notasSalida": "Cardamomo, notas especiadas",
    "notasCorazon": "notas florales, maderas",
    "notasFondo": "sándalo, almizcle",
    "notes": [
      "Cardamomo",
      "notas especiadas",
      "notas florales",
      "maderas",
      "sándalo",
      "almizcle"
    ],
    "ml": 100,
    "precioUSD": 35,
    "categoria": "arabes"
  },
  {
    "id": 103,
    "house": "EMPER",
    "name": "Emper The Black 92",
    "image": "emper-the-black-92-100ml-u.webp",
    "familia": "Amaderado",
    "tipo": "Eau de Parfum",
    "genero": "Unisex",
    "descripcion": "Unisex y sofisticado, este perfume se mueve entre lo amaderado aromático y un matiz especiado-afrutado con carácter elegante. Emper The Black 92 se inspira claramente en el estilo de The Noir 29 de Le Labo, ofreciendo una firma moderna, envolvente y de lujo para quienes buscan una fragancia distintiva y versátil.",
    "description": "Unisex y sofisticado, este perfume se mueve entre lo amaderado aromático y un matiz especiado-afrutado con carácter elegante. Emper The Black 92 se inspira claramente en el estilo de The Noir 29 de Le Labo, ofreciendo una firma moderna, envolvente y de lujo para quienes buscan una fragancia distintiva y versátil.",
    "notasSalida": "Higo, Hoja de laurel, Bergamota",
    "notasCorazon": "Cedro, Vetiver, Almizcle",
    "notasFondo": "Tabaco, Heno",
    "notes": [
      "Higo",
      "Hoja de laurel",
      "Bergamota",
      "Cedro",
      "Vetiver",
      "Almizcle",
      "Tabaco",
      "Heno"
    ],
    "ml": 100,
    "precioUSD": 40,
    "categoria": "arabes"
  },
  {
    "id": 104,
    "house": "GIORGIO ARMANI",
    "name": "Giorgio Armani Armani Code Eau de Toilette",
    "image": "giorgio-armani-armani-code-eau-de-toilette-75ml-m.webp",
    "familia": "Amaderado",
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": "Una fragancia para el hombre de estilo oriental amaderado, elegante y seductor, con una estela moderna y sofisticada. Armani Code Eau de Toilette de Giorgio Armani destaca por su carácter sensual y refinado, ideal para quienes buscan un aroma nocturno con aire de diseñador.",
    "description": "Una fragancia para el hombre de estilo oriental amaderado, elegante y seductor, con una estela moderna y sofisticada. Armani Code Eau de Toilette de Giorgio Armani destaca por su carácter sensual y refinado, ideal para quienes buscan un aroma nocturno con aire de diseñador.",
    "notasSalida": "Mandarina verde, bergamota",
    "notasCorazon": "lavanda, haba tonka",
    "notasFondo": "cedro",
    "notes": [
      "Mandarina verde",
      "bergamota",
      "lavanda",
      "haba tonka",
      "cedro"
    ],
    "ml": 75,
    "precioUSD": 130,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 105,
    "house": "GIORGIO ARMANI",
    "name": "Giorgio Armani Code for Women",
    "image": "giorgio-armani-code-for-women-75ml-f.webp",
    "familia": "Floral Amaderado",
    "tipo": "Eau de Toilette",
    "genero": "Femenino",
    "descripcion": "Una fragancia floral oriental para la mujer, elegante y seductora, con un aura luminosa de azahar, jazmín y cítricos. Inspirada en el estilo refinado de Armani Code for Women, combina sensualidad moderna con un fondo cremoso de vainilla, miel y sándalo.",
    "description": "Una fragancia floral oriental para la mujer, elegante y seductora, con un aura luminosa de azahar, jazmín y cítricos. Inspirada en el estilo refinado de Armani Code for Women, combina sensualidad moderna con un fondo cremoso de vainilla, miel y sándalo.",
    "notasSalida": "Jazmín, Naranja italiana, Naranja amarga",
    "notasCorazon": "Flor de azahar del naranjo, Jengibre, Miel",
    "notasFondo": "Vainilla, Sándalo",
    "notes": [
      "Jazmín",
      "Naranja italiana",
      "Naranja amarga",
      "Flor de azahar del naranjo",
      "Jengibre",
      "Miel",
      "Vainilla",
      "Sándalo"
    ],
    "ml": 75,
    "precioUSD": 130,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 106,
    "house": "GIORGIO ARMANI",
    "name": "Emporio Armani Stronger With You Parfum",
    "image": "giorgio-armani-emporio-armani-stronger-with-you-parfum-100ml-m.webp",
    "familia": "Floral",
    "tipo": "Parfum",
    "genero": "Masculino",
    "descripcion": "Un perfume para el hombre con un carácter cálido, elegante y adictivo, que combina una sensualidad moderna con un estilo oriental fougère sofisticado. Se inspira en la firma de Stronger With You de Giorgio Armani, ofreciendo una estela envolvente y masculina ideal para quienes buscan presencia y refinamiento.",
    "description": "Un perfume para el hombre con un carácter cálido, elegante y adictivo, que combina una sensualidad moderna con un estilo oriental fougère sofisticado. Se inspira en la firma de Stronger With You de Giorgio Armani, ofreciendo una estela envolvente y masculina ideal para quienes buscan presencia y refinamiento.",
    "notasSalida": "Pimienta rosa, Lavanda",
    "notasCorazon": "Salvia, Vainilla",
    "notasFondo": "Castaña",
    "notes": [
      "Pimienta rosa",
      "Lavanda",
      "Salvia",
      "Vainilla",
      "Castaña"
    ],
    "ml": 100,
    "precioUSD": 195,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 107,
    "house": "GUCCI",
    "name": "Gucci Bloom",
    "image": "gucci-bloom-100ml-f.webp",
    "familia": "Floral Amaderado",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Un perfume floral de lujo para la mujer, con un carácter cremoso, intenso y delicadamente sofisticado que evoca un jardín en plena floración. Gucci Bloom se inspira en el estilo de un gran floral blanco de diseñador, destacando por su elegancia natural y su feminidad contemporánea.",
    "description": "Un perfume floral de lujo para la mujer, con un carácter cremoso, intenso y delicadamente sofisticado que evoca un jardín en plena floración. Gucci Bloom se inspira en el estilo de un gran floral blanco de diseñador, destacando por su elegancia natural y su feminidad contemporánea.",
    "notasSalida": "Jazmín, Nardo",
    "notasCorazon": "Capullos de jazmín, Madreselva",
    "notasFondo": "Raíz de lirio, Sándalo",
    "notes": [
      "Jazmín",
      "Nardo",
      "Capullos de jazmín",
      "Madreselva",
      "Raíz de lirio",
      "Sándalo"
    ],
    "ml": 100,
    "precioUSD": 135,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 108,
    "house": "GUCCI",
    "name": "Gucci Bloom Eau de Toilette",
    "image": "gucci-bloom-eau-de-toilette-100ml-f.webp",
    "familia": "Floral Amaderado",
    "tipo": "Eau de Toilette",
    "genero": "Femenino",
    "descripcion": "Un floral luminoso y sofisticado para la mujer, inspirado en la feminidad contemporánea y en la línea icónica Gucci Bloom. Su carácter fresco y delicado combina un aire elegante de jardín blanco con un acabado suave y sensual, ideal para quien busca una firma refinada y moderna.",
    "description": "Un floral luminoso y sofisticado para la mujer, inspirado en la feminidad contemporánea y en la línea icónica Gucci Bloom. Su carácter fresco y delicado combina un aire elegante de jardín blanco con un acabado suave y sensual, ideal para quien busca una firma refinada y moderna.",
    "notasSalida": "Neroli, Jazmín",
    "notasCorazon": "Tuberosa, Azahar",
    "notasFondo": "Sándalo, Almizcle",
    "notes": [
      "Neroli",
      "Jazmín",
      "Tuberosa",
      "Azahar",
      "Sándalo",
      "Almizcle"
    ],
    "ml": 100,
    "precioUSD": 120,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 109,
    "house": "GUCCI",
    "name": "Gucci Rush",
    "image": "gucci-rush-75ml-f.webp",
    "familia": "Floral Frutal",
    "tipo": "Eau de Toilette",
    "genero": "Femenino",
    "descripcion": "Un clásico para la mujer con un carácter floral-frutal, sensual y muy magnético, ideal para quienes buscan una firma olfativa intensa y sofisticada. Gucci Rush se inspira en el icónico estilo de Gucci de finales de los 90, con un aura audaz, adictiva y memorable.",
    "description": "Un clásico para la mujer con un carácter floral-frutal, sensual y muy magnético, ideal para quienes buscan una firma olfativa intensa y sofisticada. Gucci Rush se inspira en el icónico estilo de Gucci de finales de los 90, con un aura audaz, adictiva y memorable.",
    "notasSalida": "Durazno, gardenia, fresia africana",
    "notasCorazon": "cilantro, rosa de Damasco, jazmín",
    "notasFondo": "pachulí, vainilla, vetiver",
    "notes": [
      "Durazno",
      "gardenia",
      "fresia africana",
      "cilantro",
      "rosa de Damasco",
      "jazmín",
      "pachulí",
      "vainilla",
      "vetiver"
    ],
    "ml": 75,
    "precioUSD": 90,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 110,
    "house": "GUESS",
    "name": "Guess for Women",
    "image": "guess-guess-for-women-75ml-f.webp",
    "familia": "Floral Frutal",
    "tipo": "Eau de Toilette",
    "genero": "Femenino",
    "descripcion": "Fragancia floral frutal para la mujer, fresca, luminosa y coqueta, con un estilo juvenil y elegante que resulta fácil de llevar a diario. Se inspira en el universo de los florales frutales de diseñador, con un aire limpio y femenino que recuerda a un clásico comercial moderno.",
    "description": "Fragancia floral frutal para la mujer, fresca, luminosa y coqueta, con un estilo juvenil y elegante que resulta fácil de llevar a diario. Se inspira en el universo de los florales frutales de diseñador, con un aire limpio y femenino que recuerda a un clásico comercial moderno.",
    "notasSalida": "Manzana verde, Fresa, Naranja tangerina, Bergamota, Magnolia, Peonía",
    "notasCorazon": "Fresia, Lirio de los valles, Bayas rojas, Jazmín, Durazno",
    "notasFondo": "Ylang-ylang, Almizcle, Cedro, Ámbar, Musgo de roble",
    "notes": [
      "Manzana verde",
      "Fresa",
      "Naranja tangerina",
      "Bergamota",
      "Magnolia",
      "Peonía",
      "Fresia",
      "Lirio de los valles",
      "Bayas rojas",
      "Jazmín",
      "Durazno",
      "Ylang-ylang",
      "Almizcle",
      "Cedro",
      "Ámbar",
      "Musgo de roble"
    ],
    "ml": 75,
    "precioUSD": 35,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 111,
    "house": "GUESS",
    "name": "Guess Seductive",
    "image": "guess-guess-seductive-75ml-f.webp",
    "familia": "Floral Frutal",
    "tipo": "Eau de Toilette",
    "genero": "Femenino",
    "descripcion": "Guess Seductive es una fragancia para la mujer, de estilo floral frutal con un aura sensual y sofisticada. Su composición recuerda a un perfume de diseñador moderno, con una salida jugosa y un fondo suave y cremoso que la hace ideal para el día o la noche.",
    "description": "Guess Seductive es una fragancia para la mujer, de estilo floral frutal con un aura sensual y sofisticada. Su composición recuerda a un perfume de diseñador moderno, con una salida jugosa y un fondo suave y cremoso que la hace ideal para el día o la noche.",
    "notasSalida": "Pera, Bergamota, Grosella negra",
    "notasCorazon": "Jazmín, Flor de naranjo africano, Raíz de lirio",
    "notasFondo": "Vainilla, Madera de cachemira, Incienso de olíbano",
    "notes": [
      "Pera",
      "Bergamota",
      "Grosella negra",
      "Jazmín",
      "Flor de naranjo africano",
      "Raíz de lirio",
      "Vainilla",
      "Madera de cachemira",
      "Incienso de olíbano"
    ],
    "ml": 75,
    "precioUSD": 40,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 112,
    "house": "HALLOWEEN",
    "name": "Halloween for Woman",
    "image": "halloween-halloween-for-woman-100ml-f.webp",
    "familia": "Oriental",
    "tipo": "Eau de Toilette",
    "genero": "Femenino",
    "descripcion": "Un perfume para la mujer de estilo floral ámbar con un aura misteriosa, elegante y ligeramente sensual. Halloween for Woman es una fragancia icónica que evoca el universo de los perfumes de diseñador con un carácter propio, reconocible por su mezcla de frescura, flores y un fondo cálido y envolvente.",
    "description": "Un perfume para la mujer de estilo floral ámbar con un aura misteriosa, elegante y ligeramente sensual. Halloween for Woman es una fragancia icónica que evoca el universo de los perfumes de diseñador con un carácter propio, reconocible por su mezcla de frescura, flores y un fondo cálido y envolvente.",
    "notasSalida": "Violeta, notas marinas, hoja de plátano, petit grain",
    "notasCorazon": "magnolia, lirio de los valles, nardos, pimienta",
    "notasFondo": "incienso, sándalo, vainilla de Madagascar, mirra",
    "notes": [
      "Violeta",
      "notas marinas",
      "hoja de plátano",
      "petit grain",
      "magnolia",
      "lirio de los valles",
      "nardos",
      "pimienta",
      "incienso",
      "sándalo",
      "vainilla de Madagascar",
      "mirra"
    ],
    "ml": 100,
    "precioUSD": 55,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 113,
    "house": "HUGO BOSS",
    "name": "Boss Woman",
    "image": "hugo-boss-boss-orange-75ml-f.webp",
    "familia": "Frutal",
    "tipo": "Eau de Toilette",
    "genero": "Femenino",
    "descripcion": "Una fragancia floral y moderna para la mujer, con un carácter cálido, delicado y luminoso. Boss Orange de Hugo Boss destaca por su estilo casual-chic y femenino, ideal para el día a día, con una inspiración suave y contemporánea en perfumes de diseñador.",
    "description": "Una fragancia floral y moderna para la mujer, con un carácter cálido, delicado y luminoso. Boss Orange de Hugo Boss destaca por su estilo casual-chic y femenino, ideal para el día a día, con una inspiración suave y contemporánea en perfumes de diseñador.",
    "notasSalida": "Manzana roja, flores blancas",
    "notasCorazon": "flor de azahar africano, sándalo",
    "notasFondo": "olivo, vainilla",
    "notes": [
      "Manzana roja",
      "flores blancas",
      "flor de azahar africano",
      "sándalo",
      "olivo",
      "vainilla"
    ],
    "ml": 75,
    "precioUSD": 40,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 114,
    "house": "HUGO BOSS",
    "name": "Hugo Boss Deep Red",
    "image": "hugo-boss-deep-red-90ml-f.webp",
    "familia": "Floral Frutal",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Un perfume para la mujer de carácter floral frutal, sensual y vibrante, con una salida jugosa y luminosa que evoluciona hacia un corazón especiado y elegante. Deep Red Hugo Boss es una fragancia femenina icónica, ideal si buscas un aroma seductor con un aire sofisticado y moderno, sin inspiración declarada en otro perfume de diseñador.",
    "description": "Un perfume para la mujer de carácter floral frutal, sensual y vibrante, con una salida jugosa y luminosa que evoluciona hacia un corazón especiado y elegante. Deep Red Hugo Boss es una fragancia femenina icónica, ideal si buscas un aroma seductor con un aire sofisticado y moderno, sin inspiración declarada en otro perfume de diseñador.",
    "notasSalida": "Clementina, Grosella negra, Mandarina, Naranja sanguina, Pera",
    "notasCorazon": "Fresia, Flor de jengibre, Hibisco, Tuberosa",
    "notasFondo": "Sándalo, Cedro, Almizcle, Vainilla",
    "notes": [
      "Clementina",
      "Grosella negra",
      "Mandarina",
      "Naranja sanguina",
      "Pera",
      "Fresia",
      "Flor de jengibre",
      "Hibisco",
      "Tuberosa",
      "Sándalo",
      "Cedro",
      "Almizcle",
      "Vainilla"
    ],
    "ml": 90,
    "precioUSD": 45,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 115,
    "house": "HUGO BOSS",
    "name": "Hugo Boss Hugo XX",
    "image": "hugo-boss-hugo-xx-100ml-f.webp",
    "familia": "Floral Frutal",
    "tipo": "Eau de Toilette",
    "genero": "Femenino",
    "descripcion": "Una fragancia para la mujer de estilo floral frutal, elegante y moderna, con un inicio jugoso y luminoso que evoluciona hacia un corazón femenino y delicado. Hugo XX de Hugo Boss se inspira en un perfil contemporáneo y sofisticado, ideal para uso diario o para ocasiones especiales.",
    "description": "Una fragancia para la mujer de estilo floral frutal, elegante y moderna, con un inicio jugoso y luminoso que evoluciona hacia un corazón femenino y delicado. Hugo XX de Hugo Boss se inspira en un perfil contemporáneo y sofisticado, ideal para uso diario o para ocasiones especiales.",
    "notasSalida": "Lichi, Brotes de grosella negra, Mandarina",
    "notasCorazon": "Rosa, Jazmín, Ámbar",
    "notasFondo": "Almizcle, Sándalo",
    "notes": [
      "Lichi",
      "Brotes de grosella negra",
      "Mandarina",
      "Rosa",
      "Jazmín",
      "Ámbar",
      "Almizcle",
      "Sándalo"
    ],
    "ml": 100,
    "precioUSD": 40,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 116,
    "house": "ISSEY MIYAKE",
    "name": "Issey Miyake A Drop d'Issey",
    "image": "issey-miyake-a-drop-dissey-90ml-f.webp",
    "familia": "Floral Amaderado",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "A Drop d'Issey de Issey Miyake es una fragancia para la mujer de carácter floral, luminoso y contemporáneo, con una estela limpia y elegante. Su estilo etéreo y moderno la convierte en una opción sofisticada para quienes buscan un perfume delicado, inspirado en la estética fresca y acuática propia de la casa Issey Miyake.",
    "description": "A Drop d'Issey de Issey Miyake es una fragancia para la mujer de carácter floral, luminoso y contemporáneo, con una estela limpia y elegante. Su estilo etéreo y moderno la convierte en una opción sofisticada para quienes buscan un perfume delicado, inspirado en la estética fresca y acuática propia de la casa Issey Miyake.",
    "notasSalida": "Lila, Almizcle, Almendra",
    "notasCorazon": "Rosa damascena, Flor de azahar",
    "notasFondo": "Vainilla, Sándalo",
    "notes": [
      "Lila",
      "Almizcle",
      "Almendra",
      "Rosa damascena",
      "Flor de azahar",
      "Vainilla",
      "Sándalo"
    ],
    "ml": 90,
    "precioUSD": 50,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 117,
    "house": "ISSEY MIYAKE",
    "name": "Issey Miyake Pleats Please L'Eau",
    "image": "issey-miyake-pleats-please-leau-100ml-f.webp",
    "familia": "Floral Amaderado",
    "tipo": "Eau de Toilette",
    "genero": "Femenino",
    "descripcion": "Una fragancia floral y almizclada para la mujer, luminosa y delicadamente moderna, con un aire fresco y elegante que realza la feminidad sin exceso. Pleats Please L'Eau se inspira en el universo creativo y sofisticado de Issey Miyake, con una estela limpia, rosada y suave que resulta ideal para el día a día.",
    "description": "Una fragancia floral y almizclada para la mujer, luminosa y delicadamente moderna, con un aire fresco y elegante que realza la feminidad sin exceso. Pleats Please L'Eau se inspira en el universo creativo y sofisticado de Issey Miyake, con una estela limpia, rosada y suave que resulta ideal para el día a día.",
    "notasSalida": "Rosa silvestre, Rosa búlgara, Pimienta rosa",
    "notasCorazon": "Néroli, Almizcle blanco",
    "notasFondo": "Cedro, Pachulí",
    "notes": [
      "Rosa silvestre",
      "Rosa búlgara",
      "Pimienta rosa",
      "Néroli",
      "Almizcle blanco",
      "Cedro",
      "Pachulí"
    ],
    "ml": 100,
    "precioUSD": 45,
    "categoria": "disenador"
  },
  {
    "id": 118,
    "house": "JACQUES SAINT PRÉ",
    "name": "Jacques Saint Pres Ciel",
    "image": "jacques-saint-pres-ciel-100ml-f.webp",
    "familia": "Floral",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Un Eau de Parfum para la mujer de estilo suave, polvoriento y floral, con un aire clásico y elegante. Se inspira en la feminidad romántica de los grandes florales orientales, evocando un perfil cercano a perfumes de diseñador de carácter envolvente y atemporal.",
    "description": "Un Eau de Parfum para la mujer de estilo suave, polvoriento y floral, con un aire clásico y elegante. Se inspira en la feminidad romántica de los grandes florales orientales, evocando un perfil cercano a perfumes de diseñador de carácter envolvente y atemporal.",
    "notasSalida": "Vainilla, Rosa blanca",
    "notasCorazon": "Flores blancas, Almizcle",
    "notasFondo": "Acuerdo polvoroso",
    "notes": [
      "Vainilla",
      "Rosa blanca",
      "Flores blancas",
      "Almizcle",
      "Acuerdo polvoroso"
    ],
    "ml": 100,
    "precioUSD": 20,
    "categoria": "disenador"
  },
  {
    "id": 119,
    "house": "JIMMY CHOO",
    "name": "Jimmy Choo Flash",
    "image": "jimmy-choo-flash-100ml-f.webp",
    "familia": "Floral Frutal",
    "tipo": "Eau de Toilette",
    "genero": "Femenino",
    "descripcion": "Flash by Jimmy Choo es una fragancia floral y luminosa para la mujer moderna, con un carácter sofisticado, sensual y lleno de energía. Su estilo vibrante recuerda a una creación de diseñador pensada para destacar en ocasiones especiales, con una estela femenina y elegante.",
    "description": "Flash by Jimmy Choo es una fragancia floral y luminosa para la mujer moderna, con un carácter sofisticado, sensual y lleno de energía. Su estilo vibrante recuerda a una creación de diseñador pensada para destacar en ocasiones especiales, con una estela femenina y elegante.",
    "notasSalida": "Fresa, Pimienta rosa, Naranja, Tuberosa",
    "notasCorazon": "Jazmín, Lirio, Heliotropo",
    "notasFondo": "Vainilla, Almizcle, Madera",
    "notes": [
      "Fresa",
      "Pimienta rosa",
      "Naranja",
      "Tuberosa",
      "Jazmín",
      "Lirio",
      "Heliotropo",
      "Vainilla",
      "Almizcle",
      "Madera"
    ],
    "ml": 100,
    "precioUSD": 55,
    "categoria": "disenador"
  },
  {
    "id": 120,
    "house": "JO MILANO PARIS",
    "name": "Jo Milano Paris Game of Spades Full House",
    "image": "jo-milano-paris-game-of-spades-full-house-100ml-u.webp",
    "familia": "Oriental",
    "tipo": "Eau de Parfum",
    "genero": "Unisex",
    "descripcion": "Una fragancia unisex sofisticada y vibrante, con un perfil cítrico-especiado que transmite frescura moderna y elegancia nocturna. Se inspira en el estilo de nicho con un carácter muy cercano al universo de diseñador, ideal para quien busca presencia, versatilidad y un acabado distintivo.",
    "description": "Una fragancia unisex sofisticada y vibrante, con un perfil cítrico-especiado que transmite frescura moderna y elegancia nocturna. Se inspira en el estilo de nicho con un carácter muy cercano al universo de diseñador, ideal para quien busca presencia, versatilidad y un acabado distintivo.",
    "notasSalida": "Cidra, Bergamota de Calabria, Naranja siciliana, Jengibre",
    "notasCorazon": "Neroli de Túnez, Canela de Ceilán, Té negro",
    "notasFondo": "Ambroxan, Madera de gaiac, Incienso",
    "notes": [
      "Cidra",
      "Bergamota de Calabria",
      "Naranja siciliana",
      "Jengibre",
      "Neroli de Túnez",
      "Canela de Ceilán",
      "Té negro",
      "Ambroxan",
      "Madera de gaiac",
      "Incienso"
    ],
    "ml": 100,
    "precioUSD": 120,
    "categoria": "arabes",
    "agotado": true
  },
  {
    "id": 121,
    "house": "JO MILANO PARIS",
    "name": "Jo Milano Paris Game of Spades Gold",
    "image": "jo-milano-paris-game-of-spades-gold-90ml-u.webp",
    "familia": "Amaderado",
    "tipo": "Eau de Parfum",
    "genero": "Unisex",
    "descripcion": "Game of Spades Gold de Jo Milano Paris es una fragancia unisex de perfil oriental amaderado, elegante y envolvente, ideal para quienes buscan un aroma sofisticado con presencia. Su estilo lujoso y cálido recuerda a perfumes nicho de carácter ámbar y amaderado, con un aire inspirado en propuestas modernas de diseñador de alta gama.",
    "description": "Game of Spades Gold de Jo Milano Paris es una fragancia unisex de perfil oriental amaderado, elegante y envolvente, ideal para quienes buscan un aroma sofisticado con presencia. Su estilo lujoso y cálido recuerda a perfumes nicho de carácter ámbar y amaderado, con un aire inspirado en propuestas modernas de diseñador de alta gama.",
    "notasSalida": "Ámbar, Brandy, Madera de gaiac",
    "notasCorazon": "Pachulí, Notas afrutadas, Almizcle",
    "notasFondo": "Dulce, Notas amaderadas",
    "notes": [
      "Ámbar",
      "Brandy",
      "Madera de gaiac",
      "Pachulí",
      "Notas afrutadas",
      "Almizcle",
      "Dulce",
      "Notas amaderadas"
    ],
    "ml": 90,
    "precioUSD": 120,
    "categoria": "arabes"
  },
  {
    "id": 122,
    "house": "JO MILANO PARIS",
    "name": "Jo Milano Paris Game of Spades Rouge",
    "image": "jo-milano-paris-game-of-spades-rouge-100ml-u.webp",
    "familia": "Floral Amaderado",
    "tipo": "Eau de Parfum",
    "genero": "Unisex",
    "descripcion": "Game of Spades Rouge de Jo Milano Paris es un perfume unisex de estilo sofisticado y envolvente, creado para quienes buscan una presencia elegante y moderna. Su perfil olfativo opulento recuerda a una inspiración de lujo contemporáneo, con un carácter similar al universo de *Baccarat Rouge 540* de Maison Francis Kurkdjian.",
    "description": "Game of Spades Rouge de Jo Milano Paris es un perfume unisex de estilo sofisticado y envolvente, creado para quienes buscan una presencia elegante y moderna. Su perfil olfativo opulento recuerda a una inspiración de lujo contemporáneo, con un carácter similar al universo de *Baccarat Rouge 540* de Maison Francis Kurkdjian.",
    "notasSalida": "Azafrán, Jazmín",
    "notasCorazon": "Ámbar gris, Madera de cedro",
    "notasFondo": "Resina, Almizcle",
    "notes": [
      "Azafrán",
      "Jazmín",
      "Ámbar gris",
      "Madera de cedro",
      "Resina",
      "Almizcle"
    ],
    "ml": 100,
    "precioUSD": 110,
    "categoria": "arabes",
    "agotado": true
  },
  {
    "id": 123,
    "house": "KATY PERRY",
    "name": "Katy Perry Mad Love",
    "image": "katy-perry-katy-perrys-mad-love-100ml-f.webp",
    "familia": "Floral Frutal",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Una fragancia para la mujer de estilo floral frutal gourmand, dulce y juguetón, con un aire juvenil y adictivo. Katy Perry's Mad Love evoca el carácter divertido de un perfume de diseñador contemporáneo, con una estela cremosa y femenina ideal para el día a día.",
    "description": "Una fragancia para la mujer de estilo floral frutal gourmand, dulce y juguetón, con un aire juvenil y adictivo. Katy Perry's Mad Love evoca el carácter divertido de un perfume de diseñador contemporáneo, con una estela cremosa y femenina ideal para el día a día.",
    "notasSalida": "Sorbete, Fresa, Manzana, Pomelo rosado",
    "notasCorazon": "Notas florales, Peonía, Jazmín",
    "notasFondo": "Coco, Almizcle, Sándalo",
    "notes": [
      "Sorbete",
      "Fresa",
      "Manzana",
      "Pomelo rosado",
      "Notas florales",
      "Peonía",
      "Jazmín",
      "Coco",
      "Almizcle",
      "Sándalo"
    ],
    "ml": 100,
    "precioUSD": 30,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 124,
    "house": "KATY PERRY",
    "name": "Katy Perry Killer Queen",
    "image": "katy-perry-killer-queen-100ml-f.webp",
    "familia": "Floral Frutal",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Un perfume para la mujer de estilo floral frutal gourmand, vibrante y seductor, con un aire dulce y sofisticado. Se inspira en la estética de grandes florales gourmand de diseñador, evocando ese carácter intenso y adictivo que recuerda a fragancias como La Vie Est Belle o Flowerbomb, sin ser un dupe directo.",
    "description": "Un perfume para la mujer de estilo floral frutal gourmand, vibrante y seductor, con un aire dulce y sofisticado. Se inspira en la estética de grandes florales gourmand de diseñador, evocando ese carácter intenso y adictivo que recuerda a fragancias como La Vie Est Belle o Flowerbomb, sin ser un dupe directo.",
    "notasSalida": "Ciruela, Frutos del bosque, Bergamota",
    "notasCorazon": "Celosía, Jazmín Sambac, Frangipani",
    "notasFondo": "Praliné, Cachemira, Pachulí",
    "notes": [
      "Ciruela",
      "Frutos del bosque",
      "Bergamota",
      "Celosía",
      "Jazmín Sambac",
      "Frangipani",
      "Praliné",
      "Cachemira",
      "Pachulí"
    ],
    "ml": 100,
    "precioUSD": 45,
    "categoria": "disenador"
  },
  {
    "id": 125,
    "house": "KATY PERRY",
    "name": "Katy Perry Meow",
    "image": "katy-perry-meow-100ml-f.webp",
    "familia": "Floral Frutal",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Un perfume para la mujer de estilo floral frutal, dulce y coqueto, ideal para quienes buscan una firma olfativa juvenil y adictiva. Meow de Katy Perry destaca por su aire gourmand suave y su carácter femenino, con un perfil que recuerda a perfumes comerciales de diseñador por su mezcla de frutas luminosas, flores blancas y una base cremosa de vainilla.",
    "description": "Un perfume para la mujer de estilo floral frutal, dulce y coqueto, ideal para quienes buscan una firma olfativa juvenil y adictiva. Meow de Katy Perry destaca por su aire gourmand suave y su carácter femenino, con un perfil que recuerda a perfumes comerciales de diseñador por su mezcla de frutas luminosas, flores blancas y una base cremosa de vainilla.",
    "notasSalida": "Pera, Naranja tangerina, Jazmín, Gardenia",
    "notasCorazon": "Madreselva, Flor del naranjo africano, Lirio de los valles, Vainilla",
    "notasFondo": "Ámbar, Almizcle, Sándalo",
    "notes": [
      "Pera",
      "Naranja tangerina",
      "Jazmín",
      "Gardenia",
      "Madreselva",
      "Flor del naranjo africano",
      "Lirio de los valles",
      "Vainilla",
      "Ámbar",
      "Almizcle",
      "Sándalo"
    ],
    "ml": 100,
    "precioUSD": 30,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 126,
    "house": "KATY PERRY",
    "name": "Katy Perry Purr",
    "image": "katy-perry-purr-100ml-f.webp",
    "familia": "Floral Frutal",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Un perfume para la mujer de estilo floral afrutado, dulce y juvenil, con un carácter coqueto y femenino muy fácil de llevar a diario. Su perfil recuerda al encanto de los perfumes de celebridad con un aire romántico y cremoso, ideal si buscas una fragancia alegre y sensual.",
    "description": "Un perfume para la mujer de estilo floral afrutado, dulce y juvenil, con un carácter coqueto y femenino muy fácil de llevar a diario. Su perfil recuerda al encanto de los perfumes de celebridad con un aire romántico y cremoso, ideal si buscas una fragancia alegre y sensual.",
    "notasSalida": "Gardenia, Bambú, Durazno, Manzana roja, Jazmín",
    "notasCorazon": "Fresia, Rosa, Almizcle, Sándalo",
    "notasFondo": "Ámbar, Vainilla, Coco, Orquídea",
    "notes": [
      "Gardenia",
      "Bambú",
      "Durazno",
      "Manzana roja",
      "Jazmín",
      "Fresia",
      "Rosa",
      "Almizcle",
      "Sándalo",
      "Ámbar",
      "Vainilla",
      "Coco",
      "Orquídea"
    ],
    "ml": 100,
    "precioUSD": 30,
    "categoria": "disenador"
  },
  {
    "id": 127,
    "house": "KATY PERRY",
    "name": "Katy Perry Royal Revolution",
    "image": "katy-perry-royal-revolution-100ml-f.webp",
    "familia": "Floral Amaderado",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Una fragancia para la mujer de estilo floral afrutado, luminosa y seductora, con un carácter juvenil y sofisticado. Su perfil evoca una estética de lujo contemporáneo con un toque dulce y envolvente, inspirada en la feminidad moderna y, por su combinación de frutas rojas, flores y cuero, recuerda el estilo de un perfume de diseñador con aire glamuroso.",
    "description": "Una fragancia para la mujer de estilo floral afrutado, luminosa y seductora, con un carácter juvenil y sofisticado. Su perfil evoca una estética de lujo contemporáneo con un toque dulce y envolvente, inspirada en la feminidad moderna y, por su combinación de frutas rojas, flores y cuero, recuerda el estilo de un perfume de diseñador con aire glamuroso.",
    "notasSalida": "Granada, Fresia rosa, Flor de azahar del naranjo",
    "notasCorazon": "Jazmín, Sándalo, Endrino",
    "notasFondo": "Orquídea de vainilla, Almizcle, Cuero",
    "notes": [
      "Granada",
      "Fresia rosa",
      "Flor de azahar del naranjo",
      "Jazmín",
      "Sándalo",
      "Endrino",
      "Orquídea de vainilla",
      "Almizcle",
      "Cuero"
    ],
    "ml": 100,
    "precioUSD": 45,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 128,
    "house": "KENZO",
    "name": "Kenzo Amour",
    "image": "kenzo-amour-100ml-f.webp",
    "familia": "Oriental",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Un perfume para la mujer sensual y delicada, con un aura floral amaderada y almizclada que envuelve con suavidad y calidez. Kenzo Amour se inspira en un viaje de amor y recuerda a un estilo elegante y romántico, con matices que evocan el universo olfativo de fragancias de diseñador sofisticadas.",
    "description": "Un perfume para la mujer sensual y delicada, con un aura floral amaderada y almizclada que envuelve con suavidad y calidez. Kenzo Amour se inspira en un viaje de amor y recuerda a un estilo elegante y romántico, con matices que evocan el universo olfativo de fragancias de diseñador sofisticadas.",
    "notasSalida": "Arroz, Té blanco, Frangipani",
    "notasCorazon": "Flor del cerezo, Heliotropo, Vainilla",
    "notasFondo": "Almizcle, Incienso, Notas amaderadas",
    "notes": [
      "Arroz",
      "Té blanco",
      "Frangipani",
      "Flor del cerezo",
      "Heliotropo",
      "Vainilla",
      "Almizcle",
      "Incienso",
      "Notas amaderadas"
    ],
    "ml": 100,
    "precioUSD": 85,
    "categoria": "disenador"
  },
  {
    "id": 129,
    "house": "KENZO",
    "name": "Kenzo Homme Eau de Toilette",
    "image": "kenzo-homme-eau-de-toilette-110ml-m.webp",
    "familia": "Amaderado",
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": "Kenzo Homme Eau de Toilette es una fragancia para el hombre de carácter amaderado marino, fresca y elegante, con una vibra moderna y natural. Su perfil recuerda al estilo de los grandes perfumes masculinos de diseñador de corte acuático y aromático, con una estela limpia, sofisticada y muy versátil para el día a día.",
    "description": "Kenzo Homme Eau de Toilette es una fragancia para el hombre de carácter amaderado marino, fresca y elegante, con una vibra moderna y natural. Su perfil recuerda al estilo de los grandes perfumes masculinos de diseñador de corte acuático y aromático, con una estela limpia, sofisticada y muy versátil para el día a día.",
    "notasSalida": "Notas marinas, Algas, Bergamota, Menta",
    "notasCorazon": "Pino, Nuez moscada, Sándalo",
    "notasFondo": "Cedro, Vetiver, Almizcle",
    "notes": [
      "Notas marinas",
      "Algas",
      "Bergamota",
      "Menta",
      "Pino",
      "Nuez moscada",
      "Sándalo",
      "Cedro",
      "Vetiver",
      "Almizcle"
    ],
    "ml": 110,
    "precioUSD": 85,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 130,
    "house": "KENZO",
    "name": "Kenzo World Power",
    "image": "kenzo-world-power-75ml-f.webp",
    "familia": "Amaderado",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Kenzo World Power es una fragancia para la mujer de carácter sensual y moderno, con un perfil oriental amaderado que juega entre la sal y la suavidad cremosa. Su estilo olfativo elegante y envolvente recuerda a una lectura más audaz y minimalista de perfumes femeninos de diseñador, ideal para quien busca una firma sofisticada y distintiva.",
    "description": "Kenzo World Power es una fragancia para la mujer de carácter sensual y moderno, con un perfil oriental amaderado que juega entre la sal y la suavidad cremosa. Su estilo olfativo elegante y envolvente recuerda a una lectura más audaz y minimalista de perfumes femeninos de diseñador, ideal para quien busca una firma sofisticada y distintiva.",
    "notasSalida": "Ciprés",
    "notasCorazon": "Sal de mar",
    "notasFondo": "Haba tonka",
    "notes": [
      "Ciprés",
      "Sal de mar",
      "Haba tonka"
    ],
    "ml": 75,
    "precioUSD": 95,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 131,
    "house": "LACOSTE",
    "name": "Lacoste Touch of Pink",
    "image": "lacoste-fragrances-touch-of-pink-90ml-f.webp",
    "familia": "Floral Frutal",
    "tipo": "Eau de Toilette",
    "genero": "Femenino",
    "descripcion": "Una fragancia para la mujer vibrante y juvenil, de estilo floral afrutado con un toque fresco y luminoso. Touch of Pink de Lacoste se inspira en una feminidad espontánea y moderna, con un carácter que recuerda a un perfume de diseñador fácil de llevar en el día a día.",
    "description": "Una fragancia para la mujer vibrante y juvenil, de estilo floral afrutado con un toque fresco y luminoso. Touch of Pink de Lacoste se inspira en una feminidad espontánea y moderna, con un carácter que recuerda a un perfume de diseñador fácil de llevar en el día a día.",
    "notasSalida": "Naranja sanguina, Cardamomo, Durazno",
    "notasCorazon": "Jazmín, Hojas de violeta, Semillas de zanahoria",
    "notasFondo": "Almizcle, Sándalo, Vainilla",
    "notes": [
      "Naranja sanguina",
      "Cardamomo",
      "Durazno",
      "Jazmín",
      "Hojas de violeta",
      "Semillas de zanahoria",
      "Almizcle",
      "Sándalo",
      "Vainilla"
    ],
    "ml": 90,
    "precioUSD": 55,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 132,
    "house": "LATTAFA",
    "name": "Lattafa Afeef",
    "image": "lattafa-afeef-100ml-u.webp",
    "familia": "Floral Frutal",
    "tipo": "Eau de Parfum",
    "genero": "Unisex",
    "descripcion": "Afeef de Lattafa es una fragancia unisex de carácter floral, frutal y ambarado, elegante y moderna, pensada para quienes buscan un aroma envolvente con presencia y sofisticación. Su estela equilibrada recuerda el estilo de un perfume de diseñador con aire refinado y sensual, ideal para destacar en cualquier ocasión.",
    "description": "Afeef de Lattafa es una fragancia unisex de carácter floral, frutal y ambarado, elegante y moderna, pensada para quienes buscan un aroma envolvente con presencia y sofisticación. Su estela equilibrada recuerda el estilo de un perfume de diseñador con aire refinado y sensual, ideal para destacar en cualquier ocasión.",
    "notasSalida": "Durazno, Pimienta rosa, Bergamota, Lavanda",
    "notasCorazon": "Rosa, Nardo, Flor de azahar del naranjo, Jazmín",
    "notasFondo": "Sándalo, Praliné, Ámbar, Pachulí",
    "notes": [
      "Durazno",
      "Pimienta rosa",
      "Bergamota",
      "Lavanda",
      "Rosa",
      "Nardo",
      "Flor de azahar del naranjo",
      "Jazmín",
      "Sándalo",
      "Praliné",
      "Ámbar",
      "Pachulí"
    ],
    "ml": 100,
    "precioUSD": 60,
    "categoria": "arabes",
    "agotado": true
  },
  {
    "id": 133,
    "house": "LATTAFA",
    "name": "Lattafa Ajwad",
    "image": "lattafa-ajwad-60ml-u.webp",
    "familia": "Floral Frutal",
    "tipo": "Eau de Parfum",
    "genero": "Unisex",
    "descripcion": "Ajwad de Lattafa es una fragancia unisex de estilo dulce, afrutado y floral, con un fondo cálido y elegante de maderas y almizcle. Su perfil envolvente y sofisticado recuerda a un perfume de diseñador en clave oriental, ideal para quienes buscan un aroma opulento y versátil.",
    "description": "Ajwad de Lattafa es una fragancia unisex de estilo dulce, afrutado y floral, con un fondo cálido y elegante de maderas y almizcle. Su perfil envolvente y sofisticado recuerda a un perfume de diseñador en clave oriental, ideal para quienes buscan un aroma opulento y versátil.",
    "notasSalida": "Notas afrutadas, Bergamota, Lichi, Rosa",
    "notasCorazon": "Jazmín, Canela, Almizcle, Vainilla",
    "notasFondo": "Ámbar, Cedro, Sándalo",
    "notes": [
      "Notas afrutadas",
      "Bergamota",
      "Lichi",
      "Rosa",
      "Jazmín",
      "Canela",
      "Almizcle",
      "Vainilla",
      "Ámbar",
      "Cedro",
      "Sándalo"
    ],
    "ml": 60,
    "precioUSD": 40,
    "categoria": "arabes"
  },
  {
    "id": 134,
    "house": "LATTAFA",
    "name": "Lattafa Ajwad Pink to Pink",
    "image": "lattafa-ajwad-pink-to-pink-60ml-u.webp",
    "familia": "Floral Frutal",
    "tipo": "Eau de Parfum",
    "genero": "Unisex",
    "descripcion": "Ajwad Pink to Pink de Lattafa es una fragancia unisex luminosa, moderna y elegante, con un perfil floral-afrutado de aire sofisticado. Su estilo recuerda a una composición de nicho contemporánea, con un toque envolvente y sensual que la hace ideal para quienes buscan una firma versátil y refinada.",
    "description": "Ajwad Pink to Pink de Lattafa es una fragancia unisex luminosa, moderna y elegante, con un perfil floral-afrutado de aire sofisticado. Su estilo recuerda a una composición de nicho contemporánea, con un toque envolvente y sensual que la hace ideal para quienes buscan una firma versátil y refinada.",
    "notasSalida": "Guayaba, Pomelo rosado, Lichi, Pimienta rosa",
    "notasCorazon": "Magnolia, Peonía, Rosa, Ámbar",
    "notasFondo": "Vainilla, Cuero, Almizcle, Musgo",
    "notes": [
      "Guayaba",
      "Pomelo rosado",
      "Lichi",
      "Pimienta rosa",
      "Magnolia",
      "Peonía",
      "Rosa",
      "Ámbar",
      "Vainilla",
      "Cuero",
      "Almizcle",
      "Musgo"
    ],
    "ml": 60,
    "precioUSD": 40,
    "categoria": "arabes"
  },
  {
    "id": 135,
    "house": "LATTAFA",
    "name": "Lattafa Al Areeq Silver",
    "image": "lattafa-al-areeq-silver-100ml-u.webp",
    "familia": "Floral Frutal",
    "tipo": "Eau de Parfum",
    "genero": "Unisex",
    "descripcion": "Unisex de carácter elegante y magnético, con una faceta aromática, amaderada y de cuero que deja una estela sofisticada y contundente. Se inspira en el estilo de fragancias nicho intensas y oscuras, con un aire que recuerda a perfumes de diseñador de cuero y tabaco.",
    "description": "Unisex de carácter elegante y magnético, con una faceta aromática, amaderada y de cuero que deja una estela sofisticada y contundente. Se inspira en el estilo de fragancias nicho intensas y oscuras, con un aire que recuerda a perfumes de diseñador de cuero y tabaco.",
    "notasSalida": "Lavanda, Manzana, Cardamomo, Tabaco",
    "notasCorazon": "Madera de oud, Ciprés, Jazmín, Cuero",
    "notasFondo": "Vainilla, Sándalo, Almizcle",
    "notes": [
      "Lavanda",
      "Manzana",
      "Cardamomo",
      "Tabaco",
      "Madera de oud",
      "Ciprés",
      "Jazmín",
      "Cuero",
      "Vainilla",
      "Sándalo",
      "Almizcle"
    ],
    "ml": 100,
    "precioUSD": 50,
    "categoria": "arabes"
  },
  {
    "id": 136,
    "house": "LATTAFA",
    "name": "Lattafa Ana Abiyedh",
    "image": "lattafa-ana-abiyedh-60ml-u.webp",
    "familia": "Cítrico",
    "tipo": "Eau de Parfum",
    "genero": "Unisex",
    "descripcion": "Lattafa Ana Abiyedh es un perfume unisex de estilo dulce, afrutado y almizclado, con un carácter limpio y envolvente que resulta muy versátil para el día a día. Su perfil recuerda a una creación moderna de nicho con aire gourmand, ideal para quienes buscan una fragancia elegante y adictiva sin una dirección claramente masculina o femenina.",
    "description": "Lattafa Ana Abiyedh es un perfume unisex de estilo dulce, afrutado y almizclado, con un carácter limpio y envolvente que resulta muy versátil para el día a día. Su perfil recuerda a una creación moderna de nicho con aire gourmand, ideal para quienes buscan una fragancia elegante y adictiva sin una dirección claramente masculina o femenina.",
    "notasSalida": "Bergamota, Naranja",
    "notasCorazon": "Vainilla, Pera",
    "notasFondo": "Almizcle, Ámbar",
    "notes": [
      "Bergamota",
      "Naranja",
      "Vainilla",
      "Pera",
      "Almizcle",
      "Ámbar"
    ],
    "ml": 60,
    "precioUSD": 30,
    "categoria": "arabes"
  },
  {
    "id": 137,
    "house": "LATTAFA",
    "name": "Lattafa Ana Abiyedh Rouge",
    "image": "lattafa-ana-abiyedh-rouge-60ml-u.webp",
    "familia": "Cítrico",
    "tipo": "Eau de Parfum",
    "genero": "Unisex",
    "descripcion": "Unisex y sofisticado, este perfume envuelve con un acorde dulce-ambarado de gran presencia, ideal para quienes buscan una estela moderna y elegante. Inspirado en el icónico Baccarat Rouge 540, ofrece un perfil cálido, luminoso y adictivo con un aire de lujo contemporáneo.",
    "description": "Unisex y sofisticado, este perfume envuelve con un acorde dulce-ambarado de gran presencia, ideal para quienes buscan una estela moderna y elegante. Inspirado en el icónico Baccarat Rouge 540, ofrece un perfil cálido, luminoso y adictivo con un aire de lujo contemporáneo.",
    "notasSalida": "Pera Nashi, Kumquat, Bergamota",
    "notasCorazon": "Caramelo, Geranio, Ámbar gris",
    "notasFondo": "Almizcle, Musgo de roble",
    "notes": [
      "Pera Nashi",
      "Kumquat",
      "Bergamota",
      "Caramelo",
      "Geranio",
      "Ámbar gris",
      "Almizcle",
      "Musgo de roble"
    ],
    "ml": 60,
    "precioUSD": 30,
    "categoria": "arabes"
  },
  {
    "id": 138,
    "house": "LATTAFA",
    "name": "Lattafa Ana Abiyedh Scarlet",
    "image": "lattafa-ana-abiyedh-scarlet-60ml-f.webp",
    "familia": "Floral Frutal",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Un perfume floral frutal gourmand para la mujer, vibrante y adictivo, con un equilibrio seductor entre dulzura, flores y calidez especiada. Se inspira en un estilo moderno de perfumería de diseñador, con un carácter sofisticado y muy femenino.",
    "description": "Un perfume floral frutal gourmand para la mujer, vibrante y adictivo, con un equilibrio seductor entre dulzura, flores y calidez especiada. Se inspira en un estilo moderno de perfumería de diseñador, con un carácter sofisticado y muy femenino.",
    "notasSalida": "Pimienta rosa, cereza negra, rosa blanca, peonía",
    "notasCorazon": "haba tonka, nuez moscada, azúcar",
    "notasFondo": "pachulí, caramelo, vainilla",
    "notes": [
      "Pimienta rosa",
      "cereza negra",
      "rosa blanca",
      "peonía",
      "haba tonka",
      "nuez moscada",
      "azúcar",
      "pachulí",
      "caramelo",
      "vainilla"
    ],
    "ml": 60,
    "precioUSD": 30,
    "categoria": "arabes"
  },
  {
    "id": 139,
    "house": "LATTAFA",
    "name": "Lattafa Angham",
    "image": "lattafa-angham-100ml-u.webp",
    "familia": "Floral",
    "tipo": "Eau de Parfum",
    "genero": "Unisex",
    "descripcion": "Angham de Lattafa es un perfume unisex de estilo oriental vainilla, elegante y envolvente, con un toque dulce, especiado y moderno. Se inspira en el universo de Burberry Goddess, ofreciendo una interpretación más cálida y accesible para quienes buscan una firma sofisticada y versátil.",
    "description": "Angham de Lattafa es un perfume unisex de estilo oriental vainilla, elegante y envolvente, con un toque dulce, especiado y moderno. Se inspira en el universo de Burberry Goddess, ofreciendo una interpretación más cálida y accesible para quienes buscan una firma sofisticada y versátil.",
    "notasSalida": "Jengibre, Clementina, Pimienta rosa",
    "notasCorazon": "Lavanda, Praliné",
    "notasFondo": "Vainilla, Ámbar",
    "notes": [
      "Jengibre",
      "Clementina",
      "Pimienta rosa",
      "Lavanda",
      "Praliné",
      "Vainilla",
      "Ámbar"
    ],
    "ml": 100,
    "precioUSD": 45,
    "categoria": "arabes"
  },
  {
    "id": 140,
    "house": "LATTAFA",
    "name": "Lattafa Art of Universe",
    "image": "lattafa-art-of-universe-100ml-u.webp",
    "familia": "Cítrico",
    "tipo": "Eau de Parfum",
    "genero": "Unisex",
    "descripcion": "Una fragancia unisex de estilo cítrico aromático, fresca, luminosa y moderna, ideal para quienes buscan un aroma elegante y versátil para el día a día. Se inspira en el perfil sofisticado de Blue Talisman de Ex Nihilo, con un carácter vibrante y limpio que deja una estela contemporánea y refinada.",
    "description": "Una fragancia unisex de estilo cítrico aromático, fresca, luminosa y moderna, ideal para quienes buscan un aroma elegante y versátil para el día a día. Se inspira en el perfil sofisticado de Blue Talisman de Ex Nihilo, con un carácter vibrante y limpio que deja una estela contemporánea y refinada.",
    "notasSalida": "Bergamota, Mandarina, Jengibre, Menta",
    "notasCorazon": "Pera, Flor de naranja, Jengibre",
    "notasFondo": "Ámbar, Almizcle, Notas amaderadas",
    "notes": [
      "Bergamota",
      "Mandarina",
      "Jengibre",
      "Menta",
      "Pera",
      "Flor de naranja",
      "Jengibre",
      "Ámbar",
      "Almizcle",
      "Notas amaderadas"
    ],
    "ml": 100,
    "precioUSD": 50,
    "categoria": "arabes",
    "agotado": true
  },
  {
    "id": 141,
    "house": "LATTAFA",
    "name": "Lattafa Asad",
    "image": "lattafa-asad-100ml-m.webp",
    "familia": "Oriental",
    "tipo": "Eau de Parfum",
    "genero": "Masculino",
    "descripcion": "Una fragancia para el hombre intensa, oriental y seductora, con un perfil especiado y amaderado que deja una estela cálida y sofisticada. Se inspira en el estilo de perfumes masculinos modernos de gran presencia, con un aire que recuerda a composiciones densas y elegantes de diseñador.",
    "description": "Una fragancia para el hombre intensa, oriental y seductora, con un perfil especiado y amaderado que deja una estela cálida y sofisticada. Se inspira en el estilo de perfumes masculinos modernos de gran presencia, con un aire que recuerda a composiciones densas y elegantes de diseñador.",
    "notasSalida": "Pimienta negra, Tabaco, Piña, Pachulí",
    "notasCorazon": "Café, Iris, Vainilla, Ámbar",
    "notasFondo": "Madera seca, Benjuí, Ládano",
    "notes": [
      "Pimienta negra",
      "Tabaco",
      "Piña",
      "Pachulí",
      "Café",
      "Iris",
      "Vainilla",
      "Ámbar",
      "Madera seca",
      "Benjuí",
      "Ládano"
    ],
    "ml": 100,
    "precioUSD": 40,
    "categoria": "arabes"
  },
  {
    "id": 142,
    "house": "LATTAFA",
    "name": "Lattafa Asad Bourbon",
    "image": "lattafa-asad-bourbon-100ml-m.webp",
    "familia": "Gourmand",
    "tipo": "Eau de Parfum",
    "genero": "Masculino",
    "descripcion": "Asad Bourbon de Lattafa es un perfume para el hombre con una estela cálida, especiada y gourmand, pensado para transmitir presencia y sofisticación. Su carácter se inspira en el estilo de fragancias masculinas intensas y seductoras, con un aire que recuerda a creaciones modernas de diseñador como Valentino Born in Roma Intense o Stronger With You Intensely.",
    "description": "Asad Bourbon de Lattafa es un perfume para el hombre con una estela cálida, especiada y gourmand, pensado para transmitir presencia y sofisticación. Su carácter se inspira en el estilo de fragancias masculinas intensas y seductoras, con un aire que recuerda a creaciones modernas de diseñador como Valentino Born in Roma Intense o Stronger With You Intensely.",
    "notasSalida": "Lavanda, Ciruela Mirabel, Pimienta rosa",
    "notasCorazon": "Nuez moscada, Davana, Cacao",
    "notasFondo": "Vainilla Bourbon, Ámbar, Vetiver",
    "notes": [
      "Lavanda",
      "Ciruela Mirabel",
      "Pimienta rosa",
      "Nuez moscada",
      "Davana",
      "Cacao",
      "Vainilla Bourbon",
      "Ámbar",
      "Vetiver"
    ],
    "ml": 100,
    "precioUSD": 40,
    "categoria": "arabes"
  },
  {
    "id": 143,
    "house": "LATTAFA",
    "name": "Lattafa Asad Elixir",
    "image": "lattafa-asad-elixir-100ml-m.webp",
    "familia": "Floral Amaderado",
    "tipo": "Elixir",
    "genero": "Masculino",
    "descripcion": "Perfume para el hombre de carácter oriental, intenso y elegante, con una estela cálida y moderna. Inspirado en el estilo de fragancias masculinas especiadas y amaderadas, recuerda a un perfil sofisticado con guiños al ADN de Asad y al universo de perfumes tipo diseñador de corte avainillado y tabacoso.",
    "description": "Perfume para el hombre de carácter oriental, intenso y elegante, con una estela cálida y moderna. Inspirado en el estilo de fragancias masculinas especiadas y amaderadas, recuerda a un perfil sofisticado con guiños al ADN de Asad y al universo de perfumes tipo diseñador de corte avainillado y tabacoso.",
    "notasSalida": "Pimienta rosa, Azafrán, Pomelo, Tabaco",
    "notasCorazon": "Cedro, Vainilla, Pachulí",
    "notasFondo": "Incienso, Madera de cachemira, Ámbar seco",
    "notes": [
      "Pimienta rosa",
      "Azafrán",
      "Pomelo",
      "Tabaco",
      "Cedro",
      "Vainilla",
      "Pachulí",
      "Incienso",
      "Madera de cachemira",
      "Ámbar seco"
    ],
    "ml": 100,
    "precioUSD": 40,
    "categoria": "arabes"
  },
  {
    "id": 144,
    "house": "LATTAFA",
    "name": "Lattafa Asad Zanzibar",
    "image": "lattafa-asad-zanzibar-100ml-m.webp",
    "familia": "Oriental",
    "tipo": "Eau de Parfum",
    "genero": "Masculino",
    "descripcion": "Una fragancia para el hombre fresca, seductora y con un giro gourmand, ideal para destacar con una elegancia moderna y versátil. Su carácter tropical y aromático la convierte en una opción atractiva para el día a día, con un aire inspirado en la sofisticación de perfumes de diseñador contemporáneos.",
    "description": "Una fragancia para el hombre fresca, seductora y con un giro gourmand, ideal para destacar con una elegancia moderna y versátil. Su carácter tropical y aromático la convierte en una opción atractiva para el día a día, con un aire inspirado en la sofisticación de perfumes de diseñador contemporáneos.",
    "notasSalida": "Lavanda, Pimienta negra, Agua de coco",
    "notasCorazon": "Iris, Sal",
    "notasFondo": "Vainilla, Incienso",
    "notes": [
      "Lavanda",
      "Pimienta negra",
      "Agua de coco",
      "Iris",
      "Sal",
      "Vainilla",
      "Incienso"
    ],
    "ml": 100,
    "precioUSD": 40,
    "categoria": "arabes"
  },
  {
    "id": 145,
    "house": "LATTAFA",
    "name": "Lattafa Atheeri",
    "image": "lattafa-atheeri-100ml-f.webp",
    "familia": "Floral",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Una fragancia para la mujer de estilo floral oriental gourmand, luminosa y delicadamente dulce, pensada para transmitir elegancia y suavidad moderna. Atheeri de Lattafa se inspira en el universo de los perfumes florales sofisticados, con un carácter femenino y envolvente ideal para quienes buscan un aroma refinado y actual.",
    "description": "Una fragancia para la mujer de estilo floral oriental gourmand, luminosa y delicadamente dulce, pensada para transmitir elegancia y suavidad moderna. Atheeri de Lattafa se inspira en el universo de los perfumes florales sofisticados, con un carácter femenino y envolvente ideal para quienes buscan un aroma refinado y actual.",
    "notasSalida": "Flor de pasionaria, Gota del rocío",
    "notasCorazon": "Orquídea, Jazmín",
    "notasFondo": "Vainilla, Amberwood",
    "notes": [
      "Flor de pasionaria",
      "Gota del rocío",
      "Orquídea",
      "Jazmín",
      "Vainilla",
      "Amberwood"
    ],
    "ml": 100,
    "precioUSD": 60,
    "categoria": "arabes",
    "agotado": true
  },
  {
    "id": 146,
    "house": "LATTAFA",
    "name": "Lattafa Atlas",
    "image": "lattafa-atlas-55ml-u.webp",
    "familia": "Amaderado",
    "tipo": "Eau de Parfum",
    "genero": "Unisex",
    "descripcion": "Atlas de Lattafa es un perfume unisex de carácter acuático y marino, fresco pero con un fondo cálido y sofisticado que le da presencia. Su estilo olfativo evoca una brisa salina moderna con un matiz amaderado, y se mueve en una línea nicho que recuerda a fragancias marinas de diseñador contemporáneo.",
    "description": "Atlas de Lattafa es un perfume unisex de carácter acuático y marino, fresco pero con un fondo cálido y sofisticado que le da presencia. Su estilo olfativo evoca una brisa salina moderna con un matiz amaderado, y se mueve en una línea nicho que recuerda a fragancias marinas de diseñador contemporáneo.",
    "notasSalida": "Notas marinas, Sal, Limón",
    "notasCorazon": "Davana, Iris, Ámbar gris",
    "notasFondo": "Musgo de roble, Sándalo",
    "notes": [
      "Notas marinas",
      "Sal",
      "Limón",
      "Davana",
      "Iris",
      "Ámbar gris",
      "Musgo de roble",
      "Sándalo"
    ],
    "ml": 55,
    "precioUSD": 45,
    "categoria": "arabes"
  },
  {
    "id": 147,
    "house": "LATTAFA",
    "name": "Lattafa Bade'e Al Oud Amethyst",
    "image": "lattafa-badee-al-oud-amethyst-100ml-u.webp",
    "familia": "Floral Amaderado",
    "tipo": "Eau de Parfum",
    "genero": "Unisex",
    "descripcion": "Unisex, con un perfil ámbar-vainilla de carácter floral y opulento, Bade'e Al Oud Amethyst combina un aura elegante de rosas con un fondo cálido y seductor de oud, ámbar y vainilla. Se inspira en el estilo de fragancias orientales de lujo con un toque muy sofisticado y envolvente, ideal para quienes buscan presencia y distinción.",
    "description": "Unisex, con un perfil ámbar-vainilla de carácter floral y opulento, Bade'e Al Oud Amethyst combina un aura elegante de rosas con un fondo cálido y seductor de oud, ámbar y vainilla. Se inspira en el estilo de fragancias orientales de lujo con un toque muy sofisticado y envolvente, ideal para quienes buscan presencia y distinción.",
    "notasSalida": "Pimienta rosa, Bergamota, Rosa turca",
    "notasCorazon": "Rosa búlgara, Jazmín, Madera de oud",
    "notasFondo": "Ámbar, Vainilla",
    "notes": [
      "Pimienta rosa",
      "Bergamota",
      "Rosa turca",
      "Rosa búlgara",
      "Jazmín",
      "Madera de oud",
      "Ámbar",
      "Vainilla"
    ],
    "ml": 100,
    "precioUSD": 35,
    "categoria": "arabes"
  },
  {
    "id": 148,
    "house": "LATTAFA",
    "name": "Lattafa Bade'e Al Oud Honor & Glory",
    "image": "lattafa-badee-al-oud-honor--glory-100ml-u.webp",
    "familia": "Frutal",
    "tipo": "Eau de Parfum",
    "genero": "Unisex",
    "descripcion": "Una fragancia unisex de estilo dulce, oriental y gourmand, creada para quienes buscan un perfume envolvente y sofisticado. Bade'e Al Oud Honor & Glory de Lattafa destaca por su carácter exótico y moderno, con un perfil que recuerda a un postre cremoso y especiado, muy en la línea de las fragancias árabes contemporáneas.",
    "description": "Una fragancia unisex de estilo dulce, oriental y gourmand, creada para quienes buscan un perfume envolvente y sofisticado. Bade'e Al Oud Honor & Glory de Lattafa destaca por su carácter exótico y moderno, con un perfil que recuerda a un postre cremoso y especiado, muy en la línea de las fragancias árabes contemporáneas.",
    "notasSalida": "Piña, crème brûlée, canela, benjuí",
    "notasCorazon": "cúrcuma, pimienta negra, vainilla",
    "notasFondo": "sándalo, cachemira, musgo",
    "notes": [
      "Piña",
      "crème brûlée",
      "canela",
      "benjuí",
      "cúrcuma",
      "pimienta negra",
      "vainilla",
      "sándalo",
      "cachemira",
      "musgo"
    ],
    "ml": 100,
    "precioUSD": 40,
    "categoria": "arabes"
  },
  {
    "id": 149,
    "house": "LATTAFA",
    "name": "Lattafa Bade'e Al Oud Noble Blush",
    "image": "lattafa-badee-al-oud-noble-blush-100ml-f.webp",
    "familia": "Floral Amaderado",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Un perfume para la mujer, delicado y sofisticado, que envuelve la piel con un aura floral gourmand cremosa y elegante. Badee Al Oud Noble Blush destaca por su carácter romántico y aterciopelado, con un estilo refinado que recuerda a las propuestas femeninas más dulces y lujosas del universo nicho.",
    "description": "Un perfume para la mujer, delicado y sofisticado, que envuelve la piel con un aura floral gourmand cremosa y elegante. Badee Al Oud Noble Blush destaca por su carácter romántico y aterciopelado, con un estilo refinado que recuerda a las propuestas femeninas más dulces y lujosas del universo nicho.",
    "notasSalida": "Leche de rosa, Merengue",
    "notasCorazon": "Almendra, Vainilla",
    "notasFondo": "Sándalo, Almizcle",
    "notes": [
      "Leche de rosa",
      "Merengue",
      "Almendra",
      "Vainilla",
      "Sándalo",
      "Almizcle"
    ],
    "ml": 100,
    "precioUSD": 35,
    "categoria": "arabes"
  },
  {
    "id": 150,
    "house": "LATTAFA",
    "name": "Lattafa Bade'e Al Oud for Glory",
    "image": "lattafa-badee-al-oud-oud-for-glory-100ml-u.webp",
    "familia": "Amaderado",
    "tipo": "Eau de Parfum",
    "genero": "Unisex",
    "descripcion": "Unisex de carácter intenso y sofisticado, con un perfil ámbar amaderado oscuro, envolvente y misterioso. Se inspira claramente en el estilo de oud de lujo popularizado por Initio Oud for Greatness, ofreciendo una presencia poderosa y elegante para quien busca una firma olfativa memorable.",
    "description": "Unisex de carácter intenso y sofisticado, con un perfil ámbar amaderado oscuro, envolvente y misterioso. Se inspira claramente en el estilo de oud de lujo popularizado por Initio Oud for Greatness, ofreciendo una presencia poderosa y elegante para quien busca una firma olfativa memorable.",
    "notasSalida": "Azafrán, Nuez moscada",
    "notasCorazon": "Lavanda, Madera de oud",
    "notasFondo": "Pachulí, Almizcle",
    "notes": [
      "Azafrán",
      "Nuez moscada",
      "Lavanda",
      "Madera de oud",
      "Pachulí",
      "Almizcle"
    ],
    "ml": 100,
    "precioUSD": 35,
    "categoria": "arabes"
  },
  {
    "id": 151,
    "house": "LATTAFA",
    "name": "Lattafa Bade'e Al Oud Sublime",
    "image": "lattafa-badee-al-oud-sublime-100ml-u.webp",
    "familia": "Floral Frutal",
    "tipo": "Eau de Parfum",
    "genero": "Unisex",
    "descripcion": "Una fragancia unisex de estilo afrutado-amaderado, luminosa y elegante, con un carácter moderno que equilibra dulzura y sofisticación. Bade'e Al Oud Sublime de Lattafa se inspira en el perfil popular de fragancias nicho contemporáneas, con una salida fresca y jugosa y un fondo cremoso y envolvente.",
    "description": "Una fragancia unisex de estilo afrutado-amaderado, luminosa y elegante, con un carácter moderno que equilibra dulzura y sofisticación. Bade'e Al Oud Sublime de Lattafa se inspira en el perfil popular de fragancias nicho contemporáneas, con una salida fresca y jugosa y un fondo cremoso y envolvente.",
    "notasSalida": "Manzana, Lichi, Rosa",
    "notasCorazon": "Ciruela, Jazmín, Vainilla",
    "notasFondo": "Musgo, Pachulí",
    "notes": [
      "Manzana",
      "Lichi",
      "Rosa",
      "Ciruela",
      "Jazmín",
      "Vainilla",
      "Musgo",
      "Pachulí"
    ],
    "ml": 100,
    "precioUSD": 35,
    "categoria": "arabes",
    "agotado": true
  },
  {
    "id": 152,
    "house": "LATTAFA",
    "name": "Lattafa Berry on Top",
    "image": "lattafa-berry-on-top-75ml-f.webp",
    "familia": "Amaderado",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Berry On Top de Lattafa es una fragancia para la mujer dulce, cremosa y adictiva, con un perfil floral-frutal gourmand que recuerda a un postre de fresa con nata y vainilla. Su estilo juvenil y goloso la convierte en una opción encantadora para quienes buscan un aroma femenino inspirado en el universo de los perfumes gourmand de diseño.",
    "description": "Berry On Top de Lattafa es una fragancia para la mujer dulce, cremosa y adictiva, con un perfil floral-frutal gourmand que recuerda a un postre de fresa con nata y vainilla. Su estilo juvenil y goloso la convierte en una opción encantadora para quienes buscan un aroma femenino inspirado en el universo de los perfumes gourmand de diseño.",
    "notasSalida": "Fresa, nata montada, mermelada de fresa",
    "notasCorazon": "azúcar, flores blancas",
    "notasFondo": "vainilla, almizcle",
    "notes": [
      "Fresa",
      "nata montada",
      "mermelada de fresa",
      "azúcar",
      "flores blancas",
      "vainilla",
      "almizcle"
    ],
    "ml": 75,
    "precioUSD": 60,
    "categoria": "arabes"
  },
  {
    "id": 153,
    "house": "LATTAFA",
    "name": "Lattafa Choco Overdose",
    "image": "lattafa-choco-overdose-75ml-f.webp",
    "familia": "Gourmand",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Choco Overdose de Lattafa es un perfume unisex de carácter gourmand y oriental-vainilla, ideal para quienes buscan una estela dulce, envolvente y adictiva. Su perfil olfativo se inspira en el estilo goloso de fragancias como *Ari by Ariana Grande* o *Chocolate Greedy*, con un efecto cálido y cremoso de lujo contemporáneo.",
    "description": "Choco Overdose de Lattafa es un perfume unisex de carácter gourmand y oriental-vainilla, ideal para quienes buscan una estela dulce, envolvente y adictiva. Su perfil olfativo se inspira en el estilo goloso de fragancias como *Ari by Ariana Grande* o *Chocolate Greedy*, con un efecto cálido y cremoso de lujo contemporáneo.",
    "notasSalida": "Chocolate, Fudge de chocolate",
    "notasCorazon": "Cacao, Vainilla",
    "notasFondo": "Benjuí, Caramelo",
    "notes": [
      "Chocolate",
      "Fudge de chocolate",
      "Cacao",
      "Vainilla",
      "Benjuí",
      "Caramelo"
    ],
    "ml": 75,
    "precioUSD": 60,
    "categoria": "arabes"
  },
  {
    "id": 154,
    "house": "LATTAFA",
    "name": "Lattafa Confidential Platinum",
    "image": "lattafa-confidential-platinum-100ml-m.webp",
    "familia": "Oriental",
    "tipo": "Eau de Parfum",
    "genero": "Masculino",
    "descripcion": "Una fragancia amaderada aromática para el hombre, elegante y moderna, con una apertura fresca y especiada que transmite sofisticación inmediata. Se inspira en el estilo de perfumes masculinos de diseñador con aire limpio, marino e incienso, ideal para quien busca presencia y refinamiento sin excesos.",
    "description": "Una fragancia amaderada aromática para el hombre, elegante y moderna, con una apertura fresca y especiada que transmite sofisticación inmediata. Se inspira en el estilo de perfumes masculinos de diseñador con aire limpio, marino e incienso, ideal para quien busca presencia y refinamiento sin excesos.",
    "notasSalida": "Bergamota, Cítricos, Ciprés, Pimienta negra",
    "notasCorazon": "Té negro, Incienso, Notas marinas, Notas amaderadas",
    "notasFondo": "Ámbar, Ládano, Musgo, Almizcle",
    "notes": [
      "Bergamota",
      "Cítricos",
      "Ciprés",
      "Pimienta negra",
      "Té negro",
      "Incienso",
      "Notas marinas",
      "Notas amaderadas",
      "Ámbar",
      "Ládano",
      "Musgo",
      "Almizcle"
    ],
    "ml": 100,
    "precioUSD": 30,
    "categoria": "arabes",
    "agotado": true
  },
  {
    "id": 155,
    "house": "LATTAFA",
    "name": "Lattafa Cookie Crave",
    "image": "lattafa-cookie-crave-75ml-f.webp",
    "familia": "Gourmand",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Una fragancia gourmand oriental para la mujer, diseñada para envolver con una sensación dulce, cremosa y absolutamente adictiva. Cookie Crave de Lattafa evoca el aroma de galletas recién horneadas con cacao, vainilla y un acorde lácteo, en una estela cálida y reconfortante que recuerda al estilo de un perfume de diseñador gourmand.",
    "description": "Una fragancia gourmand oriental para la mujer, diseñada para envolver con una sensación dulce, cremosa y absolutamente adictiva. Cookie Crave de Lattafa evoca el aroma de galletas recién horneadas con cacao, vainilla y un acorde lácteo, en una estela cálida y reconfortante que recuerda al estilo de un perfume de diseñador gourmand.",
    "notasSalida": "Galleta, Cacao, Vainilla",
    "notasCorazon": "Crema batida, Notas lácteas",
    "notasFondo": "Mantequilla, Azúcar",
    "notes": [
      "Galleta",
      "Cacao",
      "Vainilla",
      "Crema batida",
      "Notas lácteas",
      "Mantequilla",
      "Azúcar"
    ],
    "ml": 75,
    "precioUSD": 60,
    "categoria": "arabes"
  },
  {
    "id": 156,
    "house": "LATTAFA",
    "name": "Lattafa Eclaire",
    "image": "lattafa-eclaire-100ml-f.webp",
    "familia": "Amaderado",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Un perfume para la mujer de estilo gourmand, cremoso y adictivo, con una estela dulce y envolvente que recuerda a un postre de leche, caramelo y vainilla. Se inspira en el universo de los perfumes nicho gourmand y en el perfil olfativo de fragancias femeninas modernas, elegantes y muy sensuales.",
    "description": "Un perfume para la mujer de estilo gourmand, cremoso y adictivo, con una estela dulce y envolvente que recuerda a un postre de leche, caramelo y vainilla. Se inspira en el universo de los perfumes nicho gourmand y en el perfil olfativo de fragancias femeninas modernas, elegantes y muy sensuales.",
    "notasSalida": "Caramelo, Leche, Azúcar",
    "notasCorazon": "Miel, Flores blancas, Praliné",
    "notasFondo": "Vainilla, Almizcle",
    "notes": [
      "Caramelo",
      "Leche",
      "Azúcar",
      "Miel",
      "Flores blancas",
      "Praliné",
      "Vainilla",
      "Almizcle"
    ],
    "ml": 100,
    "precioUSD": 35,
    "categoria": "arabes",
    "agotado": true
  },
  {
    "id": 157,
    "house": "LATTAFA",
    "name": "Lattafa Eclaire Banoffi",
    "image": "lattafa-eclaire-banoffi-100ml-u.webp",
    "familia": "Gourmand",
    "tipo": "Eau de Parfum",
    "genero": "Unisex",
    "descripcion": "Unisex, Lattafa Eclaire Banoffi es una fragancia gourmand, cremosa y muy adictiva, inspirada en un postre de plátano, crema y dulce de leche. Su perfil dulce y envolvente la hace ideal para quienes buscan un perfume cálido y goloso, con un estilo que recuerda a una creación de nicho de carácter gourmand.",
    "description": "Unisex, Lattafa Eclaire Banoffi es una fragancia gourmand, cremosa y muy adictiva, inspirada en un postre de plátano, crema y dulce de leche. Su perfil dulce y envolvente la hace ideal para quienes buscan un perfume cálido y goloso, con un estilo que recuerda a una creación de nicho de carácter gourmand.",
    "notasSalida": "Plátano, dulce de leche, crema batida",
    "notasCorazon": "vainilla, praliné",
    "notasFondo": "bizcocho, almizcle",
    "notes": [
      "Plátano",
      "dulce de leche",
      "crema batida",
      "vainilla",
      "praliné",
      "bizcocho",
      "almizcle"
    ],
    "ml": 100,
    "precioUSD": 50,
    "categoria": "arabes"
  },
  {
    "id": 158,
    "house": "LATTAFA",
    "name": "Lattafa Eclaire Pistache",
    "image": "lattafa-eclaire-pistache-100ml-u.webp",
    "familia": "Gourmand",
    "tipo": "Eau de Parfum",
    "genero": "Unisex",
    "descripcion": "Una fragancia unisex para quienes buscan un gourmand sofisticado, cremoso y adictivo, con un perfil oriental-vainilla muy envolvente. Eclaire Pistache de Lattafa se inspira en el universo de los postres de alta pastelería y en el estilo de fragancias gourmand de diseñador, ofreciendo una estela cálida y elegante.",
    "description": "Una fragancia unisex para quienes buscan un gourmand sofisticado, cremoso y adictivo, con un perfil oriental-vainilla muy envolvente. Eclaire Pistache de Lattafa se inspira en el universo de los postres de alta pastelería y en el estilo de fragancias gourmand de diseñador, ofreciendo una estela cálida y elegante.",
    "notasSalida": "Pistacho, crema de pistacho, crema batida",
    "notasCorazon": "cacao, coco, vainilla",
    "notasFondo": "leche, almizcle",
    "notes": [
      "Pistacho",
      "crema de pistacho",
      "crema batida",
      "cacao",
      "coco",
      "vainilla",
      "leche",
      "almizcle"
    ],
    "ml": 100,
    "precioUSD": 60,
    "categoria": "arabes"
  },
  {
    "id": 159,
    "house": "LATTAFA",
    "name": "Lattafa Ejaazi",
    "image": "lattafa-ejaazi-100ml-u.webp",
    "familia": "Floral Amaderado",
    "tipo": "Eau de Parfum",
    "genero": "Unisex",
    "descripcion": "Una fragancia unisex de perfil ámbar-fresco, elegante y versátil, perfecta para quienes buscan un aroma limpio con presencia moderna. Se inspira en el estilo de los grandes cítricos aromáticos de diseñador, con una vibra sofisticada y muy llevadera para el día a día.",
    "description": "Una fragancia unisex de perfil ámbar-fresco, elegante y versátil, perfecta para quienes buscan un aroma limpio con presencia moderna. Se inspira en el estilo de los grandes cítricos aromáticos de diseñador, con una vibra sofisticada y muy llevadera para el día a día.",
    "notasSalida": "Cítricos, Lavanda, Mirto, Cardamomo, Naranja",
    "notasCorazon": "Pimienta, Pachulí, Lirio de los valles, Hoja de canela, Ambroxan",
    "notasFondo": "Notas amaderadas, Ámbar, Cedro, Musgo de roble, Vetiver",
    "notes": [
      "Cítricos",
      "Lavanda",
      "Mirto",
      "Cardamomo",
      "Naranja",
      "Pimienta",
      "Pachulí",
      "Lirio de los valles",
      "Hoja de canela",
      "Ambroxan",
      "Notas amaderadas",
      "Ámbar",
      "Cedro",
      "Musgo de roble",
      "Vetiver"
    ],
    "ml": 100,
    "precioUSD": 30,
    "categoria": "arabes",
    "agotado": true
  },
  {
    "id": 160,
    "house": "LATTAFA",
    "name": "Lattafa Eternal Oud",
    "image": "lattafa-eternal-oud-100ml-u.webp",
    "familia": "Oriental",
    "tipo": "Eau de Parfum",
    "genero": "Unisex",
    "descripcion": "Eternal Oud de Lattafa es una fragancia unisex de estilo oriental ambarado y amaderado, intensa, cálida y con un oud elegante que aporta profundidad y sofisticación. Se inspira en el universo de los perfumes árabes de lujo, con un perfil envolvente que recuerda a composiciones de diseñador opulentas y seductoras.",
    "description": "Eternal Oud de Lattafa es una fragancia unisex de estilo oriental ambarado y amaderado, intensa, cálida y con un oud elegante que aporta profundidad y sofisticación. Se inspira en el universo de los perfumes árabes de lujo, con un perfil envolvente que recuerda a composiciones de diseñador opulentas y seductoras.",
    "notasSalida": "Ciruela, Pomelo, Heliotropo, Orquídea",
    "notasCorazon": "Ámbar, Benjuí, Vainilla",
    "notasFondo": "Ládano, Haba tonka, Madera de oud",
    "notes": [
      "Ciruela",
      "Pomelo",
      "Heliotropo",
      "Orquídea",
      "Ámbar",
      "Benjuí",
      "Vainilla",
      "Ládano",
      "Haba tonka",
      "Madera de oud"
    ],
    "ml": 100,
    "precioUSD": 65,
    "categoria": "arabes"
  },
  {
    "id": 161,
    "house": "LATTAFA",
    "name": "Lattafa Fakhar Black",
    "image": "lattafa-fakhar-black-100ml-m.webp",
    "familia": "Frutal",
    "tipo": "Eau de Parfum",
    "genero": "Masculino",
    "descripcion": "Un perfume para el hombre moderno que busca una firma elegante, fresca y magnética. Su estilo aromático-ámbar con matices especiados y amaderados recuerda al carácter de Y Eau de Parfum de Yves Saint Laurent, ofreciendo una presencia sofisticada y versátil para el día o la noche.",
    "description": "Un perfume para el hombre moderno que busca una firma elegante, fresca y magnética. Su estilo aromático-ámbar con matices especiados y amaderados recuerda al carácter de Y Eau de Parfum de Yves Saint Laurent, ofreciendo una presencia sofisticada y versátil para el día o la noche.",
    "notasSalida": "Manzana, Bergamota, Jengibre, Lavanda",
    "notasCorazon": "Salvia, Bayas de enebro, Geranio, Haba tonka",
    "notasFondo": "Amberwood, Cedro, Vetiver",
    "notes": [
      "Manzana",
      "Bergamota",
      "Jengibre",
      "Lavanda",
      "Salvia",
      "Bayas de enebro",
      "Geranio",
      "Haba tonka",
      "Amberwood",
      "Cedro",
      "Vetiver"
    ],
    "ml": 100,
    "precioUSD": 35,
    "categoria": "arabes",
    "agotado": true
  },
  {
    "id": 162,
    "house": "LATTAFA",
    "name": "Lattafa Fakhar Extrait de Parfum",
    "image": "lattafa-fakhar-extrait-100ml-u.webp",
    "familia": "Floral Frutal",
    "tipo": "Extrait de Parfum",
    "genero": "Unisex",
    "descripcion": "Fakhar Extrait de Lattafa es una fragancia de carácter elegante y envolvente, con un perfil floral amaderado ámbar que aporta sofisticación y presencia. Aunque muchas tiendas lo presentan como femenino o masculino según el mercado, esta versión se comercializa también como unisex y suele gustar a quienes buscan un aroma intenso, moderno y con aire de diseñador, a menudo comparado con propuestas de lujo de gran proyección.",
    "description": "Fakhar Extrait de Lattafa es una fragancia de carácter elegante y envolvente, con un perfil floral amaderado ámbar que aporta sofisticación y presencia. Aunque muchas tiendas lo presentan como femenino o masculino según el mercado, esta versión se comercializa también como unisex y suele gustar a quienes buscan un aroma intenso, moderno y con aire de diseñador, a menudo comparado con propuestas de lujo de gran proyección.",
    "notasSalida": "Toronja, Cardamomo, Pimienta rosa",
    "notasCorazon": "Notas solares, Tuberosa, Cachemira",
    "notasFondo": "Cuero, Cistus, Ámbar",
    "notes": [
      "Toronja",
      "Cardamomo",
      "Pimienta rosa",
      "Notas solares",
      "Tuberosa",
      "Cachemira",
      "Cuero",
      "Cistus",
      "Ámbar"
    ],
    "ml": 100,
    "precioUSD": 35,
    "categoria": "arabes"
  },
  {
    "id": 163,
    "house": "LATTAFA",
    "name": "Lattafa Fakhar Platin",
    "image": "lattafa-fakhar-platin-100ml-m.webp",
    "familia": "Oriental",
    "tipo": "Eau de Parfum",
    "genero": "Masculino",
    "descripcion": "Fakhar Platin de Lattafa es una fragancia para el hombre de perfil aromático fougère, fresca y moderna, con un carácter especiado y ligeramente ahumado. Su estilo elegante y contemporáneo recuerda a un perfume de diseñador de corte limpio y sofisticado, ideal para quien busca presencia y distinción.",
    "description": "Fakhar Platin de Lattafa es una fragancia para el hombre de perfil aromático fougère, fresca y moderna, con un carácter especiado y ligeramente ahumado. Su estilo elegante y contemporáneo recuerda a un perfume de diseñador de corte limpio y sofisticado, ideal para quien busca presencia y distinción.",
    "notasSalida": "Bergamota, Jengibre",
    "notasCorazon": "Guayaba, Palo santo",
    "notasFondo": "Incienso, Almizcle",
    "notes": [
      "Bergamota",
      "Jengibre",
      "Guayaba",
      "Palo santo",
      "Incienso",
      "Almizcle"
    ],
    "ml": 100,
    "precioUSD": 35,
    "categoria": "arabes",
    "agotado": true
  },
  {
    "id": 164,
    "house": "LATTAFA",
    "name": "Lattafa Fakhar Rose Woman",
    "image": "lattafa-fakhar-rose-100ml-f.webp",
    "familia": "Floral Amaderado",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Una fragancia floral elegante y moderna para la mujer, con una salida luminosa y jugosa que se funde en un corazón opulento de flores blancas y rosa. Inspirada en el estilo de los perfumes femeninos de diseñador, transmite sofisticación, feminidad y un halo limpio y seductor.",
    "description": "Una fragancia floral elegante y moderna para la mujer, con una salida luminosa y jugosa que se funde en un corazón opulento de flores blancas y rosa. Inspirada en el estilo de los perfumes femeninos de diseñador, transmite sofisticación, feminidad y un halo limpio y seductor.",
    "notasSalida": "Frutas, Azucena, Granada, Aldehídos, Tuberosa",
    "notasCorazon": "Jazmín, Gardenia, Madreselva, Ylang-ylang, Rosa",
    "notasFondo": "Peonía, Vainilla, Almizcle blanco, Ambroxan, Sándalo",
    "notes": [
      "Frutas",
      "Azucena",
      "Granada",
      "Aldehídos",
      "Tuberosa",
      "Jazmín",
      "Gardenia",
      "Madreselva",
      "Ylang-ylang",
      "Rosa",
      "Peonía",
      "Vainilla",
      "Almizcle blanco",
      "Ambroxan",
      "Sándalo"
    ],
    "ml": 100,
    "precioUSD": 35,
    "categoria": "arabes",
    "agotado": true
  },
  {
    "id": 165,
    "house": "LATTAFA",
    "name": "Lattafa Haya",
    "image": "lattafa-haya-100ml-f.webp",
    "familia": "Floral Frutal",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Una fragancia para la mujer de estilo oriental floral, luminosa y seductora, con un aire moderno y elegante. Se inspira en el universo de los perfumes femeninos de diseñador con un toque chispeante y cremoso que la hace ideal para el día y la noche.",
    "description": "Una fragancia para la mujer de estilo oriental floral, luminosa y seductora, con un aire moderno y elegante. Se inspira en el universo de los perfumes femeninos de diseñador con un toque chispeante y cremoso que la hace ideal para el día y la noche.",
    "notasSalida": "Naranja roja, Mandarina, Fresa, Champán",
    "notasCorazon": "Rosa, Jazmín, Orquídea de vainilla, Gardenia",
    "notasFondo": "Castañas, Sándalo, Ámbar",
    "notes": [
      "Naranja roja",
      "Mandarina",
      "Fresa",
      "Champán",
      "Rosa",
      "Jazmín",
      "Orquídea de vainilla",
      "Gardenia",
      "Castañas",
      "Sándalo",
      "Ámbar"
    ],
    "ml": 100,
    "precioUSD": 40,
    "categoria": "arabes",
    "agotado": true
  },
  {
    "id": 166,
    "house": "LATTAFA",
    "name": "Lattafa Hayaati",
    "image": "lattafa-hayaati-100ml-m.webp",
    "familia": "Cítrico",
    "tipo": "Eau de Parfum",
    "genero": "Masculino",
    "descripcion": "Una fragancia para el hombre de carácter amaderado y aromático, con un perfil moderno, limpio y versátil que transmite confianza sin perder elegancia. Se inspira en el estilo de fragancias masculinas contemporáneas con un toque afrutado-especiado, ideal para quien busca una presencia sofisticada y fácil de llevar en el día a día.",
    "description": "Una fragancia para el hombre de carácter amaderado y aromático, con un perfil moderno, limpio y versátil que transmite confianza sin perder elegancia. Se inspira en el estilo de fragancias masculinas contemporáneas con un toque afrutado-especiado, ideal para quien busca una presencia sofisticada y fácil de llevar en el día a día.",
    "notasSalida": "Manzana, Bergamota",
    "notasCorazon": "Canela, Notas amaderadas",
    "notasFondo": "Almizcle, Vainilla",
    "notes": [
      "Manzana",
      "Bergamota",
      "Canela",
      "Notas amaderadas",
      "Almizcle",
      "Vainilla"
    ],
    "ml": 100,
    "precioUSD": 30,
    "categoria": "arabes"
  },
  {
    "id": 167,
    "house": "LATTAFA",
    "name": "Lattafa Hayaati Al Maleky",
    "image": "lattafa-hayaati-al-maleky-lattafa-100ml-u.webp",
    "familia": "Floral Amaderado",
    "tipo": "Eau de Parfum",
    "genero": "Unisex",
    "descripcion": "Fragancia unisex de carácter oriental especiado, elegante y envolvente, ideal para quienes buscan un aroma moderno con presencia y sofisticación. Su perfil recuerda al estilo de diseñadores como *Phantom* de Paco Rabanne, con un acabado amaderado-ambarado muy atractivo para uso diario o nocturno.",
    "description": "Fragancia unisex de carácter oriental especiado, elegante y envolvente, ideal para quienes buscan un aroma moderno con presencia y sofisticación. Su perfil recuerda al estilo de diseñadores como *Phantom* de Paco Rabanne, con un acabado amaderado-ambarado muy atractivo para uso diario o nocturno.",
    "notasSalida": "Nuez moscada, pimienta rosa, bergamota, jengibre, lavanda",
    "notasCorazon": "salvia esclarea, cedro, notas amaderadas, ládano",
    "notasFondo": "incienso, almizcle, ámbar gris, ámbar",
    "notes": [
      "Nuez moscada",
      "pimienta rosa",
      "bergamota",
      "jengibre",
      "lavanda",
      "salvia esclarea",
      "cedro",
      "notas amaderadas",
      "ládano",
      "incienso",
      "almizcle",
      "ámbar gris",
      "ámbar"
    ],
    "ml": 100,
    "precioUSD": 30,
    "categoria": "arabes",
    "agotado": true
  },
  {
    "id": 168,
    "house": "LATTAFA",
    "name": "Lattafa Hayaatim",
    "image": "lattafa-hayaatim-100ml-u.webp",
    "familia": "Frutal",
    "tipo": "Eau de Parfum",
    "genero": "Unisex",
    "descripcion": "Un perfume unisex para quienes buscan una firma sofisticada, moderna y envolvente. Su perfil olfativo combina matices frutales, florales y almizclados con un fondo cálido y seductor, inspirado en el estilo elegante de fragancias de diseñador contemporáneas.",
    "description": "Un perfume unisex para quienes buscan una firma sofisticada, moderna y envolvente. Su perfil olfativo combina matices frutales, florales y almizclados con un fondo cálido y seductor, inspirado en el estilo elegante de fragancias de diseñador contemporáneas.",
    "notasSalida": "Uva, Piña, Lima",
    "notasCorazon": "Notas florales, Almizcle",
    "notasFondo": "Sándalo, Caramelo",
    "notes": [
      "Uva",
      "Piña",
      "Lima",
      "Notas florales",
      "Almizcle",
      "Sándalo",
      "Caramelo"
    ],
    "ml": 100,
    "precioUSD": 30,
    "categoria": "arabes"
  },
  {
    "id": 169,
    "house": "LATTAFA",
    "name": "Lattafa His Confession",
    "image": "lattafa-his-confession-100ml-m.webp",
    "familia": "Oriental",
    "tipo": "Eau de Parfum",
    "genero": "Masculino",
    "descripcion": "His Confession de Lattafa es una fragancia para el hombre moderna y seductora, de carácter oriental amaderado, con una apertura especiada y luminosa que evoluciona hacia un corazón elegante y resinouso. Su estela cálida, envolvente y sofisticada lo convierte en una opción ideal para quienes buscan un perfume masculino con inspiración en el lujo contemporáneo.",
    "description": "His Confession de Lattafa es una fragancia para el hombre moderna y seductora, de carácter oriental amaderado, con una apertura especiada y luminosa que evoluciona hacia un corazón elegante y resinouso. Su estela cálida, envolvente y sofisticada lo convierte en una opción ideal para quienes buscan un perfume masculino con inspiración en el lujo contemporáneo.",
    "notasSalida": "Canela, Lavanda, Mandarina, Iris, Benjuí",
    "notasCorazon": "Ciprés, Mahonial, Vainilla, Haba tonka",
    "notasFondo": "Incienso, Ámbar, Cedro, Pachulí",
    "notes": [
      "Canela",
      "Lavanda",
      "Mandarina",
      "Iris",
      "Benjuí",
      "Ciprés",
      "Mahonial",
      "Vainilla",
      "Haba tonka",
      "Incienso",
      "Ámbar",
      "Cedro",
      "Pachulí"
    ],
    "ml": 100,
    "precioUSD": 50,
    "categoria": "arabes"
  },
  {
    "id": 170,
    "house": "LATTAFA",
    "name": "Lattafa Ishq Al Shuyukh Gold",
    "image": "lattafa-ishq-al-shuyukh-gold-100ml-u.webp",
    "familia": "Amaderado",
    "tipo": "Eau de Parfum",
    "genero": "Unisex",
    "descripcion": "Unisex, este perfume destaca por su carácter ámbar-dulce y gourmand, con un aire intenso, elegante y seductor. Su estilo oriental moderno evoca una composición de lujo con matices cálidos y envolventes, a menudo asociada a un perfil inspirado en creaciones de diseñador de corte opulento.",
    "description": "Unisex, este perfume destaca por su carácter ámbar-dulce y gourmand, con un aire intenso, elegante y seductor. Su estilo oriental moderno evoca una composición de lujo con matices cálidos y envolventes, a menudo asociada a un perfil inspirado en creaciones de diseñador de corte opulento.",
    "notasSalida": "Azafrán, Cuero, Ámbar",
    "notasCorazon": "Caramelo, Haba tonka",
    "notasFondo": "Vainilla, Oud",
    "notes": [
      "Azafrán",
      "Cuero",
      "Ámbar",
      "Caramelo",
      "Haba tonka",
      "Vainilla",
      "Oud"
    ],
    "ml": 100,
    "precioUSD": 40,
    "categoria": "arabes",
    "agotado": true
  },
  {
    "id": 171,
    "house": "LATTAFA",
    "name": "Lattafa Ishq Al Shuyukh Silver",
    "image": "lattafa-ishq-al-shuyukh-silver-100ml-u.webp",
    "familia": "Frutal",
    "tipo": "Eau de Parfum",
    "genero": "Unisex",
    "descripcion": "Una fragancia unisex de carácter oriental y moderno, luminosa y adictiva, con un inicio cítrico fresco que evoluciona hacia un corazón afrutado-especiado muy elegante. Se inspira en el estilo de fragrancias tipo diseñador gourmand-aromáticas, con un aire que puede recordar a propuestas masculinas sofisticadas por su fondo amaderado y vainillado.",
    "description": "Una fragancia unisex de carácter oriental y moderno, luminosa y adictiva, con un inicio cítrico fresco que evoluciona hacia un corazón afrutado-especiado muy elegante. Se inspira en el estilo de fragrancias tipo diseñador gourmand-aromáticas, con un aire que puede recordar a propuestas masculinas sofisticadas por su fondo amaderado y vainillado.",
    "notasSalida": "Bergamota, Limón, Pimienta negra",
    "notasCorazon": "Piña, Ámbar, Cedro",
    "notasFondo": "Pachulí, Vainilla",
    "notes": [
      "Bergamota",
      "Limón",
      "Pimienta negra",
      "Piña",
      "Ámbar",
      "Cedro",
      "Pachulí",
      "Vainilla"
    ],
    "ml": 100,
    "precioUSD": 45,
    "categoria": "arabes",
    "agotado": true
  },
  {
    "id": 172,
    "house": "LATTAFA",
    "name": "Lattafa Khamrah",
    "image": "lattafa-khamrah-100ml-u.webp",
    "familia": "Oriental",
    "tipo": "Eau de Parfum",
    "genero": "Unisex",
    "descripcion": "Lattafa Khamrah es un perfume unisex de perfil oriental especiado, cálido y envolvente, con un carácter dulce y sofisticado que destaca por su aire opulento. Su estela recuerda a fragancias de diseñador de estilo gourmand, con una vibra que evoca a Angel's Share de Kilian, pero con una lectura más árabe y avainillada.",
    "description": "Lattafa Khamrah es un perfume unisex de perfil oriental especiado, cálido y envolvente, con un carácter dulce y sofisticado que destaca por su aire opulento. Su estela recuerda a fragancias de diseñador de estilo gourmand, con una vibra que evoca a Angel's Share de Kilian, pero con una lectura más árabe y avainillada.",
    "notasSalida": "Bergamota, Nuez moscada, Canela, Dátiles",
    "notasCorazon": "Praliné, Tuberosa, Mirra, Haba tonka tostada",
    "notasFondo": "Vainilla, Benjuí, Ámbar, Madera de Akigala",
    "notes": [
      "Bergamota",
      "Nuez moscada",
      "Canela",
      "Dátiles",
      "Praliné",
      "Tuberosa",
      "Mirra",
      "Haba tonka tostada",
      "Vainilla",
      "Benjuí",
      "Ámbar",
      "Madera de Akigala"
    ],
    "ml": 100,
    "precioUSD": 35,
    "categoria": "arabes"
  },
  {
    "id": 173,
    "house": "LATTAFA",
    "name": "Lattafa Khamrah Dukhan",
    "image": "lattafa-khamrah-dukhan-100ml-u.webp",
    "familia": "Oriental",
    "tipo": "Eau de Parfum",
    "genero": "Unisex",
    "descripcion": "Una fragancia unisex de carácter oriental, intensa y seductora, con un perfil ahumado, especiado y dulcemente adictivo. Inspirada en la tradición del *dukhan* y con una vibra elegante de perfumería de nicho, recuerda al universo olfativo de los grandes ámbar-tabaco de diseñador, pero con una firma árabe más envolvente.",
    "description": "Una fragancia unisex de carácter oriental, intensa y seductora, con un perfil ahumado, especiado y dulcemente adictivo. Inspirada en la tradición del *dukhan* y con una vibra elegante de perfumería de nicho, recuerda al universo olfativo de los grandes ámbar-tabaco de diseñador, pero con una firma árabe más envolvente.",
    "notasSalida": "Especias, Pimiento morrón, Mandarina, Incienso",
    "notasCorazon": "Ládano, Flor de azahar del naranjo, Pachulí, Tabaco",
    "notasFondo": "Praliné, Ámbar, Haba tonka, Benjuí",
    "notes": [
      "Especias",
      "Pimiento morrón",
      "Mandarina",
      "Incienso",
      "Ládano",
      "Flor de azahar del naranjo",
      "Pachulí",
      "Tabaco",
      "Praliné",
      "Ámbar",
      "Haba tonka",
      "Benjuí"
    ],
    "ml": 100,
    "precioUSD": 35,
    "categoria": "arabes"
  },
  {
    "id": 174,
    "house": "LATTAFA",
    "name": "Lattafa Khamrah Qahwa",
    "image": "lattafa-khamrah-qahwa-100ml-u.webp",
    "familia": "Amaderado",
    "tipo": "Eau de Parfum",
    "genero": "Unisex",
    "descripcion": "Un perfume unisex para quienes buscan una estela opulenta, cálida y envolvente, con un carácter gourmand oriental de gran presencia. Khamrah Qahwa se inspira en la riqueza de la perfumería árabe y ofrece un perfil adictivo con aire sofisticado, perfecto para amantes de fragancias intensas y elegantes.",
    "description": "Un perfume unisex para quienes buscan una estela opulenta, cálida y envolvente, con un carácter gourmand oriental de gran presencia. Khamrah Qahwa se inspira en la riqueza de la perfumería árabe y ofrece un perfil adictivo con aire sofisticado, perfecto para amantes de fragancias intensas y elegantes.",
    "notasSalida": "Canela, Cardamomo, Jengibre, Praliné",
    "notasCorazon": "Frutas confitadas, Flores blancas, Café, Vainilla",
    "notasFondo": "Haba tonka, Benjuí, Almizcle",
    "notes": [
      "Canela",
      "Cardamomo",
      "Jengibre",
      "Praliné",
      "Frutas confitadas",
      "Flores blancas",
      "Café",
      "Vainilla",
      "Haba tonka",
      "Benjuí",
      "Almizcle"
    ],
    "ml": 100,
    "precioUSD": 35,
    "categoria": "arabes"
  },
  {
    "id": 175,
    "house": "LATTAFA",
    "name": "Lattafa Lail Maleki",
    "image": "lattafa-lail-maleki-100ml-u.webp",
    "familia": "Floral Amaderado",
    "tipo": "Eau de Parfum",
    "genero": "Unisex",
    "descripcion": "Un perfume unisex de carácter opulento y envolvente, con un perfil ámbar-floral que combina dulzor, especias y un fondo cálido muy elegante. Su estilo sensual y refinado lo convierte en una propuesta ideal para quienes buscan una fragancia de presencia, con un aire oriental que recuerda a perfumes nicho de corte lujoso, sin una inspiración de diseñador claramente confirmada.",
    "description": "Un perfume unisex de carácter opulento y envolvente, con un perfil ámbar-floral que combina dulzor, especias y un fondo cálido muy elegante. Su estilo sensual y refinado lo convierte en una propuesta ideal para quienes buscan una fragancia de presencia, con un aire oriental que recuerda a perfumes nicho de corte lujoso, sin una inspiración de diseñador claramente confirmada.",
    "notasSalida": "Notas afrutadas, Cítricos, Canela, Jazmín",
    "notasCorazon": "Rosa, Gardenia, Orquídea, Miel",
    "notasFondo": "Almizcle, Sándalo, Ámbar, Caramelo",
    "notes": [
      "Notas afrutadas",
      "Cítricos",
      "Canela",
      "Jazmín",
      "Rosa",
      "Gardenia",
      "Orquídea",
      "Miel",
      "Almizcle",
      "Sándalo",
      "Ámbar",
      "Caramelo"
    ],
    "ml": 100,
    "precioUSD": 40,
    "categoria": "arabes"
  },
  {
    "id": 176,
    "house": "LATTAFA",
    "name": "Lattafa Mashrabya",
    "image": "lattafa-mashrabya-100ml-u.webp",
    "familia": "Floral Frutal",
    "tipo": "Eau de Parfum",
    "genero": "Unisex",
    "descripcion": "Unisex y sofisticado, Mashrabya de Lattafa envuelve con un carácter oriental vainilla, dulce y especiado, ideal para quienes buscan una estela moderna con personalidad. Su perfil se inspira en la vibra intensa de fragancias tipo *Smoking Hot* de By Kilian, aportando un aire cálido, adictivo y lujoso.",
    "description": "Unisex y sofisticado, Mashrabya de Lattafa envuelve con un carácter oriental vainilla, dulce y especiado, ideal para quienes buscan una estela moderna con personalidad. Su perfil se inspira en la vibra intensa de fragancias tipo *Smoking Hot* de By Kilian, aportando un aire cálido, adictivo y lujoso.",
    "notasSalida": "Acorde de manzana y shisha, Pimienta rosa, Canela",
    "notasCorazon": "Dátiles, Flor del tabaco, Vainilla",
    "notasFondo": "Caramelo, Pachulí",
    "notes": [
      "Acorde de manzana y shisha",
      "Pimienta rosa",
      "Canela",
      "Dátiles",
      "Flor del tabaco",
      "Vainilla",
      "Caramelo",
      "Pachulí"
    ],
    "ml": 100,
    "precioUSD": 40,
    "categoria": "arabes"
  },
  {
    "id": 177,
    "house": "LATTAFA",
    "name": "Lattafa Mayar Cherry Intense",
    "image": "lattafa-mayar-cherry-intense-100ml-u.webp",
    "familia": "Gourmand",
    "tipo": "Eau de Parfum",
    "genero": "Unisex",
    "descripcion": "Mayar Cherry Intense de Lattafa es una fragancia unisex de vainilla oriental, jugosa y envolvente, ideal para quienes buscan un aroma dulce con un toque sofisticado. Su carácter gourmand y elegante recuerda al estilo de perfumes nicho frutales-ambarados, con una vibra que puede gustar a quienes disfrutan de composiciones intensas y modernas.",
    "description": "Mayar Cherry Intense de Lattafa es una fragancia unisex de vainilla oriental, jugosa y envolvente, ideal para quienes buscan un aroma dulce con un toque sofisticado. Su carácter gourmand y elegante recuerda al estilo de perfumes nicho frutales-ambarados, con una vibra que puede gustar a quienes disfrutan de composiciones intensas y modernas.",
    "notasSalida": "Fresa, Bergamota, Mermelada de cereza",
    "notasCorazon": "Cacao, Vainilla",
    "notasFondo": "Pachulí, Ámbar",
    "notes": [
      "Fresa",
      "Bergamota",
      "Mermelada de cereza",
      "Cacao",
      "Vainilla",
      "Pachulí",
      "Ámbar"
    ],
    "ml": 100,
    "precioUSD": 40,
    "categoria": "arabes"
  },
  {
    "id": 178,
    "house": "LATTAFA",
    "name": "Lattafa Mayar Natural Intense",
    "image": "lattafa-mayar-natural-intense-100ml-f.webp",
    "familia": "Floral Amaderado",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Una fragancia para la mujer fresca, luminosa y elegante, con un perfil frutal, floral y acuático ideal para climas cálidos. Su carácter recuerda al estilo de Acqua di Gioia, con un aire moderno, limpio y delicadamente sensual.",
    "description": "Una fragancia para la mujer fresca, luminosa y elegante, con un perfil frutal, floral y acuático ideal para climas cálidos. Su carácter recuerda al estilo de Acqua di Gioia, con un aire moderno, limpio y delicadamente sensual.",
    "notasSalida": "Higo, Melón, Mandarina verde, Agua de coco",
    "notasCorazon": "Flor de loto, Nenúfar, Jazmín, Ambroxan",
    "notasFondo": "Sándalo, Almizcle, Vainilla",
    "notes": [
      "Higo",
      "Melón",
      "Mandarina verde",
      "Agua de coco",
      "Flor de loto",
      "Nenúfar",
      "Jazmín",
      "Ambroxan",
      "Sándalo",
      "Almizcle",
      "Vainilla"
    ],
    "ml": 100,
    "precioUSD": 40,
    "categoria": "arabes",
    "agotado": true
  },
  {
    "id": 179,
    "house": "LATTAFA",
    "name": "Lattafa Musamam",
    "image": "lattafa-musamam-100ml-u.webp",
    "familia": "Amaderado",
    "tipo": "Eau de Parfum",
    "genero": "Unisex",
    "descripcion": "Una fragancia unisex de carácter sofisticado, con un perfil amaderado-cuero y un aire elegante que combina frescura especiada con matices florales. Lattafa Musamam se presenta como una propuesta refinada y moderna, ideal para quienes buscan una estela lujosa con inspiración contemporánea y un toque oriental.",
    "description": "Una fragancia unisex de carácter sofisticado, con un perfil amaderado-cuero y un aire elegante que combina frescura especiada con matices florales. Lattafa Musamam se presenta como una propuesta refinada y moderna, ideal para quienes buscan una estela lujosa con inspiración contemporánea y un toque oriental.",
    "notasSalida": "Azafrán, Lavanda",
    "notasCorazon": "Mandarina italiana, Cedro de Virginia",
    "notasFondo": "Ámbar, Cistus",
    "notes": [
      "Azafrán",
      "Lavanda",
      "Mandarina italiana",
      "Cedro de Virginia",
      "Ámbar",
      "Cistus"
    ],
    "ml": 100,
    "precioUSD": 60,
    "categoria": "arabes"
  },
  {
    "id": 180,
    "house": "LATTAFA",
    "name": "Lattafa Najdia",
    "image": "lattafa-najdia-100ml-u.webp",
    "familia": "Frutal",
    "tipo": "Eau de Parfum",
    "genero": "Unisex",
    "descripcion": "Un perfume unisex de carácter fresco, dulce y ligeramente especiado, ideal para el uso diario y con una estela agradable y versátil. Najdia de Lattafa se inspira en el estilo de los perfumes deportivos y aromáticos de diseñador, con un aire limpio, juvenil y muy fácil de llevar.",
    "description": "Un perfume unisex de carácter fresco, dulce y ligeramente especiado, ideal para el uso diario y con una estela agradable y versátil. Najdia de Lattafa se inspira en el estilo de los perfumes deportivos y aromáticos de diseñador, con un aire limpio, juvenil y muy fácil de llevar.",
    "notasSalida": "Bergamota, limón, manzana, romero",
    "notasCorazon": "lavanda, cardamomo, ámbar",
    "notasFondo": "almizcle, cedro, madera de oud",
    "notes": [
      "Bergamota",
      "limón",
      "manzana",
      "romero",
      "lavanda",
      "cardamomo",
      "ámbar",
      "almizcle",
      "cedro",
      "madera de oud"
    ],
    "ml": 100,
    "precioUSD": 35,
    "categoria": "arabes"
  },
  {
    "id": 181,
    "house": "LATTAFA",
    "name": "Lattafa Najdia Intense",
    "image": "lattafa-najdia-intense-100ml-m.webp",
    "familia": "Frutal",
    "tipo": "Eau de Parfum",
    "genero": "Masculino",
    "descripcion": "Una fragancia para el hombre fresca, vibrante y sofisticada, ideal para quienes buscan un aroma versátil con carácter moderno. Lattafa Najdia Intense combina una salida frutal y especiada con un corazón limpio y acuático, y se apoya en un fondo cálido y amaderado que le da profundidad; algunas descripciones lo sitúan en la línea de perfumes deportivos frescos de diseñador.",
    "description": "Una fragancia para el hombre fresca, vibrante y sofisticada, ideal para quienes buscan un aroma versátil con carácter moderno. Lattafa Najdia Intense combina una salida frutal y especiada con un corazón limpio y acuático, y se apoya en un fondo cálido y amaderado que le da profundidad; algunas descripciones lo sitúan en la línea de perfumes deportivos frescos de diseñador.",
    "notasSalida": "Piña, Menta, Jengibre, Manzana",
    "notasCorazon": "Notas marinas, Lavanda, Geranio, Pachulí",
    "notasFondo": "Sándalo, Haba tonka, Almizcle",
    "notes": [
      "Piña",
      "Menta",
      "Jengibre",
      "Manzana",
      "Notas marinas",
      "Lavanda",
      "Geranio",
      "Pachulí",
      "Sándalo",
      "Haba tonka",
      "Almizcle"
    ],
    "ml": 100,
    "precioUSD": 35,
    "categoria": "arabes"
  },
  {
    "id": 182,
    "house": "LATTAFA",
    "name": "Lattafa Nebras",
    "image": "lattafa-nebras-100ml-u.webp",
    "familia": "Gourmand",
    "tipo": "Eau de Parfum",
    "genero": "Unisex",
    "descripcion": "Nebras de Lattafa es un perfume unisex de perfil gourmand, cálido y adictivo, con una dulzura envolvente que recuerda a una nube de vainilla y cacao. Se inspira en el estilo de fragancias golosas tipo *Eilish*, ideal para quienes buscan una estela moderna, sensual y sofisticada.",
    "description": "Nebras de Lattafa es un perfume unisex de perfil gourmand, cálido y adictivo, con una dulzura envolvente que recuerda a una nube de vainilla y cacao. Se inspira en el estilo de fragancias golosas tipo *Eilish*, ideal para quienes buscan una estela moderna, sensual y sofisticada.",
    "notasSalida": "Bayas rojas, Mandarina, Vainilla",
    "notasCorazon": "Cacao, Rosa, Azúcar",
    "notasFondo": "Haba tonka, Ámbar, Almizcle",
    "notes": [
      "Bayas rojas",
      "Mandarina",
      "Vainilla",
      "Cacao",
      "Rosa",
      "Azúcar",
      "Haba tonka",
      "Ámbar",
      "Almizcle"
    ],
    "ml": 100,
    "precioUSD": 40,
    "categoria": "arabes"
  },
  {
    "id": 183,
    "house": "LATTAFA",
    "name": "Lattafa Petra",
    "image": "lattafa-petra-100ml-u.webp",
    "familia": "Amaderado",
    "tipo": "Eau de Parfum",
    "genero": "Unisex",
    "descripcion": "Fragancia unisex de carácter floral-frutal gourmand, sofisticada y envolvente, con una dulzura adictiva que deja una estela cálida y seductora. Petra de Lattafa se presenta como una propuesta moderna con un aire de perfume nicho, inspirada en un estilo elegante y sensual de la perfumería de diseñador.",
    "description": "Fragancia unisex de carácter floral-frutal gourmand, sofisticada y envolvente, con una dulzura adictiva que deja una estela cálida y seductora. Petra de Lattafa se presenta como una propuesta moderna con un aire de perfume nicho, inspirada en un estilo elegante y sensual de la perfumería de diseñador.",
    "notasSalida": "Ron, Ciruela, Coco",
    "notasCorazon": "Nardos, Vainilla",
    "notasFondo": "Praliné, Almizcle",
    "notes": [
      "Ron",
      "Ciruela",
      "Coco",
      "Nardos",
      "Vainilla",
      "Praliné",
      "Almizcle"
    ],
    "ml": 100,
    "precioUSD": 55,
    "categoria": "arabes"
  },
  {
    "id": 184,
    "house": "LATTAFA",
    "name": "Lattafa Qaed Al Fursan",
    "image": "lattafa-qaed-al-fursan-90ml-m.webp",
    "familia": "Floral Frutal",
    "tipo": "Eau de Parfum",
    "genero": "Masculino",
    "descripcion": "Perfume para el hombre de carácter moderno, intenso y sofisticado, con un perfil aromático-amaderado que destaca por su apertura frutal y su fondo cálido y elegante. Se inspira en el estilo de fragancias orientales masculinas con un aire cercano a Aventus de Creed, especialmente por su protagonismo de piña y su acabado ahumado y amaderado.",
    "description": "Perfume para el hombre de carácter moderno, intenso y sofisticado, con un perfil aromático-amaderado que destaca por su apertura frutal y su fondo cálido y elegante. Se inspira en el estilo de fragancias orientales masculinas con un aire cercano a Aventus de Creed, especialmente por su protagonismo de piña y su acabado ahumado y amaderado.",
    "notasSalida": "Piña, azafrán, jazmín",
    "notasCorazon": "abeto balsámico, ámbar",
    "notasFondo": "cedro, madera de oud",
    "notes": [
      "Piña",
      "azafrán",
      "jazmín",
      "abeto balsámico",
      "ámbar",
      "cedro",
      "madera de oud"
    ],
    "ml": 90,
    "precioUSD": 35,
    "categoria": "arabes"
  },
  {
    "id": 185,
    "house": "LATTAFA",
    "name": "Lattafa Qaed Al Fursan Unlimited",
    "image": "lattafa-qaed-al-fursan-unlimited-90ml-u.webp",
    "familia": "Floral Frutal",
    "tipo": "Eau de Parfum",
    "genero": "Unisex",
    "descripcion": "Una fragancia unisex vibrante y exótica, con un perfil tropical cremoso y dulce que resulta moderno y muy versátil. Lattafa Qaed Al Fursan Unlimited se inspira en el placer de un oasis tropical, ideal para quienes buscan una estela luminosa, suave y envolvente con aire de perfumería de diseñador.",
    "description": "Una fragancia unisex vibrante y exótica, con un perfil tropical cremoso y dulce que resulta moderno y muy versátil. Lattafa Qaed Al Fursan Unlimited se inspira en el placer de un oasis tropical, ideal para quienes buscan una estela luminosa, suave y envolvente con aire de perfumería de diseñador.",
    "notasSalida": "Coco, Cítricos, Piña",
    "notasCorazon": "Ylang-ylang, Jazmín, Frangipani",
    "notasFondo": "Vainilla, Almizcle, Sándalo",
    "notes": [
      "Coco",
      "Cítricos",
      "Piña",
      "Ylang-ylang",
      "Jazmín",
      "Frangipani",
      "Vainilla",
      "Almizcle",
      "Sándalo"
    ],
    "ml": 90,
    "precioUSD": 35,
    "categoria": "arabes"
  },
  {
    "id": 186,
    "house": "LATTAFA",
    "name": "Lattafa Qaed Al Fursan Untamed",
    "image": "lattafa-qaed-al-fursan-untamed-90ml-u.webp",
    "familia": "Oriental",
    "tipo": "Eau de Parfum",
    "genero": "Unisex",
    "descripcion": "Fragancia unisex de carácter intenso, especiado y amaderado, con un aire moderno y seductor que combina calidez, dulzor y un fondo resinoso elegante. Se percibe como una propuesta oriental sofisticada, con un perfil que algunos distribuidores comparan con el estilo de ciertos perfumes de diseñador por su mezcla de especias, caramelo y maderas.",
    "description": "Fragancia unisex de carácter intenso, especiado y amaderado, con un aire moderno y seductor que combina calidez, dulzor y un fondo resinoso elegante. Se percibe como una propuesta oriental sofisticada, con un perfil que algunos distribuidores comparan con el estilo de ciertos perfumes de diseñador por su mezcla de especias, caramelo y maderas.",
    "notasSalida": "Canela, Mandarina, Nuez moscada, Cardamomo",
    "notasCorazon": "Lavanda, Incienso, Caramelo",
    "notasFondo": "Notas amaderadas, Ámbar, Almizcle",
    "notes": [
      "Canela",
      "Mandarina",
      "Nuez moscada",
      "Cardamomo",
      "Lavanda",
      "Incienso",
      "Caramelo",
      "Notas amaderadas",
      "Ámbar",
      "Almizcle"
    ],
    "ml": 90,
    "precioUSD": 40,
    "categoria": "arabes"
  },
  {
    "id": 187,
    "house": "LATTAFA",
    "name": "Lattafa Qimmah for Men",
    "image": "lattafa-qimmah-for-men-100ml-m.webp",
    "familia": "Amaderado",
    "tipo": "Eau de Parfum",
    "genero": "Masculino",
    "descripcion": "Una fragancia para el hombre de carácter amaderado aromático con un perfil elegante, fresco y refinado. Inspirada en el estilo de Prada L’Homme, combina una presencia limpia y sofisticada con un fondo cálido y masculino ideal para el día a día o la oficina.",
    "description": "Una fragancia para el hombre de carácter amaderado aromático con un perfil elegante, fresco y refinado. Inspirada en el estilo de Prada L’Homme, combina una presencia limpia y sofisticada con un fondo cálido y masculino ideal para el día a día o la oficina.",
    "notasSalida": "Madera de oud, Cedro, Lavanda",
    "notasCorazon": "Salvia, Romero",
    "notasFondo": "Pachulí, Vetiver",
    "notes": [
      "Madera de oud",
      "Cedro",
      "Lavanda",
      "Salvia",
      "Romero",
      "Pachulí",
      "Vetiver"
    ],
    "ml": 100,
    "precioUSD": 30,
    "categoria": "arabes"
  },
  {
    "id": 188,
    "house": "LATTAFA",
    "name": "Lattafa Sakeena",
    "image": "lattafa-sakeena-100ml-f.webp",
    "familia": "Gourmand",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Sakeena de Lattafa es una fragancia para la mujer de perfil floral, frutal y gourmand, luminosa y envolvente, con un toque moderno y seductor. Su faceta golosa y elegante la hace ideal para quienes buscan una estela femenina inspirada en la sofisticación de perfumes de diseñador con acorde dulce y refinado.",
    "description": "Sakeena de Lattafa es una fragancia para la mujer de perfil floral, frutal y gourmand, luminosa y envolvente, con un toque moderno y seductor. Su faceta golosa y elegante la hace ideal para quienes buscan una estela femenina inspirada en la sofisticación de perfumes de diseñador con acorde dulce y refinado.",
    "notasSalida": "Maracuyá, Mandarina, Acorde ozónico, Frambuesa",
    "notasCorazon": "Rosa, Flor de naranjo, Sal marina, Toffee",
    "notasFondo": "Praliné, Vainilla, Almizcle",
    "notes": [
      "Maracuyá",
      "Mandarina",
      "Acorde ozónico",
      "Frambuesa",
      "Rosa",
      "Flor de naranjo",
      "Sal marina",
      "Toffee",
      "Praliné",
      "Vainilla",
      "Almizcle"
    ],
    "ml": 100,
    "precioUSD": 40,
    "categoria": "arabes"
  },
  {
    "id": 189,
    "house": "LATTAFA",
    "name": "Lattafa Sehr",
    "image": "lattafa-sehr-100ml-u.webp",
    "familia": "Floral Frutal",
    "tipo": "Eau de Parfum",
    "genero": "Unisex",
    "descripcion": "Lattafa Sehr es una fragancia unisex de carácter oriental-floral, sofisticada y envolvente, pensada para quienes buscan un aroma moderno con profundidad sensual. Su perfil olfativo se inspira en la elegancia de los perfumes nicho de lujo, con un aire refinado y magnético que la hace ideal para destacar con distinción.",
    "description": "Lattafa Sehr es una fragancia unisex de carácter oriental-floral, sofisticada y envolvente, pensada para quienes buscan un aroma moderno con profundidad sensual. Su perfil olfativo se inspira en la elegancia de los perfumes nicho de lujo, con un aire refinado y magnético que la hace ideal para destacar con distinción.",
    "notasSalida": "Almendra amarga, Frambuesa, Rosa",
    "notasCorazon": "Peonía, Jazmín, Vainilla",
    "notasFondo": "Sándalo, Cachemira, Almizcle",
    "notes": [
      "Almendra amarga",
      "Frambuesa",
      "Rosa",
      "Peonía",
      "Jazmín",
      "Vainilla",
      "Sándalo",
      "Cachemira",
      "Almizcle"
    ],
    "ml": 100,
    "precioUSD": 50,
    "categoria": "arabes"
  },
  {
    "id": 190,
    "house": "LATTAFA",
    "name": "Lattafa Shaheen Gold",
    "image": "lattafa-shaheen-gold-100ml-u.webp",
    "familia": "Cítrico",
    "tipo": "Eau de Parfum",
    "genero": "Unisex",
    "descripcion": "Shaheen Gold de Lattafa es una fragancia unisex sofisticada y luminosa, con un inicio fresco y afrutado que evoluciona hacia un corazón elegante y envolvente. Su estilo oriental moderno combina una vibra cremosa y cálida, inspirada en la perfumería de diseñador de lujo para quienes buscan presencia y distinción.",
    "description": "Shaheen Gold de Lattafa es una fragancia unisex sofisticada y luminosa, con un inicio fresco y afrutado que evoluciona hacia un corazón elegante y envolvente. Su estilo oriental moderno combina una vibra cremosa y cálida, inspirada en la perfumería de diseñador de lujo para quienes buscan presencia y distinción.",
    "notasSalida": "Piña, pomelo, higo",
    "notasCorazon": "lavanda, vainilla",
    "notasFondo": "haba tonka, pachulí",
    "notes": [
      "Piña",
      "pomelo",
      "higo",
      "lavanda",
      "vainilla",
      "haba tonka",
      "pachulí"
    ],
    "ml": 100,
    "precioUSD": 35,
    "categoria": "arabes"
  },
  {
    "id": 191,
    "house": "LATTAFA",
    "name": "Lattafa Sheikh Al Shuyukh Luxe Edition",
    "image": "lattafa-sheikh-al-shuyukh-luxe-edition-100ml-u.webp",
    "familia": "Floral",
    "tipo": "Eau de Parfum",
    "genero": "Unisex",
    "descripcion": "Unisex con un carácter oriental ámbar especiado, sofisticado y envolvente, ideal para quien busca una estela dulce, cálida y con presencia. Se inspira en el estilo de fragancias opulentas de nicho árabe, con un perfil que puede recordar a un gourmand elegante de sello moderno.",
    "description": "Unisex con un carácter oriental ámbar especiado, sofisticado y envolvente, ideal para quien busca una estela dulce, cálida y con presencia. Se inspira en el estilo de fragancias opulentas de nicho árabe, con un perfil que puede recordar a un gourmand elegante de sello moderno.",
    "notasSalida": "Rosa, Canela, Azafrán",
    "notasCorazon": "Caramelo, Pachulí, Notas amaderadas",
    "notasFondo": "Vainilla, Ámbar, Ambroxan",
    "notes": [
      "Rosa",
      "Canela",
      "Azafrán",
      "Caramelo",
      "Pachulí",
      "Notas amaderadas",
      "Vainilla",
      "Ámbar",
      "Ambroxan"
    ],
    "ml": 100,
    "precioUSD": 40,
    "categoria": "arabes",
    "agotado": true
  },
  {
    "id": 192,
    "house": "LATTAFA",
    "name": "Lattafa Taweel",
    "image": "lattafa-taweel-100ml-u.webp",
    "familia": "Amaderado",
    "tipo": "Eau de Parfum",
    "genero": "Unisex",
    "descripcion": "Un perfume unisex de carácter moderno y elegante, con un inicio especiado y cítrico que evoluciona hacia un corazón aromático refinado. Su estela combina matices de lujo contemporáneo con un aire oscuro y sofisticado, y se inspira en el estilo de fragancias frescas-amaderadas de diseñador.",
    "description": "Un perfume unisex de carácter moderno y elegante, con un inicio especiado y cítrico que evoluciona hacia un corazón aromático refinado. Su estela combina matices de lujo contemporáneo con un aire oscuro y sofisticado, y se inspira en el estilo de fragancias frescas-amaderadas de diseñador.",
    "notasSalida": "Bergamota, Jengibre, Enebro de Virginia, Nuez moscada",
    "notasCorazon": "Flor de azahar del naranjo, Lavanda, Ambroxan",
    "notasFondo": "Cuero, Vetiver, Pachulí",
    "notes": [
      "Bergamota",
      "Jengibre",
      "Enebro de Virginia",
      "Nuez moscada",
      "Flor de azahar del naranjo",
      "Lavanda",
      "Ambroxan",
      "Cuero",
      "Vetiver",
      "Pachulí"
    ],
    "ml": 100,
    "precioUSD": 45,
    "categoria": "arabes"
  },
  {
    "id": 193,
    "house": "LATTAFA",
    "name": "Lattafa Teriaq",
    "image": "lattafa-teriaq-100ml-u.webp",
    "familia": "Floral",
    "tipo": "Eau de Parfum",
    "genero": "Unisex",
    "descripcion": "Lattafa Teriaq es un perfume unisex con un perfil goloso, especiado y sensual, ideal para quienes buscan una fragancia con carácter y presencia. Su estilo olfativo combina un dulzor adictivo con matices cálidos y envolventes, y se inspira en el universo de los perfumes nicho gourmand modernos, con una personalidad propia y muy sofisticada.",
    "description": "Lattafa Teriaq es un perfume unisex con un perfil goloso, especiado y sensual, ideal para quienes buscan una fragancia con carácter y presencia. Su estilo olfativo combina un dulzor adictivo con matices cálidos y envolventes, y se inspira en el universo de los perfumes nicho gourmand modernos, con una personalidad propia y muy sofisticada.",
    "notasSalida": "Caramelo, Albaricoque, Pimienta rosa, Almendra amarga, Ruibarbo",
    "notasCorazon": "Flores blancas, Rosa, Miel, Vainilla",
    "notasFondo": "Benjuí, Lábdano, Almizcle, Cuero",
    "notes": [
      "Caramelo",
      "Albaricoque",
      "Pimienta rosa",
      "Almendra amarga",
      "Ruibarbo",
      "Flores blancas",
      "Rosa",
      "Miel",
      "Vainilla",
      "Benjuí",
      "Lábdano",
      "Almizcle",
      "Cuero"
    ],
    "ml": 100,
    "precioUSD": 50,
    "categoria": "arabes"
  },
  {
    "id": 194,
    "house": "LATTAFA",
    "name": "Lattafa Teriaq Intense",
    "image": "lattafa-teriaq-intense-100ml-u.webp",
    "familia": "Cítrico",
    "tipo": "Eau de Parfum",
    "genero": "Unisex",
    "descripcion": "Teriaq Intense de Lattafa es una fragancia unisex de estilo oriental especiado, intensa y envolvente, pensada para quienes buscan una estela cálida, moderna y sofisticada. Su carácter opulento y sensual recuerda a la elegancia de un perfume de nicho, con una inspiración claramente contemporánea en perfumes de diseñador de perfil ámbar-especiado.",
    "description": "Teriaq Intense de Lattafa es una fragancia unisex de estilo oriental especiado, intensa y envolvente, pensada para quienes buscan una estela cálida, moderna y sofisticada. Su carácter opulento y sensual recuerda a la elegancia de un perfume de nicho, con una inspiración claramente contemporánea en perfumes de diseñador de perfil ámbar-especiado.",
    "notasSalida": "Azafrán, Bergamota, Licor de ciruela",
    "notasCorazon": "Canela, Ámbar",
    "notasFondo": "Haba tonka, Benjuí",
    "notes": [
      "Azafrán",
      "Bergamota",
      "Licor de ciruela",
      "Canela",
      "Ámbar",
      "Haba tonka",
      "Benjuí"
    ],
    "ml": 100,
    "precioUSD": 50,
    "categoria": "arabes"
  },
  {
    "id": 195,
    "house": "LATTAFA",
    "name": "Lattafa The Kingdom for Men",
    "image": "lattafa-the-kingdom-for-men-100ml-m.webp",
    "familia": "Oriental",
    "tipo": "Eau de Parfum",
    "genero": "Masculino",
    "descripcion": "Lattafa The Kingdom For Men es un perfume para el hombre que combina frescura aromática y un fondo cálido gourmand con un toque elegante y moderno. Su estela recuerda el estilo de fragancias masculinas sofisticadas, con una inspiración cercana al universo de Jean Paul Gaultier Le Male Elixir por su mezcla de lavanda, menta, vainilla y tabaco.",
    "description": "Lattafa The Kingdom For Men es un perfume para el hombre que combina frescura aromática y un fondo cálido gourmand con un toque elegante y moderno. Su estela recuerda el estilo de fragancias masculinas sofisticadas, con una inspiración cercana al universo de Jean Paul Gaultier Le Male Elixir por su mezcla de lavanda, menta, vainilla y tabaco.",
    "notasSalida": "Salvia, Lavanda, Menta",
    "notasCorazon": "Tabaco, Vainilla, Flor de azahar",
    "notasFondo": "Benjuí, Haba tonka, Ládano",
    "notes": [
      "Salvia",
      "Lavanda",
      "Menta",
      "Tabaco",
      "Vainilla",
      "Flor de azahar",
      "Benjuí",
      "Haba tonka",
      "Ládano"
    ],
    "ml": 100,
    "precioUSD": 40,
    "categoria": "arabes",
    "agotado": true
  },
  {
    "id": 196,
    "house": "LATTAFA",
    "name": "Lattafa The Kingdom for Women",
    "image": "lattafa-the-kingdom-for-women-100ml-f.webp",
    "familia": "Floral Frutal",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Un perfume para la mujer de carácter dulce y sofisticado, con un perfil floral-frutal gourmand que envuelve la piel con elegancia y calidez. Lattafa The Kingdom For Women se inspira en un estilo moderno y lujoso, con un aire afrutado y cremoso que recuerda a fragancias femeninas de diseño contemporáneo.",
    "description": "Un perfume para la mujer de carácter dulce y sofisticado, con un perfil floral-frutal gourmand que envuelve la piel con elegancia y calidez. Lattafa The Kingdom For Women se inspira en un estilo moderno y lujoso, con un aire afrutado y cremoso que recuerda a fragancias femeninas de diseño contemporáneo.",
    "notasSalida": "Pera, Grosellero negro, Peonía, Praliné",
    "notasCorazon": "Haba tonka, Jazmín, Almizcle",
    "notasFondo": "Vainilla, Sándalo, Ámbar",
    "notes": [
      "Pera",
      "Grosellero negro",
      "Peonía",
      "Praliné",
      "Haba tonka",
      "Jazmín",
      "Almizcle",
      "Vainilla",
      "Sándalo",
      "Ámbar"
    ],
    "ml": 100,
    "precioUSD": 40,
    "categoria": "arabes"
  },
  {
    "id": 197,
    "house": "LATTAFA",
    "name": "Lattafa Thouq",
    "image": "lattafa-thouq-80ml-u.webp",
    "familia": "Floral Frutal",
    "tipo": "Eau de Parfum",
    "genero": "Unisex",
    "descripcion": "Unisex de carácter floral-afrutado con un aire elegante y moderno, ideal para quien busca una fragancia luminosa pero con profundidad. Combina una salida fresca de bergamota y lichi con un corazón romántico de rosa e incienso, y un fondo cálido de ámbar y vetiver; se percibe como una creación contemporánea, sin una inspiración de diseñador claramente establecida en las fuentes consultadas.",
    "description": "Unisex de carácter floral-afrutado con un aire elegante y moderno, ideal para quien busca una fragancia luminosa pero con profundidad. Combina una salida fresca de bergamota y lichi con un corazón romántico de rosa e incienso, y un fondo cálido de ámbar y vetiver; se percibe como una creación contemporánea, sin una inspiración de diseñador claramente establecida en las fuentes consultadas.",
    "notasSalida": "Bergamota, Lichi",
    "notasCorazon": "Rosa, Incienso",
    "notasFondo": "Ámbar, Vetiver",
    "notes": [
      "Bergamota",
      "Lichi",
      "Rosa",
      "Incienso",
      "Ámbar",
      "Vetiver"
    ],
    "ml": 80,
    "precioUSD": 40,
    "categoria": "arabes"
  },
  {
    "id": 198,
    "house": "LATTAFA",
    "name": "Lattafa Vintage Radio",
    "image": "lattafa-vintage-radio-100ml-u.webp",
    "familia": "Floral Amaderado",
    "tipo": "Eau de Parfum",
    "genero": "Unisex",
    "descripcion": "Una fragancia unisex sofisticada, de estilo oriental amaderado, que combina una estela moderna con un aire elegante y contemplativo. Vintage Radio de Lattafa Pride se inspira en la icónica Paragon de Initio, ofreciendo un carácter cálido, refinado y distintivo para quienes buscan una firma olfativa de lujo.",
    "description": "Una fragancia unisex sofisticada, de estilo oriental amaderado, que combina una estela moderna con un aire elegante y contemplativo. Vintage Radio de Lattafa Pride se inspira en la icónica Paragon de Initio, ofreciendo un carácter cálido, refinado y distintivo para quienes buscan una firma olfativa de lujo.",
    "notasSalida": "Lavanda, Salvia esclarea, Bergamota",
    "notasCorazon": "Ciruela, Palo de rosa, Sándalo",
    "notasFondo": "Ámbar, Oud",
    "notes": [
      "Lavanda",
      "Salvia esclarea",
      "Bergamota",
      "Ciruela",
      "Palo de rosa",
      "Sándalo",
      "Ámbar",
      "Oud"
    ],
    "ml": 100,
    "precioUSD": 40,
    "categoria": "arabes"
  },
  {
    "id": 199,
    "house": "LATTAFA",
    "name": "Lattafa Winners Trophy Gold",
    "image": "lattafa-winners-trophy-gold-100ml-u.webp",
    "familia": "Floral",
    "tipo": "Eau de Parfum",
    "genero": "Unisex",
    "descripcion": "Winners Trophy Gold de Lattafa es una fragancia unisex de carácter elegante y sofisticado, pensada para quienes buscan un aura moderna y refinada. Su estilo floral amaderado con toques almizclados y dulces evoca el lujo contemporáneo, con una personalidad versátil que recuerda a creaciones de diseñador de corte floral oriental.",
    "description": "Winners Trophy Gold de Lattafa es una fragancia unisex de carácter elegante y sofisticado, pensada para quienes buscan un aura moderna y refinada. Su estilo floral amaderado con toques almizclados y dulces evoca el lujo contemporáneo, con una personalidad versátil que recuerda a creaciones de diseñador de corte floral oriental.",
    "notasSalida": "Pimienta rosa, Jazmín",
    "notasCorazon": "Rosa, Pachulí",
    "notasFondo": "Vainilla, Almizcle",
    "notes": [
      "Pimienta rosa",
      "Jazmín",
      "Rosa",
      "Pachulí",
      "Vainilla",
      "Almizcle"
    ],
    "ml": 100,
    "precioUSD": 40,
    "categoria": "arabes",
    "agotado": true
  },
  {
    "id": 200,
    "house": "LATTAFA",
    "name": "Lattafa Yara Candy",
    "image": "lattafa-yara-candy-100ml-f.webp",
    "familia": "Floral Frutal",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Un perfume para la mujer dulce, chispeante y juvenil, ideal para quienes buscan una estela divertida y gourmand con un toque sofisticado. Yara Candy de Lattafa se inspira en el estilo de los perfumes frutales azucarados de diseñador, con un carácter moderno y adictivo que destaca por su equilibrio entre frescura y cremosidad.",
    "description": "Un perfume para la mujer dulce, chispeante y juvenil, ideal para quienes buscan una estela divertida y gourmand con un toque sofisticado. Yara Candy de Lattafa se inspira en el estilo de los perfumes frutales azucarados de diseñador, con un carácter moderno y adictivo que destaca por su equilibrio entre frescura y cremosidad.",
    "notasSalida": "Grosellas negras, Mandarina verde, Caramelo de fresa",
    "notasCorazon": "Gardenia, Vainilla, Ámbar",
    "notasFondo": "Sándalo, Almizcle",
    "notes": [
      "Grosellas negras",
      "Mandarina verde",
      "Caramelo de fresa",
      "Gardenia",
      "Vainilla",
      "Ámbar",
      "Sándalo",
      "Almizcle"
    ],
    "ml": 100,
    "precioUSD": 30,
    "categoria": "arabes",
    "agotado": true
  },
  {
    "id": 201,
    "house": "LATTAFA",
    "name": "Lattafa Yara Elixir",
    "image": "lattafa-yara-elixir-100ml-f.webp",
    "familia": "Floral Frutal",
    "tipo": "Elixir",
    "genero": "Femenino",
    "descripcion": "Una fragancia para la mujer de estilo oriental-vainilla, intensa, dulce y seductora, pensada para dejar una estela cremosa y adictiva. Yara Elixir se percibe como un perfume gourmand femenino con un toque afrutado y floral, inspirado en el universo de los perfumes dulces de diseñador, aunque con identidad propia.",
    "description": "Una fragancia para la mujer de estilo oriental-vainilla, intensa, dulce y seductora, pensada para dejar una estela cremosa y adictiva. Yara Elixir se percibe como un perfume gourmand femenino con un toque afrutado y floral, inspirado en el universo de los perfumes dulces de diseñador, aunque con identidad propia.",
    "notasSalida": "Fresa, grosellas negras, jazmín",
    "notasCorazon": "flor de azahar del naranjo, caramelo, vainilla",
    "notasFondo": "ámbar, almizcle",
    "notes": [
      "Fresa",
      "grosellas negras",
      "jazmín",
      "flor de azahar del naranjo",
      "caramelo",
      "vainilla",
      "ámbar",
      "almizcle"
    ],
    "ml": 100,
    "precioUSD": 45,
    "categoria": "arabes",
    "agotado": true
  },
  {
    "id": 202,
    "house": "LATTAFA",
    "name": "Lattafa Yara",
    "image": "lattafa-yara-lattafa-100ml-f.webp",
    "familia": "Amaderado",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Una fragancia para la mujer de estilo ámbar vainilla, cremosa y envolvente, con un aire dulce y sofisticado que resulta muy fácil de llevar en el día a día. Yara de Lattafa se inspira en el gusto por los perfumes gourmand modernos, con un perfil sensual y femenino que recuerda a las creaciones orientales más suaves.",
    "description": "Una fragancia para la mujer de estilo ámbar vainilla, cremosa y envolvente, con un aire dulce y sofisticado que resulta muy fácil de llevar en el día a día. Yara de Lattafa se inspira en el gusto por los perfumes gourmand modernos, con un perfil sensual y femenino que recuerda a las creaciones orientales más suaves.",
    "notasSalida": "Orquídea, Heliotropo, Mandarina",
    "notasCorazon": "Acuerdo goloso, Frutas tropicales, Vainilla",
    "notasFondo": "Almizcle, Sándalo",
    "notes": [
      "Orquídea",
      "Heliotropo",
      "Mandarina",
      "Acuerdo goloso",
      "Frutas tropicales",
      "Vainilla",
      "Almizcle",
      "Sándalo"
    ],
    "ml": 100,
    "precioUSD": 30,
    "categoria": "arabes",
    "agotado": true
  },
  {
    "id": 203,
    "house": "LATTAFA",
    "name": "Lattafa Yara Moi",
    "image": "lattafa-yara-moi-100ml-f.webp",
    "familia": "Floral Frutal",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Una fragancia para la mujer de estilo ámbar floral oriental, sofisticada y envolvente, con un perfil dulce y elegante. Yara Moi de Lattafa destaca por su carácter sensual y cálido, inspirado en la estética de un perfume femenino de diseño contemporáneo, con una estela cremosa y seductora.",
    "description": "Una fragancia para la mujer de estilo ámbar floral oriental, sofisticada y envolvente, con un perfil dulce y elegante. Yara Moi de Lattafa destaca por su carácter sensual y cálido, inspirado en la estética de un perfume femenino de diseño contemporáneo, con una estela cremosa y seductora.",
    "notasSalida": "Jazmín, Melocotón",
    "notasCorazon": "Ámbar, Caramelo",
    "notasFondo": "Pachulí, Sándalo",
    "notes": [
      "Jazmín",
      "Melocotón",
      "Ámbar",
      "Caramelo",
      "Pachulí",
      "Sándalo"
    ],
    "ml": 100,
    "precioUSD": 30,
    "categoria": "arabes"
  },
  {
    "id": 204,
    "house": "LATTAFA",
    "name": "Lattafa Yara Tous",
    "image": "lattafa-yara-tous-100ml-f.webp",
    "familia": "Floral Frutal",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Una fragancia para la mujer moderna que envuelve con un aire tropical, dulce y sofisticado. Yara Tous de Lattafa destaca por su carácter jugoso y cremoso, con una estela femenina y luminosa que evoca el estilo de un perfume nicho veraniego, sin inspirarse de forma declarada en un diseñador concreto.",
    "description": "Una fragancia para la mujer moderna que envuelve con un aire tropical, dulce y sofisticado. Yara Tous de Lattafa destaca por su carácter jugoso y cremoso, con una estela femenina y luminosa que evoca el estilo de un perfume nicho veraniego, sin inspirarse de forma declarada en un diseñador concreto.",
    "notasSalida": "Mango, Coco, Maracuyá",
    "notasCorazon": "Jazmín, Heliotropo, Flor de azahar del naranjo",
    "notasFondo": "Vainilla, Cachemira, Almizcle",
    "notes": [
      "Mango",
      "Coco",
      "Maracuyá",
      "Jazmín",
      "Heliotropo",
      "Flor de azahar del naranjo",
      "Vainilla",
      "Cachemira",
      "Almizcle"
    ],
    "ml": 100,
    "precioUSD": 30,
    "categoria": "arabes"
  },
  {
    "id": 205,
    "house": "LOMANI",
    "name": "Lomani AB Spirit Millionaire Women",
    "image": "lomani-ab-spirit-millionaire-women-100ml-f.webp",
    "familia": "Floral Frutal",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Una fragancia para la mujer de estilo floral-frutal con un fondo cálido y envolvente, pensada para transmitir feminidad luminosa y sofisticación. AB Spirit Millionaire Women de Lomani se inspira en el universo elegante y adictivo de los perfumes florales modernos, con un aire cercano a propuestas de diseñador de carácter dulce y sensual.",
    "description": "Una fragancia para la mujer de estilo floral-frutal con un fondo cálido y envolvente, pensada para transmitir feminidad luminosa y sofisticación. AB Spirit Millionaire Women de Lomani se inspira en el universo elegante y adictivo de los perfumes florales modernos, con un aire cercano a propuestas de diseñador de carácter dulce y sensual.",
    "notasSalida": "Lima, Naranja, Flor de azahar del naranjo, Manzana",
    "notasCorazon": "Jazmín egipcio, Frambuesa, Rosa, Almizcle",
    "notasFondo": "Pachulí, Ámbar, Vainilla, Miel",
    "notes": [
      "Lima",
      "Naranja",
      "Flor de azahar del naranjo",
      "Manzana",
      "Jazmín egipcio",
      "Frambuesa",
      "Rosa",
      "Almizcle",
      "Pachulí",
      "Ámbar",
      "Vainilla",
      "Miel"
    ],
    "ml": 100,
    "precioUSD": 20,
    "categoria": "disenador"
  },
  {
    "id": 206,
    "house": "MAISON ALHAMBRA",
    "name": "Maison Alhambra Infini Elixir",
    "image": "maison-alhambra-infini-elixir-100ml-u.webp",
    "familia": "Amaderado",
    "tipo": "Eau de Parfum",
    "genero": "Unisex",
    "descripcion": "Infini Elixir de Maison Alhambra es una fragancia unisex de carácter oriental amaderado, intensa, cálida y envolvente. Su perfil olfativo se inspira en Side Effect de Initio Parfums Privés, aportando un aire sofisticado, especiado y adictivo, ideal para quienes buscan una estela elegante y memorable.",
    "description": "Infini Elixir de Maison Alhambra es una fragancia unisex de carácter oriental amaderado, intensa, cálida y envolvente. Su perfil olfativo se inspira en Side Effect de Initio Parfums Privés, aportando un aire sofisticado, especiado y adictivo, ideal para quienes buscan una estela elegante y memorable.",
    "notasSalida": "Canela, Nuez moscada, Ron",
    "notasCorazon": "Tabaco, Azafrán, Sándalo",
    "notasFondo": "Vainilla, Pachulí",
    "notes": [
      "Canela",
      "Nuez moscada",
      "Ron",
      "Tabaco",
      "Azafrán",
      "Sándalo",
      "Vainilla",
      "Pachulí"
    ],
    "ml": 100,
    "precioUSD": 30,
    "categoria": "arabes",
    "agotado": true
  },
  {
    "id": 207,
    "house": "MAISON ALHAMBRA",
    "name": "Maison Alhambra Kismet for Women",
    "image": "maison-alhambra-kismet-for-women-100ml-f.webp",
    "familia": "Floral Amaderado",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Una fragancia para la mujer de estilo floral blanco y frutal, elegante y sofisticado, con una estela cálida y cremosa. Se inspira en el carácter clásico y envolvente de perfumes florales de diseñador, con un aire afrutado y ambarado muy refinado.",
    "description": "Una fragancia para la mujer de estilo floral blanco y frutal, elegante y sofisticado, con una estela cálida y cremosa. Se inspira en el carácter clásico y envolvente de perfumes florales de diseñador, con un aire afrutado y ambarado muy refinado.",
    "notasSalida": "Chabacano, Rosa, Jazmín",
    "notasCorazon": "Nardo, Narciso",
    "notasFondo": "Ámbar, Cedro",
    "notes": [
      "Chabacano",
      "Rosa",
      "Jazmín",
      "Nardo",
      "Narciso",
      "Ámbar",
      "Cedro"
    ],
    "ml": 100,
    "precioUSD": 40,
    "categoria": "arabes"
  },
  {
    "id": 208,
    "house": "MONTBLANC",
    "name": "Montblanc Femme Individuelle",
    "image": "montblanc-femme-individuelle-75ml-f.webp",
    "familia": "Floral",
    "tipo": "Eau de Toilette",
    "genero": "Femenino",
    "descripcion": "Una fragancia oriental floral para la mujer, sofisticada y con carácter, que combina un aire sensual con un fondo cálido y elegante. Montblanc Femme Individuelle, creada por Michel Almairac, se inspira en el estilo de un perfume de diseñador femenino con un toque gourmand y ambarado muy refinado.",
    "description": "Una fragancia oriental floral para la mujer, sofisticada y con carácter, que combina un aire sensual con un fondo cálido y elegante. Montblanc Femme Individuelle, creada por Michel Almairac, se inspira en el estilo de un perfume de diseñador femenino con un toque gourmand y ambarado muy refinado.",
    "notasSalida": "Pimienta rosa, Grosella roja, Hoja de laurel",
    "notasCorazon": "Rosa, Flor de loto, Pachulí",
    "notasFondo": "Vainilla, Ámbar, Almizcle",
    "notes": [
      "Pimienta rosa",
      "Grosella roja",
      "Hoja de laurel",
      "Rosa",
      "Flor de loto",
      "Pachulí",
      "Vainilla",
      "Ámbar",
      "Almizcle"
    ],
    "ml": 75,
    "precioUSD": 45,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 209,
    "house": "MONTBLANC",
    "name": "Montblanc Lady Emblem",
    "image": "montblanc-lady-emblem-75ml-f.webp",
    "familia": "Floral Frutal",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Lady Emblem de Montblanc es una fragancia sofisticada para la mujer, de estilo chipre frutal, elegante y sensual, con un perfil luminoso y refinado. Se inspira en la feminidad clásica y en la elegancia de un perfume de diseñador contemporáneo, ideal para quien busca una estela distinguida y envolvente.",
    "description": "Lady Emblem de Montblanc es una fragancia sofisticada para la mujer, de estilo chipre frutal, elegante y sensual, con un perfil luminoso y refinado. Se inspira en la feminidad clásica y en la elegancia de un perfume de diseñador contemporáneo, ideal para quien busca una estela distinguida y envolvente.",
    "notasSalida": "Rosa, Mermelada de rosa, Grosella roja",
    "notasCorazon": "Durazno, Sándalo, Pachulí",
    "notasFondo": "Palo de rosa, Frambuesa, Almizcle",
    "notes": [
      "Rosa",
      "Mermelada de rosa",
      "Grosella roja",
      "Durazno",
      "Sándalo",
      "Pachulí",
      "Palo de rosa",
      "Frambuesa",
      "Almizcle"
    ],
    "ml": 75,
    "precioUSD": 45,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 210,
    "house": "MONTBLANC",
    "name": "Montblanc Lady Emblem Elixir",
    "image": "montblanc-lady-emblem-elixir-75ml-f.webp",
    "familia": "Floral Frutal",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Lady Emblem Elixir de Montblanc es un perfume para la mujer elegante, seductor y muy sofisticado, con un perfil chipre floral de carácter luminoso y refinado. Se inspira en la feminidad clásica de un floral opulento, con un aire de lujo contemporáneo y una estela que recuerda a la perfumería de diseñador más distinguida.",
    "description": "Lady Emblem Elixir de Montblanc es un perfume para la mujer elegante, seductor y muy sofisticado, con un perfil chipre floral de carácter luminoso y refinado. Se inspira en la feminidad clásica de un floral opulento, con un aire de lujo contemporáneo y una estela que recuerda a la perfumería de diseñador más distinguida.",
    "notasSalida": "Lichi, Pimienta, Mandarina, Rosa de Damasco",
    "notasCorazon": "Iris, Jazmín, Flor de azahar del naranjo",
    "notasFondo": "Vainilla, Pachulí, Sándalo",
    "notes": [
      "Lichi",
      "Pimienta",
      "Mandarina",
      "Rosa de Damasco",
      "Iris",
      "Jazmín",
      "Flor de azahar del naranjo",
      "Vainilla",
      "Pachulí",
      "Sándalo"
    ],
    "ml": 75,
    "precioUSD": 55,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 211,
    "house": "MOSCHINO",
    "name": "Moschino Toy 2",
    "image": "moschino-toy-2-100ml-u.webp",
    "familia": "Floral Frutal",
    "tipo": "Eau de Parfum",
    "genero": "Unisex",
    "descripcion": "Toy 2 de Moschino es una fragancia unisex luminosa y sofisticada, de estilo floral amaderado almizclado, con una salida fresca y afrutada que evoluciona hacia un corazón limpio y elegante. Su carácter juguetón y moderno la convierte en una opción versátil, con una inspiración claramente contemporánea dentro del universo de diseñador de Moschino.",
    "description": "Toy 2 de Moschino es una fragancia unisex luminosa y sofisticada, de estilo floral amaderado almizclado, con una salida fresca y afrutada que evoluciona hacia un corazón limpio y elegante. Su carácter juguetón y moderno la convierte en una opción versátil, con una inspiración claramente contemporánea dentro del universo de diseñador de Moschino.",
    "notasSalida": "Magnolia, Manzana Granny Smith, Mandarina",
    "notasCorazon": "Grosella blanca, Peonía, Pétalos de jazmín",
    "notasFondo": "Sándalo, Almizcle, Madera de ámbar",
    "notes": [
      "Magnolia",
      "Manzana Granny Smith",
      "Mandarina",
      "Grosella blanca",
      "Peonía",
      "Pétalos de jazmín",
      "Sándalo",
      "Almizcle",
      "Madera de ámbar"
    ],
    "ml": 100,
    "precioUSD": 70,
    "categoria": "disenador"
  },
  {
    "id": 212,
    "house": "MOSCHINO",
    "name": "Moschino Toy 2 Bubble Gum",
    "image": "moschino-toy-2-bubble-gum-100ml-f.webp",
    "familia": "Floral Frutal",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Una fragancia para la mujer alegre, juguetona y deliciosamente adictiva, con un perfil floral-frutal de aire dulce y chispeante. Se inspira en el universo creativo y pop de Moschino Toy 2, llevando el concepto a un acorde de chicle sofisticado con un toque moderno y luminoso.",
    "description": "Una fragancia para la mujer alegre, juguetona y deliciosamente adictiva, con un perfil floral-frutal de aire dulce y chispeante. Se inspira en el universo creativo y pop de Moschino Toy 2, llevando el concepto a un acorde de chicle sofisticado con un toque moderno y luminoso.",
    "notasSalida": "Frutas confitadas, Limón, Naranja amarga, Chicle",
    "notasCorazon": "Rosa de Bulgaria, Arándano, Melocotón, Canela",
    "notasFondo": "Jengibre, Cedro, Almizcle, Ambroxán",
    "notes": [
      "Frutas confitadas",
      "Limón",
      "Naranja amarga",
      "Chicle",
      "Rosa de Bulgaria",
      "Arándano",
      "Melocotón",
      "Canela",
      "Jengibre",
      "Cedro",
      "Almizcle",
      "Ambroxán"
    ],
    "ml": 100,
    "precioUSD": 75,
    "categoria": "disenador"
  },
  {
    "id": 213,
    "house": "MOSCHINO",
    "name": "Moschino Toy 2 Pearl",
    "image": "moschino-toy-2-pearl-100ml-u.webp",
    "familia": "Floral Amaderado",
    "tipo": "Eau de Parfum",
    "genero": "Unisex",
    "descripcion": "Una fragancia unisex de estilo luminoso, fresco y elegante, perfecta para quienes buscan un aroma vibrante con personalidad moderna. Moschino Toy 2 Pearl se inspira en el universo cítrico, floral y almizclado, con un aire que algunos distribuidores describen como versátil y con guiños a un estilo de diseñador contemporáneo, cercano al espíritu de fragancias tipo Light Blue.",
    "description": "Una fragancia unisex de estilo luminoso, fresco y elegante, perfecta para quienes buscan un aroma vibrante con personalidad moderna. Moschino Toy 2 Pearl se inspira en el universo cítrico, floral y almizclado, con un aire que algunos distribuidores describen como versátil y con guiños a un estilo de diseñador contemporáneo, cercano al espíritu de fragancias tipo Light Blue.",
    "notasSalida": "Limón, Orégano, Sorbete de jazmín",
    "notasCorazon": "Arena, Fresia, Jazmín",
    "notasFondo": "Vetiver, Ciprés, Almizcle",
    "notes": [
      "Limón",
      "Orégano",
      "Sorbete de jazmín",
      "Arena",
      "Fresia",
      "Jazmín",
      "Vetiver",
      "Ciprés",
      "Almizcle"
    ],
    "ml": 100,
    "precioUSD": 80,
    "categoria": "disenador"
  },
  {
    "id": 214,
    "house": "ORIENTICA",
    "name": "Orientica Premium Royal Amber",
    "image": "orientica-premium-royal-amber-orientica-100ml-u.webp",
    "familia": "Cítrico",
    "tipo": "Eau de Parfum",
    "genero": "Unisex",
    "descripcion": "Unisex de carácter lujoso y envolvente, con un perfil ambarado, afrutado y cremoso que destaca por su sensualidad elegante. Inspirado en la idea de un oriental moderno, recuerda al estilo de fragancias de diseñador con ámbar, vainilla y un toque frutal luminoso.",
    "description": "Unisex de carácter lujoso y envolvente, con un perfil ambarado, afrutado y cremoso que destaca por su sensualidad elegante. Inspirado en la idea de un oriental moderno, recuerda al estilo de fragancias de diseñador con ámbar, vainilla y un toque frutal luminoso.",
    "notasSalida": "Ámbar, Vainilla",
    "notasCorazon": "Pera, Manzana",
    "notasFondo": "Cítricos, Bergamota",
    "notes": [
      "Ámbar",
      "Vainilla",
      "Pera",
      "Manzana",
      "Cítricos",
      "Bergamota"
    ],
    "ml": 100,
    "precioUSD": 90,
    "categoria": "arabes"
  },
  {
    "id": 215,
    "house": "ORIENTICA",
    "name": "Orientica Premium Royal Bleu",
    "image": "orientica-premium-royal-bleu-80ml-u.webp",
    "familia": "Floral Frutal",
    "tipo": "Eau de Parfum",
    "genero": "Unisex",
    "descripcion": "Unisex, Royal Bleu Orientica Premium es una fragancia elegante y moderna, de perfil amaderado especiado con salida fresca y un fondo cremoso y sensual. Se inspira claramente en el estilo de Parfums de Marly Layton, ofreciendo una estela refinada y versátil para quien busca distinción con un toque oriental de lujo.",
    "description": "Unisex, Royal Bleu Orientica Premium es una fragancia elegante y moderna, de perfil amaderado especiado con salida fresca y un fondo cremoso y sensual. Se inspira claramente en el estilo de Parfums de Marly Layton, ofreciendo una estela refinada y versátil para quien busca distinción con un toque oriental de lujo.",
    "notasSalida": "Manzana verde, Lavanda, Mandarina, Bergamota, Pimienta",
    "notasCorazon": "Cardamomo, Violeta, Geranio, Jazmín, Vainilla",
    "notasFondo": "Sándalo, Madera de gaiac, Pachulí, Almizcle",
    "notes": [
      "Manzana verde",
      "Lavanda",
      "Mandarina",
      "Bergamota",
      "Pimienta",
      "Cardamomo",
      "Violeta",
      "Geranio",
      "Jazmín",
      "Vainilla",
      "Sándalo",
      "Madera de gaiac",
      "Pachulí",
      "Almizcle"
    ],
    "ml": 80,
    "precioUSD": 100,
    "categoria": "arabes"
  },
  {
    "id": 216,
    "house": "PARIS HILTON",
    "name": "Paris Hilton Gold Rush",
    "image": "paris-hilton-gold-rush-100ml-f.webp",
    "familia": "Floral Amaderado",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Una fragancia luminosa y femenina para la mujer, de estilo floral frutal gourmand con un fondo cálido y elegante. Paris Hilton Gold Rush se inspira en la estética de los grandes perfumes de diseñador con un toque moderno, combinando dulzura, sensualidad y un acabado sofisticado ideal para el día o la noche.",
    "description": "Una fragancia luminosa y femenina para la mujer, de estilo floral frutal gourmand con un fondo cálido y elegante. Paris Hilton Gold Rush se inspira en la estética de los grandes perfumes de diseñador con un toque moderno, combinando dulzura, sensualidad y un acabado sofisticado ideal para el día o la noche.",
    "notasSalida": "Bergamota, Limón, Nectarina",
    "notasCorazon": "Orquídea, Pétalos de rosa, Violeta",
    "notasFondo": "Madera de cachemira, Trufa, Vainilla",
    "notes": [
      "Bergamota",
      "Limón",
      "Nectarina",
      "Orquídea",
      "Pétalos de rosa",
      "Violeta",
      "Madera de cachemira",
      "Trufa",
      "Vainilla"
    ],
    "ml": 100,
    "precioUSD": 45,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 217,
    "house": "PARIS HILTON",
    "name": "Paris Hilton Just Me",
    "image": "paris-hilton-just-me-100ml-f.webp",
    "familia": "Floral Amaderado",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Just Me Paris Hilton es una fragancia para la mujer de estilo floral afrutado, fresca y femenina, con un aire moderno y luminoso. Su carácter delicado y elegante recuerda a una propuesta de diseñador juvenil y sofisticada, pensada para destacar con suavidad y encanto.",
    "description": "Just Me Paris Hilton es una fragancia para la mujer de estilo floral afrutado, fresca y femenina, con un aire moderno y luminoso. Su carácter delicado y elegante recuerda a una propuesta de diseñador juvenil y sofisticada, pensada para destacar con suavidad y encanto.",
    "notasSalida": "Iris, Fresia, Rosa blanca",
    "notasCorazon": "Ylang-ylang, Lirio de los valles, Sándalo",
    "notasFondo": "Almizcle, Vainilla de Tahití, Notas amaderadas",
    "notes": [
      "Iris",
      "Fresia",
      "Rosa blanca",
      "Ylang-ylang",
      "Lirio de los valles",
      "Sándalo",
      "Almizcle",
      "Vainilla de Tahití",
      "Notas amaderadas"
    ],
    "ml": 100,
    "precioUSD": 35,
    "categoria": "disenador"
  },
  {
    "id": 218,
    "house": "PARIS HILTON",
    "name": "Paris Hilton Clasica",
    "image": "paris-hilton-paris-hilton-clasica-100ml-f.webp",
    "familia": "Floral Frutal",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Una fragancia floral frutal para la mujer, luminosa y seductora, con un aire juvenil y glamuroso que refleja el estilo icónico de Paris Hilton. Su carácter recuerda a un perfume de diseñador fresco y femenino, ideal para quienes buscan elegancia moderna con un toque divertido.",
    "description": "Una fragancia floral frutal para la mujer, luminosa y seductora, con un aire juvenil y glamuroso que refleja el estilo icónico de Paris Hilton. Su carácter recuerda a un perfume de diseñador fresco y femenino, ideal para quienes buscan elegancia moderna con un toque divertido.",
    "notasSalida": "Durazno, Manzana, Fresia",
    "notasCorazon": "Jazmín, Ylang-ylang",
    "notasFondo": "Roble, Sándalo",
    "notes": [
      "Durazno",
      "Manzana",
      "Fresia",
      "Jazmín",
      "Ylang-ylang",
      "Roble",
      "Sándalo"
    ],
    "ml": 100,
    "precioUSD": 35,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 219,
    "house": "PARIS HILTON",
    "name": "Paris Hilton Pink Rush",
    "image": "paris-hilton-pink-rush-100ml-f.webp",
    "familia": "Floral Frutal",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Una fragancia para la mujer, luminosa y sofisticada, con un carácter floral-frutal que aporta frescura, feminidad y un acabado limpio y elegante. Pink Rush de Paris Hilton evoca el estilo de un perfume moderno de diseñador, con un aire romántico y delicado ideal para el día a día.",
    "description": "Una fragancia para la mujer, luminosa y sofisticada, con un carácter floral-frutal que aporta frescura, feminidad y un acabado limpio y elegante. Pink Rush de Paris Hilton evoca el estilo de un perfume moderno de diseñador, con un aire romántico y delicado ideal para el día a día.",
    "notasSalida": "Bergamota, Lycium, Membrillo, Pera nashi",
    "notasCorazon": "Hibisco, Osmanto, Peonía rosa, Rosa de mayo",
    "notasFondo": "Ámbar, Almizcle blanco, Cedro",
    "notes": [
      "Bergamota",
      "Lycium",
      "Membrillo",
      "Pera nashi",
      "Hibisco",
      "Osmanto",
      "Peonía rosa",
      "Rosa de mayo",
      "Ámbar",
      "Almizcle blanco",
      "Cedro"
    ],
    "ml": 100,
    "precioUSD": 45,
    "categoria": "disenador"
  },
  {
    "id": 220,
    "house": "PARIS HILTON",
    "name": "Paris Hilton Rose Rush",
    "image": "paris-hilton-rose-rush-100ml-f.webp",
    "familia": "Floral Frutal",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Rosé Rush de Paris Hilton es un perfume para la mujer de estilo floral frutal, pensado para quienes buscan una estela romántica, luminosa y sofisticada. Su aroma se inspira en la feminidad moderna y en el universo de los perfumes de diseñador con un toque dulce y elegante, ideal para el día a día o para ocasiones especiales.",
    "description": "Rosé Rush de Paris Hilton es un perfume para la mujer de estilo floral frutal, pensado para quienes buscan una estela romántica, luminosa y sofisticada. Su aroma se inspira en la feminidad moderna y en el universo de los perfumes de diseñador con un toque dulce y elegante, ideal para el día a día o para ocasiones especiales.",
    "notasSalida": "Pétalos de rosa, Lichi, Neroli",
    "notasCorazon": "Rosa de mayo, Peonía, Papaya",
    "notasFondo": "Almizcle blanco, Ámbar, Cedro",
    "notes": [
      "Pétalos de rosa",
      "Lichi",
      "Neroli",
      "Rosa de mayo",
      "Peonía",
      "Papaya",
      "Almizcle blanco",
      "Ámbar",
      "Cedro"
    ],
    "ml": 100,
    "precioUSD": 45,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 221,
    "house": "PARIS HILTON",
    "name": "Paris Hilton Ruby Rush",
    "image": "paris-hilton-ruby-rush-100ml-f.webp",
    "familia": "Frutal",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Ruby Rush de Paris Hilton es un perfume para la mujer con un carácter floral frutal, dulce y seductor, ideal para quienes buscan una estela moderna y juvenil. Su estilo gourmand y vibrante recuerda al universo de fragancias femeninas inspiradas en perfumes de diseñador como *Pink Sugar* por su faceta golosa y coqueta, aunque con una firma más afrutada y cremosa.",
    "description": "Ruby Rush de Paris Hilton es un perfume para la mujer con un carácter floral frutal, dulce y seductor, ideal para quienes buscan una estela moderna y juvenil. Su estilo gourmand y vibrante recuerda al universo de fragancias femeninas inspiradas en perfumes de diseñador como *Pink Sugar* por su faceta golosa y coqueta, aunque con una firma más afrutada y cremosa.",
    "notasSalida": "Cereza, Hibisco, Crema batida",
    "notasCorazon": "Flor de frambueso, Dalia, Nenúfar",
    "notasFondo": "Vainilla, Almizcle, Sándalo",
    "notes": [
      "Cereza",
      "Hibisco",
      "Crema batida",
      "Flor de frambueso",
      "Dalia",
      "Nenúfar",
      "Vainilla",
      "Almizcle",
      "Sándalo"
    ],
    "ml": 100,
    "precioUSD": 45,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 222,
    "house": "PERRY ELLIS",
    "name": "Perry Ellis 360",
    "image": "perry-ellis-360-100ml-f.webp",
    "familia": "Floral Amaderado",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Una fragancia floral fresca para la mujer, luminosa y elegante, con un aire limpio y romántico que la hace muy fácil de llevar a diario. Perry Ellis 360° se inspira en el espíritu de aventura y amor, con un perfil clásico de los 90 que recuerda a los florales frescos de diseñador.",
    "description": "Una fragancia floral fresca para la mujer, luminosa y elegante, con un aire limpio y romántico que la hace muy fácil de llevar a diario. Perry Ellis 360° se inspira en el espíritu de aventura y amor, con un perfil clásico de los 90 que recuerda a los florales frescos de diseñador.",
    "notasSalida": "Rosa, Azucena, Mandarina, Osmanto, Melón",
    "notasCorazon": "Lirio de los Valles, Nenúfar, Lavanda, Salvia, Vainilla",
    "notasFondo": "Ámbar, Vetiver, Sándalo, Almizcle",
    "notes": [
      "Rosa",
      "Azucena",
      "Mandarina",
      "Osmanto",
      "Melón",
      "Lirio de los Valles",
      "Nenúfar",
      "Lavanda",
      "Salvia",
      "Vainilla",
      "Ámbar",
      "Vetiver",
      "Sándalo",
      "Almizcle"
    ],
    "ml": 100,
    "precioUSD": 40,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 223,
    "house": "PERRY ELLIS",
    "name": "Perry Ellis 360 Red",
    "image": "perry-ellis-360-red-100ml-f.webp",
    "familia": "Floral",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Un perfume para la mujer de estilo floral-frutal, elegante y luminoso, con una salida fresca y un corazón blanco muy refinado. 360° Red Perry Ellis Women se inspira en la feminidad romántica y sofisticada de un floral de diseñador, con un aire que recuerda a las composiciones clásicas de perfumería americana de principios de los 2000.",
    "description": "Un perfume para la mujer de estilo floral-frutal, elegante y luminoso, con una salida fresca y un corazón blanco muy refinado. 360° Red Perry Ellis Women se inspira en la feminidad romántica y sofisticada de un floral de diseñador, con un aire que recuerda a las composiciones clásicas de perfumería americana de principios de los 2000.",
    "notasSalida": "Ciclamen, hojas de naranjo, ciruela",
    "notasCorazon": "flor de azahar del naranjo, orquídea, mimosa",
    "notasFondo": "jazmín, ámbar, almizcle",
    "notes": [
      "Ciclamen",
      "hojas de naranjo",
      "ciruela",
      "flor de azahar del naranjo",
      "orquídea",
      "mimosa",
      "jazmín",
      "ámbar",
      "almizcle"
    ],
    "ml": 100,
    "precioUSD": 40,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 224,
    "house": "PRADA",
    "name": "Prada Luna Rossa Carbon Eau de Toilette",
    "image": "prada-luna-rossa-carbon-eau-de-toilette-100ml-m.webp",
    "familia": "Cítrico",
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": "Fragancia para el hombre de carácter fougère aromático, fresca y moderna, con una estela limpia, mineral y elegante. Luna Rossa Carbon de Prada se inspira en el estilo contemporáneo de un perfume de diseñador con un perfil versátil y sofisticado, ideal para uso diario y para quien busca una masculinidad refinada.",
    "description": "Fragancia para el hombre de carácter fougère aromático, fresca y moderna, con una estela limpia, mineral y elegante. Luna Rossa Carbon de Prada se inspira en el estilo contemporáneo de un perfume de diseñador con un perfil versátil y sofisticado, ideal para uso diario y para quien busca una masculinidad refinada.",
    "notasSalida": "Bergamota de Italia, Pimienta, Lavanda",
    "notasCorazon": "Notas acuosas, Tierra, Carbón",
    "notasFondo": "Pachulí, Ambroxan",
    "notes": [
      "Bergamota de Italia",
      "Pimienta",
      "Lavanda",
      "Notas acuosas",
      "Tierra",
      "Carbón",
      "Pachulí",
      "Ambroxan"
    ],
    "ml": 100,
    "precioUSD": 150,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 225,
    "house": "RALPH LAUREN",
    "name": "Ralph Lauren Big Pony 2 for Women",
    "image": "ralph-lauren-big-pony-2-for-women-100ml-f.webp",
    "familia": "Amaderado",
    "tipo": "Eau de Toilette",
    "genero": "Femenino",
    "descripcion": "Una fragancia para la mujer vibrante y moderna, de estilo floral frutal con un aire joven, alegre y seductor. Inspirada en el universo deportivo y elegante de Ralph Lauren, destaca por un carácter dulce y sofisticado con un toque muy femenino.",
    "description": "Una fragancia para la mujer vibrante y moderna, de estilo floral frutal con un aire joven, alegre y seductor. Inspirada en el universo deportivo y elegante de Ralph Lauren, destaca por un carácter dulce y sofisticado con un toque muy femenino.",
    "notasSalida": "Arándano rojo",
    "notasCorazon": "haba tonka",
    "notasFondo": "notas florales",
    "notes": [
      "Arándano rojo",
      "haba tonka",
      "notas florales"
    ],
    "ml": 100,
    "precioUSD": 85,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 226,
    "house": "RASASI",
    "name": "Rasasi Daarej Women",
    "image": "rasasi-daarej-pour-femme-100ml-f.webp",
    "familia": "Frutal",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Rasasi Daarej Pour Femme es un perfume para la mujer de estilo floral frutal gourmand, con un carácter cálido, dulce y envolvente que aporta una presencia sofisticada y femenina. Su perfil recuerda a una fragancia de inspiración oriental con matices de diseñador, ideal para quien busca un aroma sensual y elegante para el día a día o para ocasiones especiales.",
    "description": "Rasasi Daarej Pour Femme es un perfume para la mujer de estilo floral frutal gourmand, con un carácter cálido, dulce y envolvente que aporta una presencia sofisticada y femenina. Su perfil recuerda a una fragancia de inspiración oriental con matices de diseñador, ideal para quien busca un aroma sensual y elegante para el día a día o para ocasiones especiales.",
    "notasSalida": "Durazno, Naranja, Notas verdes",
    "notasCorazon": "Caramelo, Heliotropo, Pachulí",
    "notasFondo": "Sándalo, Vetiver, Almizcle",
    "notes": [
      "Durazno",
      "Naranja",
      "Notas verdes",
      "Caramelo",
      "Heliotropo",
      "Pachulí",
      "Sándalo",
      "Vetiver",
      "Almizcle"
    ],
    "ml": 100,
    "precioUSD": 45,
    "categoria": "arabes"
  },
  {
    "id": 227,
    "house": "RASASI",
    "name": "Rasasi Egra for Women",
    "image": "rasasi-egra-women-100ml-f.webp",
    "familia": "Floral Amaderado",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Una fragancia para la mujer sofisticada y elegante, con un estilo floral amaderado almizclado que equilibra frescura, suavidad y carácter. Rasasi Egra Women evoca una estela refinada y moderna, con una inspiración claramente ligada a los perfumes florales de diseñador de corte clásico contemporáneo.",
    "description": "Una fragancia para la mujer sofisticada y elegante, con un estilo floral amaderado almizclado que equilibra frescura, suavidad y carácter. Rasasi Egra Women evoca una estela refinada y moderna, con una inspiración claramente ligada a los perfumes florales de diseñador de corte clásico contemporáneo.",
    "notasSalida": "Bergamota, Nenúfar, Fresia, Pimienta",
    "notasCorazon": "Flor de azahar del naranjo, Jazmín, Ciruela",
    "notasFondo": "Almizcle, Pachulí, Madera de sándalo",
    "notes": [
      "Bergamota",
      "Nenúfar",
      "Fresia",
      "Pimienta",
      "Flor de azahar del naranjo",
      "Jazmín",
      "Ciruela",
      "Almizcle",
      "Pachulí",
      "Madera de sándalo"
    ],
    "ml": 100,
    "precioUSD": 35,
    "categoria": "arabes"
  },
  {
    "id": 228,
    "house": "RASASI",
    "name": "Rasasi Hawas Elixir",
    "image": "rasasi-hawas-elixir-100ml-u.webp",
    "familia": "Amaderado",
    "tipo": "Eau de Parfum",
    "genero": "Unisex",
    "descripcion": "Hawas Elixir de Rasasi es un unisex de carácter oriental vainilla, con un perfil moderno, intenso y seductor que equilibra frescura especiada y un fondo cálido y envolvente. Su estilo sofisticado recuerda a propuestas de diseñador de corte ambarado-aromático, con una vibra elegante y magnética para quien busca destacar con presencia.",
    "description": "Hawas Elixir de Rasasi es un unisex de carácter oriental vainilla, con un perfil moderno, intenso y seductor que equilibra frescura especiada y un fondo cálido y envolvente. Su estilo sofisticado recuerda a propuestas de diseñador de corte ambarado-aromático, con una vibra elegante y magnética para quien busca destacar con presencia.",
    "notasSalida": "Pomelo, menta, lavanda, bergamota, abrótano",
    "notasCorazon": "cardamomo, canela, vainilla, benjuí, miel",
    "notasFondo": "haba tonka, tabaco, cedro, almizcle, sándalo",
    "notes": [
      "Pomelo",
      "menta",
      "lavanda",
      "bergamota",
      "abrótano",
      "cardamomo",
      "canela",
      "vainilla",
      "benjuí",
      "miel",
      "haba tonka",
      "tabaco",
      "cedro",
      "almizcle",
      "sándalo"
    ],
    "ml": 100,
    "precioUSD": 45,
    "categoria": "arabes",
    "agotado": true
  },
  {
    "id": 229,
    "house": "RASASI",
    "name": "Rasasi Hawas for Her",
    "image": "rasasi-hawas-for-her-100ml-f.webp",
    "familia": "Floral Frutal",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Hawas For Her de Rasasi es una fragancia para la mujer de estilo floral afrutado con un toque gourmand, pensada para dejar una estela elegante y moderna. Se inspira en una feminidad sofisticada y seductora, con un aire contemporáneo que encaja perfectamente en una colección de lujo.",
    "description": "Hawas For Her de Rasasi es una fragancia para la mujer de estilo floral afrutado con un toque gourmand, pensada para dejar una estela elegante y moderna. Se inspira en una feminidad sofisticada y seductora, con un aire contemporáneo que encaja perfectamente en una colección de lujo.",
    "notasSalida": "Granada, Manzana, Pomelo",
    "notasCorazon": "Jazmín sambac, Iris, Cítricos",
    "notasFondo": "Pachulí, Vetiver, Praliné",
    "notes": [
      "Granada",
      "Manzana",
      "Pomelo",
      "Jazmín sambac",
      "Iris",
      "Cítricos",
      "Pachulí",
      "Vetiver",
      "Praliné"
    ],
    "ml": 100,
    "precioUSD": 35,
    "categoria": "arabes",
    "agotado": true
  },
  {
    "id": 230,
    "house": "RASASI",
    "name": "Rasasi Hawas for Her Eclat",
    "image": "rasasi-hawas-for-her-clat-100ml-f.webp",
    "familia": "Floral",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Una fragancia para la mujer de carácter floral oriental, luminosa y sensual, creada para envolver con una estela elegante y moderna. Rasasi Hawas For Her Éclat destaca por un estilo femenino sofisticado, con un aire seductor que se mueve entre lo frutal, lo cremoso y lo ambarado; algunas tiendas la presentan con una inspiración de estilo similar a perfumes de diseñador, aunque no siempre especifican una equivalencia exacta.",
    "description": "Una fragancia para la mujer de carácter floral oriental, luminosa y sensual, creada para envolver con una estela elegante y moderna. Rasasi Hawas For Her Éclat destaca por un estilo femenino sofisticado, con un aire seductor que se mueve entre lo frutal, lo cremoso y lo ambarado; algunas tiendas la presentan con una inspiración de estilo similar a perfumes de diseñador, aunque no siempre especifican una equivalencia exacta.",
    "notasSalida": "Flor de azahar del naranjo, rosa, jazmín",
    "notasCorazon": "frutas tropicales, vainilla",
    "notasFondo": "ámbar, almizcle",
    "notes": [
      "Flor de azahar del naranjo",
      "rosa",
      "jazmín",
      "frutas tropicales",
      "vainilla",
      "ámbar",
      "almizcle"
    ],
    "ml": 100,
    "precioUSD": 60,
    "categoria": "arabes"
  },
  {
    "id": 231,
    "house": "RASASI",
    "name": "Rasasi Hawas for Him",
    "image": "rasasi-hawas-for-him-100ml-m.webp",
    "familia": "Frutal",
    "tipo": "Eau de Parfum",
    "genero": "Masculino",
    "descripcion": "Fragancia para el hombre de estilo aromático acuático, fresca y seductora, pensada para proyectar presencia con un aire moderno y elegante. Se inspira en el ADN de los grandes perfumes frescos masculinos, con una vibra contemporánea y muy versátil para el día a día o la noche.",
    "description": "Fragancia para el hombre de estilo aromático acuático, fresca y seductora, pensada para proyectar presencia con un aire moderno y elegante. Se inspira en el ADN de los grandes perfumes frescos masculinos, con una vibra contemporánea y muy versátil para el día a día o la noche.",
    "notasSalida": "Manzana, Bergamota, Limón, Canela",
    "notasCorazon": "Notas acuosas, Ciruela, Cardamomo, Flor de azahar del naranjo",
    "notasFondo": "Almizcle, Ámbar gris, Pachulí, Madera a la deriva",
    "notes": [
      "Manzana",
      "Bergamota",
      "Limón",
      "Canela",
      "Notas acuosas",
      "Ciruela",
      "Cardamomo",
      "Flor de azahar del naranjo",
      "Almizcle",
      "Ámbar gris",
      "Pachulí",
      "Madera a la deriva"
    ],
    "ml": 100,
    "precioUSD": 45,
    "categoria": "arabes",
    "agotado": true
  },
  {
    "id": 232,
    "house": "RASASI",
    "name": "Rasasi Hawas Kobra",
    "image": "rasasi-hawas-kobra-100ml-m.webp",
    "familia": "Cítrico",
    "tipo": "Eau de Parfum",
    "genero": "Masculino",
    "descripcion": "Una fragancia para el hombre de perfil oriental amaderado y especiado, creada para proyectar presencia, elegancia e intensidad. Hawas Kobra de Rasasi se inspira en el estilo moderno de los perfumes masculinos de lujo, con una apertura fresca y vibrante que evoluciona hacia un fondo cálido, sensual y sofisticado.",
    "description": "Una fragancia para el hombre de perfil oriental amaderado y especiado, creada para proyectar presencia, elegancia e intensidad. Hawas Kobra de Rasasi se inspira en el estilo moderno de los perfumes masculinos de lujo, con una apertura fresca y vibrante que evoluciona hacia un fondo cálido, sensual y sofisticado.",
    "notasSalida": "Jengibre, Bergamota, Mandarina",
    "notasCorazon": "Canela, Neroli, Té verde",
    "notasFondo": "Almizcle, Notas amaderadas, Ámbar",
    "notes": [
      "Jengibre",
      "Bergamota",
      "Mandarina",
      "Canela",
      "Neroli",
      "Té verde",
      "Almizcle",
      "Notas amaderadas",
      "Ámbar"
    ],
    "ml": 100,
    "precioUSD": 65,
    "categoria": "arabes",
    "agotado": true
  },
  {
    "id": 233,
    "house": "RASASI",
    "name": "Rasasi Hawas London",
    "image": "rasasi-hawas-london-100ml-u.webp",
    "familia": "Floral Frutal",
    "tipo": "Eau de Parfum",
    "genero": "Unisex",
    "descripcion": "Unisex de carácter oriental amaderado, con un perfil sofisticado que combina frescura frutal y un fondo cálido y sensual. Su aroma recuerda a una creación de diseñador contemporánea, con un aire elegante y misterioso inspirado en fragancias especiadas y modernas.",
    "description": "Unisex de carácter oriental amaderado, con un perfil sofisticado que combina frescura frutal y un fondo cálido y sensual. Su aroma recuerda a una creación de diseñador contemporánea, con un aire elegante y misterioso inspirado en fragancias especiadas y modernas.",
    "notasSalida": "Pimienta rosa, Azafrán, Pera",
    "notasCorazon": "Incienso, Rosa, Flores blancas",
    "notasFondo": "Almizcle, Ámbar, Vainilla",
    "notes": [
      "Pimienta rosa",
      "Azafrán",
      "Pera",
      "Incienso",
      "Rosa",
      "Flores blancas",
      "Almizcle",
      "Ámbar",
      "Vainilla"
    ],
    "ml": 100,
    "precioUSD": 75,
    "categoria": "arabes",
    "agotado": true
  },
  {
    "id": 234,
    "house": "RASASI",
    "name": "Rasasi Hawas Malibu",
    "image": "rasasi-hawas-malibu-100ml-m.webp",
    "familia": "Floral Frutal",
    "tipo": "Eau de Parfum",
    "genero": "Masculino",
    "descripcion": "Rasasi Hawas Malibu es una fragancia fresca, luminosa y moderna para el hombre, con un perfil aromático afrutado y ambarado que transmite energía y elegancia casual. Se inspira en el estilo vibrante de las fragancias masculinas contemporáneas, con un carácter que puede recordar a un *designer scent* veraniego y pulido, pensado para destacar con una estela limpia y sensual.",
    "description": "Rasasi Hawas Malibu es una fragancia fresca, luminosa y moderna para el hombre, con un perfil aromático afrutado y ambarado que transmite energía y elegancia casual. Se inspira en el estilo vibrante de las fragancias masculinas contemporáneas, con un carácter que puede recordar a un *designer scent* veraniego y pulido, pensado para destacar con una estela limpia y sensual.",
    "notasSalida": "Piña, Naranja, Pomelo, Raíz de lirio",
    "notasCorazon": "Ámbar, Lavanda, Haba tonka",
    "notasFondo": "Almizcle, Pachulí, Madera de cachemira",
    "notes": [
      "Piña",
      "Naranja",
      "Pomelo",
      "Raíz de lirio",
      "Ámbar",
      "Lavanda",
      "Haba tonka",
      "Almizcle",
      "Pachulí",
      "Madera de cachemira"
    ],
    "ml": 100,
    "precioUSD": 60,
    "categoria": "arabes"
  },
  {
    "id": 235,
    "house": "RASASI",
    "name": "Rasasi La Yuqawam Orchid Prairie",
    "image": "rasasi-la-yuqawam-orchid-prairie-75ml-f.webp",
    "familia": "Floral",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Una fragancia oriental floral para la mujer, con un carácter elegante, sensual y moderno. Rasasi La Yuqawam Orchid Prairie se asocia a un estilo sofisticado y lujoso, con un perfil que puede recordar a propuestas femeninas intensas y seductoras del mundo nicho y de diseñador.",
    "description": "Una fragancia oriental floral para la mujer, con un carácter elegante, sensual y moderno. Rasasi La Yuqawam Orchid Prairie se asocia a un estilo sofisticado y lujoso, con un perfil que puede recordar a propuestas femeninas intensas y seductoras del mundo nicho y de diseñador.",
    "notasSalida": "Almizcle, Ámbar, Cashmeran",
    "notasCorazon": "Cumarina, Geranio, Oud",
    "notasFondo": "Rosa, Vainilla",
    "notes": [
      "Almizcle",
      "Ámbar",
      "Cashmeran",
      "Cumarina",
      "Geranio",
      "Oud",
      "Rosa",
      "Vainilla"
    ],
    "ml": 75,
    "precioUSD": 50,
    "categoria": "arabes",
    "agotado": true
  },
  {
    "id": 236,
    "house": "RAVE",
    "name": "Rave Now White",
    "image": "rave-now-white-100ml-u.webp",
    "familia": "Cítrico",
    "tipo": "Eau de Parfum",
    "genero": "Unisex",
    "descripcion": "RAVE Now White es una fragancia unisex fresca, luminosa y elegante, ideal para quienes buscan un aroma limpio con un toque moderno y sofisticado. Se inspira en un estilo cítrico-amaderado con matices frutales, recordando el perfil versátil de perfumes de diseñador contemporáneos de uso diario.",
    "description": "RAVE Now White es una fragancia unisex fresca, luminosa y elegante, ideal para quienes buscan un aroma limpio con un toque moderno y sofisticado. Se inspira en un estilo cítrico-amaderado con matices frutales, recordando el perfil versátil de perfumes de diseñador contemporáneos de uso diario.",
    "notasSalida": "Toronja (pomelo), Bergamota, Manzana verde",
    "notasCorazon": "Pachulí, Almizcle",
    "notasFondo": "Ámbar, Cachemira",
    "notes": [
      "Toronja (pomelo)",
      "Bergamota",
      "Manzana verde",
      "Pachulí",
      "Almizcle",
      "Ámbar",
      "Cachemira"
    ],
    "ml": 100,
    "precioUSD": 30,
    "categoria": "arabes"
  },
  {
    "id": 237,
    "house": "RAVE",
    "name": "Rave Now Women",
    "image": "rave-now-women-100ml-f.webp",
    "familia": "Floral Frutal",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Fragancia para la mujer de estilo dulce, afrutado y cremoso, con un aire juvenil y muy femenino. Se inspira en el universo olfativo de Burberry Her Elixir, con una estela moderna y envolvente ideal para quienes buscan un perfume delicado pero llamativo.",
    "description": "Fragancia para la mujer de estilo dulce, afrutado y cremoso, con un aire juvenil y muy femenino. Se inspira en el universo olfativo de Burberry Her Elixir, con una estela moderna y envolvente ideal para quienes buscan un perfume delicado pero llamativo.",
    "notasSalida": "Frutos rojos, Naranja, Malvavisco",
    "notasCorazon": "Lirio de los valles, Jazmín, Vainilla",
    "notasFondo": "Almizcle, Musgo",
    "notes": [
      "Frutos rojos",
      "Naranja",
      "Malvavisco",
      "Lirio de los valles",
      "Jazmín",
      "Vainilla",
      "Almizcle",
      "Musgo"
    ],
    "ml": 100,
    "precioUSD": 35,
    "categoria": "arabes"
  },
  {
    "id": 238,
    "house": "RAYHAAN",
    "name": "Rayhaan Elixir",
    "image": "rayhaan-elixir-100ml-u.webp",
    "familia": "Cítrico",
    "tipo": "Eau de Parfum",
    "genero": "Unisex",
    "descripcion": "Rayhaan Elixir es un perfume unisex de carácter oriental amaderado, elegante y envolvente, ideal para quienes buscan una estela moderna con presencia y sofisticación. Se inspira claramente en el estilo de Le Male Elixir de Jean Paul Gaultier, reinterpretado con un toque más versátil y lujoso para el día o la noche.",
    "description": "Rayhaan Elixir es un perfume unisex de carácter oriental amaderado, elegante y envolvente, ideal para quienes buscan una estela moderna con presencia y sofisticación. Se inspira claramente en el estilo de Le Male Elixir de Jean Paul Gaultier, reinterpretado con un toque más versátil y lujoso para el día o la noche.",
    "notasSalida": "Benjuí, Bergamota",
    "notasCorazon": "Lavanda, Menta",
    "notasFondo": "Haba tonka, Vainilla",
    "notes": [
      "Benjuí",
      "Bergamota",
      "Lavanda",
      "Menta",
      "Haba tonka",
      "Vainilla"
    ],
    "ml": 100,
    "precioUSD": 50,
    "categoria": "arabes",
    "agotado": true
  },
  {
    "id": 239,
    "house": "REVLON",
    "name": "Revlon Ciara",
    "image": "revlon-ciara-68ml-f.webp",
    "familia": "Floral Frutal",
    "tipo": "Eau de Cologne",
    "genero": "Femenino",
    "descripcion": "Un clásico oriental y sensual para la mujer, con un carácter elegante, cálido y de aire vintage. Su estela mezcla frutos rojos, flores blancas y un fondo profundo de incienso, cuero y vainilla, evocando el estilo sofisticado de las grandes fragancias femeninas de los años 70.",
    "description": "Un clásico oriental y sensual para la mujer, con un carácter elegante, cálido y de aire vintage. Su estela mezcla frutos rojos, flores blancas y un fondo profundo de incienso, cuero y vainilla, evocando el estilo sofisticado de las grandes fragancias femeninas de los años 70.",
    "notasSalida": "Frambuesa, Neroli, Bergamota, Limón, Palo de rosa de Brasil",
    "notasCorazon": "Ylang-ylang, Raíz de lirio, Jazmín, Palmarosa, Opopónaco",
    "notasFondo": "Incienso, Cuero, Vainilla, Almizcle, Cedro",
    "notes": [
      "Frambuesa",
      "Neroli",
      "Bergamota",
      "Limón",
      "Palo de rosa de Brasil",
      "Ylang-ylang",
      "Raíz de lirio",
      "Jazmín",
      "Palmarosa",
      "Opopónaco",
      "Incienso",
      "Cuero",
      "Vainilla",
      "Almizcle",
      "Cedro"
    ],
    "ml": 68,
    "precioUSD": 20,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 240,
    "house": "SALVATORE FERRAGAMO",
    "name": "Salvatore Ferragamo Signorina Eau de Toilette",
    "image": "salvatore-ferragamo-signorina-eau-de-toilette-100ml-f.webp",
    "familia": "Floral",
    "tipo": "Eau de Toilette",
    "genero": "Femenino",
    "descripcion": "Un perfume para la mujer de estilo floral afrutado, delicado y elegante, con un aire juvenil y sofisticado. Signorina Eau de Toilette de Salvatore Ferragamo se inspira en el encanto femenino contemporáneo y evoca la estética refinada de la casa italiana.",
    "description": "Un perfume para la mujer de estilo floral afrutado, delicado y elegante, con un aire juvenil y sofisticado. Signorina Eau de Toilette de Salvatore Ferragamo se inspira en el encanto femenino contemporáneo y evoca la estética refinada de la casa italiana.",
    "notasSalida": "Pimienta rosa, Grosella roja, Jazmín",
    "notasCorazon": "Rosa, Peonía, Almizcle",
    "notasFondo": "Panacotta, Pachulí",
    "notes": [
      "Pimienta rosa",
      "Grosella roja",
      "Jazmín",
      "Rosa",
      "Peonía",
      "Almizcle",
      "Panacotta",
      "Pachulí"
    ],
    "ml": 100,
    "precioUSD": 55,
    "categoria": "disenador"
  },
  {
    "id": 241,
    "house": "SALVATORE FERRAGAMO",
    "name": "Eau de Parfum Leather Edition",
    "image": "salvatore-ferragamo-signorina-leather-edition-100ml-f.webp",
    "familia": "Floral",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Un perfume para la mujer con un carácter oriental floral sofisticado, elegante y sensual. Signorina Leather Edition de Salvatore Ferragamo se inspira en el universo refinado de la línea Signorina, con un toque de cuero que aporta más profundidad y un aire más intenso y moderno, ideal para una presencia distinguida.",
    "description": "Un perfume para la mujer con un carácter oriental floral sofisticado, elegante y sensual. Signorina Leather Edition de Salvatore Ferragamo se inspira en el universo refinado de la línea Signorina, con un toque de cuero que aporta más profundidad y un aire más intenso y moderno, ideal para una presencia distinguida.",
    "notasSalida": "Cassis, Rosa",
    "notasCorazon": "Osmanto, Cuero",
    "notasFondo": "Pachulí, Vainilla",
    "notes": [
      "Cassis",
      "Rosa",
      "Osmanto",
      "Cuero",
      "Pachulí",
      "Vainilla"
    ],
    "ml": 100,
    "precioUSD": 65,
    "categoria": "disenador"
  },
  {
    "id": 242,
    "house": "VALENTINO",
    "name": "Valentino Donna Born in Roma",
    "image": "valentino-donna-born-in-roma-100ml-f.webp",
    "familia": "Floral Amaderado",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Una fragancia para la mujer moderna y sofisticada, de estilo floriental y sensual, con un aura elegante y femenina. Valentino Donna Born In Roma se inspira en la alta costura italiana y destaca por su equilibrio entre flores luminosas, frutas jugosas y un fondo cremoso y envolvente.",
    "description": "Una fragancia para la mujer moderna y sofisticada, de estilo floriental y sensual, con un aura elegante y femenina. Valentino Donna Born In Roma se inspira en la alta costura italiana y destaca por su equilibrio entre flores luminosas, frutas jugosas y un fondo cremoso y envolvente.",
    "notasSalida": "Grosella negra, Bergamota, Pimienta rosa",
    "notasCorazon": "Jazmín, Jazmín sambac, Té de jazmín",
    "notasFondo": "Vainilla Bourbon, Cachemira, Madera de gaiac",
    "notes": [
      "Grosella negra",
      "Bergamota",
      "Pimienta rosa",
      "Jazmín",
      "Jazmín sambac",
      "Té de jazmín",
      "Vainilla Bourbon",
      "Cachemira",
      "Madera de gaiac"
    ],
    "ml": 100,
    "precioUSD": 190,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 243,
    "house": "VALENTINO",
    "name": "Valentino Donna Born in Roma Green Stravaganza",
    "image": "valentino-donna-born-in-roma-green-stravaganza-100ml-f.webp",
    "familia": "Floral",
    "tipo": "Eau de Toilette",
    "genero": "Femenino",
    "descripcion": "Una fragancia para la mujer floral ámbar amaderada, con un carácter elegante, moderno y ligeramente ahumado que envuelve con sofisticación. Su estilo olfativo combina la frescura del té lapsang souchong con un corazón floral luminoso y una base cremosa de vainilla, evocando el universo refinado y sensual de la línea Valentino Born in Roma.",
    "description": "Una fragancia para la mujer floral ámbar amaderada, con un carácter elegante, moderno y ligeramente ahumado que envuelve con sofisticación. Su estilo olfativo combina la frescura del té lapsang souchong con un corazón floral luminoso y una base cremosa de vainilla, evocando el universo refinado y sensual de la línea Valentino Born in Roma.",
    "notasSalida": "Té Lapsang Souchong",
    "notasCorazon": "Jazmín",
    "notasFondo": "Vainilla",
    "notes": [
      "Té Lapsang Souchong",
      "Jazmín",
      "Vainilla"
    ],
    "ml": 100,
    "precioUSD": 195,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 244,
    "house": "VALENTINO",
    "name": "Valentino Donna Born in Roma Intense",
    "image": "valentino-donna-born-in-roma-intense-100ml-f.webp",
    "familia": "Floral",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Un perfume para la mujer con carácter sensual y sofisticado, de estela ámbar floral y una vainilla intensa que envuelve la piel con elegancia. Valentino Donna Born In Roma Intense se inspira en el universo olfativo de la línea Born in Roma, con un perfil que recuerda a la feminidad moderna de un diseñador de lujo.",
    "description": "Un perfume para la mujer con carácter sensual y sofisticado, de estela ámbar floral y una vainilla intensa que envuelve la piel con elegancia. Valentino Donna Born In Roma Intense se inspira en el universo olfativo de la línea Born in Roma, con un perfil que recuerda a la feminidad moderna de un diseñador de lujo.",
    "notasSalida": "Vainilla Bourbon, Ámbar",
    "notasCorazon": "Jazmín",
    "notasFondo": "Benjuí",
    "notes": [
      "Vainilla Bourbon",
      "Ámbar",
      "Jazmín",
      "Benjuí"
    ],
    "ml": 100,
    "precioUSD": 230,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 245,
    "house": "VALENTINO",
    "name": "Valentino Valentina 2025",
    "image": "valentino-valentina-2025-50ml-f.webp",
    "familia": "Floral Frutal",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Valentina 2025 de Valentino es una fragancia floral frutal para la mujer, sofisticada y luminosa, con un aire romántico y moderno. Su estilo elegante recuerda a la feminidad contemporánea de Valentino, con una estela cremosa y seductora que encaja muy bien en cualquier ocasión.",
    "description": "Valentina 2025 de Valentino es una fragancia floral frutal para la mujer, sofisticada y luminosa, con un aire romántico y moderno. Su estilo elegante recuerda a la feminidad contemporánea de Valentino, con una estela cremosa y seductora que encaja muy bien en cualquier ocasión.",
    "notasSalida": "Fresa, Bergamota",
    "notasCorazon": "Jazmín sambac, Pachulí",
    "notasFondo": "Vainilla, Cedro",
    "notes": [
      "Fresa",
      "Bergamota",
      "Jazmín sambac",
      "Pachulí",
      "Vainilla",
      "Cedro"
    ],
    "ml": 50,
    "precioUSD": 125,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 246,
    "house": "VERSACE",
    "name": "Versace Red Jeans",
    "image": "versace-red-jeans-75ml-f.webp",
    "familia": "Floral Frutal",
    "tipo": "Eau de Toilette",
    "genero": "Femenino",
    "descripcion": "Versace Red Jeans es un perfume para la mujer de estilo floral frutal, alegre y juvenil, con un aire clásico de los años 90. Su carácter vibrante y femenino lo convierte en una fragancia fácil de llevar, inspirada en la frescura de un diseñador italiano icónico.",
    "description": "Versace Red Jeans es un perfume para la mujer de estilo floral frutal, alegre y juvenil, con un aire clásico de los años 90. Su carácter vibrante y femenino lo convierte en una fragancia fácil de llevar, inspirada en la frescura de un diseñador italiano icónico.",
    "notasSalida": "Chabacano, Fresia, Melocotón, Grosella negra",
    "notasCorazon": "Violeta, Nenúfar, Ylang-ylang, Lirio de los valles",
    "notasFondo": "Rosa, Sándalo, Almizcle, Vainilla",
    "notes": [
      "Chabacano",
      "Fresia",
      "Melocotón",
      "Grosella negra",
      "Violeta",
      "Nenúfar",
      "Ylang-ylang",
      "Lirio de los valles",
      "Rosa",
      "Sándalo",
      "Almizcle",
      "Vainilla"
    ],
    "ml": 75,
    "precioUSD": 30,
    "categoria": "disenador"
  },
  {
    "id": 247,
    "house": "YSL",
    "name": "Yves Saint Laurent La Nuit de l'Homme",
    "image": "yves-saint-laurent-la-nuit-de-lhomme-100ml-m.webp",
    "familia": "Amaderado",
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": "Un perfume para el hombre elegante y seductor, de estilo amaderado especiado con un aura nocturna sofisticada. Su carácter recuerda al ADN de un gran diseñador, con una salida vibrante de cardamomo y un fondo cálido y masculino que lo ha convertido en un clásico moderno.",
    "description": "Un perfume para el hombre elegante y seductor, de estilo amaderado especiado con un aura nocturna sofisticada. Su carácter recuerda al ADN de un gran diseñador, con una salida vibrante de cardamomo y un fondo cálido y masculino que lo ha convertido en un clásico moderno.",
    "notasSalida": "Cardamomo, Lavanda",
    "notasCorazon": "Cedro de Virginia, Bergamota",
    "notasFondo": "Vetiver, Alcaravea",
    "notes": [
      "Cardamomo",
      "Lavanda",
      "Cedro de Virginia",
      "Bergamota",
      "Vetiver",
      "Alcaravea"
    ],
    "ml": 100,
    "precioUSD": 150,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 248,
    "house": "ZIMAYA",
    "name": "Zimaya Tiramisu Caramel",
    "image": "zimaya-tiramisu-caramel-100ml-u.webp",
    "familia": "Amaderado",
    "tipo": "Eau de Parfum",
    "genero": "Unisex",
    "descripcion": "Tiramisu Caramel de Zimaya es una fragancia unisex gourmand y envolvente, con un perfil dulce, cremoso y cálido que resulta irresistible para quienes aman los aromas opulentos. Su carácter recuerda a un postre de lujo, con una estela moderna e intensa inspirada en la faceta más golosa de la perfumería nicho y oriental.",
    "description": "Tiramisu Caramel de Zimaya es una fragancia unisex gourmand y envolvente, con un perfil dulce, cremoso y cálido que resulta irresistible para quienes aman los aromas opulentos. Su carácter recuerda a un postre de lujo, con una estela moderna e intensa inspirada en la faceta más golosa de la perfumería nicho y oriental.",
    "notasSalida": "Caramelo, Vainilla",
    "notasCorazon": "Almizcle, Miel",
    "notasFondo": "Whisky",
    "notes": [
      "Caramelo",
      "Vainilla",
      "Almizcle",
      "Miel",
      "Whisky"
    ],
    "ml": 100,
    "precioUSD": 45,
    "categoria": "arabes"
  },
  {
    "id": 249,
    "house": "PACO RABANNE",
    "name": "Paco Rabanne Paco",
    "image": "paco-rabanne-paco-100ml-m.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Mandarina, Lima, Bergamota",
    "notasCorazon": "Lavanda, Cilantro, Rosa, Geranio",
    "notasFondo": "Pachulí, Almizcle, Musgo de roble, Cedro",
    "notes": [
      "Mandarina",
      "Lima",
      "Bergamota",
      "Lavanda",
      "Cilantro",
      "Rosa",
      "Geranio",
      "Pachulí",
      "Almizcle",
      "Musgo de roble",
      "Cedro"
    ],
    "ml": 100,
    "precioUSD": 45,
    "categoria": "disenador"
  },
  {
    "id": 250,
    "house": "CREED",
    "name": "Creed Aventus",
    "image": "creed-aventus-creed-100ml-m.webp",
    "familia": null,
    "tipo": "Eau de Parfum",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Piña, Bergamota, Manzana verde, Grosella negra",
    "notasCorazon": "Abedul, Jazmín, Rosa, Pachulí",
    "notasFondo": "Almizcle, Ámbar, Musgo de roble, Vetiver",
    "notes": [
      "Piña",
      "Bergamota",
      "Manzana verde",
      "Grosella negra",
      "Abedul",
      "Jazmín",
      "Rosa",
      "Pachulí",
      "Almizcle",
      "Ámbar",
      "Musgo de roble",
      "Vetiver"
    ],
    "ml": 100,
    "precioUSD": 480,
    "categoria": "nicho"
  },
  {
    "id": 251,
    "house": "MOSCHINO",
    "name": "Moschino Toy Boy",
    "image": "moschino-toy-boy-100ml-m.webp",
    "familia": null,
    "tipo": "Eau de Parfum",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Cardamomo, Bergamota, Pomelo",
    "notasCorazon": "Rosa, Davana, Iris",
    "notasFondo": "Cachemira, Almizcle blanco, Ámbar",
    "notes": [
      "Cardamomo",
      "Bergamota",
      "Pomelo",
      "Rosa",
      "Davana",
      "Iris",
      "Cachemira",
      "Almizcle blanco",
      "Ámbar"
    ],
    "ml": 100,
    "precioUSD": 70,
    "categoria": "disenador"
  },
  {
    "id": 252,
    "house": "DOLCE & GABBANA",
    "name": "Dolce & Gabbana The One Gold Eau de Parfum Intense",
    "image": "dolcegabbana-the-one-gold-eau-de-parfum-intense-spray-50ml-m.webp",
    "familia": null,
    "tipo": "Eau de Parfum",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Bergamota, Pomelo, Albahaca",
    "notasCorazon": "Cardamomo, Jengibre, Iris",
    "notasFondo": "Tabaco, Ámbar, Cedro, Vetiver",
    "notes": [
      "Bergamota",
      "Pomelo",
      "Albahaca",
      "Cardamomo",
      "Jengibre",
      "Iris",
      "Tabaco",
      "Ámbar",
      "Cedro",
      "Vetiver"
    ],
    "ml": 50,
    "precioUSD": 145,
    "categoria": "disenador"
  },
  {
    "id": 253,
    "house": "DOLCE & GABBANA",
    "name": "Dolce & Gabbana K by Dolce & Gabbana",
    "image": "dolce--gabbana-k-by-dolce--gabbana-for-men-100ml-m.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Cardamomo, Lavanda, Bergamota",
    "notasCorazon": "Vetiver, Geranio, Iris",
    "notasFondo": "Sándalo, Almizcle, Ámbar gris",
    "notes": [
      "Cardamomo",
      "Lavanda",
      "Bergamota",
      "Vetiver",
      "Geranio",
      "Iris",
      "Sándalo",
      "Almizcle",
      "Ámbar gris"
    ],
    "ml": 100,
    "precioUSD": 110,
    "categoria": "disenador"
  },
  {
    "id": 254,
    "house": "DOLCE & GABBANA",
    "name": "Dolce & Gabbana K Eau de Parfum",
    "image": "dolce--gabbana-k-by-dolce--gabbana-for-men-eau-de-parfum-100ml-m.webp",
    "familia": null,
    "tipo": "Eau de Parfum",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Cardamomo, Lavanda, Bergamota",
    "notasCorazon": "Vetiver, Geranio, Iris",
    "notasFondo": "Oud, Sándalo, Ámbar",
    "notes": [
      "Cardamomo",
      "Lavanda",
      "Bergamota",
      "Vetiver",
      "Geranio",
      "Iris",
      "Oud",
      "Sándalo",
      "Ámbar"
    ],
    "ml": 100,
    "precioUSD": 110,
    "categoria": "disenador"
  },
  {
    "id": 255,
    "house": "VERSACE",
    "name": "Versace Eros Parfum",
    "image": "versace-eros-parfum-100ml-m.webp",
    "familia": null,
    "tipo": "Parfum",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Menta, Manzana verde, Limón",
    "notasCorazon": "Flor de acacia, Cardamomo",
    "notasFondo": "Vetiver, Ámbar, Cedro, Almizcle",
    "notes": [
      "Menta",
      "Manzana verde",
      "Limón",
      "Flor de acacia",
      "Cardamomo",
      "Vetiver",
      "Ámbar",
      "Cedro",
      "Almizcle"
    ],
    "ml": 100,
    "precioUSD": 100,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 256,
    "house": "VERSACE",
    "name": "Versace Eros Eau de Parfum",
    "image": "versace-eros-eau-de-parfum-100ml-m.webp",
    "familia": null,
    "tipo": "Eau de Parfum",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Menta, Manzana verde, Limón",
    "notasCorazon": "Flor de acacia, Geranio, Cardamomo",
    "notasFondo": "Vetiver, Haba tonka, Ámbar, Almizcle",
    "notes": [
      "Menta",
      "Manzana verde",
      "Limón",
      "Flor de acacia",
      "Geranio",
      "Cardamomo",
      "Vetiver",
      "Haba tonka",
      "Ámbar",
      "Almizcle"
    ],
    "ml": 100,
    "precioUSD": 100,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 257,
    "house": "PRADA",
    "name": "Prada Paradoxe Intense",
    "image": "prada-paradoxe-intense-90ml-f.webp",
    "familia": null,
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Bergamota, Neroli, Ámbar gris",
    "notasCorazon": "Jazmín sambac, Pachulí, Ylang ylang",
    "notasFondo": "Almizcle blanco, Sándalo, Vainilla",
    "notes": [
      "Bergamota",
      "Neroli",
      "Ámbar gris",
      "Jazmín sambac",
      "Pachulí",
      "Ylang ylang",
      "Almizcle blanco",
      "Sándalo",
      "Vainilla"
    ],
    "ml": 90,
    "precioUSD": 230,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 258,
    "house": "MONTALE",
    "name": "Montale Arabians Tonka",
    "image": "montale-paris-arabians-tonka-100ml-u.webp",
    "familia": null,
    "tipo": "Eau de Parfum",
    "genero": "Unisex",
    "descripcion": null,
    "description": null,
    "notasSalida": "Bergamota, Canela",
    "notasCorazon": "Rosa turca, Neroli, Haba tonka",
    "notasFondo": "Oud, Sándalo, Almizcle blanco",
    "notes": [
      "Bergamota",
      "Canela",
      "Rosa turca",
      "Neroli",
      "Haba tonka",
      "Oud",
      "Sándalo",
      "Almizcle blanco"
    ],
    "ml": 100,
    "precioUSD": 180,
    "categoria": "nicho"
  },
  {
    "id": 259,
    "house": "DIOR",
    "name": "Dior Homme Intense Eau de Parfum",
    "image": "christian-dior-homme-intense-eau-de-parfum-100ml-m.webp",
    "familia": null,
    "tipo": "Eau de Parfum",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Lavanda, Iris, Bergamota",
    "notasCorazon": "Iris, Cedro, Cuero",
    "notasFondo": "Vetiver, Ámbar, Almizcle",
    "notes": [
      "Lavanda",
      "Iris",
      "Bergamota",
      "Iris",
      "Cedro",
      "Cuero",
      "Vetiver",
      "Ámbar",
      "Almizcle"
    ],
    "ml": 100,
    "precioUSD": 220,
    "categoria": "disenador"
  },
  {
    "id": 260,
    "house": "ARIANA GRANDE",
    "name": "Ariana Grande Cloud",
    "image": "ariana-grande-cloud-ariana-grande-100ml-f.webp",
    "familia": null,
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Lavanda, Bergamota, Coco",
    "notasCorazon": "Jazmín, Orquídea, Musgo",
    "notasFondo": "Almizcle, Vainilla, Cachemira",
    "notes": [
      "Lavanda",
      "Bergamota",
      "Coco",
      "Jazmín",
      "Orquídea",
      "Musgo",
      "Almizcle",
      "Vainilla",
      "Cachemira"
    ],
    "ml": 100,
    "precioUSD": 75,
    "categoria": "disenador"
  },
  {
    "id": 261,
    "house": "ARIANA GRANDE",
    "name": "Ariana Grande Cloud Intense",
    "image": "ariana-grande-cloud-intense-ariana-grande-100ml-f.webp",
    "familia": null,
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Bergamota, Pera",
    "notasCorazon": "Iris, Jazmín, Orquídea",
    "notasFondo": "Almizcle, Vainilla, Sándalo",
    "notes": [
      "Bergamota",
      "Pera",
      "Iris",
      "Jazmín",
      "Orquídea",
      "Almizcle",
      "Vainilla",
      "Sándalo"
    ],
    "ml": 100,
    "precioUSD": 75,
    "categoria": "disenador"
  },
  {
    "id": 262,
    "house": "ARIANA GRANDE",
    "name": "Ariana Grande Thank U Next",
    "image": "ariana-grande-thank-u-next-ariana-grande-100ml-f.webp",
    "familia": null,
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Pera, Frambuesa",
    "notasCorazon": "Rosa, Jazmín",
    "notasFondo": "Almizcle blanco, Coco, Vainilla",
    "notes": [
      "Pera",
      "Frambuesa",
      "Rosa",
      "Jazmín",
      "Almizcle blanco",
      "Coco",
      "Vainilla"
    ],
    "ml": 100,
    "precioUSD": 65,
    "categoria": "disenador"
  },
  {
    "id": 263,
    "house": "RALPH LAUREN",
    "name": "Ralph Lauren Polo Black",
    "image": "ralph-lauren-polo-black-ralph-lauren-125ml-m.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Mango, Osmanto, Limón",
    "notasCorazon": "Pachulí, Artemisa, Abedul",
    "notasFondo": "Ámbar, Almizcle, Madera de teca",
    "notes": [
      "Mango",
      "Osmanto",
      "Limón",
      "Pachulí",
      "Artemisa",
      "Abedul",
      "Ámbar",
      "Almizcle",
      "Madera de teca"
    ],
    "ml": 125,
    "precioUSD": 55,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 264,
    "house": "RALPH LAUREN",
    "name": "Ralph Lauren Polo 67",
    "image": "ralph-lauren-polo-67-ralph-lauren-100ml-m.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Bergamota, Neroli, Limón",
    "notasCorazon": "Lavanda, Rosa, Geranio",
    "notasFondo": "Cedro, Almizcle, Ámbar",
    "notes": [
      "Bergamota",
      "Neroli",
      "Limón",
      "Lavanda",
      "Rosa",
      "Geranio",
      "Cedro",
      "Almizcle",
      "Ámbar"
    ],
    "ml": 100,
    "precioUSD": 95,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 265,
    "house": "RALPH LAUREN",
    "name": "Ralph Lauren Big Pony 1",
    "image": "ralph-lauren-big-pony-1-ralph-lauren-100ml-m.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Bergamota, Pomelo, Lavanda",
    "notasCorazon": "Geranio, Rosa, Cedro",
    "notasFondo": "Almizcle, Ámbar, Cuero",
    "notes": [
      "Bergamota",
      "Pomelo",
      "Lavanda",
      "Geranio",
      "Rosa",
      "Cedro",
      "Almizcle",
      "Ámbar",
      "Cuero"
    ],
    "ml": 100,
    "precioUSD": 40,
    "categoria": "disenador"
  },
  {
    "id": 266,
    "house": "JEAN PAUL GAULTIER",
    "name": "Jean Paul Gaultier Scandal Pour Homme",
    "image": "jean-paul-gaultier-scandal-pour-homme-100ml-m.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Bergamota, Naranja sanguina, Pimienta",
    "notasCorazon": "Salvia, Neroli",
    "notasFondo": "Vetiver, Styrax, Tabaco",
    "notes": [
      "Bergamota",
      "Naranja sanguina",
      "Pimienta",
      "Salvia",
      "Neroli",
      "Vetiver",
      "Styrax",
      "Tabaco"
    ],
    "ml": 100,
    "precioUSD": 150,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 267,
    "house": "ARIANA GRANDE",
    "name": "Ariana Grande Ari",
    "image": "ariana-grande-ari-ariana-grande-100ml-f.webp",
    "familia": null,
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Pera, Pomelo, Cereza",
    "notasCorazon": "Lila, Jazmín, Nectarina",
    "notasFondo": "Almizcle, Madera, Vainilla",
    "notes": [
      "Pera",
      "Pomelo",
      "Cereza",
      "Lila",
      "Jazmín",
      "Nectarina",
      "Almizcle",
      "Madera",
      "Vainilla"
    ],
    "ml": 100,
    "precioUSD": 55,
    "categoria": "disenador"
  },
  {
    "id": 268,
    "house": "GIORGIO ARMANI",
    "name": "Giorgio Armani My Way Ylang",
    "image": "giorgio-armani-my-way-ylang-90ml-f.webp",
    "familia": null,
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Bergamota, Ylang ylang, Neroli",
    "notasCorazon": "Ylang ylang, Rosa, Jazmín",
    "notasFondo": "Cedro blanco, Vainilla, Almizcle",
    "notes": [
      "Bergamota",
      "Ylang ylang",
      "Neroli",
      "Ylang ylang",
      "Rosa",
      "Jazmín",
      "Cedro blanco",
      "Vainilla",
      "Almizcle"
    ],
    "ml": 90,
    "precioUSD": 180,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 269,
    "house": "VALENTINO",
    "name": "Valentino Donna Born in Roma Coral Fantasy",
    "image": "valentino-donna-born-in-roma-coral-fantasy-100ml-f.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Femenino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Bergamota, Neroli, Maracuyá",
    "notasCorazon": "Rosa, Iris, Flor de naranjo",
    "notasFondo": "Vainilla, Ámbar, Sándalo",
    "notes": [
      "Bergamota",
      "Neroli",
      "Maracuyá",
      "Rosa",
      "Iris",
      "Flor de naranjo",
      "Vainilla",
      "Ámbar",
      "Sándalo"
    ],
    "ml": 100,
    "precioUSD": 190,
    "categoria": "disenador"
  },
  {
    "id": 270,
    "house": "RABANNE",
    "name": "Rabanne Fame Parfum",
    "image": "rabanne-fame-parfum-rabanne-80ml-f.webp",
    "familia": null,
    "tipo": "Parfum",
    "genero": "Femenino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Bergamota, Mandarina, Jazmín",
    "notasCorazon": "Jazmín, Rosa, Orquídea",
    "notasFondo": "Almizcle, Vetiver, Cedro",
    "notes": [
      "Bergamota",
      "Mandarina",
      "Jazmín",
      "Jazmín",
      "Rosa",
      "Orquídea",
      "Almizcle",
      "Vetiver",
      "Cedro"
    ],
    "ml": 80,
    "precioUSD": 180,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 271,
    "house": "CAROLINA HERRERA",
    "name": "Carolina Herrera CH Men Sport",
    "image": "carolina-herrera-ch-men-sport-100ml-m.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Bergamota, Pomelo, Jengibre",
    "notasCorazon": "Lavanda, Cedro, Salvia",
    "notasFondo": "Ámbar, Almizcle, Madera",
    "notes": [
      "Bergamota",
      "Pomelo",
      "Jengibre",
      "Lavanda",
      "Cedro",
      "Salvia",
      "Ámbar",
      "Almizcle",
      "Madera"
    ],
    "ml": 100,
    "precioUSD": 65,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 272,
    "house": "RABANNE",
    "name": "Rabanne Phantom Intense",
    "image": "rabanne-phantom-intense-100ml-m.webp",
    "familia": null,
    "tipo": "Eau de Parfum",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Limón, Lavanda, Bergamota",
    "notasCorazon": "Salvia, Lavanda, Roble",
    "notasFondo": "Vainilla, Vetiver, Almizcle",
    "notes": [
      "Limón",
      "Lavanda",
      "Bergamota",
      "Salvia",
      "Lavanda",
      "Roble",
      "Vainilla",
      "Vetiver",
      "Almizcle"
    ],
    "ml": 100,
    "precioUSD": 140,
    "categoria": "disenador"
  },
  {
    "id": 273,
    "house": "RABANNE",
    "name": "Rabanne Invictus Victory Absolu",
    "image": "rabanne-invictus-victory-absolu-100ml-m.webp",
    "familia": null,
    "tipo": "Parfum",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Pomelo, Mandarina",
    "notasCorazon": "Laurel, Jazmín",
    "notasFondo": "Madera de guayaco, Pachulí, Ámbar",
    "notes": [
      "Pomelo",
      "Mandarina",
      "Laurel",
      "Jazmín",
      "Madera de guayaco",
      "Pachulí",
      "Ámbar"
    ],
    "ml": 100,
    "precioUSD": 125,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 274,
    "house": "MARC JACOBS",
    "name": "Marc Jacobs Daisy",
    "image": "marc-jacobs-daisy-marc-jacobs-100ml-f.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Femenino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Fresa, Violeta, Pomelo",
    "notasCorazon": "Gardenia, Jazmín, Violeta",
    "notasFondo": "Almizcle, Vainilla, Madera blanca",
    "notes": [
      "Fresa",
      "Violeta",
      "Pomelo",
      "Gardenia",
      "Jazmín",
      "Violeta",
      "Almizcle",
      "Vainilla",
      "Madera blanca"
    ],
    "ml": 100,
    "precioUSD": 100,
    "categoria": "disenador"
  },
  {
    "id": 275,
    "house": "ARIANA GRANDE",
    "name": "Ariana Grande Sweet Like Candy",
    "image": "ariana-grande-sweet-like-candy-100ml-f.webp",
    "familia": null,
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Pera, Frambuesa, Bergamota",
    "notasCorazon": "Algodón de azúcar, Flor de saúco",
    "notasFondo": "Vainilla, Almizcle, Madera",
    "notes": [
      "Pera",
      "Frambuesa",
      "Bergamota",
      "Algodón de azúcar",
      "Flor de saúco",
      "Vainilla",
      "Almizcle",
      "Madera"
    ],
    "ml": 100,
    "precioUSD": 55,
    "categoria": "disenador"
  },
  {
    "id": 276,
    "house": "VIKTOR&ROLF",
    "name": "Viktor&Rolf Spicebomb Extreme",
    "image": "viktorrolf-spicebomb-extreme-90ml-m.webp",
    "familia": null,
    "tipo": "Eau de Parfum",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Pomelo, Lavanda",
    "notasCorazon": "Tabaco, Vainilla, Vetiver",
    "notasFondo": "Sándalo, Almizcle, Incienso",
    "notes": [
      "Pomelo",
      "Lavanda",
      "Tabaco",
      "Vainilla",
      "Vetiver",
      "Sándalo",
      "Almizcle",
      "Incienso"
    ],
    "ml": 90,
    "precioUSD": 125,
    "categoria": "disenador"
  },
  {
    "id": 277,
    "house": "VIKTOR&ROLF",
    "name": "Viktor&Rolf Flowerbomb Nectar",
    "image": "viktorrolf-flowerbomb-nectar-90ml-f.webp",
    "familia": null,
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Jazmín, Rosa, Bergamota",
    "notasCorazon": "Rosa centifolia, Jazmín, Orquídea",
    "notasFondo": "Pachulí, Almizcle, Ámbar",
    "notes": [
      "Jazmín",
      "Rosa",
      "Bergamota",
      "Rosa centifolia",
      "Jazmín",
      "Orquídea",
      "Pachulí",
      "Almizcle",
      "Ámbar"
    ],
    "ml": 90,
    "precioUSD": 155,
    "categoria": "disenador"
  },
  {
    "id": 278,
    "house": "GIVENCHY",
    "name": "Givenchy Ange ou Démon",
    "image": "givenchy-ange-ou-demon-100ml-f.webp",
    "familia": null,
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Mandarina, Bergamota, Flor de lirio",
    "notasCorazon": "Iris, Ylang ylang, Magnolia",
    "notasFondo": "Vetiver, Sándalo, Ámbar",
    "notes": [
      "Mandarina",
      "Bergamota",
      "Flor de lirio",
      "Iris",
      "Ylang ylang",
      "Magnolia",
      "Vetiver",
      "Sándalo",
      "Ámbar"
    ],
    "ml": 100,
    "precioUSD": 120,
    "categoria": "disenador"
  },
  {
    "id": 279,
    "house": "CAROLINA HERRERA",
    "name": "Carolina Herrera 212 VIP Black Elixir",
    "image": "carolina-herrera-212-vip-black-elixir-100ml-m.webp",
    "familia": null,
    "tipo": "Elixir",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Pera, Bergamota, Cardamomo",
    "notasCorazon": "Vainilla negra, Ámbar, Pachulí",
    "notasFondo": "Sándalo, Almizcle, Cedro",
    "notes": [
      "Pera",
      "Bergamota",
      "Cardamomo",
      "Vainilla negra",
      "Ámbar",
      "Pachulí",
      "Sándalo",
      "Almizcle",
      "Cedro"
    ],
    "ml": 100,
    "precioUSD": 180,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 280,
    "house": "RABANNE",
    "name": "Rabanne 1 Million Gold Elixir",
    "image": "rabanne-million-gold-elixir-100ml-m.webp",
    "familia": null,
    "tipo": "Elixir",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Mandarina sanguina, Pomelo, Menta",
    "notasCorazon": "Rosa, Canela, Cuero",
    "notasFondo": "Pachulí, Ámbar dorado, Madera",
    "notes": [
      "Mandarina sanguina",
      "Pomelo",
      "Menta",
      "Rosa",
      "Canela",
      "Cuero",
      "Pachulí",
      "Ámbar dorado",
      "Madera"
    ],
    "ml": 100,
    "precioUSD": 150,
    "categoria": "disenador"
  },
  {
    "id": 281,
    "house": "CAROLINA HERRERA",
    "name": "Carolina Herrera Chic",
    "image": "carolina-herrera-chic-carolina-herrera-100ml-f.webp",
    "familia": null,
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Pomelo, Bergamota, Durazno",
    "notasCorazon": "Rosa, Jazmín, Magnolia",
    "notasFondo": "Almizcle, Cedro, Ámbar",
    "notes": [
      "Pomelo",
      "Bergamota",
      "Durazno",
      "Rosa",
      "Jazmín",
      "Magnolia",
      "Almizcle",
      "Cedro",
      "Ámbar"
    ],
    "ml": 100,
    "precioUSD": 70,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 282,
    "house": "RABANNE",
    "name": "Rabanne Black XS for Her",
    "image": "rabanne-black-xs-for-her-80ml-f.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Femenino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Anís estrellado, Jazmín",
    "notasCorazon": "Rosa negra, Orquídea",
    "notasFondo": "Pachulí, Cuero, Almizcle",
    "notes": [
      "Anís estrellado",
      "Jazmín",
      "Rosa negra",
      "Orquídea",
      "Pachulí",
      "Cuero",
      "Almizcle"
    ],
    "ml": 80,
    "precioUSD": 85,
    "categoria": "disenador"
  },
  {
    "id": 283,
    "house": "YVES SAINT LAURENT",
    "name": "Yves Saint Laurent Libre L'Eau Nue",
    "image": "yves-saint-laurent-libre-leau-nue-50ml-f.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Femenino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Bergamota, Naranja amarga, Lavanda",
    "notasCorazon": "Flor de naranjo, Jazmín, Iris",
    "notasFondo": "Almizcle blanco, Cedro, Ámbar",
    "notes": [
      "Bergamota",
      "Naranja amarga",
      "Lavanda",
      "Flor de naranjo",
      "Jazmín",
      "Iris",
      "Almizcle blanco",
      "Cedro",
      "Ámbar"
    ],
    "ml": 50,
    "precioUSD": 230,
    "categoria": "disenador"
  },
  {
    "id": 284,
    "house": "LACOSTE",
    "name": "Lacoste Eau de Lacoste L.12.12 White",
    "image": "lacoste-fragrances-eau-de-lacoste-l1212-white-100ml-m.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Bergamota, Naranja, Limón",
    "notasCorazon": "Cardamomo, Geranio, Cedro",
    "notasFondo": "Almizcle blanco, Ámbar, Madera",
    "notes": [
      "Bergamota",
      "Naranja",
      "Limón",
      "Cardamomo",
      "Geranio",
      "Cedro",
      "Almizcle blanco",
      "Ámbar",
      "Madera"
    ],
    "ml": 100,
    "precioUSD": 70,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 285,
    "house": "VERSACE",
    "name": "Versace Eros Flame",
    "image": "versace-eros-flame-100ml-m.webp",
    "familia": null,
    "tipo": "Eau de Parfum",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Mandarina, Pomelo, Pimienta negra",
    "notasCorazon": "Salvia, Geranio, Pimienta rosa",
    "notasFondo": "Cedro, Almizcle, Ámbar",
    "notes": [
      "Mandarina",
      "Pomelo",
      "Pimienta negra",
      "Salvia",
      "Geranio",
      "Pimienta rosa",
      "Cedro",
      "Almizcle",
      "Ámbar"
    ],
    "ml": 100,
    "precioUSD": 135,
    "categoria": "disenador"
  },
  {
    "id": 286,
    "house": "LE LABO",
    "name": "Le Labo Santal 33",
    "image": "le-labo-santal-33-50ml-u.webp",
    "familia": null,
    "tipo": "Eau de Parfum",
    "genero": "Unisex",
    "descripcion": null,
    "description": null,
    "notasSalida": "Cardamomo, Iris, Violeta",
    "notasCorazon": "Sándalo, Ámbar gris, Cedro",
    "notasFondo": "Cuero, Almizcle, Papiro",
    "notes": [
      "Cardamomo",
      "Iris",
      "Violeta",
      "Sándalo",
      "Ámbar gris",
      "Cedro",
      "Cuero",
      "Almizcle",
      "Papiro"
    ],
    "ml": 50,
    "precioUSD": 580,
    "categoria": "nicho"
  },
  {
    "id": 287,
    "house": "PACO RABANNE",
    "name": "Paco Rabanne Paco XS",
    "image": "paco-rabanne-paco-xs-100ml-m.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Bergamota, Lavanda, Menta",
    "notasCorazon": "Geranio, Rosa, Lirio",
    "notasFondo": "Almizcle, Cedro, Vetiver",
    "notes": [
      "Bergamota",
      "Lavanda",
      "Menta",
      "Geranio",
      "Rosa",
      "Lirio",
      "Almizcle",
      "Cedro",
      "Vetiver"
    ],
    "ml": 100,
    "precioUSD": 85,
    "categoria": "disenador"
  },
  {
    "id": 288,
    "house": "RABANNE",
    "name": "Rabanne Black XS",
    "image": "rabanne-black-xs-100ml-m.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Anís estrellado, Bergamota, Pimienta",
    "notasCorazon": "Cuero, Pachulí, Roble",
    "notasFondo": "Ámbar, Almizcle, Madera",
    "notes": [
      "Anís estrellado",
      "Bergamota",
      "Pimienta",
      "Cuero",
      "Pachulí",
      "Roble",
      "Ámbar",
      "Almizcle",
      "Madera"
    ],
    "ml": 100,
    "precioUSD": 105,
    "categoria": "disenador"
  },
  {
    "id": 289,
    "house": "XERJOFF",
    "name": "Xerjoff Erba Pura",
    "image": "xerjoff-erba-pura-100ml-u.webp",
    "familia": null,
    "tipo": "Eau de Parfum",
    "genero": "Unisex",
    "descripcion": null,
    "description": null,
    "notasSalida": "Naranja siciliana, Pomelo, Bergamota",
    "notasCorazon": "Jazmín, Rosa, Aldehídos",
    "notasFondo": "Almizcle blanco, Cedro, Ámbar",
    "notes": [
      "Naranja siciliana",
      "Pomelo",
      "Bergamota",
      "Jazmín",
      "Rosa",
      "Aldehídos",
      "Almizcle blanco",
      "Cedro",
      "Ámbar"
    ],
    "ml": 100,
    "precioUSD": 320,
    "categoria": "nicho",
    "agotado": true
  },
  {
    "id": 290,
    "house": "TOM FORD",
    "name": "Tom Ford Velvet Orchid",
    "image": "tom-ford-velvet-orchid-50ml-f.webp",
    "familia": null,
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Bergamota, Aldehídos, Flor de naranjo",
    "notasCorazon": "Orquídea, Rosa, Jazmín",
    "notasFondo": "Ámbar, Almizcle, Oud",
    "notes": [
      "Bergamota",
      "Aldehídos",
      "Flor de naranjo",
      "Orquídea",
      "Rosa",
      "Jazmín",
      "Ámbar",
      "Almizcle",
      "Oud"
    ],
    "ml": 50,
    "precioUSD": 270,
    "categoria": "disenador"
  },
  {
    "id": 291,
    "house": "AZZARO",
    "name": "Azzaro The Most Wanted Intense",
    "image": "azzaro-the-most-wanted-intense-100ml-m.webp",
    "familia": null,
    "tipo": "Eau de Parfum",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Cardamomo, Lavanda, Bergamota",
    "notasCorazon": "Tabaco, Haba tonka, Pimienta negra",
    "notasFondo": "Cedro, Ámbar, Vainilla",
    "notes": [
      "Cardamomo",
      "Lavanda",
      "Bergamota",
      "Tabaco",
      "Haba tonka",
      "Pimienta negra",
      "Cedro",
      "Ámbar",
      "Vainilla"
    ],
    "ml": 100,
    "precioUSD": 110,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 292,
    "house": "ARIANA GRANDE",
    "name": "Ariana Grande Cloud Pink",
    "image": "ariana-grande-cloud-pink-100ml-f.webp",
    "familia": null,
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Fresa, Mandarina, Flor de cerezo",
    "notasCorazon": "Orquídea, Rosa, Jazmín",
    "notasFondo": "Almizcle, Vainilla, Cachemira",
    "notes": [
      "Fresa",
      "Mandarina",
      "Flor de cerezo",
      "Orquídea",
      "Rosa",
      "Jazmín",
      "Almizcle",
      "Vainilla",
      "Cachemira"
    ],
    "ml": 100,
    "precioUSD": 75,
    "categoria": "disenador"
  },
  {
    "id": 293,
    "house": "ARIANA GRANDE",
    "name": "Ariana Grande Thank U Next 2.0",
    "image": "ariana-grande-thank-u-next-20-100ml-f.webp",
    "familia": null,
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Pera, Frambuesa, Bergamota",
    "notasCorazon": "Rosa, Jazmín, Orquídea",
    "notasFondo": "Almizcle blanco, Vainilla, Cachemira",
    "notes": [
      "Pera",
      "Frambuesa",
      "Bergamota",
      "Rosa",
      "Jazmín",
      "Orquídea",
      "Almizcle blanco",
      "Vainilla",
      "Cachemira"
    ],
    "ml": 100,
    "precioUSD": 75,
    "categoria": "disenador"
  },
  {
    "id": 293,
    "house": "ARIANA GRANDE",
    "name": "Ariana Grande Thank U Next 2.0",
    "image": "ariana-grande-thank-u-next-20-100ml-f.webp",
    "familia": null,
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Pera, Frambuesa, Bergamota",
    "notasCorazon": "Rosa, Jazmín, Orquídea",
    "notasFondo": "Almizcle blanco, Vainilla, Cachemira",
    "notes": [
      "Pera",
      "Frambuesa",
      "Bergamota",
      "Rosa",
      "Jazmín",
      "Orquídea",
      "Almizcle blanco",
      "Vainilla",
      "Cachemira"
    ],
    "ml": 100,
    "precioUSD": 65,
    "categoria": "disenador"
  },
  {
    "id": 294,
    "house": "RALPH LAUREN",
    "name": "Ralph Lauren Polo Red Eau de Parfum",
    "image": "ralph-lauren-polo-red-eau-de-parfum-125ml-m.webp",
    "familia": null,
    "tipo": "Eau de Parfum",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Mandarina, Pomelo, Canela",
    "notasCorazon": "Salvia, Geranio, Rosa",
    "notasFondo": "Cedro, Ámbar, Sándalo",
    "notes": [
      "Mandarina",
      "Pomelo",
      "Canela",
      "Salvia",
      "Geranio",
      "Rosa",
      "Cedro",
      "Ámbar",
      "Sándalo"
    ],
    "ml": 125,
    "precioUSD": 60,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 295,
    "house": "RALPH LAUREN",
    "name": "Ralph Lauren Polo Sport",
    "image": "ralph-lauren-polo-sport-125ml-m.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Bergamota, Pomelo, Menta",
    "notasCorazon": "Lavanda, Geranio, Cedro",
    "notasFondo": "Almizcle, Ámbar, Madera",
    "notes": [
      "Bergamota",
      "Pomelo",
      "Menta",
      "Lavanda",
      "Geranio",
      "Cedro",
      "Almizcle",
      "Ámbar",
      "Madera"
    ],
    "ml": 125,
    "precioUSD": 60,
    "categoria": "disenador"
  },
  {
    "id": 296,
    "house": "RALPH LAUREN",
    "name": "Ralph Lauren Ralph's Club",
    "image": "ralph-lauren-ralphs-club-100ml-m.webp",
    "familia": null,
    "tipo": "Eau de Parfum",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Bergamota, Limón, Cardamomo",
    "notasCorazon": "Iris, Rosa, Vetiver",
    "notasFondo": "Oud, Ámbar, Almizcle",
    "notes": [
      "Bergamota",
      "Limón",
      "Cardamomo",
      "Iris",
      "Rosa",
      "Vetiver",
      "Oud",
      "Ámbar",
      "Almizcle"
    ],
    "ml": 100,
    "precioUSD": 85,
    "categoria": "disenador"
  },
  {
    "id": 297,
    "house": "JEAN PAUL GAULTIER",
    "name": "Jean Paul Gaultier Scandal Pour Homme Intense",
    "image": "jean-paul-gaultier-scandal-pour-homme-intense-100ml-m.webp",
    "familia": null,
    "tipo": "Eau de Parfum",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Naranja sanguina, Bergamota, Pimienta",
    "notasCorazon": "Salvia, Neroli, Labdano",
    "notasFondo": "Vetiver, Styrax, Tabaco",
    "notes": [
      "Naranja sanguina",
      "Bergamota",
      "Pimienta",
      "Salvia",
      "Neroli",
      "Labdano",
      "Vetiver",
      "Styrax",
      "Tabaco"
    ],
    "ml": 100,
    "precioUSD": 180,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 298,
    "house": "JEAN PAUL GAULTIER",
    "name": "Jean Paul Gaultier Le Beau Paradise Garden",
    "image": "jean-paul-gaultier-le-beau-paradise-garden-125ml-m.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Pera, Kiwi, Pomelo",
    "notasCorazon": "Flor de coco, Nenúfar",
    "notasFondo": "Cedro, Almizcle, Ámbar",
    "notes": [
      "Pera",
      "Kiwi",
      "Pomelo",
      "Flor de coco",
      "Nenúfar",
      "Cedro",
      "Almizcle",
      "Ámbar"
    ],
    "ml": 125,
    "precioUSD": 185,
    "categoria": "disenador"
  },
  {
    "id": 299,
    "house": "DOLCE & GABBANA",
    "name": "Dolce & Gabbana The One Pour Homme Eau de Parfum",
    "image": "dolcegabbana-the-one-pour-homme-eau-de-parfum-100ml-m.webp",
    "familia": null,
    "tipo": "Eau de Parfum",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Cardamomo, Bergamota, Pomelo",
    "notasCorazon": "Jengibre, Albahaca, Geranio",
    "notasFondo": "Tabaco, Cedro, Ámbar, Almizcle",
    "notes": [
      "Cardamomo",
      "Bergamota",
      "Pomelo",
      "Jengibre",
      "Albahaca",
      "Geranio",
      "Tabaco",
      "Cedro",
      "Ámbar",
      "Almizcle"
    ],
    "ml": 100,
    "precioUSD": 95,
    "categoria": "disenador"
  },
  {
    "id": 300,
    "house": "DOLCE & GABBANA",
    "name": "Dolce & Gabbana Light Blue Eau Intense Pour Homme",
    "image": "dolcegabbana-light-blue-eau-intense-pour-homme-100ml-m.webp",
    "familia": null,
    "tipo": "Eau de Parfum",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Manzana de Sicilia, Mandarina, Bergamota",
    "notasCorazon": "Enebro, Romero, Cachemira",
    "notasFondo": "Almizcle, Ámbar, Cedro",
    "notes": [
      "Manzana de Sicilia",
      "Mandarina",
      "Bergamota",
      "Enebro",
      "Romero",
      "Cachemira",
      "Almizcle",
      "Ámbar",
      "Cedro"
    ],
    "ml": 100,
    "precioUSD": 125,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 301,
    "house": "CAROLINA HERRERA",
    "name": "Carolina Herrera CH Men",
    "image": "carolina-herrera-ch-men-100ml-m.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Bergamota, Lavanda, Limón",
    "notasCorazon": "Cedro, Sándalo, Cuero",
    "notasFondo": "Vetiver, Almizcle, Ámbar",
    "notes": [
      "Bergamota",
      "Lavanda",
      "Limón",
      "Cedro",
      "Sándalo",
      "Cuero",
      "Vetiver",
      "Almizcle",
      "Ámbar"
    ],
    "ml": 100,
    "precioUSD": 130,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 302,
    "house": "RABANNE",
    "name": "Rabanne Phantom",
    "image": "rabanne-phantom-rabanne-100ml-m.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Limón, Lavanda, Bergamota",
    "notasCorazon": "Salvia, Lavanda, Roble",
    "notasFondo": "Vainilla, Vetiver, Almizcle",
    "notes": [
      "Limón",
      "Lavanda",
      "Bergamota",
      "Salvia",
      "Lavanda",
      "Roble",
      "Vainilla",
      "Vetiver",
      "Almizcle"
    ],
    "ml": 100,
    "precioUSD": 125,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 303,
    "house": "RABANNE",
    "name": "Rabanne Invictus Victory Elixir",
    "image": "rabanne-invictus-victory-elixir-100ml-m.webp",
    "familia": null,
    "tipo": "Elixir",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Pomelo, Mandarina",
    "notasCorazon": "Laurel, Jazmín, Incienso",
    "notasFondo": "Madera de guayaco, Pachulí, Ámbar intenso",
    "notes": [
      "Pomelo",
      "Mandarina",
      "Laurel",
      "Jazmín",
      "Incienso",
      "Madera de guayaco",
      "Pachulí",
      "Ámbar intenso"
    ],
    "ml": 100,
    "precioUSD": 150,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 304,
    "house": "RABANNE",
    "name": "Rabanne Invictus Victory",
    "image": "rabanne-invictus-victory-100ml-m.webp",
    "familia": null,
    "tipo": "Eau de Parfum",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Pomelo, Mandarina",
    "notasCorazon": "Laurel, Neroli",
    "notasFondo": "Madera de guayaco, Almizcle, Ámbar",
    "notes": [
      "Pomelo",
      "Mandarina",
      "Laurel",
      "Neroli",
      "Madera de guayaco",
      "Almizcle",
      "Ámbar"
    ],
    "ml": 100,
    "precioUSD": 125,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 305,
    "house": "CAROLINA HERRERA",
    "name": "Carolina Herrera CH Men Hot Hot Hot",
    "image": "carolina-herrera-ch-men-hot-hot-hot-100ml-m.webp",
    "familia": null,
    "tipo": "Eau de Parfum",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Pimienta, Naranja, Limón",
    "notasCorazon": "Salvia, Cardamomo, Cedro",
    "notasFondo": "Ámbar, Almizcle, Cuero",
    "notes": [
      "Pimienta",
      "Naranja",
      "Limón",
      "Salvia",
      "Cardamomo",
      "Cedro",
      "Ámbar",
      "Almizcle",
      "Cuero"
    ],
    "ml": 100,
    "precioUSD": 160,
    "categoria": "disenador"
  },
  {
    "id": 306,
    "house": "MARC JACOBS",
    "name": "Marc Jacobs Perfect Eau de Toilette",
    "image": "marc-jacobs-perfect-eau-de-toilette-100ml-f.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Femenino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Almendra, Bergamota, Iris",
    "notasCorazon": "Guisante de olor, Rosa, Iris",
    "notasFondo": "Sándalo, Almizcle, Cedro",
    "notes": [
      "Almendra",
      "Bergamota",
      "Iris",
      "Guisante de olor",
      "Rosa",
      "Iris",
      "Sándalo",
      "Almizcle",
      "Cedro"
    ],
    "ml": 100,
    "precioUSD": 110,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 307,
    "house": "ARIANA GRANDE",
    "name": "Ariana Grande Mod Blush",
    "image": "ariana-grande-mod-blush-100ml-f.webp",
    "familia": null,
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Ruibarbo, Pera, Frambuesa",
    "notasCorazon": "Flor de cerezo, Rosa, Jazmín",
    "notasFondo": "Almizcle blanco, Vainilla, Madera",
    "notes": [
      "Ruibarbo",
      "Pera",
      "Frambuesa",
      "Flor de cerezo",
      "Rosa",
      "Jazmín",
      "Almizcle blanco",
      "Vainilla",
      "Madera"
    ],
    "ml": 100,
    "precioUSD": 75,
    "categoria": "disenador"
  },
  {
    "id": 308,
    "house": "GIORGIO ARMANI",
    "name": "Giorgio Armani Acqua di Giò for Women",
    "image": "giorgio-armani-acqua-di-gio-eau-de-toilette-for-women-100ml-f.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Femenino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Mandarina, Bergamota, Jazmín",
    "notasCorazon": "Rosa, Ciclamen, Narciso",
    "notasFondo": "Almizcle, Cedro, Ámbar",
    "notes": [
      "Mandarina",
      "Bergamota",
      "Jazmín",
      "Rosa",
      "Ciclamen",
      "Narciso",
      "Almizcle",
      "Cedro",
      "Ámbar"
    ],
    "ml": 100,
    "precioUSD": 100,
    "categoria": "disenador"
  },
  {
    "id": 309,
    "house": "HUGO BOSS",
    "name": "Hugo Boss Boss The Scent for Her",
    "image": "hugo-boss-boss-the-scent-for-her-100ml-f.webp",
    "familia": null,
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Melocotón, Osmanto",
    "notasCorazon": "Orquídea, Jazmín",
    "notasFondo": "Almizcle, Vainilla, Madera",
    "notes": [
      "Melocotón",
      "Osmanto",
      "Orquídea",
      "Jazmín",
      "Almizcle",
      "Vainilla",
      "Madera"
    ],
    "ml": 100,
    "precioUSD": 90,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 310,
    "house": "CAROLINA HERRERA",
    "name": "Carolina Herrera 212 VIP Black",
    "image": "carolina-herrera-212-vip-black-100ml-m.webp",
    "familia": null,
    "tipo": "Eau de Parfum",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Pera, Bergamota, Cardamomo",
    "notasCorazon": "Pachulí, Vainilla negra, Ámbar",
    "notasFondo": "Sándalo, Almizcle, Cedro",
    "notes": [
      "Pera",
      "Bergamota",
      "Cardamomo",
      "Pachulí",
      "Vainilla negra",
      "Ámbar",
      "Sándalo",
      "Almizcle",
      "Cedro"
    ],
    "ml": 100,
    "precioUSD": 110,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 311,
    "house": "RABANNE",
    "name": "Rabanne 1 Million",
    "image": "rabanne-1-million-100ml-m.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Mandarina sanguina, Pomelo, Menta",
    "notasCorazon": "Rosa, Canela, Cuero",
    "notasFondo": "Pachulí, Ámbar, Maderas blancas",
    "notes": [
      "Mandarina sanguina",
      "Pomelo",
      "Menta",
      "Rosa",
      "Canela",
      "Cuero",
      "Pachulí",
      "Ámbar",
      "Maderas blancas"
    ],
    "ml": 100,
    "precioUSD": 150,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 312,
    "house": "CAROLINA HERRERA",
    "name": "Carolina Herrera 212 Heroes Forever",
    "image": "carolina-herrera-212-heroes-100ml-m.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Bergamota, Pomelo, Romero",
    "notasCorazon": "Lavanda, Salvia, Pimienta",
    "notasFondo": "Cedro, Vetiver, Almizcle",
    "notes": [
      "Bergamota",
      "Pomelo",
      "Romero",
      "Lavanda",
      "Salvia",
      "Pimienta",
      "Cedro",
      "Vetiver",
      "Almizcle"
    ],
    "ml": 100,
    "precioUSD": 115,
    "categoria": "disenador"
  },
  {
    "id": 313,
    "house": "RABANNE",
    "name": "Rabanne Lady Million Royal",
    "image": "rabanne-lady-million-royal-80ml-f.webp",
    "familia": null,
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Bergamota, Albaricoque, Rosa",
    "notasCorazon": "Rosa turca, Jazmín, Naranja amarga",
    "notasFondo": "Pachulí, Almizcle, Madera",
    "notes": [
      "Bergamota",
      "Albaricoque",
      "Rosa",
      "Rosa turca",
      "Jazmín",
      "Naranja amarga",
      "Pachulí",
      "Almizcle",
      "Madera"
    ],
    "ml": 80,
    "precioUSD": 110,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 314,
    "house": "RABANNE",
    "name": "Rabanne Fame Intense",
    "image": "rabanne-fame-intense-80ml-f.webp",
    "familia": null,
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Bergamota, Mandarina",
    "notasCorazon": "Jazmín, Rosa, Flor de azahar",
    "notasFondo": "Vetiver, Almizcle, Ámbar",
    "notes": [
      "Bergamota",
      "Mandarina",
      "Jazmín",
      "Rosa",
      "Flor de azahar",
      "Vetiver",
      "Almizcle",
      "Ámbar"
    ],
    "ml": 80,
    "precioUSD": 200,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 315,
    "house": "JEAN PAUL GAULTIER",
    "name": "Jean Paul Gaultier Le Beau",
    "image": "jean-paul-gaultier-le-beau-jean-paul-gaultier-75ml-m.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Bergamota, Coco, Pomelo",
    "notasCorazon": "Flor de coco, Nenúfar, Ciclamen",
    "notasFondo": "Cedro, Almizcle, Vainilla",
    "notes": [
      "Bergamota",
      "Coco",
      "Pomelo",
      "Flor de coco",
      "Nenúfar",
      "Ciclamen",
      "Cedro",
      "Almizcle",
      "Vainilla"
    ],
    "ml": 75,
    "precioUSD": 150,
    "categoria": "disenador"
  },
  {
    "id": 316,
    "house": "DOLCE & GABBANA",
    "name": "Dolce & Gabbana Light Blue Pour Homme",
    "image": "dolcegabbana-light-blue-pour-homme-125ml-m.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Manzana de Sicilia, Bergamota, Mandarina",
    "notasCorazon": "Enebro, Pimienta rosa, Romero",
    "notasFondo": "Almizcle, Cedro, Ámbar",
    "notes": [
      "Manzana de Sicilia",
      "Bergamota",
      "Mandarina",
      "Enebro",
      "Pimienta rosa",
      "Romero",
      "Almizcle",
      "Cedro",
      "Ámbar"
    ],
    "ml": 125,
    "precioUSD": 100,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 317,
    "house": "PRADA",
    "name": "Prada Paradoxe",
    "image": "prada-prada-paradoxe-90ml-f.webp",
    "familia": null,
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Bergamota, Neroli, Ámbar gris",
    "notasCorazon": "Jazmín sambac, Magnolia",
    "notasFondo": "Almizcle blanco, Sándalo, Musgo",
    "notes": [
      "Bergamota",
      "Neroli",
      "Ámbar gris",
      "Jazmín sambac",
      "Magnolia",
      "Almizcle blanco",
      "Sándalo",
      "Musgo"
    ],
    "ml": 90,
    "precioUSD": 220,
    "categoria": "disenador"
  },
  {
    "id": 318,
    "house": "RABANNE",
    "name": "Rabanne XS 2018",
    "image": "rabanne-xs-2018-100ml-m.webp",
    "familia": null,
    "tipo": "Eau de Parfum",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Pomelo, Menta, Pimienta rosa",
    "notasCorazon": "Cardamomo, Vetiver, Flores",
    "notasFondo": "Cedro, Almizcle, Ámbar",
    "notes": [
      "Pomelo",
      "Menta",
      "Pimienta rosa",
      "Cardamomo",
      "Vetiver",
      "Flores",
      "Cedro",
      "Almizcle",
      "Ámbar"
    ],
    "ml": 100,
    "precioUSD": 85,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 319,
    "house": "CREED",
    "name": "Creed Millesime Impérial",
    "image": "creed-millsime-imprial-100ml-u.webp",
    "familia": null,
    "tipo": "Eau de Parfum",
    "genero": "Unisex",
    "descripcion": null,
    "description": null,
    "notasSalida": "Bergamota, Mandarina, Limón",
    "notasCorazon": "Iris, Algas marinas, Jazmín",
    "notasFondo": "Almizcle blanco, Ámbar gris, Musgo de roble",
    "notes": [
      "Bergamota",
      "Mandarina",
      "Limón",
      "Iris",
      "Algas marinas",
      "Jazmín",
      "Almizcle blanco",
      "Ámbar gris",
      "Musgo de roble"
    ],
    "ml": 100,
    "precioUSD": 400,
    "categoria": "nicho"
  },
  {
    "id": 320,
    "house": "CREED",
    "name": "Creed Aventus Absolu",
    "image": "creed-absolu-aventus-2023-75ml-m.webp",
    "familia": null,
    "tipo": "Eau de Parfum",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Piña, Bergamota, Manzana verde",
    "notasCorazon": "Abedul, Jazmín, Rosa",
    "notasFondo": "Almizcle, Ámbar, Vetiver, Musgo de roble",
    "notes": [
      "Piña",
      "Bergamota",
      "Manzana verde",
      "Abedul",
      "Jazmín",
      "Rosa",
      "Almizcle",
      "Ámbar",
      "Vetiver",
      "Musgo de roble"
    ],
    "ml": 75,
    "precioUSD": 580,
    "categoria": "nicho"
  },
  {
    "id": 321,
    "house": "CAROLINA HERRERA",
    "name": "Carolina Herrera Bad Boy Le Parfum",
    "image": "carolina-herrera-bad-boy-le-parfum-100ml-m.webp",
    "familia": null,
    "tipo": "Parfum",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Bergamota, Pimienta rosa, Pomelo",
    "notasCorazon": "Cedro, Salvia, Geranio",
    "notasFondo": "Almizcle negro, Cuero, Ámbar",
    "notes": [
      "Bergamota",
      "Pimienta rosa",
      "Pomelo",
      "Cedro",
      "Salvia",
      "Geranio",
      "Almizcle negro",
      "Cuero",
      "Ámbar"
    ],
    "ml": 100,
    "precioUSD": 150,
    "categoria": "disenador"
  },
  {
    "id": 322,
    "house": "ESCADA",
    "name": "Escada Magnetism",
    "image": "escada-escada-magnetism-75ml-f.webp",
    "familia": null,
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Mandarina, Bergamota, Durazno",
    "notasCorazon": "Rosa, Jazmín, Magnolia",
    "notasFondo": "Almizcle, Sándalo, Vainilla",
    "notes": [
      "Mandarina",
      "Bergamota",
      "Durazno",
      "Rosa",
      "Jazmín",
      "Magnolia",
      "Almizcle",
      "Sándalo",
      "Vainilla"
    ],
    "ml": 75,
    "precioUSD": 45,
    "categoria": "disenador"
  },
  {
    "id": 323,
    "house": "VERSACE",
    "name": "Versace Pour Femme Dylan Purple",
    "image": "versace-pour-femme-dylan-purple-100ml-f.webp",
    "familia": null,
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Bergamota, Zarzamora, Rosa",
    "notasCorazon": "Violeta, Jazmín, Iris",
    "notasFondo": "Almizcle, Sándalo, Pachulí",
    "notes": [
      "Bergamota",
      "Zarzamora",
      "Rosa",
      "Violeta",
      "Jazmín",
      "Iris",
      "Almizcle",
      "Sándalo",
      "Pachulí"
    ],
    "ml": 100,
    "precioUSD": 110,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 324,
    "house": "RALPH LAUREN",
    "name": "Set Ralph Lauren Polo Blue",
    "image": "ralph-lauren-polo-blue-travel-set-125ml-m.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Melón, Pepino, Mandarina",
    "notasCorazon": "Salvia, Geranio, Cedro",
    "notasFondo": "Almizcle, Suede, Ámbar",
    "notes": [
      "Melón",
      "Pepino",
      "Mandarina",
      "Salvia",
      "Geranio",
      "Cedro",
      "Almizcle",
      "Suede",
      "Ámbar"
    ],
    "ml": 125,
    "precioUSD": 85,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 325,
    "house": "DOLCE & GABBANA",
    "name": "Dolce & Gabbana Light Blue Summer Vibes",
    "image": "dolcegabbana-light-blue-summer-vibes-100ml-f.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Femenino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Mandarina, Durazno, Bergamota",
    "notasCorazon": "Rosa, Ciclamen, Orquídea",
    "notasFondo": "Almizcle, Cedro, Ámbar",
    "notes": [
      "Mandarina",
      "Durazno",
      "Bergamota",
      "Rosa",
      "Ciclamen",
      "Orquídea",
      "Almizcle",
      "Cedro",
      "Ámbar"
    ],
    "ml": 100,
    "precioUSD": 85,
    "categoria": "disenador"
  },
  {
    "id": 326,
    "house": "RABANNE",
    "name": "Rabanne Black XS Excess",
    "image": "rabanne-black-xs-for-her-eau-de-parfum-80ml-f.webp",
    "familia": null,
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Anís estrellado, Jazmín",
    "notasCorazon": "Rosa, Orquídea negra",
    "notasFondo": "Pachulí, Almizcle, Ámbar",
    "notes": [
      "Anís estrellado",
      "Jazmín",
      "Rosa",
      "Orquídea negra",
      "Pachulí",
      "Almizcle",
      "Ámbar"
    ],
    "ml": 80,
    "precioUSD": 90,
    "categoria": "disenador"
  },
  {
    "id": 327,
    "house": "CAROLINA HERRERA",
    "name": "Carolina Herrera CH Pasión for Him",
    "image": "carolina-herrera-ch-pasin-for-him-100ml-m.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Bergamota, Pomelo, Pimienta",
    "notasCorazon": "Vetiver, Cedro, Cuero",
    "notasFondo": "Almizcle, Ámbar, Madera",
    "notes": [
      "Bergamota",
      "Pomelo",
      "Pimienta",
      "Vetiver",
      "Cedro",
      "Cuero",
      "Almizcle",
      "Ámbar",
      "Madera"
    ],
    "ml": 100,
    "precioUSD": 85,
    "categoria": "disenador"
  },
  {
    "id": 328,
    "house": "CAROLINA HERRERA",
    "name": "Carolina Herrera 212 Sexy Men",
    "image": "carolina-herrera-212-sexy-men-100ml-m.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Cardamomo, Laurel, Pomelo",
    "notasCorazon": "Cuero, Árbol de caucho, Musgo",
    "notasFondo": "Almizcle, Cedro, Ámbar",
    "notes": [
      "Cardamomo",
      "Laurel",
      "Pomelo",
      "Cuero",
      "Árbol de caucho",
      "Musgo",
      "Almizcle",
      "Cedro",
      "Ámbar"
    ],
    "ml": 100,
    "precioUSD": 90,
    "categoria": "disenador"
  },
  {
    "id": 329,
    "house": "ARIANA GRANDE",
    "name": "Ariana Grande Moonlight",
    "image": "ariana-grande-moonlight-100ml-f.webp",
    "familia": null,
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Grosella negra, Pera, Vainilla",
    "notasCorazon": "Orquídea negra, Flor de cerezo, Jazmín",
    "notasFondo": "Almizcle, Sándalo, Ámbar",
    "notes": [
      "Grosella negra",
      "Pera",
      "Vainilla",
      "Orquídea negra",
      "Flor de cerezo",
      "Jazmín",
      "Almizcle",
      "Sándalo",
      "Ámbar"
    ],
    "ml": 100,
    "precioUSD": 55,
    "categoria": "disenador"
  },
  {
    "id": 330,
    "house": "MUGLER",
    "name": "Mugler Alien Woman",
    "image": "mugler-alien-mugler-90ml-f.webp",
    "familia": null,
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Jazmín",
    "notasCorazon": "Cachemira, Sol",
    "notasFondo": "Ámbar blanco, Almizcle",
    "notes": [
      "Jazmín",
      "Cachemira",
      "Sol",
      "Ámbar blanco",
      "Almizcle"
    ],
    "ml": 90,
    "precioUSD": 115,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 331,
    "house": "CALVIN",
    "name": "Calvin Klein Eternity for Men",
    "image": "calvin-klein-eternity-for-men-100ml-m.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Mandarina, Lavanda, Menta",
    "notasCorazon": "Geranio, Cedro, Vetiver",
    "notasFondo": "Almizcle, Sándalo, Ámbar",
    "notes": [
      "Mandarina",
      "Lavanda",
      "Menta",
      "Geranio",
      "Cedro",
      "Vetiver",
      "Almizcle",
      "Sándalo",
      "Ámbar"
    ],
    "ml": 100,
    "precioUSD": 45,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 332,
    "house": "MONTBLANC",
    "name": "Montblanc Legend Spirit",
    "image": "montblanc-legend-spirit-100ml-m.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Bergamota, Pomelo, Menta acuática",
    "notasCorazon": "Lavanda, Geranio, Cedro",
    "notasFondo": "Almizcle blanco, Cedro, Ámbar",
    "notes": [
      "Bergamota",
      "Pomelo",
      "Menta acuática",
      "Lavanda",
      "Geranio",
      "Cedro",
      "Almizcle blanco",
      "Cedro",
      "Ámbar"
    ],
    "ml": 100,
    "precioUSD": 65,
    "categoria": "disenador"
  },
  {
    "id": 333,
    "house": "AZZARO",
    "name": "Azzaro Wanted Eau de Parfum",
    "image": "azzaro-wanted-eau-de-parfum-100ml-m.webp",
    "familia": null,
    "tipo": "Eau de Parfum",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Cardamomo, Lavanda, Bergamota",
    "notasCorazon": "Salvia, Tabaco, Pimienta",
    "notasFondo": "Vetiver, Cedro, Ámbar",
    "notes": [
      "Cardamomo",
      "Lavanda",
      "Bergamota",
      "Salvia",
      "Tabaco",
      "Pimienta",
      "Vetiver",
      "Cedro",
      "Ámbar"
    ],
    "ml": 100,
    "precioUSD": 110,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 334,
    "house": "VIKTOR&ROLF",
    "name": "Viktor&Rolf Spicebomb",
    "image": "viktorrolf-spicebomb-90ml-m.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Pomelo, Bergamota",
    "notasCorazon": "Canela, Pimienta roja, Tabaco",
    "notasFondo": "Cuero, Vetiver, Almizcle",
    "notes": [
      "Pomelo",
      "Bergamota",
      "Canela",
      "Pimienta roja",
      "Tabaco",
      "Cuero",
      "Vetiver",
      "Almizcle"
    ],
    "ml": 90,
    "precioUSD": 90,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 335,
    "house": "GIORGIO ARMANI",
    "name": "Giorgio Armani Acqua di Giò Profondo",
    "image": "giorgio-armani-acqua-di-gi-profondo-100ml-m.webp",
    "familia": null,
    "tipo": "Eau de Parfum",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Bergamota, Mandarina, Incienso marino",
    "notasCorazon": "Romero, Ciprés, Pachulí",
    "notasFondo": "Madera de guayaco, Almizcle, Ámbar",
    "notes": [
      "Bergamota",
      "Mandarina",
      "Incienso marino",
      "Romero",
      "Ciprés",
      "Pachulí",
      "Madera de guayaco",
      "Almizcle",
      "Ámbar"
    ],
    "ml": 100,
    "precioUSD": 100,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 336,
    "house": "VIKTOR&ROLF",
    "name": "Viktor&Rolf Bonbon",
    "image": "viktorrolf-bonbon-50ml-f.webp",
    "familia": null,
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Mandarina, Naranja, Caramelo",
    "notasCorazon": "Peonía, Rosa, Caramelo",
    "notasFondo": "Almizcle blanco, Pachulí, Cachemira",
    "notes": [
      "Mandarina",
      "Naranja",
      "Caramelo",
      "Peonía",
      "Rosa",
      "Caramelo",
      "Almizcle blanco",
      "Pachulí",
      "Cachemira"
    ],
    "ml": 90,
    "precioUSD": 150,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 337,
    "house": "CUBA",
    "name": "Cuba Paris Cuba Green",
    "image": "cuba-paris-cuba-green-100ml-m.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Bergamota, Lavanda, Menta",
    "notasCorazon": "Geranio, Cedro, Rosa",
    "notasFondo": "Almizcle, Ámbar, Vetiver",
    "notes": [
      "Bergamota",
      "Lavanda",
      "Menta",
      "Geranio",
      "Cedro",
      "Rosa",
      "Almizcle",
      "Ámbar",
      "Vetiver"
    ],
    "ml": 100,
    "precioUSD": 20,
    "categoria": "disenador"
  },
  {
    "id": 338,
    "house": "BENETTON",
    "name": "Benetton Paradiso Inferno Blue",
    "image": "benetton-paradiso-inferno-blue-100ml-m.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Bergamota, Limón, Menta",
    "notasCorazon": "Lavanda, Geranio, Rosa",
    "notasFondo": "Cedro, Almizcle, Ámbar",
    "notes": [
      "Bergamota",
      "Limón",
      "Menta",
      "Lavanda",
      "Geranio",
      "Rosa",
      "Cedro",
      "Almizcle",
      "Ámbar"
    ],
    "ml": 100,
    "precioUSD": 20,
    "categoria": "disenador"
  },
  {
    "id": 339,
    "house": "NAUTICA",
    "name": "Nautica Voyage Sport",
    "image": "nautica-nautica-voyage-sport-100ml-m.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Manzana verde, Lima, Pomelo",
    "notasCorazon": "Cedro, Algas marinas, Cardamomo",
    "notasFondo": "Almizcle, Ámbar, Madera",
    "notes": [
      "Manzana verde",
      "Lima",
      "Pomelo",
      "Cedro",
      "Algas marinas",
      "Cardamomo",
      "Almizcle",
      "Ámbar",
      "Madera"
    ],
    "ml": 100,
    "precioUSD": 25,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 340,
    "house": "GUESS",
    "name": "Guess Seductive Red Homme",
    "image": "guess-seductive-red-homme-100ml-m.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Naranja, Limón, Lavanda",
    "notasCorazon": "Salvia, Cedro, Vetiver",
    "notasFondo": "Almizcle, Ámbar, Madera",
    "notes": [
      "Naranja",
      "Limón",
      "Lavanda",
      "Salvia",
      "Cedro",
      "Vetiver",
      "Almizcle",
      "Ámbar",
      "Madera"
    ],
    "ml": 100,
    "precioUSD": 35,
    "categoria": "disenador"
  },
  {
    "id": 341,
    "house": "CALVIN",
    "name": "Calvin Klein CK One",
    "image": "calvin-klein-ck-one-200ml-u.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Unisex",
    "descripcion": null,
    "description": null,
    "notasSalida": "Bergamota, Cardamomo, Piña",
    "notasCorazon": "Violeta, Rosa, Jazmín, Lirio",
    "notasFondo": "Almizcle, Cedro, Ámbar",
    "notes": [
      "Bergamota",
      "Cardamomo",
      "Piña",
      "Violeta",
      "Rosa",
      "Jazmín",
      "Lirio",
      "Almizcle",
      "Cedro",
      "Ámbar"
    ],
    "ml": 200,
    "precioUSD": 50,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 342,
    "house": "CALVIN",
    "name": "Calvin Klein CK One",
    "image": "calvin-klein-ck-one-100ml-u.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Unisex",
    "descripcion": null,
    "description": null,
    "notasSalida": "Bergamota, Cardamomo, Piña",
    "notasCorazon": "Violeta, Rosa, Jazmín, Lirio",
    "notasFondo": "Almizcle, Cedro, Ámbar",
    "notes": [
      "Bergamota",
      "Cardamomo",
      "Piña",
      "Violeta",
      "Rosa",
      "Jazmín",
      "Lirio",
      "Almizcle",
      "Cedro",
      "Ámbar"
    ],
    "ml": 100,
    "precioUSD": 40,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 343,
    "house": "CALVIN",
    "name": "Calvin Klein CK Be",
    "image": "calvin-klein-ck-be-100ml-u.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Unisex",
    "descripcion": null,
    "description": null,
    "notasSalida": "Bergamota, Mandarina, Lavanda",
    "notasCorazon": "Iris, Muguete, Rosa, Cedro",
    "notasFondo": "Almizcle, Ámbar, Sándalo",
    "notes": [
      "Bergamota",
      "Mandarina",
      "Lavanda",
      "Iris",
      "Muguete",
      "Rosa",
      "Cedro",
      "Almizcle",
      "Ámbar",
      "Sándalo"
    ],
    "ml": 100,
    "precioUSD": 35,
    "categoria": "disenador",
    "variantIds": [
      344
    ],
    "descuento": 20
  },
  {
    "id": 344,
    "house": "CALVIN",
    "name": "Calvin Klein CK Be",
    "image": "calvin-klein-ck-be-200ml-u.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Unisex",
    "descripcion": null,
    "description": null,
    "notasSalida": "Bergamota, Mandarina, Lavanda",
    "notasCorazon": "Iris, Muguete, Rosa, Cedro",
    "notasFondo": "Almizcle, Ámbar, Sándalo",
    "notes": [
      "Bergamota",
      "Mandarina",
      "Lavanda",
      "Iris",
      "Muguete",
      "Rosa",
      "Cedro",
      "Almizcle",
      "Ámbar",
      "Sándalo"
    ],
    "ml": 200,
    "precioUSD": 45,
    "categoria": "disenador"
  },
  {
    "id": 345,
    "house": "ARMAF",
    "name": "Armaf The Pride of Armaf for Men",
    "image": "armaf-the-pride-of-armaf-for-men-100ml-m.webp",
    "familia": null,
    "tipo": "Eau de Parfum",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Bergamota, Cardamomo, Pimienta",
    "notasCorazon": "Cuero, Oud, Rosa",
    "notasFondo": "Almizcle, Ámbar, Cedro",
    "notes": [
      "Bergamota",
      "Cardamomo",
      "Pimienta",
      "Cuero",
      "Oud",
      "Rosa",
      "Almizcle",
      "Ámbar",
      "Cedro"
    ],
    "ml": 100,
    "precioUSD": 45,
    "categoria": "arabes"
  },
  {
    "id": 346,
    "house": "ANTONIO BANDERAS",
    "name": "Antonio Banderas The Golden Secret",
    "image": "antonio-banderas-the-golden-secret-100ml-m.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Pomelo, Limón, Cardamomo",
    "notasCorazon": "Lavanda, Cedro, Geranio",
    "notasFondo": "Almizcle, Ámbar, Madera",
    "notes": [
      "Pomelo",
      "Limón",
      "Cardamomo",
      "Lavanda",
      "Cedro",
      "Geranio",
      "Almizcle",
      "Ámbar",
      "Madera"
    ],
    "ml": 100,
    "precioUSD": 25,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 346,
    "house": "ANTONIO BANDERAS",
    "name": "Antonio Banderas The Golden Secret",
    "image": "antonio-banderas-the-golden-secret-100ml-m.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Pomelo, Limón, Cardamomo",
    "notasCorazon": "Lavanda, Cedro, Geranio",
    "notasFondo": "Almizcle, Ámbar, Madera",
    "notes": [
      "Pomelo",
      "Limón",
      "Cardamomo",
      "Lavanda",
      "Cedro",
      "Geranio",
      "Almizcle",
      "Ámbar",
      "Madera"
    ],
    "ml": 100,
    "precioUSD": 25,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 347,
    "house": "NAUTICA",
    "name": "Nautica Voyage Heritage",
    "image": "nautica-nautica-voyage-heritage-100ml-m.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Bergamota, Pomelo, Menta",
    "notasCorazon": "Lavanda, Cedro, Geranio",
    "notasFondo": "Almizcle, Ámbar, Vetiver",
    "notes": [
      "Bergamota",
      "Pomelo",
      "Menta",
      "Lavanda",
      "Cedro",
      "Geranio",
      "Almizcle",
      "Ámbar",
      "Vetiver"
    ],
    "ml": 100,
    "precioUSD": 25,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 348,
    "house": "MONTBLANC",
    "name": "Montblanc Star Walker",
    "image": "montblanc-starwalker-75ml-m.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Bergamota, Menta, Pomelo",
    "notasCorazon": "Lavanda, Cedro, Geranio",
    "notasFondo": "Vetiver, Almizcle, Ámbar",
    "notes": [
      "Bergamota",
      "Menta",
      "Pomelo",
      "Lavanda",
      "Cedro",
      "Geranio",
      "Vetiver",
      "Almizcle",
      "Ámbar"
    ],
    "ml": 75,
    "precioUSD": 45,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 349,
    "house": "MONTBLANC",
    "name": "Montblanc Legend Red",
    "image": "montblanc-legend-red-100ml-m.webp",
    "familia": null,
    "tipo": "Eau de Parfum",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Pomelo, Bergamota, Pimienta roja",
    "notasCorazon": "Salvia, Geranio, Lavanda",
    "notasFondo": "Almizcle, Cedro, Ámbar rojo",
    "notes": [
      "Pomelo",
      "Bergamota",
      "Pimienta roja",
      "Salvia",
      "Geranio",
      "Lavanda",
      "Almizcle",
      "Cedro",
      "Ámbar rojo"
    ],
    "ml": 100,
    "precioUSD": 60,
    "categoria": "disenador"
  },
  {
    "id": 350,
    "house": "MONTBLANC",
    "name": "Montblanc Legend Blue",
    "image": "montblanc-legend-blue-100ml-m.webp",
    "familia": null,
    "tipo": "Eau de Parfum",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Bergamota, Pomelo, Cardamomo",
    "notasCorazon": "Lavanda, Cedro, Iris",
    "notasFondo": "Almizcle, Ámbar, Sándalo",
    "notes": [
      "Bergamota",
      "Pomelo",
      "Cardamomo",
      "Lavanda",
      "Cedro",
      "Iris",
      "Almizcle",
      "Ámbar",
      "Sándalo"
    ],
    "ml": 100,
    "precioUSD": 85,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 351,
    "house": "ISSEY MIYAKE",
    "name": "Issey Miyake L'Eau d'Issey Pour Homme",
    "image": "issey-miyake-leau-dissey-pour-homme-125ml-m.webp",
    "familia": null,
    "tipo": "Parfum",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Yuzu, Bergamota, Mandarina",
    "notasCorazon": "Cactus, Nuez moscada, Ciclamen",
    "notasFondo": "Cedro, Almizcle, Ámbar",
    "notes": [
      "Yuzu",
      "Bergamota",
      "Mandarina",
      "Cactus",
      "Nuez moscada",
      "Ciclamen",
      "Cedro",
      "Almizcle",
      "Ámbar"
    ],
    "ml": 125,
    "precioUSD": 90,
    "categoria": "disenador"
  },
  {
    "id": 352,
    "house": "ISSEY MIYAKE",
    "name": "Issey Miyake L'Eau d'Issey Pour Homme",
    "image": "issey-miyake-leau-dissey-pour-homme-200ml-m.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Yuzu, Bergamota, Mandarina",
    "notasCorazon": "Cactus, Nuez moscada, Ciclamen",
    "notasFondo": "Cedro, Almizcle, Ámbar",
    "notes": [
      "Yuzu",
      "Bergamota",
      "Mandarina",
      "Cactus",
      "Nuez moscada",
      "Ciclamen",
      "Cedro",
      "Almizcle",
      "Ámbar"
    ],
    "ml": 200,
    "precioUSD": 90,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 353,
    "house": "PARIS HILTON",
    "name": "Paris Hilton Gold Rush Man",
    "image": "paris-hilton-gold-rush-man-100ml-m.webp",
    "familia": null,
    "tipo": "Eau de Parfum",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Bergamota, Limón, Pomelo",
    "notasCorazon": "Cardamomo, Cedro, Iris",
    "notasFondo": "Almizcle, Ámbar, Sándalo",
    "notes": [
      "Bergamota",
      "Limón",
      "Pomelo",
      "Cardamomo",
      "Cedro",
      "Iris",
      "Almizcle",
      "Ámbar",
      "Sándalo"
    ],
    "ml": 100,
    "precioUSD": 45,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 354,
    "house": "PERRY ELLIS",
    "name": "Perry Ellis Aqua Extreme",
    "image": "perry-ellis-aqua-extreme-100ml-m.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Lima, Menta, Bergamota",
    "notasCorazon": "Jazmín acuático, Ciclamen, Lavanda",
    "notasFondo": "Cedro, Almizcle, Ámbar",
    "notes": [
      "Lima",
      "Menta",
      "Bergamota",
      "Jazmín acuático",
      "Ciclamen",
      "Lavanda",
      "Cedro",
      "Almizcle",
      "Ámbar"
    ],
    "ml": 100,
    "precioUSD": 35,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 355,
    "house": "ANTONIO BANDERAS",
    "name": "Antonio Banderas Black Seduction",
    "image": "antonio-banderas-black-seduction-200ml-m.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Bergamota, Pomelo, Pimienta",
    "notasCorazon": "Lavanda, Cedro, Jazmín",
    "notasFondo": "Almizcle, Ámbar, Cuero",
    "notes": [
      "Bergamota",
      "Pomelo",
      "Pimienta",
      "Lavanda",
      "Cedro",
      "Jazmín",
      "Almizcle",
      "Ámbar",
      "Cuero"
    ],
    "ml": 200,
    "precioUSD": 35,
    "categoria": "disenador"
  },
  {
    "id": 356,
    "house": "CALVIN",
    "name": "Calvin Klein Obsession for Men",
    "image": "calvin-klein-obsession-for-men-200ml-m.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Mandarina, Bergamota, Lavanda",
    "notasCorazon": "Cedro, Sándalo, Vetiver",
    "notasFondo": "Almizcle, Ámbar, Vainilla",
    "notes": [
      "Mandarina",
      "Bergamota",
      "Lavanda",
      "Cedro",
      "Sándalo",
      "Vetiver",
      "Almizcle",
      "Ámbar",
      "Vainilla"
    ],
    "ml": 200,
    "precioUSD": 50,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 357,
    "house": "MONTBLANC",
    "name": "Montblanc Legend Night",
    "image": "montblanc-legend-night-100ml-m.webp",
    "familia": null,
    "tipo": "Eau de Parfum",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Cardamomo, Lavanda, Bergamota",
    "notasCorazon": "Oud, Cedro, Lirio",
    "notasFondo": "Vetiver, Ámbar, Almizcle",
    "notes": [
      "Cardamomo",
      "Lavanda",
      "Bergamota",
      "Oud",
      "Cedro",
      "Lirio",
      "Vetiver",
      "Ámbar",
      "Almizcle"
    ],
    "ml": 100,
    "precioUSD": 60,
    "categoria": "disenador"
  },
  {
    "id": 358,
    "house": "ANIMALE",
    "name": "Animale for Men",
    "image": "animale-animale-for-men-100ml-m.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Bergamota, Mandarina, Limón",
    "notasCorazon": "Lavanda, Cedro, Geranio",
    "notasFondo": "Almizcle, Ámbar, Madera",
    "notes": [
      "Bergamota",
      "Mandarina",
      "Limón",
      "Lavanda",
      "Cedro",
      "Geranio",
      "Almizcle",
      "Ámbar",
      "Madera"
    ],
    "ml": 100,
    "precioUSD": 40,
    "categoria": "disenador"
  },
  {
    "id": 359,
    "house": "ANTONIO BANDERAS",
    "name": "Antonio Banderas The Icon",
    "image": "antonio-banderas-the-icon-100ml-m.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Bergamota, Mandarina, Pimienta rosa",
    "notasCorazon": "Salvia, Cedro, Cachemira",
    "notasFondo": "Pachulí, Ámbar, Almizcle",
    "notes": [
      "Bergamota",
      "Mandarina",
      "Pimienta rosa",
      "Salvia",
      "Cedro",
      "Cachemira",
      "Pachulí",
      "Ámbar",
      "Almizcle"
    ],
    "ml": 100,
    "precioUSD": 30,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 360,
    "house": "ADIDAS",
    "name": "Adidas Victory League",
    "image": "adidas-victory-league-eau-de-toilette-100ml-m.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Bergamota, Lavanda, Menta",
    "notasCorazon": "Geranio, Cedro, Rosa",
    "notasFondo": "Almizcle, Ámbar, Madera",
    "notes": [
      "Bergamota",
      "Lavanda",
      "Menta",
      "Geranio",
      "Cedro",
      "Rosa",
      "Almizcle",
      "Ámbar",
      "Madera"
    ],
    "ml": 100,
    "precioUSD": 15,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 361,
    "house": "ADIDAS",
    "name": "Adidas Pure Game",
    "image": "adidas-pure-game-eau-de-toilette-100ml-m.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Bergamota, Lima, Pomelo",
    "notasCorazon": "Lavanda, Cedro, Menta",
    "notasFondo": "Almizcle, Ámbar, Madera",
    "notes": [
      "Bergamota",
      "Lima",
      "Pomelo",
      "Lavanda",
      "Cedro",
      "Menta",
      "Almizcle",
      "Ámbar",
      "Madera"
    ],
    "ml": 100,
    "precioUSD": 15,
    "categoria": "disenador"
  },
  {
    "id": 362,
    "house": "MONTBLANC",
    "name": "Montblanc Explorer",
    "image": "montblanc-explorer-100ml-m.webp",
    "familia": null,
    "tipo": "Eau de Parfum",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Bergamota, Vetiver, Cardamomo",
    "notasCorazon": "Vetiver, Cuero, Cardamomo",
    "notasFondo": "Ámbar, Almizcle, Pachulí",
    "notes": [
      "Bergamota",
      "Vetiver",
      "Cardamomo",
      "Vetiver",
      "Cuero",
      "Cardamomo",
      "Ámbar",
      "Almizcle",
      "Pachulí"
    ],
    "ml": 100,
    "precioUSD": 80,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 363,
    "house": "SALVATORE FERRAGAMO",
    "name": "Salvatore Ferragamo F by Ferragamo Pour Homme",
    "image": "salvatore-ferragamo-f-by-ferragamo-pour-homme-100ml-m.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Bergamota, Mandarina, Pomelo",
    "notasCorazon": "Lavanda, Cedro, Geranio",
    "notasFondo": "Almizcle, Ámbar, Madera",
    "notes": [
      "Bergamota",
      "Mandarina",
      "Pomelo",
      "Lavanda",
      "Cedro",
      "Geranio",
      "Almizcle",
      "Ámbar",
      "Madera"
    ],
    "ml": 100,
    "precioUSD": 45,
    "categoria": "disenador"
  },
  {
    "id": 364,
    "house": "ISSEY MIYAKE",
    "name": "Issey Miyake L'Eau d'Issey Pour Homme Intense",
    "image": "issey-miyake-leau-dissey-pour-homme-intense-75ml-m.webp",
    "familia": null,
    "tipo": "Eau de Parfum",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Yuzu, Jengibre, Bergamota",
    "notasCorazon": "Nuez moscada, Cedro, Ciprés",
    "notasFondo": "Oud, Vetiver, Almizcle",
    "notes": [
      "Yuzu",
      "Jengibre",
      "Bergamota",
      "Nuez moscada",
      "Cedro",
      "Ciprés",
      "Oud",
      "Vetiver",
      "Almizcle"
    ],
    "ml": 75,
    "precioUSD": 35,
    "categoria": "disenador",
    "variantIds": [
      372
    ]
  },
  {
    "id": 365,
    "house": "MAISON ALHAMBRA",
    "name": "Maison Alhambra Glacier Ultra",
    "image": "maison-alhambra-glacier-ultra-100ml-m.webp",
    "familia": null,
    "tipo": "Eau de Parfum",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Bergamota, Pomelo, Limón",
    "notasCorazon": "Lavanda, Cedro, Notas acuáticas",
    "notasFondo": "Almizcle, Ámbar, Madera",
    "notes": [
      "Bergamota",
      "Pomelo",
      "Limón",
      "Lavanda",
      "Cedro",
      "Notas acuáticas",
      "Almizcle",
      "Ámbar",
      "Madera"
    ],
    "ml": 100,
    "precioUSD": 30,
    "categoria": "arabes",
    "agotado": true
  },
  {
    "id": 366,
    "house": "ANTONIO BANDERAS",
    "name": "Antonio Banderas King of Seduction",
    "image": "antonio-banderas-king-of-seduction-100ml-m.webp",
    "familia": "amadera especiada",
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "piña, melón, bergamota, manzana verde, toronja (pomelo)",
    "notasCorazon": "notas marinas, jazmin, cardamomo, neroli",
    "notasFondo": "vetiver, almizcle blanco, cedro, gamuza, ambar",
    "notes": [
      "piña",
      "melón",
      "bergamota",
      "manzana verde",
      "toronja (pomelo)",
      "notas marinas",
      "jazmin",
      "cardamomo",
      "neroli",
      "vetiver",
      "almizcle blanco",
      "cedro",
      "gamuza",
      "ambar"
    ],
    "ml": 100,
    "precioUSD": 25,
    "categoria": "disenador",
    "variantIds": [
      448
    ]
  },
  {
    "id": 367,
    "house": "ANTONIO BANDERAS",
    "name": "Antonio Banderas King of Seduction Absolute",
    "image": "antonio-banderas-king-of-seduction-absolute-100ml-m.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Bergamota, Mandarina, Pimienta",
    "notasCorazon": "Lavanda, Cedro, Sándalo",
    "notasFondo": "Almizcle, Ámbar intenso, Madera",
    "notes": [
      "Bergamota",
      "Mandarina",
      "Pimienta",
      "Lavanda",
      "Cedro",
      "Sándalo",
      "Almizcle",
      "Ámbar intenso",
      "Madera"
    ],
    "ml": 100,
    "precioUSD": 25,
    "categoria": "disenador"
  },
  {
    "id": 368,
    "house": "NAUTICA",
    "name": "Nautica Voyage",
    "image": "nautica-nautica-voyage-200ml-m.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Verde hoja, Manzana verde, Agua de mar",
    "notasCorazon": "Mimosa, Loto, Cedro",
    "notasFondo": "Almizcle, Musgo de roble, Ámbar",
    "notes": [
      "Verde hoja",
      "Manzana verde",
      "Agua de mar",
      "Mimosa",
      "Loto",
      "Cedro",
      "Almizcle",
      "Musgo de roble",
      "Ámbar"
    ],
    "ml": 200,
    "precioUSD": 40,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 369,
    "house": "ADIDAS",
    "name": "Adidas Ice Dive",
    "image": "adidas-ice-dive-eau-de-toilette-100ml-m.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Lima, Menta, Bergamota",
    "notasCorazon": "Lavanda, Geranio, Cedro",
    "notasFondo": "Almizcle, Ámbar, Madera",
    "notes": [
      "Lima",
      "Menta",
      "Bergamota",
      "Lavanda",
      "Geranio",
      "Cedro",
      "Almizcle",
      "Ámbar",
      "Madera"
    ],
    "ml": 100,
    "precioUSD": 15,
    "categoria": "disenador"
  },
  {
    "id": 370,
    "house": "MONTBLANC",
    "name": "Montblanc Explorer Platinum",
    "image": "montblanc-explorer-platinum-100ml-m.webp",
    "familia": null,
    "tipo": "Eau de Parfum",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Bergamota, Madera, Cardamomo",
    "notasCorazon": "Vetiver, Cedro, Cuero platino",
    "notasFondo": "Ámbar frío, Almizcle, Madera",
    "notes": [
      "Bergamota",
      "Madera",
      "Cardamomo",
      "Vetiver",
      "Cedro",
      "Cuero platino",
      "Ámbar frío",
      "Almizcle",
      "Madera"
    ],
    "ml": 100,
    "precioUSD": 65,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 371,
    "house": "SALVATORE FERRAGAMO",
    "name": "Salvatore Ferragamo",
    "image": "salvatore-ferragamo-ferragamo-100ml-m.webp",
    "familia": null,
    "tipo": "Eau de Parfum",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Bergamota, Mandarina, Pomelo",
    "notasCorazon": "Lavanda, Cedro, Geranio, Iris",
    "notasFondo": "Almizcle, Ámbar, Sándalo",
    "notes": [
      "Bergamota",
      "Mandarina",
      "Pomelo",
      "Lavanda",
      "Cedro",
      "Geranio",
      "Iris",
      "Almizcle",
      "Ámbar",
      "Sándalo"
    ],
    "ml": 100,
    "precioUSD": 45,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 372,
    "house": "ISSEY MIYAKE",
    "name": "Issey Miyake L'Eau d'Issey Pour Homme Intense",
    "image": "issey-miyake-nuit-dissey-125ml-m.webp",
    "familia": null,
    "tipo": "Eau de Parfum",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Bergamota, Mandarina, Pimienta",
    "notasCorazon": "Cedro, Vetiver, Cuero",
    "notasFondo": "Almizcle, Ámbar, Incienso",
    "notes": [
      "Bergamota",
      "Mandarina",
      "Pimienta",
      "Cedro",
      "Vetiver",
      "Cuero",
      "Almizcle",
      "Ámbar",
      "Incienso"
    ],
    "ml": 125,
    "precioUSD": 45,
    "categoria": "disenador"
  },
  {
    "id": 373,
    "house": "LACOSTE",
    "name": "Lacoste Booster",
    "image": "lacoste-fragrances-lacoste-booster-125ml-m.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Menta, Bergamota, Lavanda",
    "notasCorazon": "Geranio, Cedro, Rosa",
    "notasFondo": "Almizcle, Ámbar, Madera",
    "notes": [
      "Menta",
      "Bergamota",
      "Lavanda",
      "Geranio",
      "Cedro",
      "Rosa",
      "Almizcle",
      "Ámbar",
      "Madera"
    ],
    "ml": 125,
    "precioUSD": 70,
    "categoria": "disenador"
  },
  {
    "id": 374,
    "house": "PERRY ELLIS",
    "name": "Perry Ellis 360 Red for Men",
    "image": "perry-ellis-360-red-for-men-100ml-m.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Limón, Bergamota, Pomelo",
    "notasCorazon": "Lavanda, Cedro, Jengibre",
    "notasFondo": "Almizcle, Ámbar, Madera",
    "notes": [
      "Limón",
      "Bergamota",
      "Pomelo",
      "Lavanda",
      "Cedro",
      "Jengibre",
      "Almizcle",
      "Ámbar",
      "Madera"
    ],
    "ml": 100,
    "precioUSD": 40,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 375,
    "house": "ANTONIO BANDERAS",
    "name": "Antonio Banderas Black Seduction",
    "image": "antonio-banderas-black-seduction-100ml-m.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Bergamota, Pomelo, Pimienta",
    "notasCorazon": "Lavanda, Cedro, Jazmín",
    "notasFondo": "Almizcle, Ámbar, Cuero",
    "notes": [
      "Bergamota",
      "Pomelo",
      "Pimienta",
      "Lavanda",
      "Cedro",
      "Jazmín",
      "Almizcle",
      "Ámbar",
      "Cuero"
    ],
    "ml": 100,
    "precioUSD": 25,
    "categoria": "disenador",
    "variantIds": [
      355
    ]
  },
  {
    "id": 376,
    "house": "ANTONIO BANDERAS",
    "name": "Antonio Banderas The Secret Absolu",
    "image": "antonio-banderas-the-secret-absolu-100ml-m.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Mandarina, Bergamota, Pimienta",
    "notasCorazon": "Cuero, Cedro, Sándalo",
    "notasFondo": "Ámbar, Almizcle, Vainilla",
    "notes": [
      "Mandarina",
      "Bergamota",
      "Pimienta",
      "Cuero",
      "Cedro",
      "Sándalo",
      "Ámbar",
      "Almizcle",
      "Vainilla"
    ],
    "ml": 100,
    "precioUSD": 25,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 377,
    "house": "ANTONIO BANDERAS",
    "name": "Antonio Banderas The Secret",
    "image": "antonio-banderas-the-secret-100ml-m.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Pomelo, Mandarina, Bergamota",
    "notasCorazon": "Lavanda, Cedro, Cuero",
    "notasFondo": "Almizcle, Ámbar, Madera",
    "notes": [
      "Pomelo",
      "Mandarina",
      "Bergamota",
      "Lavanda",
      "Cedro",
      "Cuero",
      "Almizcle",
      "Ámbar",
      "Madera"
    ],
    "ml": 100,
    "precioUSD": 25,
    "categoria": "disenador",
    "variantIds": [
      378
    ]
  },
  {
    "id": 378,
    "house": "ANTONIO BANDERAS",
    "name": "Antonio Banderas The Secret",
    "image": "antonio-banderas-the-secret-200ml-m.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Pomelo, Mandarina, Bergamota",
    "notasCorazon": "Lavanda, Cedro, Cuero",
    "notasFondo": "Almizcle, Ámbar, Madera",
    "notes": [
      "Pomelo",
      "Mandarina",
      "Bergamota",
      "Lavanda",
      "Cedro",
      "Cuero",
      "Almizcle",
      "Ámbar",
      "Madera"
    ],
    "ml": 200,
    "precioUSD": 35,
    "categoria": "disenador"
  },
  {
    "id": 379,
    "house": "HUGO BOSS",
    "name": "Hugo Boss Man Green",
    "image": "hugo-boss-hugo-man-125ml-m.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Manzana, Albahaca, Bayas de enebro",
    "notasCorazon": "Geranio, Salvia, Lavanda",
    "notasFondo": "Cedro, Sándalo, Musgo de roble",
    "notes": [
      "Manzana",
      "Albahaca",
      "Bayas de enebro",
      "Geranio",
      "Salvia",
      "Lavanda",
      "Cedro",
      "Sándalo",
      "Musgo de roble"
    ],
    "ml": 125,
    "precioUSD": 75,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 380,
    "house": "BOUCHERON",
    "name": "Boucheron Men Pour Homme",
    "image": "boucheron-pour-homme-collector-eau-de-parfum-100ml-m.webp",
    "familia": null,
    "tipo": "Eau de Parfum",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Bergamota, Limón, Cardamomo",
    "notasCorazon": "Iris, Cedro, Cuero",
    "notasFondo": "Ámbar, Almizcle, Sándalo",
    "notes": [
      "Bergamota",
      "Limón",
      "Cardamomo",
      "Iris",
      "Cedro",
      "Cuero",
      "Ámbar",
      "Almizcle",
      "Sándalo"
    ],
    "ml": 100,
    "precioUSD": 50,
    "categoria": "disenador"
  },
  {
    "id": 381,
    "house": "HALLOWEEN",
    "name": "Halloween Man Mystery",
    "image": "halloween-halloween-man-mystery-125ml-m.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Pomelo, Bergamota, Pimienta",
    "notasCorazon": "Cardamomo, Cedro, Incienso",
    "notasFondo": "Almizcle, Ámbar, Madera",
    "notes": [
      "Pomelo",
      "Bergamota",
      "Pimienta",
      "Cardamomo",
      "Cedro",
      "Incienso",
      "Almizcle",
      "Ámbar",
      "Madera"
    ],
    "ml": 125,
    "precioUSD": 55,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 382,
    "house": "BENETTON",
    "name": "Benetton Colors de Benetton Man Blue",
    "image": "benetton-colors-de-benetton-man-blue-100ml-m.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Bergamota, Pomelo, Limón",
    "notasCorazon": "Lavanda, Cedro, Geranio",
    "notasFondo": "Almizcle, Ámbar, Madera",
    "notes": [
      "Bergamota",
      "Pomelo",
      "Limón",
      "Lavanda",
      "Cedro",
      "Geranio",
      "Almizcle",
      "Ámbar",
      "Madera"
    ],
    "ml": 100,
    "precioUSD": 25,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 383,
    "house": "NAUTICA",
    "name": "Nautica Voyage",
    "image": "nautica-nautica-voyage-100ml-m.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Verde hoja, Manzana verde, Agua de mar",
    "notasCorazon": "Mimosa, Loto, Cedro",
    "notasFondo": "Almizcle, Musgo de roble, Ámbar",
    "notes": [
      "Verde hoja",
      "Manzana verde",
      "Agua de mar",
      "Mimosa",
      "Loto",
      "Cedro",
      "Almizcle",
      "Musgo de roble",
      "Ámbar"
    ],
    "ml": 100,
    "precioUSD": 25,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 384,
    "house": "CALVIN",
    "name": "Calvin Klein Escape for Men",
    "image": "calvin-klein-escape-for-men-100ml-m.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Bergamota, Menta, Pimienta",
    "notasCorazon": "Cedro, Lavanda, Nuez moscada",
    "notasFondo": "Almizcle, Ámbar, Sándalo",
    "notes": [
      "Bergamota",
      "Menta",
      "Pimienta",
      "Cedro",
      "Lavanda",
      "Nuez moscada",
      "Almizcle",
      "Ámbar",
      "Sándalo"
    ],
    "ml": 100,
    "precioUSD": 45,
    "categoria": "disenador"
  },
  {
    "id": 385,
    "house": "MONTBLANC",
    "name": "Montblanc Emblem",
    "image": "montblanc-emblem-100ml-m.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Bergamota, Limón, Jengibre",
    "notasCorazon": "Lavanda, Cedro, Salvia",
    "notasFondo": "Almizcle, Ámbar, Madera",
    "notes": [
      "Bergamota",
      "Limón",
      "Jengibre",
      "Lavanda",
      "Cedro",
      "Salvia",
      "Almizcle",
      "Ámbar",
      "Madera"
    ],
    "ml": 100,
    "precioUSD": 60,
    "categoria": "disenador"
  },
  {
    "id": 386,
    "house": "MONTBLANC",
    "name": "Montblanc Presence",
    "image": "montblanc-presence-75ml-m.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Bergamota, Lavanda, Limón",
    "notasCorazon": "Geranio, Cedro, Rosa",
    "notasFondo": "Almizcle, Ámbar, Sándalo",
    "notes": [
      "Bergamota",
      "Lavanda",
      "Limón",
      "Geranio",
      "Cedro",
      "Rosa",
      "Almizcle",
      "Ámbar",
      "Sándalo"
    ],
    "ml": 75,
    "precioUSD": 40,
    "categoria": "disenador"
  },
  {
    "id": 387,
    "house": "SALVATORE FERRAGAMO",
    "name": "Salvatore Ferragamo F by Ferragamo Black",
    "image": "salvatore-ferragamo-f-by-ferragamo-black-100ml-m.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Bergamota, Lavanda, Cardamomo",
    "notasCorazon": "Cedro, Vetiver, Pachulí",
    "notasFondo": "Almizcle, Ámbar, Madera",
    "notes": [
      "Bergamota",
      "Lavanda",
      "Cardamomo",
      "Cedro",
      "Vetiver",
      "Pachulí",
      "Almizcle",
      "Ámbar",
      "Madera"
    ],
    "ml": 100,
    "precioUSD": 45,
    "categoria": "disenador"
  },
  {
    "id": 388,
    "house": "CALVIN",
    "name": "Calvin Klein Obsession for Men",
    "image": "calvin-klein-obsession-for-men-125ml-m.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Mandarina, Bergamota, Lavanda",
    "notasCorazon": "Cedro, Sándalo, Vetiver",
    "notasFondo": "Almizcle, Ámbar, Vainilla",
    "notes": [
      "Mandarina",
      "Bergamota",
      "Lavanda",
      "Cedro",
      "Sándalo",
      "Vetiver",
      "Almizcle",
      "Ámbar",
      "Vainilla"
    ],
    "ml": 125,
    "precioUSD": 35,
    "categoria": "disenador"
  },
  {
    "id": 389,
    "house": "HUGO BOSS",
    "name": "Hugo Boss Hugo Dark Blue",
    "image": "hugo-boss-hugo-dark-blue-75ml-m.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Bergamota, Lima, Menta",
    "notasCorazon": "Lavanda, Cedro, Salvia",
    "notasFondo": "Almizcle, Ámbar, Vetiver",
    "notes": [
      "Bergamota",
      "Lima",
      "Menta",
      "Lavanda",
      "Cedro",
      "Salvia",
      "Almizcle",
      "Ámbar",
      "Vetiver"
    ],
    "ml": 75,
    "precioUSD": 55,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 390,
    "house": "HALLOWEEN",
    "name": "Halloween Man",
    "image": "halloween-halloween-man-125ml-m.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Bergamota, Pomelo, Lavanda",
    "notasCorazon": "Cedro, Iris, Geranio",
    "notasFondo": "Almizcle, Ámbar, Madera",
    "notes": [
      "Bergamota",
      "Pomelo",
      "Lavanda",
      "Cedro",
      "Iris",
      "Geranio",
      "Almizcle",
      "Ámbar",
      "Madera"
    ],
    "ml": 125,
    "precioUSD": 55,
    "categoria": "disenador"
  },
  {
    "id": 391,
    "house": "ANTONIO BANDERAS",
    "name": "Antonio Banderas Blue Seduction",
    "image": "antonio-banderas-blue-seduction-100ml-m.webp",
    "familia": "Oriental Fougère ",
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": "Una fragancia fresca y afrutada que abre con un contraste jugoso entre el melón dulce, la bergamota cítrica y un toque verde de menta, realzado por grosellas negras. En el corazón, una brisa marina se mezcla con manzana verde, capuchino y cardamomo, creando un carácter sofisticado y ligeramente especiado. La base de notas amaderadas y ámbar deja una estela cálida y sensual. Blue Seduction combina frescura y seducción en un mismo aliento — ideal para el día a día con carácter propio.",
    "description": "Una fragancia fresca y afrutada que abre con un contraste jugoso entre el melón dulce, la bergamota cítrica y un toque verde de menta, realzado por grosellas negras. En el corazón, una brisa marina se mezcla con manzana verde, capuchino y cardamomo, creando un carácter sofisticado y ligeramente especiado. La base de notas amaderadas y ámbar deja una estela cálida y sensual. Blue Seduction combina frescura y seducción en un mismo aliento — ideal para el día a día con carácter propio.",
    "notasSalida": "melón, bergamota, menta, grosellas negras",
    "notasCorazon": "agua de mar, manzana verde, capuchino, cardamomo, nuez moscada",
    "notasFondo": "notas amaderadas, ambar",
    "notes": [
      "melón",
      "bergamota",
      "menta",
      "grosellas negras",
      "agua de mar",
      "manzana verde",
      "capuchino",
      "cardamomo",
      "nuez moscada",
      "notas amaderadas",
      "ambar"
    ],
    "ml": 100,
    "precioUSD": 25,
    "categoria": "disenador",
    "variantIds": [
      447
    ]
  },
  {
    "id": 392,
    "house": "BENETTON",
    "name": "Benetton Colors Man Black",
    "image": "benetton-colors-man-black-100ml-m.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Bergamota, Pomelo, Pimienta",
    "notasCorazon": "Cuero, Cedro, Vetiver",
    "notasFondo": "Almizcle, Ámbar, Madera",
    "notes": [
      "Bergamota",
      "Pomelo",
      "Pimienta",
      "Cuero",
      "Cedro",
      "Vetiver",
      "Almizcle",
      "Ámbar",
      "Madera"
    ],
    "ml": 100,
    "precioUSD": 25,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 393,
    "house": "GUESS",
    "name": "Guess Seductive Homme",
    "image": "guess-guess-seductive-homme-100ml-m.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Pomelo, Bergamota, Limón",
    "notasCorazon": "Lavanda, Cedro, Geranio",
    "notasFondo": "Almizcle, Ámbar, Madera",
    "notes": [
      "Pomelo",
      "Bergamota",
      "Limón",
      "Lavanda",
      "Cedro",
      "Geranio",
      "Almizcle",
      "Ámbar",
      "Madera"
    ],
    "ml": 100,
    "precioUSD": 35,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 394,
    "house": "HUGO BOSS",
    "name": "Hugo Boss Hugo XY",
    "image": "hugo-boss-hugo-xy-100ml-m.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Bergamota, Cardamomo, Pimienta",
    "notasCorazon": "Lavanda, Cedro, Cuero",
    "notasFondo": "Almizcle, Ámbar, Madera",
    "notes": [
      "Bergamota",
      "Cardamomo",
      "Pimienta",
      "Lavanda",
      "Cedro",
      "Cuero",
      "Almizcle",
      "Ámbar",
      "Madera"
    ],
    "ml": 100,
    "precioUSD": 45,
    "categoria": "disenador"
  },
  {
    "id": 395,
    "house": "SALVATORE FERRAGAMO",
    "name": "Salvatore Ferragamo Uomo",
    "image": "salvatore-ferragamo-uomo-salvatore-ferragamo-100ml-m.webp",
    "familia": null,
    "tipo": "Eau de Parfum",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Bergamota, Mandarina, Laurel",
    "notasCorazon": "Iris florentino, Lavanda, Vetiver",
    "notasFondo": "Sándalo, Almizcle, Ámbar",
    "notes": [
      "Bergamota",
      "Mandarina",
      "Laurel",
      "Iris florentino",
      "Lavanda",
      "Vetiver",
      "Sándalo",
      "Almizcle",
      "Ámbar"
    ],
    "ml": 100,
    "precioUSD": 60,
    "categoria": "disenador"
  },
  {
    "id": 396,
    "house": "NAUTICA",
    "name": "Nautica",
    "image": "nautica-nautica-100ml-m.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Bergamota, Pomelo, Menta acuática",
    "notasCorazon": "Lavanda, Cedro, Algas marinas",
    "notasFondo": "Almizcle, Ámbar, Madera",
    "notes": [
      "Bergamota",
      "Pomelo",
      "Menta acuática",
      "Lavanda",
      "Cedro",
      "Algas marinas",
      "Almizcle",
      "Ámbar",
      "Madera"
    ],
    "ml": 100,
    "precioUSD": 25,
    "categoria": "disenador"
  },
  {
    "id": 397,
    "house": "DAVIDOFF",
    "name": "Davidoff Cool Water",
    "image": "davidoff-cool-water-125ml-m.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Menta de agua, Romero, Lavanda",
    "notasCorazon": "Geranio, Jazmín, Menta",
    "notasFondo": "Cedro, Almizcle, Tabaco",
    "notes": [
      "Menta de agua",
      "Romero",
      "Lavanda",
      "Geranio",
      "Jazmín",
      "Menta",
      "Cedro",
      "Almizcle",
      "Tabaco"
    ],
    "ml": 200,
    "precioUSD": 55,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 398,
    "house": "MONTBLANC",
    "name": "Montblanc Legend Eau de Parfum",
    "image": "montblanc-legend-eau-de-parfum-100ml-m.webp",
    "familia": null,
    "tipo": "Eau de Parfum",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Bergamota, Cardamomo, Lavanda",
    "notasCorazon": "Oud, Salvia, Cedro",
    "notasFondo": "Vetiver, Almizcle, Ámbar",
    "notes": [
      "Bergamota",
      "Cardamomo",
      "Lavanda",
      "Oud",
      "Salvia",
      "Cedro",
      "Vetiver",
      "Almizcle",
      "Ámbar"
    ],
    "ml": 100,
    "precioUSD": 80,
    "categoria": "disenador"
  },
  {
    "id": 399,
    "house": "ARAMIS",
    "name": "Aramis New West for Him",
    "image": "aramis-new-west-for-him-100ml-m.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Bergamota, Lavanda, Menta",
    "notasCorazon": "Geranio, Cedro, Musgo marino",
    "notasFondo": "Almizcle, Ámbar, Madera",
    "notes": [
      "Bergamota",
      "Lavanda",
      "Menta",
      "Geranio",
      "Cedro",
      "Musgo marino",
      "Almizcle",
      "Ámbar",
      "Madera"
    ],
    "ml": 100,
    "precioUSD": 55,
    "categoria": "disenador"
  },
  {
    "id": 400,
    "house": "BENETTON",
    "name": "Benetton Cold Silver",
    "image": "benetton-cold-silver-100ml-m.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Bergamota, Pomelo, Menta",
    "notasCorazon": "Lavanda, Cedro, Geranio",
    "notasFondo": "Almizcle, Ámbar, Madera",
    "notes": [
      "Bergamota",
      "Pomelo",
      "Menta",
      "Lavanda",
      "Cedro",
      "Geranio",
      "Almizcle",
      "Ámbar",
      "Madera"
    ],
    "ml": 100,
    "precioUSD": 20,
    "categoria": "disenador"
  },
  {
    "id": 401,
    "house": "MAISON ALHAMBRA",
    "name": "Maison Alhambra Glacier Pour Homme",
    "image": "maison-alhambra-glacier-pour-homme-100ml-m.webp",
    "familia": null,
    "tipo": "Eau de Parfum",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Bergamota, Lima, Pomelo",
    "notasCorazon": "Lavanda, Cedro, Notas acuáticas",
    "notasFondo": "Almizcle, Ámbar, Madera",
    "notes": [
      "Bergamota",
      "Lima",
      "Pomelo",
      "Lavanda",
      "Cedro",
      "Notas acuáticas",
      "Almizcle",
      "Ámbar",
      "Madera"
    ],
    "ml": 100,
    "precioUSD": 30,
    "categoria": "arabes",
    "agotado": true
  },
  {
    "id": 402,
    "house": "COFINLUXE",
    "name": "Cofinluxe Taxi",
    "image": "cofinluxe-taxi-100ml-m.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Bergamota, Limón, Menta",
    "notasCorazon": "Lavanda, Geranio, Cedro",
    "notasFondo": "Almizcle, Ámbar, Madera",
    "notes": [
      "Bergamota",
      "Limón",
      "Menta",
      "Lavanda",
      "Geranio",
      "Cedro",
      "Almizcle",
      "Ámbar",
      "Madera"
    ],
    "ml": 100,
    "precioUSD": 20,
    "categoria": "disenador"
  },
  {
    "id": 403,
    "house": "HALLOWEEN",
    "name": "Halloween Man X",
    "image": "halloween-halloween-man-x-125ml-m.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Bergamota, Pomelo, Menta",
    "notasCorazon": "Cedro, Salvia, Incienso",
    "notasFondo": "Almizcle, Ámbar, Madera",
    "notes": [
      "Bergamota",
      "Pomelo",
      "Menta",
      "Cedro",
      "Salvia",
      "Incienso",
      "Almizcle",
      "Ámbar",
      "Madera"
    ],
    "ml": 125,
    "precioUSD": 55,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 404,
    "house": "JOOP!",
    "name": "Joop! Homme",
    "image": "joop-joop-homme-125ml-m.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Naranja, Bergamota, Lavanda",
    "notasCorazon": "Canela, Jazmín, Heliotropo",
    "notasFondo": "Sándalo, Almizcle, Vainilla",
    "notes": [
      "Naranja",
      "Bergamota",
      "Lavanda",
      "Canela",
      "Jazmín",
      "Heliotropo",
      "Sándalo",
      "Almizcle",
      "Vainilla"
    ],
    "ml": 125,
    "precioUSD": 35,
    "categoria": "disenador"
  },
  {
    "id": 405,
    "house": "GUESS",
    "name": "Guess 1981 Los Angeles Men",
    "image": "guess-guess-1981-los-angeles-men-100ml-m.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Bergamota, Limón, Pimienta",
    "notasCorazon": "Lavanda, Cedro, Salvia",
    "notasFondo": "Almizcle, Ámbar, Madera",
    "notes": [
      "Bergamota",
      "Limón",
      "Pimienta",
      "Lavanda",
      "Cedro",
      "Salvia",
      "Almizcle",
      "Ámbar",
      "Madera"
    ],
    "ml": 100,
    "precioUSD": 35,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 406,
    "house": "HUGO BOSS",
    "name": "Hugo Boss Boss Selection",
    "image": "hugo-boss-boss-selection-100ml-m.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Bergamota, Naranja, Cardamomo",
    "notasCorazon": "Salvia, Cedro, Sándalo",
    "notasFondo": "Vetiver, Almizcle, Ámbar",
    "notes": [
      "Bergamota",
      "Naranja",
      "Cardamomo",
      "Salvia",
      "Cedro",
      "Sándalo",
      "Vetiver",
      "Almizcle",
      "Ámbar"
    ],
    "ml": 100,
    "precioUSD": 45,
    "categoria": "disenador"
  },
  {
    "id": 407,
    "house": "HUGO BOSS",
    "name": "Hugo Boss Unlimited",
    "image": "hugo-boss-hugo-boss-unlimited-100ml-m.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Limón, Bergamota, Mandarina",
    "notasCorazon": "Lavanda, Cedro, Madera",
    "notasFondo": "Almizcle, Ámbar, Sándalo",
    "notes": [
      "Limón",
      "Bergamota",
      "Mandarina",
      "Lavanda",
      "Cedro",
      "Madera",
      "Almizcle",
      "Ámbar",
      "Sándalo"
    ],
    "ml": 100,
    "precioUSD": 65,
    "categoria": "disenador"
  },
  {
    "id": 408,
    "house": "HUGO BOSS",
    "name": "Hugo Boss Hugo Energise",
    "image": "hugo-boss-hugo-energise-eau-de-toilette-75ml-m.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Bergamota, Lima, Menta",
    "notasCorazon": "Lavanda, Cedro, Geranio",
    "notasFondo": "Almizcle, Ámbar, Madera",
    "notes": [
      "Bergamota",
      "Lima",
      "Menta",
      "Lavanda",
      "Cedro",
      "Geranio",
      "Almizcle",
      "Ámbar",
      "Madera"
    ],
    "ml": 75,
    "precioUSD": 45,
    "categoria": "disenador"
  },
  {
    "id": 409,
    "house": "SEAN",
    "name": "Sean John I Am King",
    "image": "sean-john-i-am-king-100ml-m.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Menta, Bergamota, Lavanda",
    "notasCorazon": "Cedro, Geranio, Sándalo",
    "notasFondo": "Almizcle, Ámbar, Madera",
    "notes": [
      "Menta",
      "Bergamota",
      "Lavanda",
      "Cedro",
      "Geranio",
      "Sándalo",
      "Almizcle",
      "Ámbar",
      "Madera"
    ],
    "ml": 100,
    "precioUSD": 45,
    "categoria": "disenador"
  },
  {
    "id": 410,
    "house": "ANTONIO BANDERAS",
    "name": "Antonio Banderas Power of Seduction",
    "image": "antonio-banderas-power-of-seduction-100ml-m.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Bergamota, Pomelo, Cardamomo",
    "notasCorazon": "Lavanda, Cedro, Salvia",
    "notasFondo": "Almizcle, Ámbar, Madera",
    "notes": [
      "Bergamota",
      "Pomelo",
      "Cardamomo",
      "Lavanda",
      "Cedro",
      "Salvia",
      "Almizcle",
      "Ámbar",
      "Madera"
    ],
    "ml": 100,
    "precioUSD": 25,
    "categoria": "disenador"
  },
  {
    "id": 411,
    "house": "BENETTON",
    "name": "Benetton Cold",
    "image": "benetton-benetton-cold-100ml-m.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Bergamota, Pomelo, Menta",
    "notasCorazon": "Lavanda, Cedro, Geranio",
    "notasFondo": "Almizcle, Ámbar, Madera",
    "notes": [
      "Bergamota",
      "Pomelo",
      "Menta",
      "Lavanda",
      "Cedro",
      "Geranio",
      "Almizcle",
      "Ámbar",
      "Madera"
    ],
    "ml": 100,
    "precioUSD": 20,
    "categoria": "disenador"
  },
  {
    "id": 412,
    "house": "ANTONIO BANDERAS",
    "name": "Antonio Banderas The Icon Eau de Parfum",
    "image": "antonio-banderas-the-icon-eau-de-parfum-100ml-m.webp",
    "familia": null,
    "tipo": "Eau de Parfum",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Bergamota, Mandarina, Pimienta rosa",
    "notasCorazon": "Salvia, Cedro, Cuero",
    "notasFondo": "Pachulí, Ámbar, Almizcle",
    "notes": [
      "Bergamota",
      "Mandarina",
      "Pimienta rosa",
      "Salvia",
      "Cedro",
      "Cuero",
      "Pachulí",
      "Ámbar",
      "Almizcle"
    ],
    "ml": 100,
    "precioUSD": 30,
    "categoria": "disenador"
  },
  {
    "id": 413,
    "house": "NAUTICA",
    "name": "Nautica Blue",
    "image": "nautica-nautica-blue-100ml-m.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Bergamota, Lima, Pomelo",
    "notasCorazon": "Lavanda, Cedro, Geranio",
    "notasFondo": "Almizcle, Ámbar, Madera",
    "notes": [
      "Bergamota",
      "Lima",
      "Pomelo",
      "Lavanda",
      "Cedro",
      "Geranio",
      "Almizcle",
      "Ámbar",
      "Madera"
    ],
    "ml": 100,
    "precioUSD": 25,
    "categoria": "disenador"
  },
  {
    "id": 414,
    "house": "ADIDAS",
    "name": "Adidas Dynamic Pulse",
    "image": "adidas-dynamic-pulse-eau-de-toilette-100ml-m.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Bergamota, Lima, Menta",
    "notasCorazon": "Lavanda, Cedro, Geranio",
    "notasFondo": "Almizcle, Ámbar, Madera",
    "notes": [
      "Bergamota",
      "Lima",
      "Menta",
      "Lavanda",
      "Cedro",
      "Geranio",
      "Almizcle",
      "Ámbar",
      "Madera"
    ],
    "ml": 100,
    "precioUSD": 15,
    "categoria": "disenador"
  },
  {
    "id": 415,
    "house": "GUESS",
    "name": "Guess Man Gold",
    "image": "guess-guess-man-gold-75ml-m.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Bergamota, Lima, Cardamomo",
    "notasCorazon": "Lavanda, Cedro, Iris",
    "notasFondo": "Almizcle, Ámbar, Sándalo",
    "notes": [
      "Bergamota",
      "Lima",
      "Cardamomo",
      "Lavanda",
      "Cedro",
      "Iris",
      "Almizcle",
      "Ámbar",
      "Sándalo"
    ],
    "ml": 75,
    "precioUSD": 35,
    "categoria": "disenador"
  },
  {
    "id": 416,
    "house": "ESTÉE LAUDER",
    "name": "Estée Lauder Lauder for Men",
    "image": "este-lauder-lauder-for-men-100ml-m.webp",
    "familia": null,
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "Bergamota, Lavanda, Cedro",
    "notasCorazon": "Geranio, Roble, Iris",
    "notasFondo": "Almizcle, Vetiver, Ámbar",
    "notes": [
      "Bergamota",
      "Lavanda",
      "Cedro",
      "Geranio",
      "Roble",
      "Iris",
      "Almizcle",
      "Vetiver",
      "Ámbar"
    ],
    "ml": 100,
    "precioUSD": 40,
    "categoria": "disenador"
  },
  {
    "id": 417,
    "house": "BENETTON",
    "name": "Benetton Sisterland Pink Raspberry",
    "image": "benetton-sisterland-pink-raspberry-80ml-f.webp",
    "familia": "Floral Frutal",
    "tipo": "Eau de Toilette",
    "genero": "Femenino",
    "descripcion": "Sisterland Pink Raspberry de United Colors of Benetton es una fragancia para la mujer fresca, frutal y femenina, parte de la colección Sisterland presentada en un llamativo frasco con forma de muñeca. Su apertura vibrante de frambuesa y lichi evoluciona hacia un corazón floral de peonía y rosa, cerrando con una base suave de sándalo y almizcle. Juguetona, moderna y muy fácil de llevar en el día a día.",
    "description": "Sisterland Pink Raspberry de United Colors of Benetton es una fragancia para la mujer fresca, frutal y femenina, parte de la colección Sisterland presentada en un llamativo frasco con forma de muñeca. Su apertura vibrante de frambuesa y lichi evoluciona hacia un corazón floral de peonía y rosa, cerrando con una base suave de sándalo y almizcle. Juguetona, moderna y muy fácil de llevar en el día a día.",
    "notasSalida": "Frambuesa, Lichi",
    "notasCorazon": "Peonía, Rosa",
    "notasFondo": "Sándalo, Almizcle",
    "notes": [
      "Frambuesa",
      "Lichi",
      "Peonía",
      "Rosa",
      "Sándalo",
      "Almizcle"
    ],
    "ml": 80,
    "precioUSD": 25,
    "categoria": "disenador"
  },
  {
    "id": 418,
    "house": "ARABIYAT SUGAR",
    "name": "Arabiyat Sugar Strawberry Tres Leches",
    "image": "arabiyat-sugar-strawberry-tres-leches-100ml-f.webp",
    "familia": "Gourmand",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Strawberry Tres Leches de Arabiyat Sugar es una fragancia gourmand para la mujer, inspirada en el clásico postre de tres leches con fresa. Lanzada en 2025, su apertura dulce y frutal de fresa, nectarina y leche evoluciona hacia un corazón cremoso de malvavisco, caramelo y flor de azahar, cerrando con una base de vainilla, ambergris y almizcle blanco. Adictiva, lúdica y de gran proyección.",
    "description": "Strawberry Tres Leches de Arabiyat Sugar es una fragancia gourmand para la mujer, inspirada en el clásico postre de tres leches con fresa. Lanzada en 2025, su apertura dulce y frutal de fresa, nectarina y leche evoluciona hacia un corazón cremoso de malvavisco, caramelo y flor de azahar, cerrando con una base de vainilla, ambergris y almizcle blanco. Adictiva, lúdica y de gran proyección.",
    "notasSalida": "Fresa, Nectarina, Leche",
    "notasCorazon": "Freesia, Flor de azahar, Malvavisco, Caramelo",
    "notasFondo": "Dulce de leche, Vainilla, Almizcle blanco, Ambergris",
    "notes": [
      "Fresa",
      "Nectarina",
      "Leche",
      "Freesia",
      "Flor de azahar",
      "Malvavisco",
      "Caramelo",
      "Dulce de leche",
      "Vainilla",
      "Almizcle blanco",
      "Ambergris"
    ],
    "ml": 100,
    "precioUSD": 45,
    "categoria": "arabes",
    "agotado": true
  },
  {
    "id": 419,
    "house": "ARABIYAT SUGAR",
    "name": "Arabiyat Sugar Coconut Chiffon",
    "image": "arabiyat-sugar-coconut-chiffon-100ml-u.webp",
    "familia": "Gourmand",
    "tipo": "Eau de Parfum",
    "genero": "Unisex",
    "descripcion": "Coconut Chiffon de Arabiyat Sugar es una fragancia unisex oriental vainillada, lanzada en 2025 y creada por el perfumista Lucas Sieuzac. Su corazón de coco cremoso y jazmín sobre una base de mantequilla, caramelo y vainilla recrea a la perfección la textura de un pastel chiffon de coco. Suave, golosa y envolvente, ideal para quienes aman los aromas cálidos y cremosos.",
    "description": "Coconut Chiffon de Arabiyat Sugar es una fragancia unisex oriental vainillada, lanzada en 2025 y creada por el perfumista Lucas Sieuzac. Su corazón de coco cremoso y jazmín sobre una base de mantequilla, caramelo y vainilla recrea a la perfección la textura de un pastel chiffon de coco. Suave, golosa y envolvente, ideal para quienes aman los aromas cálidos y cremosos.",
    "notasSalida": "Coco, Azúcar",
    "notasCorazon": "Jazmín, Mantequilla",
    "notasFondo": "Vainilla, Caramelo, Almizcle",
    "notes": [
      "Coco",
      "Azúcar",
      "Jazmín",
      "Mantequilla",
      "Vainilla",
      "Caramelo",
      "Almizcle"
    ],
    "ml": 100,
    "precioUSD": 45,
    "categoria": "arabes"
  },
  {
    "id": 420,
    "house": "ARABIYAT SUGAR",
    "name": "Arabiyat Sugar Lemon Sorbet",
    "image": "arabiyat-sugar-lemon-sorbet-100ml-u.webp",
    "familia": "Gourmand",
    "tipo": "Eau de Parfum",
    "genero": "Unisex",
    "descripcion": "Lemon Sorbet de Arabiyat Sugar es una fragancia unisex gourmand y cítrica, lanzada en 2025 y creada por el perfumista Mustafa Firoz. Su apertura brillante de limón con un toque de ron evoluciona hacia un corazón gourmand dulce que recuerda al postre, cerrando con una base cálida de vainilla. Fresca, veraniega y deliciosamente golosa.",
    "description": "Lemon Sorbet de Arabiyat Sugar es una fragancia unisex gourmand y cítrica, lanzada en 2025 y creada por el perfumista Mustafa Firoz. Su apertura brillante de limón con un toque de ron evoluciona hacia un corazón gourmand dulce que recuerda al postre, cerrando con una base cálida de vainilla. Fresca, veraniega y deliciosamente golosa.",
    "notasSalida": "Limón, Ron",
    "notasCorazon": "Acorde Gourmand",
    "notasFondo": "Vainilla",
    "notes": [
      "Limón",
      "Ron",
      "Acorde Gourmand",
      "Vainilla"
    ],
    "ml": 100,
    "precioUSD": 45,
    "categoria": "arabes"
  },
  {
    "id": 421,
    "house": "ARABIYAT SUGAR",
    "name": "Arabiyat Sugar Mango Affogato",
    "image": "arabiyat-sugar-mango-affogato-100ml-u.webp",
    "familia": "Oriental Especiado",
    "tipo": "Eau de Parfum",
    "genero": "Unisex",
    "descripcion": "Mango Affogato de Arabiyat Sugar es una fragancia unisex oriental especiada, lanzada en 2025 y creada por la perfumista Ana Gomez. Su apertura de mango maduro especiado con nuez moscada y clavo evoluciona hacia un corazón de cuero, azafrán y ámbar, cerrando con una base oscura de akigalawood, pachulí y vetiver. Una propuesta madura y sofisticada que contrasta lo dulce tropical con la intensidad oriental.",
    "description": "Mango Affogato de Arabiyat Sugar es una fragancia unisex oriental especiada, lanzada en 2025 y creada por la perfumista Ana Gomez. Su apertura de mango maduro especiado con nuez moscada y clavo evoluciona hacia un corazón de cuero, azafrán y ámbar, cerrando con una base oscura de akigalawood, pachulí y vetiver. Una propuesta madura y sofisticada que contrasta lo dulce tropical con la intensidad oriental.",
    "notasSalida": "Mango, Nuez moscada, Clavo",
    "notasCorazon": "Cuero, Azafrán, Ámbar",
    "notasFondo": "Musgo, Akigalawood, Cipriol, Pachulí, Vetiver",
    "notes": [
      "Mango",
      "Nuez moscada",
      "Clavo",
      "Cuero",
      "Azafrán",
      "Ámbar",
      "Musgo",
      "Akigalawood",
      "Cipriol",
      "Pachulí",
      "Vetiver"
    ],
    "ml": 100,
    "precioUSD": 45,
    "categoria": "arabes"
  },
  {
    "id": 422,
    "house": "ARABIYAT SUGAR",
    "name": "Arabiyat Sugar Cotton Blush",
    "image": "arabiyat-sugar-cotton-blush-100ml-f.webp",
    "familia": "Floral Gourmand",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Cotton Blush de Arabiyat Sugar es una fragancia para la mujer floral frutal gourmand, lanzada en 2025 y creada por Lucas Sieuzac. Su apertura cremosa de coco con frambuesa y fresa evoluciona hacia un corazón floral de rosa, peonía y malvavisco rosa, cerrando con una base cálida de vainilla, ámbar y almizcle. Dulce, femenina y muy adictiva.",
    "description": "Cotton Blush de Arabiyat Sugar es una fragancia para la mujer floral frutal gourmand, lanzada en 2025 y creada por Lucas Sieuzac. Su apertura cremosa de coco con frambuesa y fresa evoluciona hacia un corazón floral de rosa, peonía y malvavisco rosa, cerrando con una base cálida de vainilla, ámbar y almizcle. Dulce, femenina y muy adictiva.",
    "notasSalida": "Coco, Frambuesa, Fresa",
    "notasCorazon": "Rosa, Peonía, Malvavisco",
    "notasFondo": "Vainilla, Ámbar, Almizcle",
    "notes": [
      "Coco",
      "Frambuesa",
      "Fresa",
      "Rosa",
      "Peonía",
      "Malvavisco",
      "Vainilla",
      "Ámbar",
      "Almizcle"
    ],
    "ml": 100,
    "precioUSD": 45,
    "categoria": "arabes",
    "agotado": true
  },
  {
    "id": 423,
    "house": "RASASI",
    "name": "Rasasi Hawas for Her Diva",
    "image": "rasasi-hawas-for-her-diva-100ml-f.webp",
    "familia": "Floral Oriental",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Hawas for Her Diva de Rasasi es una fragancia para la mujer floral oriental intensa y seductora, lanzada en 2025. Su apertura vibrante y ácida de frutas rojas, ruibarbo y lichi evoluciona hacia un corazón floral especiado de rosa e incienso sobre cedro, cerrando con una base cálida y sensual de vainilla, almizcle y ambergris. Una fragancia inspirada en Valentino Donna Born in Roma Intense, con gran proyección y carácter.",
    "description": "Hawas for Her Diva de Rasasi es una fragancia para la mujer floral oriental intensa y seductora, lanzada en 2025. Su apertura vibrante y ácida de frutas rojas, ruibarbo y lichi evoluciona hacia un corazón floral especiado de rosa e incienso sobre cedro, cerrando con una base cálida y sensual de vainilla, almizcle y ambergris. Una fragancia inspirada en Valentino Donna Born in Roma Intense, con gran proyección y carácter.",
    "notasSalida": "Frutas rojas, Ruibarbo, Lichi",
    "notasCorazon": "Rosa, Incienso, Cedro",
    "notasFondo": "Vainilla, Almizcle, Ambergris",
    "notes": [
      "Frutas rojas",
      "Ruibarbo",
      "Lichi",
      "Rosa",
      "Incienso",
      "Cedro",
      "Vainilla",
      "Almizcle",
      "Ambergris"
    ],
    "ml": 100,
    "precioUSD": 55,
    "categoria": "arabes"
  },
  {
    "id": 424,
    "house": "LATTAFA",
    "name": "Lattafa Musamam White Intense",
    "image": "lattafa-musamam-white-intense-100ml-u.webp",
    "familia": "Floral Oriental",
    "tipo": "Eau de Parfum",
    "genero": "Unisex",
    "descripcion": "Musamam White Intense de Lattafa es una fragancia unisex oriental floral, lanzada en 2023 y creada por el perfumista Gaël Montero. Su apertura cítrica de bergamota y naranja con especias evoluciona hacia un corazón tropical y seductor de coco, ylang ylang y ambroxan, cerrando con una base cálida y cremosa de sándalo y benjuí. Elegante, sensual y con excelente longevidad.",
    "description": "Musamam White Intense de Lattafa es una fragancia unisex oriental floral, lanzada en 2023 y creada por el perfumista Gaël Montero. Su apertura cítrica de bergamota y naranja con especias evoluciona hacia un corazón tropical y seductor de coco, ylang ylang y ambroxan, cerrando con una base cálida y cremosa de sándalo y benjuí. Elegante, sensual y con excelente longevidad.",
    "notasSalida": "Bergamota, Naranja, Especias",
    "notasCorazon": "Coco, Ylang ylang, Ambroxan, Mahonial",
    "notasFondo": "Sándalo, Almizcle, Benjuí",
    "notes": [
      "Bergamota",
      "Naranja",
      "Especias",
      "Coco",
      "Ylang ylang",
      "Ambroxan",
      "Mahonial",
      "Sándalo",
      "Almizcle",
      "Benjuí"
    ],
    "ml": 100,
    "precioUSD": 50,
    "categoria": "arabes",
    "agotado": true
  },
  {
    "id": 426,
    "house": "BHARARA",
    "name": "Bharara Queen",
    "image": "bharara-queen-100ml-f.webp",
    "familia": "Floral Amaderado",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Queen de Bharara es una fragancia para la mujer floral amaderada de carácter elegante y seductor. Su apertura cítrica especiada de bergamota, grosella negra y pimienta rosa evoluciona hacia un corazón floral de jazmín y rosa, cerrando con una base cálida y sofisticada de pachulí, ámbar y madera de cachemira. Confiada, poderosa y femenina.",
    "description": "Queen de Bharara es una fragancia para la mujer floral amaderada de carácter elegante y seductor. Su apertura cítrica especiada de bergamota, grosella negra y pimienta rosa evoluciona hacia un corazón floral de jazmín y rosa, cerrando con una base cálida y sofisticada de pachulí, ámbar y madera de cachemira. Confiada, poderosa y femenina.",
    "notasSalida": "Bergamota, Pimienta rosa, Grosella negra",
    "notasCorazon": "Jazmín, Rosa",
    "notasFondo": "Pachulí, Ámbar, Almizcle, Madera de cachemira",
    "notes": [
      "Bergamota",
      "Pimienta rosa",
      "Grosella negra",
      "Jazmín",
      "Rosa",
      "Pachulí",
      "Ámbar",
      "Almizcle",
      "Madera de cachemira"
    ],
    "ml": 100,
    "precioUSD": 85,
    "categoria": "arabes",
    "agotado": true
  },
  {
    "id": 427,
    "house": "ARMAF",
    "name": "Armaf Club de Nuit Intense Woman",
    "image": "armaf-club-de-nuit-intense-woman-105ml-f.webp",
    "familia": "Chipre Floral",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Club de Nuit Intense Woman de Armaf es una fragancia para la mujer chipre floral intensa y sofisticada, considerada una de las mejores alternativas al Tom Ford Black Orchid. Su apertura floral especiada de rosa, azafrán y geranio evoluciona hacia un corazón especiado con nuez moscada, pimienta y violeta, cerrando con una base oscura y lujosa de oud, pachulí, vainilla y ámbar. Gran proyección y estela para ocasiones especiales.",
    "description": "Club de Nuit Intense Woman de Armaf es una fragancia para la mujer chipre floral intensa y sofisticada, considerada una de las mejores alternativas al Tom Ford Black Orchid. Su apertura floral especiada de rosa, azafrán y geranio evoluciona hacia un corazón especiado con nuez moscada, pimienta y violeta, cerrando con una base oscura y lujosa de oud, pachulí, vainilla y ámbar. Gran proyección y estela para ocasiones especiales.",
    "notasSalida": "Rosa, Azafrán, Geranio",
    "notasCorazon": "Nuez moscada, Pimienta, Violeta, Alcaravea",
    "notasFondo": "Pachulí, Oud, Vainilla, Ámbar",
    "notes": [
      "Rosa",
      "Azafrán",
      "Geranio",
      "Nuez moscada",
      "Pimienta",
      "Violeta",
      "Alcaravea",
      "Pachulí",
      "Oud",
      "Vainilla",
      "Ámbar"
    ],
    "ml": 105,
    "precioUSD": 40,
    "categoria": "arabes"
  },
  {
    "id": 428,
    "house": "ARMAF",
    "name": "Armaf Odyssey Marshmallow",
    "image": "armaf-odyssey-marshmallow-100ml-u.webp",
    "familia": "Gourmand",
    "tipo": "Eau de Parfum",
    "genero": "Unisex",
    "descripcion": "Odyssey Marshmallow de Armaf es una fragancia unisex floral frutal gourmand, lanzada en 2025. Su apertura chispeante de manzana, coco y limón evoluciona hacia un corazón goloso de malvavisco, fresa, melocotón y frambuesa con florales suaves, cerrando con una base cremosa y adictiva de mascarpone, praliné, vainilla y haba tonka. Una de las mejores fragancias gourmand del año, inspirada en Kayali Yum Boujee Marshmallow.",
    "description": "Odyssey Marshmallow de Armaf es una fragancia unisex floral frutal gourmand, lanzada en 2025. Su apertura chispeante de manzana, coco y limón evoluciona hacia un corazón goloso de malvavisco, fresa, melocotón y frambuesa con florales suaves, cerrando con una base cremosa y adictiva de mascarpone, praliné, vainilla y haba tonka. Una de las mejores fragancias gourmand del año, inspirada en Kayali Yum Boujee Marshmallow.",
    "notasSalida": "Coco, Manzana, Limón",
    "notasCorazon": "Peonía, Lirio de los valles, Malvavisco, Fresa, Melocotón, Albaricoque, Frambuesa, Flor de azahar",
    "notasFondo": "Mascarpone, Vainilla, Praliné, Almizcle, Haba tonka, Ámbar",
    "notes": [
      "Coco",
      "Manzana",
      "Limón",
      "Peonía",
      "Lirio de los valles",
      "Malvavisco",
      "Fresa",
      "Melocotón",
      "Albaricoque",
      "Frambuesa",
      "Flor de azahar",
      "Mascarpone",
      "Vainilla",
      "Praliné",
      "Almizcle",
      "Haba tonka",
      "Ámbar"
    ],
    "ml": 100,
    "precioUSD": 45,
    "categoria": "arabes"
  },
  {
    "id": 429,
    "house": "BHARARA",
    "name": "Rome Paradox",
    "image": "mast-perfume-rome-paradox-100ml-f.webp",
    "familia": "Floral Frutal",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Rome Paradox de Mast Perfume es una fragancia para la mujer floral frutal sofisticada, de carácter fresco y femenino. Su apertura cítrica y frutal de bergamota, grosella negra y pera evoluciona hacia un corazón floral delicado de flores blancas, lirio de los valles y notas marinas, cerrando con una base cálida de ámbar y ambergris. Elegante y versátil para el uso diario.",
    "description": "Rome Paradox de Mast Perfume es una fragancia para la mujer floral frutal sofisticada, de carácter fresco y femenino. Su apertura cítrica y frutal de bergamota, grosella negra y pera evoluciona hacia un corazón floral delicado de flores blancas, lirio de los valles y notas marinas, cerrando con una base cálida de ámbar y ambergris. Elegante y versátil para el uso diario.",
    "notasSalida": "Bergamota, Grosella negra, Naranja dulce, Pimienta negra, Pera",
    "notasCorazon": "Flores blancas, Lirio de los valles, Heliotropo, Flor de azahar, Notas marinas",
    "notasFondo": "Ámbar, Ámbar oriental, Ambergris",
    "notes": [
      "Bergamota",
      "Grosella negra",
      "Naranja dulce",
      "Pimienta negra",
      "Pera",
      "Flores blancas",
      "Lirio de los valles",
      "Heliotropo",
      "Flor de azahar",
      "Notas marinas",
      "Ámbar",
      "Ámbar oriental",
      "Ambergris"
    ],
    "ml": 100,
    "precioUSD": 45,
    "categoria": "disenador"
  },
  {
    "id": 430,
    "house": "BHARARA",
    "name": "Rome Extradose Pour Femme",
    "image": "mast-perfume-rome-extradose-pour-femme-100ml-f.webp",
    "familia": "Floral Frutal",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Rome Extradose Pour Femme de Mast Perfume es una fragancia para la mujer floral frutal cremosa, de carácter vibrante y sensual. Su apertura cítrica de bergamota y limón evoluciona hacia un corazón tropical y floral de coco, ciruela, jazmín, rosa, piña y manzana, cerrando con una base cálida y sofisticada de benjuí, ámbar, vainilla, sándalo y haba tonka.",
    "description": "Rome Extradose Pour Femme de Mast Perfume es una fragancia para la mujer floral frutal cremosa, de carácter vibrante y sensual. Su apertura cítrica de bergamota y limón evoluciona hacia un corazón tropical y floral de coco, ciruela, jazmín, rosa, piña y manzana, cerrando con una base cálida y sofisticada de benjuí, ámbar, vainilla, sándalo y haba tonka.",
    "notasSalida": "Bergamota, Limón, Coco, Ciruela",
    "notasCorazon": "Jazmín, Rosa, Piña, Manzana",
    "notasFondo": "Benjuí, Ámbar, Vainilla, Sándalo, Haba tonka, Almizcle",
    "notes": [
      "Bergamota",
      "Limón",
      "Coco",
      "Ciruela",
      "Jazmín",
      "Rosa",
      "Piña",
      "Manzana",
      "Benjuí",
      "Ámbar",
      "Vainilla",
      "Sándalo",
      "Haba tonka",
      "Almizcle"
    ],
    "ml": 100,
    "precioUSD": 45,
    "categoria": "disenador"
  },
  {
    "id": 431,
    "house": "PERRY ELLIS",
    "name": "Perry Ellis 360 Coral",
    "image": "perry-ellis-360-coral-100ml-f.webp",
    "familia": "Floral Frutal",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Perry Ellis 360 Coral es una fragancia para la mujer floral frutal fresca y femenina, lanzada en 2014. Su apertura chispeante de bergamota, melocotón y pera con pimienta rosa evoluciona hacia un corazón floral de jazmín, peonía y lirio de los valles, cerrando con una base cálida de madera de cachemira, haba tonka y almizcle. Perfecta para el día a día en temporadas cálidas.",
    "description": "Perry Ellis 360 Coral es una fragancia para la mujer floral frutal fresca y femenina, lanzada en 2014. Su apertura chispeante de bergamota, melocotón y pera con pimienta rosa evoluciona hacia un corazón floral de jazmín, peonía y lirio de los valles, cerrando con una base cálida de madera de cachemira, haba tonka y almizcle. Perfecta para el día a día en temporadas cálidas.",
    "notasSalida": "Bergamota italiana, Prosecco de melocotón, Pera asiática, Pimienta rosa",
    "notasCorazon": "Jazmín blanco, Peonía, Lirio de los valles",
    "notasFondo": "Ámbar blanco, Madera de cachemira, Haba tonka, Almizcle",
    "notes": [
      "Bergamota italiana",
      "Prosecco de melocotón",
      "Pera asiática",
      "Pimienta rosa",
      "Jazmín blanco",
      "Peonía",
      "Lirio de los valles",
      "Ámbar blanco",
      "Madera de cachemira",
      "Haba tonka",
      "Almizcle"
    ],
    "ml": 100,
    "precioUSD": 45,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 432,
    "house": "BENETTON",
    "name": "Benetton Hot",
    "image": "benetton-hot-100ml-f.webp",
    "familia": "Floral Cítrico",
    "tipo": "Eau de Toilette",
    "genero": "Femenino",
    "descripcion": "Hot de United Colors of Benetton es una fragancia para la mujer floral cítrica vibrante y desenfadada, con un frasco transparente minimalista y tapa roja llamativa. Su apertura cítrica de pomelo y mandarina con pimienta rosa evoluciona hacia un corazón floral de violeta, jazmín y rosa, cerrando con una base suave de sándalo, ámbar y almizcle. Fresca, accesible y perfecta para el uso diario.",
    "description": "Hot de United Colors of Benetton es una fragancia para la mujer floral cítrica vibrante y desenfadada, con un frasco transparente minimalista y tapa roja llamativa. Su apertura cítrica de pomelo y mandarina con pimienta rosa evoluciona hacia un corazón floral de violeta, jazmín y rosa, cerrando con una base suave de sándalo, ámbar y almizcle. Fresca, accesible y perfecta para el uso diario.",
    "notasSalida": "Pomelo, Mandarina, Pimienta rosa",
    "notasCorazon": "Violeta, Jazmín, Rosa",
    "notasFondo": "Sándalo, Almizcle, Ámbar",
    "notes": [
      "Pomelo",
      "Mandarina",
      "Pimienta rosa",
      "Violeta",
      "Jazmín",
      "Rosa",
      "Sándalo",
      "Almizcle",
      "Ámbar"
    ],
    "ml": 100,
    "precioUSD": 20,
    "categoria": "disenador"
  },
  {
    "id": 433,
    "house": "ANIMALE",
    "name": "Animale",
    "image": "animale-animale-edp-100ml-f.webp",
    "familia": "Floral Oriental",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Animale de Animale es una fragancia para la mujer floral oriental vibrante y seductora. Su apertura cítrica luminosa de bergamota y naranja evoluciona hacia un corazón floral intenso de rosa, jazmín y ylang ylang, cerrando con una base cálida y sensual de sándalo, vetiver, ámbar y almizcle. Una fragancia femenina con carácter, ideal para noches elegantes y ocasiones especiales.",
    "description": "Animale de Animale es una fragancia para la mujer floral oriental vibrante y seductora. Su apertura cítrica luminosa de bergamota y naranja evoluciona hacia un corazón floral intenso de rosa, jazmín y ylang ylang, cerrando con una base cálida y sensual de sándalo, vetiver, ámbar y almizcle. Una fragancia femenina con carácter, ideal para noches elegantes y ocasiones especiales.",
    "notasSalida": "Bergamota, Naranja, Cítricos",
    "notasCorazon": "Rosa, Jazmín, Ylang ylang",
    "notasFondo": "Sándalo, Vetiver, Almizcle, Ámbar",
    "notes": [
      "Bergamota",
      "Naranja",
      "Cítricos",
      "Rosa",
      "Jazmín",
      "Ylang ylang",
      "Sándalo",
      "Vetiver",
      "Almizcle",
      "Ámbar"
    ],
    "ml": 100,
    "precioUSD": 50,
    "categoria": "disenador"
  },
  {
    "id": 434,
    "house": "TOMMY HILFIGER",
    "name": "Tommy Hilfiger Tommy Girl",
    "image": "tommy-hilfiger-tommy-girl-100ml-f.webp",
    "familia": "Floral Verde",
    "tipo": "Eau de Toilette",
    "genero": "Femenino",
    "descripcion": "Tommy Girl de Tommy Hilfiger es una fragancia para la mujer floral verde fresca e icónica, lanzada en 1996. Su apertura luminosa de mandarina, bergamota y pera con té negro evoluciona hacia un corazón floral de lirio, lirio de los valles y violeta, cerrando con una base limpia de cedro, sándalo y almizcle. Un clásico atemporal que evoca juventud, libertad y estilo americano.",
    "description": "Tommy Girl de Tommy Hilfiger es una fragancia para la mujer floral verde fresca e icónica, lanzada en 1996. Su apertura luminosa de mandarina, bergamota y pera con té negro evoluciona hacia un corazón floral de lirio, lirio de los valles y violeta, cerrando con una base limpia de cedro, sándalo y almizcle. Un clásico atemporal que evoca juventud, libertad y estilo americano.",
    "notasSalida": "Mandarina, Bergamota, Pera, Té negro",
    "notasCorazon": "Lirio, Lirio de los valles, Violeta",
    "notasFondo": "Cedro, Almizcle, Sándalo",
    "notes": [
      "Mandarina",
      "Bergamota",
      "Pera",
      "Té negro",
      "Lirio",
      "Lirio de los valles",
      "Violeta",
      "Cedro",
      "Almizcle",
      "Sándalo"
    ],
    "ml": 100,
    "precioUSD": 45,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 435,
    "house": "CLINIQUE",
    "name": "Clinique Happy Woman",
    "image": "clinique-happy-100ml-f.webp",
    "familia": "Floral Cítrico",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Happy de Clinique es una fragancia para la mujer floral cítrica luminosa e icónica en concentración Parfum Spray 100ml, lanzada en 1997. Su apertura chispeante de pomelo, mandarina y bergamota evoluciona hacia un corazón floral brillante de flor de azahar y lirio tropical, cerrando con una base limpia y suave de almizcle y ámbar. Una fragancia optimista, fresca y reconocida mundialmente por su carácter alegre y sofisticado.",
    "description": "Happy de Clinique es una fragancia para la mujer floral cítrica luminosa e icónica en concentración Parfum Spray 100ml, lanzada en 1997. Su apertura chispeante de pomelo, mandarina y bergamota evoluciona hacia un corazón floral brillante de flor de azahar y lirio tropical, cerrando con una base limpia y suave de almizcle y ámbar. Una fragancia optimista, fresca y reconocida mundialmente por su carácter alegre y sofisticado.",
    "notasSalida": "Pomelo, Mandarina, Bergamota",
    "notasCorazon": "Flor de azahar, Lirio tropical, Rosa silvestre",
    "notasFondo": "Almizcle, Ámbar",
    "notes": [
      "Pomelo",
      "Mandarina",
      "Bergamota",
      "Flor de azahar",
      "Lirio tropical",
      "Rosa silvestre",
      "Almizcle",
      "Ámbar"
    ],
    "ml": 100,
    "precioUSD": 60,
    "categoria": "disenador"
  },
  {
    "id": 435,
    "house": "CLINIQUE",
    "name": "Clinique Happy Woman",
    "image": "clinique-happy-100ml-f.webp",
    "familia": "Floral Cítrico",
    "tipo": "Parfum",
    "genero": "Femenino",
    "descripcion": "Happy de Clinique es una fragancia para la mujer floral cítrica luminosa e icónica en concentración Parfum Spray 100ml, lanzada en 1997. Su apertura chispeante de pomelo, mandarina y bergamota evoluciona hacia un corazón floral brillante de flor de azahar y lirio tropical, cerrando con una base limpia y suave de almizcle y ámbar. Una fragancia optimista, fresca y reconocida mundialmente por su carácter alegre y sofisticado.",
    "description": "Happy de Clinique es una fragancia para la mujer floral cítrica luminosa e icónica en concentración Parfum Spray 100ml, lanzada en 1997. Su apertura chispeante de pomelo, mandarina y bergamota evoluciona hacia un corazón floral brillante de flor de azahar y lirio tropical, cerrando con una base limpia y suave de almizcle y ámbar. Una fragancia optimista, fresca y reconocida mundialmente por su carácter alegre y sofisticado.",
    "notasSalida": "Pomelo, Mandarina, Bergamota",
    "notasCorazon": "Flor de azahar, Lirio tropical, Rosa silvestre",
    "notasFondo": "Almizcle, Ámbar",
    "notes": [
      "Pomelo",
      "Mandarina",
      "Bergamota",
      "Flor de azahar",
      "Lirio tropical",
      "Rosa silvestre",
      "Almizcle",
      "Ámbar"
    ],
    "ml": 100,
    "precioUSD": 60,
    "categoria": "disenador"
  },
  {
    "id": 436,
    "house": "BENETTON",
    "name": "Benetton Sisterland Red Rose",
    "image": "benetton-sisterland-red-rose-80ml-f.webp",
    "familia": "Floral Frutal",
    "tipo": "Eau de Toilette",
    "genero": "Femenino",
    "descripcion": "Sisterland Red Rose de United Colors of Benetton es una fragancia para la mujer floral rosa fresca y apasionada, parte de la colección Sisterland en su característico frasco con forma de muñeca en rojo. Su apertura frutal de frambuesa y bergamota evoluciona hacia un corazón floral romántico de rosa roja, peonía y jazmín, cerrando con una base suave de sándalo, pachulí y almizcle. Femenina, vibrante y muy accesible.",
    "description": "Sisterland Red Rose de United Colors of Benetton es una fragancia para la mujer floral rosa fresca y apasionada, parte de la colección Sisterland en su característico frasco con forma de muñeca en rojo. Su apertura frutal de frambuesa y bergamota evoluciona hacia un corazón floral romántico de rosa roja, peonía y jazmín, cerrando con una base suave de sándalo, pachulí y almizcle. Femenina, vibrante y muy accesible.",
    "notasSalida": "Frambuesa, Bergamota",
    "notasCorazon": "Rosa roja, Peonía, Jazmín",
    "notasFondo": "Sándalo, Almizcle, Pachulí",
    "notes": [
      "Frambuesa",
      "Bergamota",
      "Rosa roja",
      "Peonía",
      "Jazmín",
      "Sándalo",
      "Almizcle",
      "Pachulí"
    ],
    "ml": 80,
    "precioUSD": 25,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 437,
    "house": "COFINLUXE",
    "name": "Cofinluxe Watt Pink",
    "image": "cofinluxe-watt-pink-100ml-f.webp",
    "familia": "Floral Frutal",
    "tipo": "Eau de Toilette",
    "genero": "Femenino",
    "descripcion": "Watt Pink de Cofinluxe es una fragancia para la mujer floral frutal delicada y femenina, lanzada en 1993. Su composición gira en torno a un bouquet floral de rosa, jacinto y violeta sobre una base afrutada de melocotón y albaricoque, con un toque suavizante de manzanilla y jazmín. Fresca, romántica y fácil de llevar, perfecta para el día a día en primavera y verano.",
    "description": "Watt Pink de Cofinluxe es una fragancia para la mujer floral frutal delicada y femenina, lanzada en 1993. Su composición gira en torno a un bouquet floral de rosa, jacinto y violeta sobre una base afrutada de melocotón y albaricoque, con un toque suavizante de manzanilla y jazmín. Fresca, romántica y fácil de llevar, perfecta para el día a día en primavera y verano.",
    "notasSalida": "Rosa, Jacinto, Violeta",
    "notasCorazon": "Jazmín, Manzanilla",
    "notasFondo": "Melocotón, Albaricoque",
    "notes": [
      "Rosa",
      "Jacinto",
      "Violeta",
      "Jazmín",
      "Manzanilla",
      "Melocotón",
      "Albaricoque"
    ],
    "ml": 100,
    "precioUSD": 20,
    "categoria": "disenador"
  },
  {
    "id": 438,
    "house": "BENETTON",
    "name": "Benetton Paradiso Inferno Pink",
    "image": "benetton-paradiso-inferno-pink-100ml-f.webp",
    "familia": "Floral Cítrico",
    "tipo": "Eau de Toilette",
    "genero": "Femenino",
    "descripcion": "Paradiso Inferno Pink de Benetton es una fragancia para la mujer floral cítrica ligera y fresca, lanzada en 2004. Su apertura luminosa de limón, naranja y hojas verdes evoluciona hacia un corazón floral de rosa, ylang ylang y jazmín, cerrando con una base limpia y suave de cedro, sándalo y almizcle. Fresca, accesible y fácil de llevar en el día a día.",
    "description": "Paradiso Inferno Pink de Benetton es una fragancia para la mujer floral cítrica ligera y fresca, lanzada en 2004. Su apertura luminosa de limón, naranja y hojas verdes evoluciona hacia un corazón floral de rosa, ylang ylang y jazmín, cerrando con una base limpia y suave de cedro, sándalo y almizcle. Fresca, accesible y fácil de llevar en el día a día.",
    "notasSalida": "Limón, Hojas verdes, Naranja",
    "notasCorazon": "Rosa, Ylang ylang, Jazmín",
    "notasFondo": "Cedro, Almizcle, Sándalo",
    "notes": [
      "Limón",
      "Hojas verdes",
      "Naranja",
      "Rosa",
      "Ylang ylang",
      "Jazmín",
      "Cedro",
      "Almizcle",
      "Sándalo"
    ],
    "ml": 100,
    "precioUSD": 30,
    "categoria": "disenador"
  },
  {
    "id": 439,
    "house": "ELIZABETH ARDEN",
    "name": "Elizabeth Arden 5th Avenue",
    "image": "elizabeth-arden-5th-avenue-125ml-f.webp",
    "familia": "Floral",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "5th Avenue de Elizabeth Arden es una fragancia para la mujer floral sofisticada e icónica, creada por Ann Gottlieb y lanzada en 1996. Inspirada en el glamour de la famosa avenida neoyorquina, su apertura luminosa de lila, magnolia y bergamota evoluciona hacia un corazón floral opulento de tuberosa, jazmín, rosa y ylang ylang con toques de melocotón y nuez moscada, cerrando con una base cálida de sándalo, iris, ámbar y vainilla. Un clásico atemporal de elegancia refinada.",
    "description": "5th Avenue de Elizabeth Arden es una fragancia para la mujer floral sofisticada e icónica, creada por Ann Gottlieb y lanzada en 1996. Inspirada en el glamour de la famosa avenida neoyorquina, su apertura luminosa de lila, magnolia y bergamota evoluciona hacia un corazón floral opulento de tuberosa, jazmín, rosa y ylang ylang con toques de melocotón y nuez moscada, cerrando con una base cálida de sándalo, iris, ámbar y vainilla. Un clásico atemporal de elegancia refinada.",
    "notasSalida": "Tilo, Lila, Magnolia, Bergamota, Mandarina",
    "notasCorazon": "Jazmín, Tuberosa, Rosa de Bulgaria, Melocotón, Ylang ylang, Violeta, Clavel, Nuez moscada",
    "notasFondo": "Almizcle, Iris, Sándalo, Ámbar, Vainilla, Clavo",
    "notes": [
      "Tilo",
      "Lila",
      "Magnolia",
      "Bergamota",
      "Mandarina",
      "Jazmín",
      "Tuberosa",
      "Rosa de Bulgaria",
      "Melocotón",
      "Ylang ylang",
      "Violeta",
      "Clavel",
      "Nuez moscada",
      "Almizcle",
      "Iris",
      "Sándalo",
      "Ámbar",
      "Vainilla",
      "Clavo"
    ],
    "ml": 125,
    "precioUSD": 40,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 440,
    "house": "ELIZABETH ARDEN",
    "name": "Elizabeth Arden Red Door",
    "image": "elizabeth-arden-red-door-100ml-f.webp",
    "familia": "Floral Oriental",
    "tipo": "Eau de Toilette",
    "genero": "Femenino",
    "descripcion": "Red Door de Elizabeth Arden es una fragancia para la mujer floral oriental clásica e icónica, lanzada en 1989 e inspirada en el famoso salón de belleza Red Door Spa de la marca. Su apertura opulenta de lirio de los valles, freesia y violeta silvestre evoluciona hacia un corazón floral lujoso de rosa roja, flor de naranja marroquí, jazmín y ylang ylang, cerrando con una base cálida y sensual de miel, musgo de roble y sándalo. Un símbolo atemporal de glamour y feminidad.",
    "description": "Red Door de Elizabeth Arden es una fragancia para la mujer floral oriental clásica e icónica, lanzada en 1989 e inspirada en el famoso salón de belleza Red Door Spa de la marca. Su apertura opulenta de lirio de los valles, freesia y violeta silvestre evoluciona hacia un corazón floral lujoso de rosa roja, flor de naranja marroquí, jazmín y ylang ylang, cerrando con una base cálida y sensual de miel, musgo de roble y sándalo. Un símbolo atemporal de glamour y feminidad.",
    "notasSalida": "Lirio de los valles, Freesia, Violeta silvestre",
    "notasCorazon": "Rosa roja, Flor de naranja marroquí, Jazmín, Ylang ylang, Orquídea",
    "notasFondo": "Miel, Musgo de roble, Sándalo",
    "notes": [
      "Lirio de los valles",
      "Freesia",
      "Violeta silvestre",
      "Rosa roja",
      "Flor de naranja marroquí",
      "Jazmín",
      "Ylang ylang",
      "Orquídea",
      "Miel",
      "Musgo de roble",
      "Sándalo"
    ],
    "ml": 100,
    "precioUSD": 45,
    "categoria": "disenador"
  },
  {
    "id": 441,
    "house": "CALVIN KLEIN",
    "name": "Calvin Klein Escape",
    "image": "calvin-klein-escape-100ml-f.webp",
    "familia": "Floral Acuático",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Escape de Calvin Klein es una fragancia para la mujer floral acuática clásica y atemporal, lanzada en 1991 y creada por el perfumista Claude Dir. Su apertura frutal y verde de melón, manzana, albaricoque y grosella negra evoluciona hacia un corazón floral romántico de rosa, jazmín y lirio de los valles con notas de melocotón, cerrando con una base terrosa y cálida de sándalo, ámbar, musgo de roble y vainilla. Un emblema de libertad femenina de los años 90.",
    "description": "Escape de Calvin Klein es una fragancia para la mujer floral acuática clásica y atemporal, lanzada en 1991 y creada por el perfumista Claude Dir. Su apertura frutal y verde de melón, manzana, albaricoque y grosella negra evoluciona hacia un corazón floral romántico de rosa, jazmín y lirio de los valles con notas de melocotón, cerrando con una base terrosa y cálida de sándalo, ámbar, musgo de roble y vainilla. Un emblema de libertad femenina de los años 90.",
    "notasSalida": "Melón, Manzanilla, Caléndula, Albaricoque, Jacinto, Manzana, Cilantro, Mandarina, Grosella negra, Lichi",
    "notasCorazon": "Ylang ylang, Melocotón, Lirio de los valles, Clavel, Rosa, Jazmín, Clavo",
    "notasFondo": "Almizcle, Sándalo, Ámbar, Vetiver, Cedro, Vainilla, Musgo de roble",
    "notes": [
      "Melón",
      "Manzanilla",
      "Caléndula",
      "Albaricoque",
      "Jacinto",
      "Manzana",
      "Cilantro",
      "Mandarina",
      "Grosella negra",
      "Lichi",
      "Ylang ylang",
      "Melocotón",
      "Lirio de los valles",
      "Clavel",
      "Rosa",
      "Jazmín",
      "Clavo",
      "Almizcle",
      "Sándalo",
      "Ámbar",
      "Vetiver",
      "Cedro",
      "Vainilla",
      "Musgo de roble"
    ],
    "ml": 100,
    "precioUSD": 40,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 442,
    "house": "BENETTON",
    "name": "Benetton United Dreams Love Yourself",
    "image": "benetton-united-dreams-love-yourself-80ml-f.webp",
    "familia": "Floral Frutal",
    "tipo": "Eau de Toilette",
    "genero": "Femenino",
    "descripcion": "United Dreams Love Yourself de United Colors of Benetton es una fragancia para la mujer floral frutal fresca y femenina, lanzada en 2014 como parte de la colección United Dreams. Su apertura afrutada de frambuesa, pera, albaricoque y bergamota evoluciona hacia un corazón floral suave de rosa, freesia y jazmín, cerrando con una base limpia y cremosa de sándalo, ámbar y almizcle. Un aroma optimista, joven y muy llevable para el día a día.",
    "description": "United Dreams Love Yourself de United Colors of Benetton es una fragancia para la mujer floral frutal fresca y femenina, lanzada en 2014 como parte de la colección United Dreams. Su apertura afrutada de frambuesa, pera, albaricoque y bergamota evoluciona hacia un corazón floral suave de rosa, freesia y jazmín, cerrando con una base limpia y cremosa de sándalo, ámbar y almizcle. Un aroma optimista, joven y muy llevable para el día a día.",
    "notasSalida": "Frambuesa, Pera, Albaricoque, Bergamota",
    "notasCorazon": "Rosa, Freesia, Jazmín",
    "notasFondo": "Almizcle, Sándalo, Ámbar",
    "notes": [
      "Frambuesa",
      "Pera",
      "Albaricoque",
      "Bergamota",
      "Rosa",
      "Freesia",
      "Jazmín",
      "Almizcle",
      "Sándalo",
      "Ámbar"
    ],
    "ml": 80,
    "precioUSD": 25,
    "categoria": "disenador"
  },
  {
    "id": 443,
    "house": "CAROLINA HERRERA",
    "name": "Chic For Men",
    "image": "carolina-herrera-chic-for-men-100ml-m.webp",
    "familia": "amadera especiada",
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": "Descubre Chic for Men, una fragancia icónica que equilibra a la perfección la frescura urbana y la calidez magnética. Con un inicio vibrante de sandía y cítricos, un corazón especiado de pimienta y canela, y un fondo profundamente masculino de madera de cedro y haba tonka, este Eau de Toilette está diseñado para el hombre creativo, elegante y seguro de sí mismo. Ideal para el día a día o para dejar una impresión inolvidable en una ocasión especial.",
    "description": "Descubre Chic for Men, una fragancia icónica que equilibra a la perfección la frescura urbana y la calidez magnética. Con un inicio vibrante de sandía y cítricos, un corazón especiado de pimienta y canela, y un fondo profundamente masculino de madera de cedro y haba tonka, este Eau de Toilette está diseñado para el hombre creativo, elegante y seguro de sí mismo. Ideal para el día a día o para dejar una impresión inolvidable en una ocasión especial.",
    "notasSalida": "cardamomo, limón, mandarina, sandía, bergamota",
    "notasCorazon": "pimienta, canela",
    "notasFondo": "sandalo, haba tonka, almizcle, ambar, cedro",
    "notes": [
      "cardamomo",
      "limón",
      "mandarina",
      "sandía",
      "bergamota",
      "pimienta",
      "canela",
      "sandalo",
      "haba tonka",
      "almizcle",
      "ambar",
      "cedro"
    ],
    "ml": 100,
    "precioUSD": 70,
    "categoria": "disenador"
  },
  {
    "id": 444,
    "house": "CAROLINA HERRERA",
    "name": "La Bomba ",
    "image": "carolina-herrera-la-bomba-80ml-w.webp",
    "familia": "Floral Frutal Ámbar",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "La Bomba de Carolina Herrera es una vibrante fragancia femenina de la familia floral, afrutada y ambarina, diseñada para mujeres exuberantes y libres. Destaca por su aroma dulce pero refrescante y su llamativo diseño, simbolizando la transformación, la energía y la pasión.",
    "description": "La Bomba de Carolina Herrera es una vibrante fragancia femenina de la familia floral, afrutada y ambarina, diseñada para mujeres exuberantes y libres. Destaca por su aroma dulce pero refrescante y su llamativo diseño, simbolizando la transformación, la energía y la pasión.",
    "notasSalida": null,
    "notasCorazon": "frangipani, peonia",
    "notasFondo": "vainilla, pachulí",
    "notes": [
      "frangipani",
      "peonia",
      "vainilla",
      "pachulí"
    ],
    "ml": 80,
    "precioUSD": 200,
    "categoria": "disenador"
  },
  {
    "id": 445,
    "house": "AL HARAMAIN",
    "name": "Al Haramain Amber Oud Carbon Edition",
    "image": "al-haramain-amber-oud-carbon-edition-60ml-u.webp",
    "familia": "Amaderado",
    "tipo": "Eau de Parfum",
    "genero": "Unisex",
    "descripcion": "Una fragancia unisex de estilo amaderado aromático, fresca, limpia y elegante, ideal para quienes buscan un perfil sofisticado con carácter moderno. Se inspira claramente en Terre d’Hermès, ofreciendo un acorde mineral y seco con una salida aromática muy refinada.",
    "description": "Una fragancia unisex de estilo amaderado aromático, fresca, limpia y elegante, ideal para quienes buscan un perfil sofisticado con carácter moderno. Se inspira claramente en Terre d’Hermès, ofreciendo un acorde mineral y seco con una salida aromática muy refinada.",
    "notasSalida": "lavanda, bergamota, romero, notas marinas",
    "notasCorazon": "geranio, salvia, ámbar",
    "notasFondo": "vetiver, musgo, cedro",
    "notes": [
      "lavanda",
      "bergamota",
      "romero",
      "notas marinas",
      "geranio",
      "salvia",
      "ámbar",
      "vetiver",
      "musgo",
      "cedro"
    ],
    "ml": 100,
    "precioUSD": 160,
    "categoria": "arabes",
    "agotado": true
  },
  {
    "id": 446,
    "house": "AL HARAMAIN",
    "name": "Al Haramain Amber Oud Dubai Night",
    "image": "al-haramain-amber-oud-dubai-night-75ml-u.webp",
    "familia": "Floral Frutal",
    "tipo": "Eau de Parfum",
    "genero": "Unisex",
    "descripcion": "Unisex y sofisticado, Amber Oud Dubai Night de Al Haramain despliega un carácter oriental amaderado con una apertura especiada y cítrica, seguida de un corazón floral-oud de gran presencia. Su perfil opulento y envolvente lo convierte en una propuesta de lujo ideal para noches especiales, con un aire de inspiración nicho que recuerda el estilo de fragancias ámbar-oud de alta gama.",
    "description": "Unisex y sofisticado, Amber Oud Dubai Night de Al Haramain despliega un carácter oriental amaderado con una apertura especiada y cítrica, seguida de un corazón floral-oud de gran presencia. Su perfil opulento y envolvente lo convierte en una propuesta de lujo ideal para noches especiales, con un aire de inspiración nicho que recuerda el estilo de fragancias ámbar-oud de alta gama.",
    "notasSalida": "azafrán, bergamota, resina de elemí, rosa búlgara",
    "notasCorazon": "oud, lirio de los valles, haba tonka",
    "notasFondo": "ambar, almizcle blanco, musgo de roble",
    "notes": [
      "azafrán",
      "bergamota",
      "resina de elemí",
      "rosa búlgara",
      "oud",
      "lirio de los valles",
      "haba tonka",
      "ambar",
      "almizcle blanco",
      "musgo de roble"
    ],
    "ml": 100,
    "precioUSD": 75,
    "categoria": "arabes"
  },
  {
    "id": 447,
    "house": "ANTONIO BANDERAS",
    "name": "Antonio Banderas Blue Seduction",
    "image": "antonio-banderas-blue-seduction-100ml-m.webp",
    "familia": "Oriental Fougère  ",
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": "Una fragancia fresca y afrutada que abre con un contraste jugoso entre el melón dulce, la bergamota cítrica y un toque verde de menta, realzado por grosellas negras. En el corazón, una brisa marina se mezcla con manzana verde, capuchino y cardamomo, creando un carácter sofisticado y ligeramente especiado. La base de notas amaderadas y ámbar deja una estela cálida y sensual. Blue Seduction combina frescura y seducción en un mismo aliento — ideal para el día a día con carácter propio.",
    "description": "Una fragancia fresca y afrutada que abre con un contraste jugoso entre el melón dulce, la bergamota cítrica y un toque verde de menta, realzado por grosellas negras. En el corazón, una brisa marina se mezcla con manzana verde, capuchino y cardamomo, creando un carácter sofisticado y ligeramente especiado. La base de notas amaderadas y ámbar deja una estela cálida y sensual. Blue Seduction combina frescura y seducción en un mismo aliento — ideal para el día a día con carácter propio.",
    "notasSalida": "melón, bergamota, menta, grosellas negras",
    "notasCorazon": "agua de mar, manzana verde, capuchino, cardamomo, nuez moscada",
    "notasFondo": "notas amaderadas, ambar",
    "notes": [
      "melón",
      "bergamota",
      "menta",
      "grosellas negras",
      "agua de mar",
      "manzana verde",
      "capuchino",
      "cardamomo",
      "nuez moscada",
      "notas amaderadas",
      "ambar"
    ],
    "ml": 200,
    "precioUSD": 35,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 448,
    "house": "ANTONIO BANDERAS",
    "name": "Antonio Banderas King of Seduction",
    "image": "antonio-banderas-king-of-seduction-100ml-m.webp",
    "familia": "amadera especiada",
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": null,
    "description": null,
    "notasSalida": "piña, melón, bergamota, manzana verde, toronja (pomelo)",
    "notasCorazon": "notas marinas, jazmin, cardamomo, neroli",
    "notasFondo": "vetiver, almizcle blanco, cedro, gamuza, ambar",
    "notes": [
      "piña",
      "melón",
      "bergamota",
      "manzana verde",
      "toronja (pomelo)",
      "notas marinas",
      "jazmin",
      "cardamomo",
      "neroli",
      "vetiver",
      "almizcle blanco",
      "cedro",
      "gamuza",
      "ambar"
    ],
    "ml": 200,
    "precioUSD": 35,
    "categoria": "disenador"
  },
  {
    "id": 449,
    "house": "CAROLINA HERRERA",
    "name": "212 VIP Rosé NYC",
    "image": null,
    "familia": "Floral Frutal",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "212 VIP Rosé de Carolina Herrera es una fragancia floral y frutal para la mujer, fresca, sensual y dinámica, lanzada en 2014 como versión femenina del icónico 212 VIP. Abre con un acorde efervescente de vino rosado (champagne rosé) y pimienta rosa que estalla como un brindis, y da paso a un corazón coqueto de flor de durazno y un ramo de rosas. El fondo de Queenwood y almizcle deja una estela suave, cálida y adictiva. Es el perfume de la mujer que va a la fiesta y marca presencia: pensado para la noche, los planes con amigas y las celebraciones.",
    "description": "212 VIP Rosé de Carolina Herrera es una fragancia floral y frutal para la mujer, fresca, sensual y dinámica, lanzada en 2014 como versión femenina del icónico 212 VIP. Abre con un acorde efervescente de vino rosado (champagne rosé) y pimienta rosa que estalla como un brindis, y da paso a un corazón coqueto de flor de durazno y un ramo de rosas. El fondo de Queenwood y almizcle deja una estela suave, cálida y adictiva. Es el perfume de la mujer que va a la fiesta y marca presencia: pensado para la noche, los planes con amigas y las celebraciones.",
    "notasSalida": "champán, pimienta rosa",
    "notasCorazon": "flor de albaricoquero, rosa",
    "notasFondo": "almizcle blanco, notas amaderadas",
    "notes": [
      "champán",
      "pimienta rosa",
      "flor de albaricoquero",
      "rosa",
      "almizcle blanco",
      "notas amaderadas"
    ],
    "ml": 80,
    "precioUSD": 140,
    "categoria": "disenador"
  },
  {
    "id": 450,
    "house": "CAROLINA HERRERA",
    "name": "Very Good Girl Elixir",
    "image": null,
    "familia": "Oriental",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Very Good Girl Elixir de Carolina Herrera es una fragancia floral oriental para la mujer, más rica, densa y sensual que la Very Good Girl original de 2021. Abre con una cereza negra jugosa y un toque de almendra amarga que la vuelve adictiva desde el primer instante, sobre un corazón floral de rosa y nardo. El fondo cremoso de vainilla y cacao deja una estela golosa, cálida y envolvente que dura horas. Es el perfume de la mujer segura que sale de noche y quiere dejar recuerdo: ideal para citas, eventos y las estaciones frías.",
    "description": "Very Good Girl Elixir de Carolina Herrera es una fragancia floral oriental para la mujer, más rica, densa y sensual que la Very Good Girl original de 2021. Abre con una cereza negra jugosa y un toque de almendra amarga que la vuelve adictiva desde el primer instante, sobre un corazón floral de rosa y nardo. El fondo cremoso de vainilla y cacao deja una estela golosa, cálida y envolvente que dura horas. Es el perfume de la mujer segura que sale de noche y quiere dejar recuerdo: ideal para citas, eventos y las estaciones frías.",
    "notasSalida": "cereza negra, almendra amarga",
    "notasCorazon": "rosa, nardos",
    "notasFondo": "vainilla, cacao",
    "notes": [
      "cereza negra",
      "almendra amarga",
      "rosa",
      "nardos",
      "vainilla",
      "cacao"
    ],
    "ml": null,
    "precioUSD": 195,
    "categoria": "disenador"
  },
  {
    "id": 451,
    "house": "DOLCE & GABBANA",
    "name": "Light Blue Eau de Toilette",
    "image": null,
    "familia": "Floral Frutal",
    "tipo": "Eau de Toilette",
    "genero": "Femenino",
    "descripcion": "Light Blue Eau de Toilette de Dolce & Gabbana es una fragancia floral frutal para la mujer, fresca y luminosa, firmada por el perfumista Olivier Cresp. Abre con limón siciliano y manzana Granny Smith crujiente sobre un toque de cedro y campanilla, que dan paso a un corazón suave de jazmín, bambú y rosa. La base cálida de cedro, almizcle y ámbar deja una estela limpia, femenina y sensual que dura todo el día. Es el perfume de la mujer que quiere sentirse fresca sin esfuerzo: ideal para el día a día, la oficina y el clima cálido.",
    "description": "Light Blue Eau de Toilette de Dolce & Gabbana es una fragancia floral frutal para la mujer, fresca y luminosa, firmada por el perfumista Olivier Cresp. Abre con limón siciliano y manzana Granny Smith crujiente sobre un toque de cedro y campanilla, que dan paso a un corazón suave de jazmín, bambú y rosa. La base cálida de cedro, almizcle y ámbar deja una estela limpia, femenina y sensual que dura todo el día. Es el perfume de la mujer que quiere sentirse fresca sin esfuerzo: ideal para el día a día, la oficina y el clima cálido.",
    "notasSalida": "limón, manzana granny smith, cedro",
    "notasCorazon": "jazmín, bambú, rosa",
    "notasFondo": "cedro, almizcle, ambar",
    "notes": [
      "limón",
      "manzana granny smith",
      "cedro",
      "jazmín",
      "bambú",
      "rosa",
      "cedro",
      "almizcle",
      "ambar"
    ],
    "ml": 100,
    "precioUSD": 90,
    "categoria": "disenador"
  },
  {
    "id": 452,
    "house": "VALENTINO",
    "name": "Valentino Uomo Born in Roma",
    "image": null,
    "familia": "amadera especiada",
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": "Valentino Uomo Born in Roma es una fragancia amaderada y especiada para el hombre, moderna, magnética y con carácter, lanzada en 2019 como himno a la energía de Roma. Abre con un acorde mineral salino y absoluto de hojas de violeta que le dan un aire fresco y contemporáneo, sobre un corazón especiado de salvia y jengibre. La base de vetiver ahumado y notas amaderadas deja una estela seca, elegante y muy persistente. Es el perfume del hombre seguro que rompe las reglas con estilo: ideal para la noche, la oficina y el uso diario en clima templado o fresco.",
    "description": "Valentino Uomo Born in Roma es una fragancia amaderada y especiada para el hombre, moderna, magnética y con carácter, lanzada en 2019 como himno a la energía de Roma. Abre con un acorde mineral salino y absoluto de hojas de violeta que le dan un aire fresco y contemporáneo, sobre un corazón especiado de salvia y jengibre. La base de vetiver ahumado y notas amaderadas deja una estela seca, elegante y muy persistente. Es el perfume del hombre seguro que rompe las reglas con estilo: ideal para la noche, la oficina y el uso diario en clima templado o fresco.",
    "notasSalida": "notas marinas, hojas de violeta, sal",
    "notasCorazon": "salvia, jengibre",
    "notasFondo": "notas amaderadas, vetiver",
    "notes": [
      "notas marinas",
      "hojas de violeta",
      "sal",
      "salvia",
      "jengibre",
      "notas amaderadas",
      "vetiver"
    ],
    "ml": 100,
    "precioUSD": 165,
    "categoria": "disenador"
  },
  {
    "id": 453,
    "house": "VERSACE",
    "name": "Dylan Blue Pour Femme",
    "image": null,
    "familia": "Floral Frutal",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Una fragancia floral frutal vibrante y sofisticada para la mujer moderna, creada en 2017 por las perfumistas Calice Becker y Natalie Gracia-Cetto. Versace Pour Femme Dylan Blue abre con un contraste jugoso de manzana Granny Smith y grosellas negras, evoluciona hacia un corazón floral delicado de durazno, rosa y jazmín, y cierra con una base sensual de almizcle, maderas blancas y pachulí que aporta calidez y carácter duradero.",
    "description": "Una fragancia floral frutal vibrante y sofisticada para la mujer moderna, creada en 2017 por las perfumistas Calice Becker y Natalie Gracia-Cetto. Versace Pour Femme Dylan Blue abre con un contraste jugoso de manzana Granny Smith y grosellas negras, evoluciona hacia un corazón floral delicado de durazno, rosa y jazmín, y cierra con una base sensual de almizcle, maderas blancas y pachulí que aporta calidez y carácter duradero.",
    "notasSalida": "manzana granny smith, grosellas negras, menta",
    "notasCorazon": "durazno, rosa, jazmín",
    "notasFondo": "almizcle, madera blanca, pachulí",
    "notes": [
      "manzana granny smith",
      "grosellas negras",
      "menta",
      "durazno",
      "rosa",
      "jazmín",
      "almizcle",
      "madera blanca",
      "pachulí"
    ],
    "ml": 100,
    "precioUSD": 95,
    "categoria": "disenador",
    "agotado": true
  },
  {
    "id": 454,
    "house": "RALPH LAUREN",
    "name": "Polo Red Eau de Toilette",
    "image": null,
    "familia": "amadera especiada",
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": "Una fragancia amaderada especiada llena de energía y frescura deportiva para el hombre moderno, lanzada en 2013 por el perfumista Olivier Gillotin. Polo Red abre con un contraste vibrante de arándano, toronja y limón italiano, evoluciona hacia un corazón especiado de azafrán y salvia, y cierra con una base cálida de ámbar, café y notas amaderadas que aportan carácter y una estela adictiva.",
    "description": "Una fragancia amaderada especiada llena de energía y frescura deportiva para el hombre moderno, lanzada en 2013 por el perfumista Olivier Gillotin. Polo Red abre con un contraste vibrante de arándano, toronja y limón italiano, evoluciona hacia un corazón especiado de azafrán y salvia, y cierra con una base cálida de ámbar, café y notas amaderadas que aportan carácter y una estela adictiva.",
    "notasSalida": "arándano, toronja (pomelo), limón",
    "notasCorazon": "azafrán, salvia",
    "notasFondo": "ambar, café, notas amaderadas",
    "notes": [
      "arándano",
      "toronja (pomelo)",
      "limón",
      "azafrán",
      "salvia",
      "ambar",
      "café",
      "notas amaderadas"
    ],
    "ml": 125,
    "precioUSD": 80,
    "categoria": "disenador"
  },
  {
    "id": 455,
    "house": "JEAN PAUL GAULTIER",
    "name": "Le Male",
    "image": null,
    "familia": "Oriental Fougère ",
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": "Un clásico icónico de la familia oriental fougère, masculino y magnético, lanzado en 1995 por el reconocido perfumista Francis Kurkdjian. Le Male abre con un contraste fresco de lavanda, menta y cardamomo, evoluciona hacia un corazón especiado de canela y flor de azahar, y cierra con una base envolvente de vainilla, haba tonka y ámbar sobre sándalo y cedro, creando una estela cálida, dulce y profundamente seductora.",
    "description": "Un clásico icónico de la familia oriental fougère, masculino y magnético, lanzado en 1995 por el reconocido perfumista Francis Kurkdjian. Le Male abre con un contraste fresco de lavanda, menta y cardamomo, evoluciona hacia un corazón especiado de canela y flor de azahar, y cierra con una base envolvente de vainilla, haba tonka y ámbar sobre sándalo y cedro, creando una estela cálida, dulce y profundamente seductora.",
    "notasSalida": "lavanda, menta, cardamomo, bergamota, abrótano",
    "notasCorazon": "canela, flor de azahar del naranjo, alcaravea",
    "notasFondo": "vainilla, haba tonka, ambar, sandalo, cedro",
    "notes": [
      "lavanda",
      "menta",
      "cardamomo",
      "bergamota",
      "abrótano",
      "canela",
      "flor de azahar del naranjo",
      "alcaravea",
      "vainilla",
      "haba tonka",
      "ambar",
      "sandalo",
      "cedro"
    ],
    "ml": 125,
    "precioUSD": 130,
    "categoria": "disenador"
  },
  {
    "id": 456,
    "house": "VERSACE",
    "name": "Versace Pour Homme",
    "image": null,
    "familia": "Aromático",
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": "Una fragancia aromática fougère fresca y clásica para el hombre elegante, lanzada en 2008 por el reconocido perfumista Alberto Morillas. Versace Pour Homme abre con un contraste cítrico de limón y bergamota realzado por neroli y rosa de Mai, evoluciona hacia un corazón floral-verde de jacinto, cedro y geranio, y cierra con una base suave de almizcle, haba tonka y ámbar que aporta calidez y una estela limpia y sofisticada, ideal para el día a día.",
    "description": "Una fragancia aromática fougère fresca y clásica para el hombre elegante, lanzada en 2008 por el reconocido perfumista Alberto Morillas. Versace Pour Homme abre con un contraste cítrico de limón y bergamota realzado por neroli y rosa de Mai, evoluciona hacia un corazón floral-verde de jacinto, cedro y geranio, y cierra con una base suave de almizcle, haba tonka y ámbar que aporta calidez y una estela limpia y sofisticada, ideal para el día a día.",
    "notasSalida": "limón, bergamota, neroli, rosa de mayo",
    "notasCorazon": "jacinto, cedro, salvia esclarea, geranio",
    "notasFondo": "almizcle, haba tonka, ambar",
    "notes": [
      "limón",
      "bergamota",
      "neroli",
      "rosa de mayo",
      "jacinto",
      "cedro",
      "salvia esclarea",
      "geranio",
      "almizcle",
      "haba tonka",
      "ambar"
    ],
    "ml": 100,
    "precioUSD": 80,
    "categoria": "disenador"
  },
  {
    "id": 457,
    "house": "YVES SAINT LAURENT",
    "name": "Kouros",
    "image": null,
    "familia": "Aromático",
    "tipo": "Eau de Toilette",
    "genero": "Masculino",
    "descripcion": "Un ícono absoluto de la perfumería masculina, intenso y de carácter animal, creado en 1981 por el perfumista Pierre Bourdon. Kouros abre con un contraste vibrante de aldehídos, cilantro y bergamota, evoluciona hacia un corazón especiado y floral de pachulí, clavel, jazmín y lavanda, y cierra con una base densa y sensual de cuero, algalia, miel, musgo de roble y vainilla que deja una estela poderosa, cálida y memorable — una fragancia atrevida pensada para quien busca dejar huella.",
    "description": "Un ícono absoluto de la perfumería masculina, intenso y de carácter animal, creado en 1981 por el perfumista Pierre Bourdon. Kouros abre con un contraste vibrante de aldehídos, cilantro y bergamota, evoluciona hacia un corazón especiado y floral de pachulí, clavel, jazmín y lavanda, y cierra con una base densa y sensual de cuero, algalia, miel, musgo de roble y vainilla que deja una estela poderosa, cálida y memorable — una fragancia atrevida pensada para quien busca dejar huella.",
    "notasSalida": "aldehídos, cilantro, salvia esclarea, abrótano, bergamota",
    "notasCorazon": "pachulí, vetiver, jazmín, geranio, raíz de lirio, canela, lavanda",
    "notasFondo": "algalia, almizcle, cuero, miel, musgo de roble, ambar, haba tonka, vainilla",
    "notes": [
      "aldehídos",
      "cilantro",
      "salvia esclarea",
      "abrótano",
      "bergamota",
      "pachulí",
      "vetiver",
      "jazmín",
      "geranio",
      "raíz de lirio",
      "canela",
      "lavanda",
      "algalia",
      "almizcle",
      "cuero",
      "miel",
      "musgo de roble",
      "ambar",
      "haba tonka",
      "vainilla"
    ],
    "ml": 100,
    "precioUSD": 75,
    "categoria": "disenador"
  },
  {
    "id": 458,
    "house": "TOM FORD",
    "name": "Ombré Leather",
    "image": null,
    "familia": "Amaderado",
    "tipo": "Eau de Parfum",
    "genero": "Unisex",
    "descripcion": "Una fragancia de cuero moderna y unisex, sensual y minimalista, creada por la perfumista Sonia Constant. Ombré Leather abre con un toque especiado de cardamomo, evoluciona hacia un corazón de cuero envolvente realzado por jazmín sambac, y cierra con una base cálida de ámbar, musgo y pachulí que deja una estela sofisticada y magnética, perfecta tanto para hombres como para mujeres.",
    "description": "Una fragancia de cuero moderna y unisex, sensual y minimalista, creada por la perfumista Sonia Constant. Ombré Leather abre con un toque especiado de cardamomo, evoluciona hacia un corazón de cuero envolvente realzado por jazmín sambac, y cierra con una base cálida de ámbar, musgo y pachulí que deja una estela sofisticada y magnética, perfecta tanto para hombres como para mujeres.",
    "notasSalida": "cardamomo",
    "notasCorazon": "cuero, jazmín sambac",
    "notasFondo": "ambar, musgo, pachulí",
    "notes": [
      "cardamomo",
      "cuero",
      "jazmín sambac",
      "ambar",
      "musgo",
      "pachulí"
    ],
    "ml": 100,
    "precioUSD": 270,
    "categoria": "disenador"
  },
  {
    "id": 460,
    "house": "GUESS",
    "name": "Guess Seductive Kiss",
    "image": null,
    "familia": "Frutal",
    "tipo": "Eau de Toilette",
    "genero": "Femenino",
    "descripcion": "Una fragancia floral fresca y juvenil para la mujer moderna, lanzada en 2020 por la casa Guess. Seductive Kiss abre con un contraste cítrico vibrante de toronja y naranja tangerina, evoluciona hacia un corazón floral delicado de flor del duraznero, flor silvestre y jazmín, y cierra con una base suave de cedro, almizcle y ámbar que aporta calidez y una estela sutilmente seductora, ideal para el uso diario.",
    "description": "Una fragancia floral fresca y juvenil para la mujer moderna, lanzada en 2020 por la casa Guess. Seductive Kiss abre con un contraste cítrico vibrante de toronja y naranja tangerina, evoluciona hacia un corazón floral delicado de flor del duraznero, flor silvestre y jazmín, y cierra con una base suave de cedro, almizcle y ámbar que aporta calidez y una estela sutilmente seductora, ideal para el uso diario.",
    "notasSalida": "toronja (pomelo), naranja",
    "notasCorazon": "flor de albaricoquero, rosa silvestre, jazmín",
    "notasFondo": "ambar, cedro, almizcle",
    "notes": [
      "toronja (pomelo)",
      "naranja",
      "flor de albaricoquero",
      "rosa silvestre",
      "jazmín",
      "ambar",
      "cedro",
      "almizcle"
    ],
    "ml": 75,
    "precioUSD": 35,
    "categoria": "disenador"
  },
  {
    "id": 461,
    "house": "GUESS",
    "name": "Bella Vita",
    "image": null,
    "familia": "Floral Frutal",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Una fragancia floral frutal gourmand dulce y vibrante para la mujer moderna, lanzada en 2020 por la casa Guess. Bella Vita abre con un contraste jugoso de cereza ácida, grosellas negras y limón italiano, evoluciona hacia un corazón floral envolvente de nardos, orquídea de vainilla y jazmín, y cierra con una base golosa de praliné, haba tonka, Amberwood y almizcle que deja una estela cálida, dulce y muy femenina.",
    "description": "Una fragancia floral frutal gourmand dulce y vibrante para la mujer moderna, lanzada en 2020 por la casa Guess. Bella Vita abre con un contraste jugoso de cereza ácida, grosellas negras y limón italiano, evoluciona hacia un corazón floral envolvente de nardos, orquídea de vainilla y jazmín, y cierra con una base golosa de praliné, haba tonka, Amberwood y almizcle que deja una estela cálida, dulce y muy femenina.",
    "notasSalida": "cereza ácida, grosellas negras, limón",
    "notasCorazon": "nardos, orquídea de vainilla, jazmín",
    "notasFondo": "praliné, haba tonka, amberwood, almizcle",
    "notes": [
      "cereza ácida",
      "grosellas negras",
      "limón",
      "nardos",
      "orquídea de vainilla",
      "jazmín",
      "praliné",
      "haba tonka",
      "amberwood",
      "almizcle"
    ],
    "ml": 100,
    "precioUSD": 45,
    "categoria": "disenador"
  },
  {
    "id": 462,
    "house": "RABANNE",
    "name": "Ultraviolet",
    "image": null,
    "familia": "Oriental",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Un clásico de culto de la familia oriental floral, envolvente y adictivo, lanzado en 1999 por el perfumista Jacques Cavallier Belletrud. Ultraviolet abre con un contraste especiado y afrutado de pimienta, chabacano y almendras frescas, evoluciona hacia un corazón floral luminoso de violeta, osmanto, rosa y jazmín, y cierra con una base cálida de vainilla, pachulí, ámbar y cedro que deja una estela sofisticada, misteriosa y profundamente femenina.",
    "description": "Un clásico de culto de la familia oriental floral, envolvente y adictivo, lanzado en 1999 por el perfumista Jacques Cavallier Belletrud. Ultraviolet abre con un contraste especiado y afrutado de pimienta, chabacano y almendras frescas, evoluciona hacia un corazón floral luminoso de violeta, osmanto, rosa y jazmín, y cierra con una base cálida de vainilla, pachulí, ámbar y cedro que deja una estela sofisticada, misteriosa y profundamente femenina.",
    "notasSalida": "pimienta, chabacano, cilantro, almendra, palo de rosa de brasil",
    "notasCorazon": "violeta, osmanto, rosa, jazmín",
    "notasFondo": "vainilla, pachulí, ambar, cedro",
    "notes": [
      "pimienta",
      "chabacano",
      "cilantro",
      "almendra",
      "palo de rosa de brasil",
      "violeta",
      "osmanto",
      "rosa",
      "jazmín",
      "vainilla",
      "pachulí",
      "ambar",
      "cedro"
    ],
    "ml": 80,
    "precioUSD": 65,
    "categoria": "disenador"
  },
  {
    "id": 463,
    "house": "BHARARA",
    "name": "Rome La Bomba ",
    "image": null,
    "familia": "Floral Frutal",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Una fragancia femenina audaz y glamorosa, pensada para la mujer que se adueña de cada lugar donde entra. Mast Perfume Rome La Bomba abre con acordes frutales vibrantes y jugosos, evoluciona hacia un corazón floral suave y radiante que aporta elegancia y feminidad, y cierra con una base cremosa de vainilla, almizcle cálido y maderas suaves que deja una estela sensual e inolvidable, una fragancia hecha para llamar la atención sin esforzarse demasiado.",
    "description": "Una fragancia femenina audaz y glamorosa, pensada para la mujer que se adueña de cada lugar donde entra. Mast Perfume Rome La Bomba abre con acordes frutales vibrantes y jugosos, evoluciona hacia un corazón floral suave y radiante que aporta elegancia y feminidad, y cierra con una base cremosa de vainilla, almizcle cálido y maderas suaves que deja una estela sensual e inolvidable, una fragancia hecha para llamar la atención sin esforzarse demasiado.",
    "notasSalida": "frutas",
    "notasCorazon": "frangipani, peonía rosa",
    "notasFondo": "vainilla, pachulí",
    "notes": [
      "frutas",
      "frangipani",
      "peonía rosa",
      "vainilla",
      "pachulí"
    ],
    "ml": 100,
    "precioUSD": 45,
    "categoria": "disenador"
  },
  {
    "id": 464,
    "house": "LATTAFA",
    "name": "Khamrah Waha",
    "image": null,
    "familia": "Aromático",
    "tipo": "Eau de Parfum",
    "genero": "Unisex",
    "descripcion": "Una fragancia unisex aromática acuática fresca y contemporánea, lanzada en 2026 por el perfumista Jordi Fernández. Khamrah Waha abre con un contraste vibrante de bergamota, yuzu, enebro y jengibre, evoluciona hacia un corazón acuático y verde de pepino, sal de mar, iris y salvia, y cierra con una base cálida de vainilla, haba tonka, almizcle y maderas ambaradas (Ambrofix™ y Akigalawood) que aporta profundidad y una estela moderna y sofisticada.",
    "description": "Una fragancia unisex aromática acuática fresca y contemporánea, lanzada en 2026 por el perfumista Jordi Fernández. Khamrah Waha abre con un contraste vibrante de bergamota, yuzu, enebro y jengibre, evoluciona hacia un corazón acuático y verde de pepino, sal de mar, iris y salvia, y cierra con una base cálida de vainilla, haba tonka, almizcle y maderas ambaradas (Ambrofix™ y Akigalawood) que aporta profundidad y una estela moderna y sofisticada.",
    "notasSalida": "bergamota, yuzu, enebro, jengibre",
    "notasCorazon": "pepino, sal de mar, iris, salvia",
    "notasFondo": "vainilla, haba tonka, almizcle, madera de akigala",
    "notes": [
      "bergamota",
      "yuzu",
      "enebro",
      "jengibre",
      "pepino",
      "sal de mar",
      "iris",
      "salvia",
      "vainilla",
      "haba tonka",
      "almizcle",
      "madera de akigala"
    ],
    "ml": 100,
    "precioUSD": 60,
    "categoria": "arabes"
  },
  {
    "id": 465,
    "house": "FRENCH AVENUE",
    "name": "Vulcan Feu",
    "image": null,
    "familia": "Floral",
    "tipo": "Extrait de Parfum",
    "genero": "Unisex",
    "descripcion": "Una fragancia floral unisex moderna y golosa, lanzada en 2025 por French Avenue. Vulcan Feu abre con un contraste frutal y picante de mango, limón, jengibre y ruibarbo, evoluciona hacia un corazón floral especiado de pimienta rosa, jazmín, violeta y praliné, y cierra con una base cálida de haba tonka, cedro, ámbar gris y musgo que deja una estela dulce, envolvente y sofisticada.",
    "description": "Una fragancia floral unisex moderna y golosa, lanzada en 2025 por French Avenue. Vulcan Feu abre con un contraste frutal y picante de mango, limón, jengibre y ruibarbo, evoluciona hacia un corazón floral especiado de pimienta rosa, jazmín, violeta y praliné, y cierra con una base cálida de haba tonka, cedro, ámbar gris y musgo que deja una estela dulce, envolvente y sofisticada.",
    "notasSalida": "mango, lima, jengibre, ruibarbo",
    "notasCorazon": "pimienta rosa, jazmín, violeta, praliné",
    "notasFondo": "haba tonka, ambergris, cedro, musgo",
    "notes": [
      "mango",
      "lima",
      "jengibre",
      "ruibarbo",
      "pimienta rosa",
      "jazmín",
      "violeta",
      "praliné",
      "haba tonka",
      "ambergris",
      "cedro",
      "musgo"
    ],
    "ml": 100,
    "precioUSD": 60,
    "categoria": "arabes"
  },
  {
    "id": 466,
    "house": "HUGO BOSS",
    "name": "Boss Ma Vie",
    "image": null,
    "familia": "Floral",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Una fragancia floral elegante y luminosa para la mujer contemporánea, lanzada en 2014 por Hugo Boss. Ma Vie Pour Femme abre con la nota única y singular de flor de cactus, evoluciona hacia un corazón floral delicado de fresia rosa, rosa y jazmín, y cierra con una base suave de notas amaderadas y cedro que aporta calidez y una estela fresca y sofisticada, ideal para el día a día.",
    "description": "Una fragancia floral elegante y luminosa para la mujer contemporánea, lanzada en 2014 por Hugo Boss. Ma Vie Pour Femme abre con la nota única y singular de flor de cactus, evoluciona hacia un corazón floral delicado de fresia rosa, rosa y jazmín, y cierra con una base suave de notas amaderadas y cedro que aporta calidez y una estela fresca y sofisticada, ideal para el día a día.",
    "notasSalida": "cactus",
    "notasCorazon": "fresia rosa, rosa, jazmín",
    "notasFondo": "notas amaderadas, cedro",
    "notes": [
      "cactus",
      "fresia rosa",
      "rosa",
      "jazmín",
      "notas amaderadas",
      "cedro"
    ],
    "ml": 75,
    "precioUSD": 50,
    "categoria": "disenador"
  },
  {
    "id": 467,
    "house": "KATY PERRY",
    "name": "Mad Potion",
    "image": null,
    "familia": "Oriental",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Una fragancia oriental vainilla dulce y envolvente para la mujer, lanzada en 2015 por Katy Perry junto a los perfumistas Stephen Nilsen y Natasha Côté. Mad Potion abre con un contraste delicado de orquídea de vainilla, manzana y peonía, evoluciona hacia un corazón cremoso de vainilla Bourbon, almizcle y jazmín, y cierra con una base cálida de vainilla, almizcle y ámbar que deja una estela golosa, sensual y muy femenina.",
    "description": "Una fragancia oriental vainilla dulce y envolvente para la mujer, lanzada en 2015 por Katy Perry junto a los perfumistas Stephen Nilsen y Natasha Côté. Mad Potion abre con un contraste delicado de orquídea de vainilla, manzana y peonía, evoluciona hacia un corazón cremoso de vainilla Bourbon, almizcle y jazmín, y cierra con una base cálida de vainilla, almizcle y ámbar que deja una estela golosa, sensual y muy femenina.",
    "notasSalida": "orquídea de vainilla, manzana, peonia",
    "notasCorazon": "vainilla bourbon, almizcle, jazmín",
    "notasFondo": "vainilla, almizcle, ambar",
    "notes": [
      "orquídea de vainilla",
      "manzana",
      "peonia",
      "vainilla bourbon",
      "almizcle",
      "jazmín",
      "vainilla",
      "almizcle",
      "ambar"
    ],
    "ml": 100,
    "precioUSD": 40,
    "categoria": "disenador"
  },
  {
    "id": 468,
    "house": "BRITNEY SPEARS",
    "name": "Fantasy Intense",
    "image": null,
    "familia": "Floral Frutal",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Una fragancia floral frutal gourmand dulce y golosa para la mujer, lanzada en 2021 como versión intensa del icónico Fantasy. Abre con un contraste frutal jugoso de lichi, pera y kiwi, evoluciona hacia un corazón dulce y cremoso de quequito, chocolate blanco, jazmín y orquídea, y cierra con una base cálida de pachulí, notas amaderadas, almizcle y raíz de lirio que deja una estela golosa, envolvente y muy femenina.",
    "description": "Una fragancia floral frutal gourmand dulce y golosa para la mujer, lanzada en 2021 como versión intensa del icónico Fantasy. Abre con un contraste frutal jugoso de lichi, pera y kiwi, evoluciona hacia un corazón dulce y cremoso de quequito, chocolate blanco, jazmín y orquídea, y cierra con una base cálida de pachulí, notas amaderadas, almizcle y raíz de lirio que deja una estela golosa, envolvente y muy femenina.",
    "notasSalida": "lichi, pera, kiwi",
    "notasCorazon": "quequito, chocolate blanco, jazmín, orquídea",
    "notasFondo": "pachulí, notas amaderadas, almizcle, raíz de lirio",
    "notes": [
      "lichi",
      "pera",
      "kiwi",
      "quequito",
      "chocolate blanco",
      "jazmín",
      "orquídea",
      "pachulí",
      "notas amaderadas",
      "almizcle",
      "raíz de lirio"
    ],
    "ml": 100,
    "precioUSD": 35,
    "categoria": "disenador"
  },
  {
    "id": 469,
    "house": "ANTONIO BANDERAS",
    "name": "The Icon Splendid Woman",
    "image": null,
    "familia": "Dulce",
    "tipo": "Eau de Parfum",
    "genero": "Femenino",
    "descripcion": "Una fragancia floral gourmand elegante y luminosa para la mujer, lanzada en 2024 dentro de la línea The Icon. Splendid abre con un contraste frutal fresco de pera, naranja tangerina y pimienta negra, evoluciona hacia un corazón floral cálido de peonía, flor de azahar del naranjo y guayacán, y cierra con una base golosa y sensual de praliné, vainilla, sándalo, almizcle y rosa que deja una estela suave, dulce y sofisticada.",
    "description": "Una fragancia floral gourmand elegante y luminosa para la mujer, lanzada en 2024 dentro de la línea The Icon. Splendid abre con un contraste frutal fresco de pera, naranja tangerina y pimienta negra, evoluciona hacia un corazón floral cálido de peonía, flor de azahar del naranjo y guayacán, y cierra con una base golosa y sensual de praliné, vainilla, sándalo, almizcle y rosa que deja una estela suave, dulce y sofisticada.",
    "notasSalida": "pera, naranja, pimienta negra",
    "notasCorazon": "peonía, flor de azahar del naranjo, madera de guayaco",
    "notasFondo": "praliné, vainilla, sandalo",
    "notes": [
      "pera",
      "naranja",
      "pimienta negra",
      "peonía",
      "flor de azahar del naranjo",
      "madera de guayaco",
      "praliné",
      "vainilla",
      "sandalo"
    ],
    "ml": 100,
    "precioUSD": 30,
    "categoria": "disenador"
  },
  {
    "id": 470,
    "house": "BENETTON",
    "name": "Colors Woman Rose ",
    "image": null,
    "familia": "Floral",
    "tipo": "Eau de Toilette",
    "genero": "Femenino",
    "descripcion": "Una fragancia floral fresca y luminosa para la mujer, lanzada en 2019 dentro de la línea Colors de Benetton. Woman Rose abre con un contraste dulce y cítrico de madreselva, bergamota y naranja, evoluciona hacia un corazón floral tropical de jazmín, frangipani y ylang-ylang, y cierra con una base cremosa de leche de coco, almizcle y notas amaderadas que deja una estela suave, dulce y veraniega, ideal para el uso diario.",
    "description": "Una fragancia floral fresca y luminosa para la mujer, lanzada en 2019 dentro de la línea Colors de Benetton. Woman Rose abre con un contraste dulce y cítrico de madreselva, bergamota y naranja, evoluciona hacia un corazón floral tropical de jazmín, frangipani y ylang-ylang, y cierra con una base cremosa de leche de coco, almizcle y notas amaderadas que deja una estela suave, dulce y veraniega, ideal para el uso diario.",
    "notasSalida": "madreselva, bergamota, naranja",
    "notasCorazon": "jazmín, frangipani, ylang-ylang",
    "notasFondo": "leche, almizcle, notas amaderadas",
    "notes": [
      "madreselva",
      "bergamota",
      "naranja",
      "jazmín",
      "frangipani",
      "ylang-ylang",
      "leche",
      "almizcle",
      "notas amaderadas"
    ],
    "ml": 80,
    "precioUSD": 25,
    "categoria": "disenador"
  }
]
