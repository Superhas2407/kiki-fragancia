...
const MEN_IDS = [19, 20, 141, 255, 272, 104, 247, 266, 311, 172]
...
export default function MustHaveMen() {
  const products = useIndexProducts()
  const items = MEN_IDS.map(id => products.find(p => p.id === id)).filter(Boolean)
  ...
  return (
    <section className="bestseller-section">
      <div className="bestseller-heading-wrap">
        ...
        <p className="mhm-subtitle">Para hombres</p>
      ...
      <div className="bestseller-cta-wrap">
        <Link to="/tienda?genero=Masculino" className="bestseller-cta">Ver fragancias para hombres</Link>
      </div>
    </section>
  )
}
